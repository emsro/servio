 ...
 * @note   Baud rate value stored in LPUART_InitStruct BaudRate field, should be valid (different
 * from 0).
 * @param  LPUARTx LPUART Instance
 ...
 */
ErrorStatus LL_LPUART_Init( USART_TypeDef* LPUARTx, LL_LPUART_InitTypeDef const* LPUART_InitStruct )
{
        ...
        if ( LL_LPUART_IsEnabled( LPUARTx ) == 0U ) {
                 ...
                 * Configure LPUARTx CR1 (LPUART Word Length, Parity and Transfer Direction bits)
                 * with parameters:
                 * - DataWidth:          USART_CR1_M bits according to LPUART_InitStruct->DataWidth
                 * value
