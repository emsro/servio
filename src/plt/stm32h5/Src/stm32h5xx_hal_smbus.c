...
static HAL_StatusTypeDef SMBUS_WaitOnFlagUntilTimeout(
    SMBUS_HandleTypeDef* hsmbus,
    uint32_t             Flag,
    FlagStatus           Status,
    uint32_t             Timeout );
...
static void SMBUS_TransferConfig(
    SMBUS_HandleTypeDef* hsmbus,
    uint16_t             DevAddress,
    uint8_t              Size,
    uint32_t             Mode,
 ...
 */
HAL_StatusTypeDef
HAL_SMBUS_ConfigDigitalFilter( SMBUS_HandleTypeDef* hsmbus, uint32_t DigitalFilter )
{
        uint32_t tmpreg;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SMBUS_RegisterCallback(
    SMBUS_HandleTypeDef*        hsmbus,
 ...
 */
HAL_StatusTypeDef
HAL_SMBUS_UnRegisterCallback( SMBUS_HandleTypeDef* hsmbus, HAL_SMBUS_CallbackIDTypeDef CallbackID )
{
        HAL_StatusTypeDef status = HAL_OK;
...
HAL_StatusTypeDef HAL_SMBUS_Master_Abort_IT( SMBUS_HandleTypeDef* hsmbus, uint16_t DevAddress )
{
        if ( hsmbus->State == HAL_SMBUS_STATE_READY ) {
                 ...
                 * current transfer */
                SMBUS_TransferConfig(
                    hsmbus, DevAddress, 1, SMBUS_AUTOEND_MODE, SMBUS_NO_STARTSTOP );
        ...
        if ( hsmbus->State == HAL_SMBUS_STATE_READY ) {
                ...
                do {
                        ...
                        if ( __HAL_SMBUS_GET_FLAG( hsmbus, SMBUS_FLAG_AF ) == RESET ) {
                                ...
                                if ( SMBUS_WaitOnFlagUntilTimeout(
                                         hsmbus, SMBUS_FLAG_STOPF, RESET, Timeout ) != HAL_OK ) {
                                        return HAL_ERROR;
                                }
 ...
 * @retval None
 */
__weak void HAL_SMBUS_AddrCallback(
    SMBUS_HandleTypeDef* hsmbus,
 ...
 * @retval HAL state
 */
uint32_t HAL_SMBUS_GetState( SMBUS_HandleTypeDef const* hsmbus )
{
 ...
 */
uint32_t HAL_SMBUS_GetError( SMBUS_HandleTypeDef const* hsmbus )
{
...
static HAL_StatusTypeDef SMBUS_Slave_ISR( SMBUS_HandleTypeDef* hsmbus, uint32_t StatusFlags )
{
        ...
        if ( SMBUS_CHECK_FLAG( StatusFlags, SMBUS_FLAG_AF ) != RESET ) {
            ...
            ( SMBUS_CHECK_FLAG( StatusFlags, SMBUS_FLAG_RXNE ) != RESET ) ||
                     ...
                     HAL_SMBUS_STATE_SLAVE_BUSY_RX ) {
                        ...
                        *hsmbus->pBuffPtr = (uint8_t) ( hsmbus->Instance->RXDR );
                        ...
                        if ( hsmbus->XferCount == 1U ) {
                                ...
                                hsmbus->XferOptions &= ~SMBUS_RELOAD_MODE;
                                SMBUS_TransferConfig(
                                    hsmbus, 0, 1, hsmbus->XferOptions, SMBUS_NO_STARTSTOP );
...
static void SMBUS_Enable_IRQ( SMBUS_HandleTypeDef* hsmbus, uint32_t InterruptRequest )
{
        ...
        if ( ( InterruptRequest & SMBUS_IT_RX ) == SMBUS_IT_RX ) {
                ...
                tmpisr |=
                    SMBUS_IT_ERRI | SMBUS_IT_TCI | SMBUS_IT_STOPI | SMBUS_IT_NACKI | SMBUS_IT_RXI;
        }
