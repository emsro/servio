/**
 ...
 HAL_FLASHEx_ConfigBBAttributes()
      ...
      (#) Get the block-based secure / privilege area configuration function: Use
...
static void
FLASH_OB_GetBootConfig( uint32_t BootOption, uint32_t* BootAddress, uint32_t* BootLockConfig );
...
static void
FLASH_OB_WMSECConfig( uint32_t Banks, uint32_t WMSecStartSector, uint32_t WMSecEndSector );
static void
FLASH_OB_GetWMSEC( uint32_t Bank, uint32_t* WMSecStartSector, uint32_t* WMSecEndSector );
...
HAL_StatusTypeDef HAL_FLASHEx_Erase( FLASH_EraseInitTypeDef* pEraseInit, uint32_t* SectorError )
{
        ...
        if ( status == HAL_OK ) {
                ...
                CLEAR_BIT(
                    ( *reg_cr ), ( pEraseInit->TypeErase ) & ( ~( FLASH_NON_SECURE_MASK ) ) );
        }
...
HAL_StatusTypeDef HAL_FLASHEx_OBProgram( FLASH_OBProgramInitTypeDef* pOBInit )
{
        ...
        if ( status == HAL_OK ) {
                ...
                if ( ( pOBInit->OptionType & OPTIONBYTE_USER ) != 0U ) {
                        ...
                        FLASH_OB_UserConfig(
                            pOBInit->USERType, pOBInit->USERConfig, pOBInit->USERConfig2 );
                }
                ...
                if ( ( pOBInit->OptionType & OPTIONBYTE_WMSEC ) != 0U ) {
                        ...
                        FLASH_OB_WMSECConfig(
                            pOBInit->Banks, pOBInit->WMSecStartSector, pOBInit->WMSecEndSector );
                }
#endif /* __ARM_FEATURE_CMSE */
                ...
                if ( ( pOBInit->OptionType & OPTIONBYTE_HDP ) != 0U ) {
                        FLASH_OB_HDPConfig(
                            pOBInit->Banks, pOBInit->HDPStartSector, pOBInit->HDPEndSector );
                }
 ...
 * @param  pOBInit pointer to an FLASH_OBInitStruct structure that
 *         contains the configuration information for the programming.
 * @note   The parameter Banks of the pOBInit structure must be set exclusively to FLASH_BANK_1 or
 * FLASH_BANK_2, as this parameter is use to get the given Bank WRP, PCROP and secured area
 * configuration.
 *
 * @retval None
 */
void HAL_FLASHEx_OBGetConfig( FLASH_OBProgramInitTypeDef* pOBInit )
{
...
#endif /* __ARM_FEATURE_CMSE */
        {
                ...
                FLASH_OB_GetBootConfig(
                    pOBInit->BootConfig, &( pOBInit->BootAddr ), &( pOBInit->BootLock ) );
        }
 ...
 *         This parameter can be a value between 0 (no OBK data swapped) and 511 (all OBK data
 * swapped). Typical value are available in @ref FLASH_OBK_SWAP_Offset
 * @retval HAL Status
 */
HAL_StatusTypeDef HAL_FLASHEx_OBK_Swap( uint32_t SwapOffset )
{
        ...
        if ( status == HAL_OK ) {
                ...
                reg_obkcfgr =
                    IS_FLASH_SECURE_OPERATION() ? &( FLASH->SECOBKCFGR ) : &( FLASH_NS->NSOBKCFGR );

                /* Set OBK swap offset */
                MODIFY_REG(
                    ( *reg_obkcfgr ),
                    FLASH_OBKCFGR_SWAP_OFFSET,
...
HAL_StatusTypeDef HAL_FLASHEx_ConfigBBAttributes( FLASH_BBAttributesTypeDef* pBBAttributes )
{
        ...
        if ( status == HAL_OK ) {
                ...
                /* ISB instruction is called to be sure next instructions are performed with correct
                 * attributes */
                __ISB();
        }
 ...
 *      @arg FLASH_SPRIV_GRANTED: access to secure Flash registers is granted to privileged or
 * unprivileged access
 *      @arg FLASH_SPRIV_DENIED: access to secure Flash registers is denied to unprivileged access
 *      @arg FLASH_NSPRIV_GRANTED: access to non-secure Flash registers is granted to privileged or
 * unprivileged access
 *      @arg FLASH_NSPRIV_DENIED: access to non-secure Flash registers is denied to unprivilege
 * access
 *
 * @retval None
 */
...
void FLASH_Erase_Sector( uint32_t Sector, uint32_t Banks )
{
        ...
        if ( ( Banks & FLASH_BANK_1 ) == FLASH_BANK_1 ) {
                ...
                ( *reg_cr ) |=
                    ( FLASH_CR_SER | FLASH_CR_BKSEL | ( Sector << FLASH_CR_SNB_Pos ) |
                      FLASH_CR_START );
        }
}
 ...
 *         This parameter can be sector number between WMSecStartSector and WMSecEndSector(max
 ...
 *         This parameter can be sector number between HDPStartSector and HDPEndSector (max number
 * of sectors in the bank - 1)
 *
 * @retval None
 */
static void FLASH_OB_HDPConfig( uint32_t Banks, uint32_t HDPStartSector, uint32_t HDPEndSector )
...
void HAL_FLASHEx_GetEccInfo( FLASH_EccInfoTypeDef* pData )
{
        ...
        /* Check that an ECC single or double error has occurred to continue the calculation of area
         * address */
        if ( addr_reg != 0xFFFFFFFFU ) {
                ...
                case FLASH_ECC_AREA_USER_BANK1:
                        /*
                         * One error detection/correction or two error detections per 128-bit flash
                         * word Therefore, the address returned by ECC registers in bank1 represents
                         * 128-bit flash word, to get the correct address value, we must do a shift
                         * by 4 bits
                         */
                ...
                case FLASH_ECC_AREA_USER_BANK2:
                        /*
                         * One error detection/correction or two error detections per 128-bit flash
                         * word Therefore, the address returned by ECC registers in bank2 represents
                         * 128-bit flash word, to get the correct address value, we must do a shift
                         * by 4 bits
                         */
                        addr_reg       = addr_reg << 4U;
                ...
                case FLASH_ECC_AREA_EDATA_BANK1:
                        ...
                        /*
                         * addr_reg is the address returned by the ECC register along with an offset
                         * value depends on area To calculate the exact address set by user while an
                         * ECC occurred, we must subtract the offset value, In addition, the address
                         * returned by ECC registers represents 128-bit flash word (multiply by 4),
                         */
                        pData->Address =
