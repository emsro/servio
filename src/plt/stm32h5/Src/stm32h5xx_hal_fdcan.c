...
#define SRAMCAN_FLESA                                                                           \
        ( (uint32_t) ( SRAMCAN_FLSSA +                                                          \
                       ( SRAMCAN_FLS_NBR * SRAMCAN_FLS_SIZE ) ) ) /* Filter List Extended Start \
                                                                     Address                  */
#define SRAMCAN_RF0SA                  \
        ( (uint32_t) ( SRAMCAN_FLESA + \
                       ( SRAMCAN_FLE_NBR * SRAMCAN_FLE_SIZE ) ) ) /* Rx FIFO 0 Start Address  */
#define SRAMCAN_RF1SA                  \
        ( (uint32_t) ( SRAMCAN_RF0SA + \
                       ( SRAMCAN_RF0_NBR * SRAMCAN_RF0_SIZE ) ) ) /* Rx FIFO 1 Start Address  */
#define SRAMCAN_TEFSA                                                                              \
        ( (uint32_t) ( SRAMCAN_RF1SA + ( SRAMCAN_RF1_NBR * SRAMCAN_RF1_SIZE ) ) ) /* Tx Event FIFO \
                                                                                     Start Address \
                                                                                   */
#define SRAMCAN_TFQSA                                                                              \
        ( (uint32_t) ( SRAMCAN_TEFSA + ( SRAMCAN_TEF_NBR * SRAMCAN_TEF_SIZE ) ) ) /* Tx FIFO/Queue \
                                                                                     Start Address \
                                                                                   */
#define SRAMCAN_SIZE                                                                             \
        ( (uint32_t) ( SRAMCAN_TFQSA + ( SRAMCAN_TFQ_NBR * SRAMCAN_TFQ_SIZE ) ) ) /* Message RAM \
                                                                                     size */
 ...
 * @{
 */
static uint8_t const DLCtoBytes[] = { 0, 1, 2, 3, 4, 5, 6, 7, 8, 12, 16, 20, 24, 32, 48, 64 };
/**
 * @}
 */
...
static void FDCAN_CopyMessageToRAM(
    ...
    FDCAN_TxHeaderTypeDef const* pTxHeader,
 ...
 *         Tx Event Fifo FDCAN Callback is redirected to the weak HAL_FDCAN_TxEventFifoCallback()
 * predefined callback
 * @param  hfdcan FDCAN handle
 * @retval HAL status
 */
 ...
 *         Rx Fifo 0 FDCAN Callback is redirected to the weak HAL_FDCAN_RxFifo0Callback() predefined
 * callback
 * @param  hfdcan FDCAN handle
 * @retval HAL status
 */
 ...
 *         Rx Fifo 1 FDCAN Callback is redirected to the weak HAL_FDCAN_RxFifo1Callback() predefined
 * callback
 * @param  hfdcan FDCAN handle
 * @retval HAL status
 */
 ...
 *         Error Status FDCAN Callback is redirected to the weak HAL_FDCAN_ErrorStatusCallback()
 * predefined callback
 * @param  hfdcan FDCAN handle
 * @retval HAL status
 */
 ...
 */
HAL_StatusTypeDef
HAL_FDCAN_ConfigFilter( FDCAN_HandleTypeDef* hfdcan, FDCAN_FilterTypeDef const* sFilterConfig )
{
        ...
        if ( ( state == HAL_FDCAN_STATE_READY ) || ( state == HAL_FDCAN_STATE_BUSY ) ) {
                ...
                if ( sFilterConfig->IdType == FDCAN_STANDARD_ID ) {
                        ...
                        assert_param( IS_FDCAN_MAX_VALUE(
                            ( sFilterConfig->FilterIndex + 1U ), hfdcan->Init.StdFiltersNbr ) );
                ...
                } else /* sFilterConfig->IdType == FDCAN_EXTENDED_ID */
                {
                        /* Check function parameters */
                        assert_param( IS_FDCAN_MAX_VALUE(
                            ( sFilterConfig->FilterIndex + 1U ), hfdcan->Init.ExtFiltersNbr ) );
 ...
 */
uint16_t HAL_FDCAN_GetTimestampCounter( FDCAN_HandleTypeDef const* hfdcan )
{
 ...
 */
uint16_t HAL_FDCAN_GetTimeoutCounter( FDCAN_HandleTypeDef const* hfdcan )
{
...
request
      ...
      (+) HAL_FDCAN_GetRxMessage                  : Get an FDCAN frame from the Rx FIFO zone into
the message RAM
 ...
 */
uint32_t HAL_FDCAN_GetLatestTxFifoQRequestBuffer( FDCAN_HandleTypeDef const* hfdcan )
{
        ...
        if ( state == HAL_FDCAN_STATE_BUSY ) {
                if ( RxLocation == FDCAN_RX_FIFO0 ) /* Rx element is assigned to the Rx FIFO 0 */
                {
                        ...
                        if ( ( hfdcan->Instance->RXF0S & FDCAN_RXF0S_F0FL ) == 0U ) {
                                       ...
                                       FDCAN_RXF0S_F0F_Pos ) == 1U ) {
                                        if ( ( ( hfdcan->Instance->RXGFC & FDCAN_RXGFC_F0OM ) >>
                                               FDCAN_RXGFC_F0OM_Pos ) == FDCAN_RX_FIFO_OVERWRITE ) {
                                                /* When overwrite status is on discard first message
                                                 * in FIFO */
                                                GetIndex = 1U;
                                        }
                                }
                                ...
                                GetIndex +=
                                    ( ( hfdcan->Instance->RXF0S & FDCAN_RXF0S_F0GI ) >>
                ...
                } else /* Rx element is assigned to the Rx FIFO 1 */
                {
                        ...
                        if ( ( hfdcan->Instance->RXF1S & FDCAN_RXF1S_F1FL ) == 0U ) {
                                ...
                                GetIndex +=
                                    ( ( hfdcan->Instance->RXF1S & FDCAN_RXF1S_F1GI ) >>
                ...
                if ( pRxHeader->IdType == FDCAN_STANDARD_ID ) /* Standard ID element */
                {
                        pRxHeader->Identifier =
                            ( ( *RxAddress & FDCAN_ELEMENT_MASK_STDID ) >> 18U );
...
HAL_FDCAN_GetTxEvent( FDCAN_HandleTypeDef* hfdcan, FDCAN_TxEventFifoTypeDef* pTxEvent )
{
        ...
        if ( state == HAL_FDCAN_STATE_BUSY ) {
                ...
                TxEventAddress =
                    (uint32_t*) ( hfdcan->msgRam.TxEventFIFOSA + ( GetIndex * SRAMCAN_TEF_SIZE ) );
 ...
 */
uint32_t
HAL_FDCAN_IsTxBufferMessagePending( FDCAN_HandleTypeDef const* hfdcan, uint32_t TxBufferIndex )
{
        /* Check function parameters */
 ...
 */
uint32_t HAL_FDCAN_GetRxFifoFillLevel( FDCAN_HandleTypeDef const* hfdcan, uint32_t RxFifo )
{
        uint32_t FillLevel;
 ...
 */
uint32_t HAL_FDCAN_GetTxFifoFreeLevel( FDCAN_HandleTypeDef const* hfdcan )
{
        uint32_t FreeLevel;
 ...
 */
uint32_t HAL_FDCAN_IsRestrictedOperationMode( FDCAN_HandleTypeDef const* hfdcan )
{
        uint32_t OperationMode;
        ...
        if ( ( state == HAL_FDCAN_STATE_READY ) || ( state == HAL_FDCAN_STATE_BUSY ) ) {
                ...
                if ( ( ActiveITs & FDCAN_IT_TX_ABORT_COMPLETE ) != 0U ) {
                        /* Enable Tx Buffer Cancellation Finished Interrupt to set TCF flag in IR
                           register, but interrupt will only occur if TCF is enabled in IE register
                         */
 ...
 * @retval HAL state
 */
HAL_FDCAN_StateTypeDef HAL_FDCAN_GetState( FDCAN_HandleTypeDef const* hfdcan )
{
        /* Return FDCAN state */
        return hfdcan->State;
}
 ...
 */
uint32_t HAL_FDCAN_GetError( FDCAN_HandleTypeDef const* hfdcan )
{
    ...
    uint32_t                     BufferIndex )
{
        ...
        TxElementW2 =
            ( ( pTxHeader->MessageMarker << 24U ) | pTxHeader->TxEventFifoControl |
              pTxHeader->FDFormat | pTxHeader->BitRateSwitch | ( pTxHeader->DataLength << 16U ) );
