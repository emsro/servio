 ...
 *          This parameter can be one of the following values:
 *            @arg VREFBUF_HIGH_IMPEDANCE_DISABLE: VREF+ pin is internally connect to VREFINT
 * output.
 ...
 *           @arg SBS_VDD_REGISTER_CODE: Select Code from the SBS compensation cell code register
 * (SBS_CCSWCR)
 * @retval None
 */
 ...
 *           @arg SBS_VDDIO_REGISTER_CODE: Select Code from the SBS compensation cell code register
 * (SBS_CCSWCR)
 * @retval None
 */
...
void HAL_SBS_VDDCompensationCodeConfig( uint32_t SBS_PMOSCode, uint32_t SBS_NMOSCode )
{
        ...
        MODIFY_REG(
            SBS->CCSWCR,
            SBS_CCSWCR_SW_ANSRC1 | SBS_CCSWCR_SW_APSRC1,
...
void HAL_SBS_VDDIOCompensationCodeConfig( uint32_t SBS_PMOSCode, uint32_t SBS_NMOSCode )
{
        ...
        MODIFY_REG(
            SBS->CCSWCR,
            SBS_CCSWCR_SW_ANSRC2 | SBS_CCSWCR_SW_APSRC2,
...
{
        return (
            ( READ_BIT( SBS->ECCNMIR, SBS_ECCNMIR_ECCNMI_MASK_EN ) == SBS_ECCNMIR_ECCNMI_MASK_EN ) ?
                1UL :
                0UL );
}
...
HAL_StatusTypeDef HAL_SBS_ConfigDebugLevel( uint32_t Level )
{
        ...
        if ( Level != SBS_HDPL_VALUE_0 ) {
                MODIFY_REG(
                    SBS->DBGCR, SBS_DBGCR_DBG_AUTH_HDPL, ( Level << SBS_DBGCR_DBG_AUTH_HDPL_Pos ) );
                return HAL_OK;
        } else {
                return HAL_ERROR;
        }
...
void HAL_SBS_Lock( uint32_t Item )
{
        ...
        SBS->CSLCKR =
            ( ( 0xFFFF0000U & Item ) >> 16U ); /* Secure-only lock item in 16 highest bits */
