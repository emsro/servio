  ...
  *** Callback registration ***
  ...
  [..]
    ...
    it allows to register following callbacks:
      ...
      (+) Read_DMALnkLstBufCpltCallback : callback when the DMA reception of linked list node buffer
  is completed.
      (+) Write_DMALnkLstBufCpltCallback : callback when the DMA transmission of linked list node
  buffer is completed.
                ...
                if ( ( ( hmmc->Ext_CSD[( MMC_EXT_CSD_DATA_SEC_SIZE_INDEX / 4 )] >>
                       ...
                       0x000000FFU ) != 0x0U ) {
                        if ( ( NumberOfBlocks % 8U ) != 0U ) {
                                /* The number of blocks should be a multiple of 8 sectors of 512
                                 * bytes = 4 KBytes */
                        ...
                        if ( ( BlockAdd % 8U ) != 0U ) {
                                /* The address should be aligned to 8 (corresponding to 4 KBytes
                                 * blocks) */
 ...
 */
HAL_StatusTypeDef
HAL_MMC_Erase( MMC_HandleTypeDef* hmmc, uint32_t BlockStartAdd, uint32_t BlockEndAdd )
{
        uint32_t errorstate;
...
void HAL_MMC_IRQHandler( MMC_HandleTypeDef* hmmc )
{
        ...
        if ( ( __HAL_MMC_GET_FLAG( hmmc, SDMMC_FLAG_RXFIFOHF ) != RESET ) &&
            ...
            ( __HAL_MMC_GET_FLAG( hmmc, SDMMC_FLAG_TXFIFOHE ) != RESET ) &&
            ( ( context & MMC_CONTEXT_IT ) != 0U ) ) {
                MMC_Write_IT( hmmc );
        }
 ...
 * @retval HAL state
 */
HAL_MMC_StateTypeDef HAL_MMC_GetState( MMC_HandleTypeDef const* hmmc )
{
        return hmmc->State;
}
 ...
 */
uint32_t HAL_MMC_GetError( MMC_HandleTypeDef const* hmmc )
{
 ...
 *          @arg @ref HAL_MMC_READ_DMA_LNKLST_BUF_CPLT_CB_ID  MMC DMA Rx Linked List Node buffer
 * Callback ID
 *          @arg @ref HAL_MMC_WRITE_DMA_LNKLST_BUF_CPLT_CB_ID MMC DMA Tx Linked List Node buffer
 * Callback ID
 ...
 */
HAL_StatusTypeDef
HAL_MMC_UnRegisterCallback( MMC_HandleTypeDef* hmmc, HAL_MMC_CallbackIDTypeDef CallbackId )
{
        HAL_StatusTypeDef status = HAL_OK;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_MMC_GetCardCID( MMC_HandleTypeDef const* hmmc, HAL_MMC_CardCIDTypeDef* pCID )
{
...
HAL_StatusTypeDef HAL_MMC_GetCardCSD( MMC_HandleTypeDef* hmmc, HAL_MMC_CardCSDTypeDef* pCSD )
{
        ...
        if ( MMC_ReadExtCSD( hmmc, &block_nbr, 212, 0x0FFFFFFFU ) != HAL_OK ) /* Field SEC_COUNT
                                                                                 [215:212] */
        {
                return HAL_ERROR;
        }
 ...
 */
HAL_StatusTypeDef
HAL_MMC_GetCardInfo( MMC_HandleTypeDef const* hmmc, HAL_MMC_CardInfoTypeDef* pCardInfo )
{
 ...
 */
HAL_StatusTypeDef
HAL_MMC_GetCardExtCSD( MMC_HandleTypeDef* hmmc, uint32_t* pExtCSD, uint32_t Timeout )
{
        SDMMC_DataInitTypeDef config;
        uint32_t              errorstate;
        uint32_t              tickstart = HAL_GetTick();
        ...
        case SDMMC_SPEED_MODE_AUTO: {
                if ( ( ( hmmc->Instance->CLKCR & SDMMC_CLKCR_WIDBUS ) != 0U ) &&
                     ( ( device_type & 0x04U ) != 0U ) ) {
                        ...
                        if ( errorstate != HAL_MMC_ERROR_NONE ) {
                                hmmc->ErrorCode |= errorstate;
                        } else if ( ( hmmc->Instance->CLKCR & SDMMC_CLKCR_CLKDIV ) != 0U ) {
...
HAL_StatusTypeDef HAL_MMC_Abort( MMC_HandleTypeDef* hmmc )
{
        ...
        if ( hmmc->State == HAL_MMC_STATE_BUSY ) {
                ...
                /*In case the data transfer aborted , the external memory will respond and will
                ...
                if ( ( hmmc->Instance->DCTRL & SDMMC_DCTRL_DTDIR ) == SDMMC_TRANSFER_DIR_TO_CARD ) {
                        if ( hmmc->ErrorCode == HAL_MMC_ERROR_NONE ) {
                                while ( !__HAL_MMC_GET_FLAG(
                                    hmmc, SDMMC_FLAG_DABORT | SDMMC_FLAG_BUSYD0END ) ) {
                                        if ( ( HAL_GetTick() - tickstart ) >= SDMMC_DATATIMEOUT ) {
                                                hmmc->ErrorCode = HAL_MMC_ERROR_TIMEOUT;
                                                hmmc->State     = HAL_MMC_STATE_READY;
                                                return HAL_TIMEOUT;
                    ...
                    ( hmmc->Instance->DCTRL & SDMMC_DCTRL_DTDIR ) == SDMMC_TRANSFER_DIR_TO_SDMMC ) {
                        while (
                ...
                /*The reason of all these while conditions previously is that we need to wait the
                  SDMMC and clear the appropriate flags that will be set depending of the abort/non
                  abort of the memory */
                /*Not waiting the SDMMC flags will cause the next SDMMC_DISABLE_IDMA to not get
                  cleared and will result in next SDMMC read/write operation to fail */
 ...
 *             @arg HAL_MMC_SECURE_ERASE Perform a secure purge according SRT on the erase groups
 * identified by CMD35 & 36
 *             @arg HAL_MMC_SECURE_TRIM_STEP1 Mark the write blocks identified by CMD35 & 36 for
 * secure erase
 *             @arg HAL_MMC_SECURE_TRIM_STEP2 Perform a secure purge according SRT on the write
 * blocks previously identified
 * @param  BlockStartAdd Start Block address
 * @param  BlockEndAdd End Block address
 * @retval HAL status
 */
                                ...
                                if ( errorstate == HAL_MMC_ERROR_NONE ) {
                                        if ( ( EraseType == HAL_MMC_SECURE_ERASE ) ||
                                             ( EraseType == HAL_MMC_SECURE_TRIM_STEP2 ) ) {
                                                /* Wait that the device is ready by checking the D0
                                                 * line */
                                                while ( ( !__HAL_MMC_GET_FLAG(
                                                            hmmc, SDMMC_FLAG_BUSYD0END ) ) &&
 ...
 *            @arg HAL_MMC_SRT_WRITE_CHAR_ERASE Information removed by an overwriting with a
 * character followed by an erase
 *            @arg HAL_MMC_SRT_WRITE_CHAR_COMPL_RANDOM Information removed by an overwriting with a
                ...
                if ( HAL_MMC_GetSupportedSecRemovalType( hmmc, &srt ) == HAL_OK ) {
                        ...
                        if ( ( SRTMode & srt ) != 0U ) {
                                ...
                                errorstate = SDMMC_CmdSwitch(
                                    hmmc->Instance, ( 0x03100000U | ( srt << 8U ) ) );
                                if ( errorstate == HAL_MMC_ERROR_NONE ) {
                                        /* While card is not ready for data and trial number for
                                         * sending CMD13 is not exceeded */
                                        count = SDMMC_MAX_TRIAL;
                                        do {
                                                errorstate = SDMMC_CmdSendStatus(
                                                    hmmc->Instance,
                                                    (uint32_t) ( ( (uint32_t)
                                                                       hmmc->MmcCard.RelCardAdd )
                                                                 << 16U ) );
                                                if ( errorstate != HAL_MMC_ERROR_NONE )
                                                        break;

                                                /* Get command response */
                                                response = SDMMC_GetResponse(
                                                    hmmc->Instance, SDMMC_RESP1 );
                                                count--;
 ...
 */
HAL_StatusTypeDef
HAL_MMC_GetSupportedSecRemovalType( MMC_HandleTypeDef* hmmc, uint32_t* SupportedSRT )
{
        /* Check the state of the driver */
        if ( hmmc->State == HAL_MMC_STATE_READY ) {
                /* Change State */
                ...
                if ( errorstate == HAL_MMC_ERROR_NONE ) {
                        ...
                        if ( count == 0U ) {
                                ...
                                if ( ( response & 0x80U ) != 0U ) {
                                        ...
                                        /* Set the power-off notification to sleep notification :
                                         * Ext_CSD[34] = 4 */
                                        errorstate =
                                            SDMMC_CmdSwitch( hmmc->Instance, ( 0x03220400U ) );
                                        if ( errorstate == HAL_MMC_ERROR_NONE ) {
                                                ...
                                                /* Sleep/Awake Timeout = 10us *
                                                 * 2^SLEEP_NOTIFICATION_TIME */
                                                /* In HAL, the tick interrupt occurs each ms */
                                                if ( ( sleep_timeout == 0U ) ||
                                                     ( sleep_timeout > 0x17U ) ) {
                                                        sleep_timeout = 0x17U; /* Max register value
                                                                                  defined is 0x17 */
                                                }
                                                timeout =
                                                    ( ( ( 1UL << sleep_timeout ) / 100U ) + 1U );
                                                ...
                                                if ( errorstate == HAL_MMC_ERROR_NONE ) {
                                                        /* While card is not ready for data and
                                                         * trial number for sending CMD13 is not
                                                         * exceeded */
                                                        count = SDMMC_MAX_TRIAL;
                                                        do {
                                                                errorstate = SDMMC_CmdSendStatus(
                                                        ...
                                                        /* Check the status after the switch command
                                                         * execution */
                                                        if ( count == 0U ) {
                                                                errorstate = SDMMC_ERROR_TIMEOUT;
                                                        } else if (
                                                            errorstate == HAL_MMC_ERROR_NONE ) {
                                                                /* Check the bit SWITCH_ERROR of the
                                                                 * device status */
                                                                if ( ( response & 0x80U ) != 0U ) {
                                                                        errorstate =
                                                                            SDMMC_ERROR_UNSUPPORTED_FEATURE;
                                                                } else {
                                                                        /* Switch the device in
                                                                         * stand-by mode */
                                                                        (void) SDMMC_CmdSelDesel(
                                                                            hmmc->Instance, 0U );
                        ...
                        if ( errorstate == HAL_MMC_ERROR_NONE ) {
                                if ( HAL_MMC_GetCardState( hmmc ) == HAL_MMC_CARD_STANDBY ) {
                                        ...
                                        errorstate = SDMMC_CmdSelDesel(
                                            hmmc->Instance, ( hmmc->MmcCard.RelCardAdd << 16U ) );
                                        if ( errorstate == HAL_MMC_ERROR_NONE ) {
                                                if ( HAL_MMC_GetCardState( hmmc ) ==
                                                     HAL_MMC_CARD_TRANSFER ) {
                                                        /* Set the power-off notification to
                                                         * powered-on : Ext_CSD[34] = 1 */
                                                        errorstate = SDMMC_CmdSwitch(
                                                            hmmc->Instance, ( 0x03220100U ) );
                                                        if ( errorstate == HAL_MMC_ERROR_NONE ) {
                                                                /* While card is not ready for data
                                                                 * and trial number for sending
                                                                 * CMD13 is not exceeded */
                                                                count = SDMMC_MAX_TRIAL;
                                                                do {
                                                                        errorstate = SDMMC_CmdSendStatus(
...
static uint32_t MMC_HighSpeed( MMC_HandleTypeDef* hmmc, FunctionalState state )
{
        ...
        if ( ( ( hmmc->Instance->CLKCR & SDMMC_CLKCR_BUSSPEED ) == 0U ) && ( state != DISABLE ) ) {
                errorstate = MMC_PwrClassUpdate(
                    hmmc, ( hmmc->Instance->CLKCR & SDMMC_CLKCR_WIDBUS ), SDMMC_SPEED_MODE_HIGH );
                if ( errorstate == HAL_MMC_ERROR_NONE ) {
                        /* Index : 185 - Value : 1 */
                ...
                if ( ( count != 0U ) && ( errorstate == HAL_MMC_ERROR_NONE ) ) {
                        ...
                        if ( ( response & 0x80U ) != 0U ) {
                                ...
                                if ( state == DISABLE ) {
                                        ...
                                        if ( hmmc->Instance == SDMMC1 ) {
                                                sdmmc_clk = HAL_RCCEx_GetPeriphCLKFreq(
                                        ...
                                        if ( sdmmc_clk == 0U ) {
                                                ...
                                                if ( sdmmc_clk <= MMC_HIGH_SPEED_FREQ ) {
                                                        ...
                                                        Init.ClockDiv =
                                                            ( sdmmc_clk /
                                                              ( 2U * MMC_HIGH_SPEED_FREQ ) ) +
                                                            1U;
                                                }
                                                (void) SDMMC_Init( hmmc->Instance, Init );

                                                SET_BIT(
                                                    hmmc->Instance->CLKCR, SDMMC_CLKCR_BUSSPEED );
                                        }
                                }
                        }
                } else if ( count == 0U ) {
 ...
 */
HAL_StatusTypeDef
HAL_MMC_SwitchPartition( MMC_HandleTypeDef* hmmc, HAL_MMC_PartitionTypeDef Partition )
{
        uint32_t errorstate;
        uint32_t response = 0U;
        uint32_t count;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_MMC_RPMB_ProgramAuthenticationKey(
    MMC_HandleTypeDef* hmmc,
    uint8_t const*     pKey,
    uint32_t           Timeout )
{
...
uint32_t HAL_MMC_RPMB_GetWriteCounter( MMC_HandleTypeDef* hmmc, uint8_t* pNonce, uint32_t Timeout )
{
        ...
        if ( hmmc->State == HAL_MMC_STATE_READY ) {
                ...
                return (
                    (uint32_t) tail_pack[3] | ( (uint32_t) tail_pack[2] << 8 ) |
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_MMC_RPMB_WriteBlocks(
    MMC_HandleTypeDef* hmmc,
    uint8_t const*     pData,
    uint16_t           BlockAdd,
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_MMC_RPMB_ReadBlocks(
    MMC_HandleTypeDef* hmmc,
    uint8_t*           pData,
    uint16_t           BlockAdd,
    uint16_t           NumberOfBlocks,
