...
static uint32_t SAI_InterruptFlag( SAI_HandleTypeDef const* hsai, SAI_ModeTypedef mode );
static HAL_StatusTypeDef
SAI_InitI2S( SAI_HandleTypeDef* hsai, uint32_t protocol, uint32_t datasize, uint32_t nbslot );
static HAL_StatusTypeDef
SAI_InitPCM( SAI_HandleTypeDef* hsai, uint32_t protocol, uint32_t datasize, uint32_t nbslot );
...
HAL_StatusTypeDef HAL_SAI_Init( SAI_HandleTypeDef* hsai )
{
        ...
        hsai->Instance->CR1 &=
            ~( SAI_xCR1_MODE | SAI_xCR1_PRTCFG | SAI_xCR1_DS | SAI_xCR1_LSBFIRST | SAI_xCR1_CKSTR |
               SAI_xCR1_SYNCEN | SAI_xCR1_MONO | SAI_xCR1_OUTDRIV | SAI_xCR1_DMAEN |
               SAI_xCR1_NODIV | SAI_xCR1_MCKDIV | SAI_xCR1_OSR | SAI_xCR1_MCKEN );
        ...
        hsai->Instance->FRCR &=
            ( ~( SAI_xFRCR_FRL | SAI_xFRCR_FSALL | SAI_xFRCR_FSDEF | SAI_xFRCR_FSPOL |
                 SAI_xFRCR_FSOFF ) );
        hsai->Instance->FRCR |=
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_SAI_RegisterCallback(
    SAI_HandleTypeDef*        hsai,
    ...
    pSAI_CallbackTypeDef      pCallback )
{
        ...
        if ( pCallback == NULL ) {
                ...
                status = HAL_ERROR;
        } else if ( HAL_SAI_STATE_READY == hsai->State ) {
                switch ( CallbackID ) {
 ...
 */
HAL_StatusTypeDef
HAL_SAI_Transmit( SAI_HandleTypeDef* hsai, uint8_t* pData, uint16_t Size, uint32_t Timeout )
{
 ...
 */
HAL_StatusTypeDef
HAL_SAI_Receive( SAI_HandleTypeDef* hsai, uint8_t* pData, uint16_t Size, uint32_t Timeout )
{
        uint32_t tickstart = HAL_GetTick();
        uint32_t temp;
...
HAL_StatusTypeDef HAL_SAI_Transmit_DMA( SAI_HandleTypeDef* hsai, uint8_t* pData, uint16_t Size )
{
        ...
        if ( hsai->State == HAL_SAI_STATE_READY ) {
                ...
                /* For transmission, the DMA source is data buffer.
                   We have to compute DMA size of a source block transfer in bytes according SAI
                   data size. */
                if ( ( hsai->Init.DataSize == SAI_DATASIZE_8 ) &&
                     ( hsai->Init.CompandingMode == SAI_NOCOMPANDING ) ) {
 ...
 */
HAL_StatusTypeDef
HAL_SAI_EnableRxMuteMode( SAI_HandleTypeDef* hsai, SAIcallback callback, uint16_t counter )
{
...
void HAL_SAI_IRQHandler( SAI_HandleTypeDef* hsai )
{
        if ( hsai->State != HAL_SAI_STATE_RESET ) {
                ...
                /* SAI Overrun error interrupt occurred ----------------------------------*/
                    ...
                    ( ( itflags & SAI_FLAG_OVRUDR ) == SAI_FLAG_OVRUDR ) &&
                    ( ( itsources & SAI_IT_OVRUDR ) == SAI_IT_OVRUDR ) ) {
                        /* Clear the SAI Overrun flag */
                        __HAL_SAI_CLEAR_FLAG( hsai, SAI_FLAG_OVRUDR );
                        /* Get the SAI error code */
                        tmperror =
                            ( ( hsai->State == HAL_SAI_STATE_BUSY_RX ) ? HAL_SAI_ERROR_OVR :
                                                                         HAL_SAI_ERROR_UDR );
                ...
                /* SAI mutedet interrupt occurred ----------------------------------*/
                    ...
                    ( ( itflags & SAI_FLAG_MUTEDET ) == SAI_FLAG_MUTEDET ) &&
                ...
                /* SAI AFSDET interrupt occurred ----------------------------------*/
                    ...
                    ( ( itflags & SAI_FLAG_AFSDET ) == SAI_FLAG_AFSDET ) &&
                ...
                /* SAI LFSDET interrupt occurred ----------------------------------*/
                    ...
                    ( ( itflags & SAI_FLAG_LFSDET ) == SAI_FLAG_LFSDET ) &&
                ...
                /* SAI WCKCFG interrupt occurred ----------------------------------*/
                    ...
                    ( ( itflags & SAI_FLAG_WCKCFG ) == SAI_FLAG_WCKCFG ) &&
                ...
                /* SAI CNRDY interrupt occurred ----------------------------------*/
                    ...
                    ( ( itflags & SAI_FLAG_CNRDY ) == SAI_FLAG_CNRDY ) &&
 ...
 * @retval HAL state
 */
HAL_SAI_StateTypeDef HAL_SAI_GetState( SAI_HandleTypeDef const* hsai )
{
        return hsai->State;
}
 ...
 */
uint32_t HAL_SAI_GetError( SAI_HandleTypeDef const* hsai )
{
