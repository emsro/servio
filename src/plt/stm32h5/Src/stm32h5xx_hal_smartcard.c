...
#define USART_CR1_FIELDS                                                                          \
        ( (uint32_t) ( USART_CR1_M | USART_CR1_PCE | USART_CR1_PS | USART_CR1_TE | USART_CR1_RE | \
                       USART_CR1_OVER8 | USART_CR1_FIFOEN ) ) /*!< USART CR1 fields of parameters \
                                                                 set by SMARTCARD_SetConfig API */
...
#define USART_CR3_FIELDS                                                                           \
        ( (uint32_t) ( USART_CR3_ONEBIT | USART_CR3_NACK | USART_CR3_SCARCNT | USART_CR3_TXFTCFG | \
        ...
        if ( HAL_SMARTCARD_STATE_READY == hsmartcard->gState ) {
                ...
                case HAL_SMARTCARD_TX_FIFO_EMPTY_CB_ID:
                        hsmartcard->TxFifoEmptyCallback =
                            HAL_SMARTCARDEx_TxFifoEmptyCallback; /* Legacy weak TxFifoEmptyCallback
                                                                  */
                        break;
        ...
        if ( hsmartcard->gState == HAL_SMARTCARD_STATE_READY ) {
                ...
                while ( hsmartcard->TxXferCount > 0U ) {
                        ...
                        if ( SMARTCARD_WaitOnFlagUntilTimeout(
                                 hsmartcard, SMARTCARD_FLAG_TXE, RESET, tickstart, Timeout ) !=
                             HAL_OK ) {
                                return HAL_TIMEOUT;
                        }
    ...
    uint32_t                 Timeout )
{
        ...
        if ( hsmartcard->RxState == HAL_SMARTCARD_STATE_READY ) {
                ...
                while ( hsmartcard->RxXferCount > 0U ) {
                        ...
                        if ( SMARTCARD_WaitOnFlagUntilTimeout(
                                 hsmartcard, SMARTCARD_FLAG_RXNE, RESET, tickstart, Timeout ) !=
                             HAL_OK ) {
                                return HAL_TIMEOUT;
                        }
 ...
 */
HAL_StatusTypeDef
HAL_SMARTCARD_Receive_IT( SMARTCARD_HandleTypeDef* hsmartcard, uint8_t* pData, uint16_t Size )
{
        /* Check that a Rx process is not already ongoing */
        if ( hsmartcard->RxState == HAL_SMARTCARD_STATE_READY ) {
                if ( ( pData == NULL ) || ( Size == 0U ) )
                ...
                if ( ( hsmartcard->FifoMode == SMARTCARD_FIFOMODE_ENABLE ) &&
                     ( Size >= hsmartcard->NbRxDataToProcess ) ) {
                         ...
                         */
                        SET_BIT(
                            hsmartcard->Instance->CR1, USART_CR1_PEIE | USART_CR1_RXNEIE_RXFNEIE );
                }
 ...
 */
HAL_StatusTypeDef
HAL_SMARTCARD_Receive_DMA( SMARTCARD_HandleTypeDef* hsmartcard, uint8_t* pData, uint16_t Size )
{
        HAL_StatusTypeDef status;
        ...
        if ( HAL_IS_BIT_SET( hsmartcard->Instance->CR3, USART_CR3_DMAT ) ) {
                ...
                if ( hsmartcard->hdmatx != NULL ) {
                        ...
                        if ( HAL_DMA_Abort( hsmartcard->hdmatx ) != HAL_OK ) {
                                if ( HAL_DMA_GetError( hsmartcard->hdmatx ) ==
        ...
        if ( HAL_IS_BIT_SET( hsmartcard->Instance->CR3, USART_CR3_DMAR ) ) {
                ...
                if ( hsmartcard->hdmarx != NULL ) {
                        ...
                        if ( HAL_DMA_Abort( hsmartcard->hdmarx ) != HAL_OK ) {
                                if ( HAL_DMA_GetError( hsmartcard->hdmarx ) ==
                                     HAL_DMA_ERROR_TIMEOUT ) {
                                        /* Set error code to DMA */
                                        hsmartcard->ErrorCode = HAL_SMARTCARD_ERROR_DMA;
             ...
             ( ( ( ( cr3its & ( USART_CR3_RXFTIE | USART_CR3_EIE ) ) != 0U ) ||
                ...
                if ( hsmartcard->ErrorCode != HAL_SMARTCARD_ERROR_NONE ) {
                        ...
                        */
                        ...
                        if ( ( HAL_IS_BIT_SET( hsmartcard->Instance->CR3, USART_CR3_DMAR ) ) ||
                             ( ( errorcode &
                                 ( HAL_SMARTCARD_ERROR_RTO | HAL_SMARTCARD_ERROR_ORE ) ) != 0U ) ) {
 ...
 */
HAL_SMARTCARD_StateTypeDef HAL_SMARTCARD_GetState( SMARTCARD_HandleTypeDef const* hsmartcard )
{
 ...
 */
uint32_t HAL_SMARTCARD_GetError( SMARTCARD_HandleTypeDef const* hsmartcard )
{
...
static HAL_StatusTypeDef SMARTCARD_SetConfig( SMARTCARD_HandleTypeDef* hsmartcard )
{
        ...
        static uint16_t const        SMARTCARDPrescTable[12] = {
         ...
         * value */
        MODIFY_REG(
            hsmartcard->Instance->PRESC, USART_PRESC_PRESCALER, hsmartcard->Init.ClockPrescaler );

        /*-------------------------- USART GTPR Configuration ----------------------*/
        tmpreg =
            ( hsmartcard->Init.Prescaler |
              ( (uint32_t) hsmartcard->Init.GuardTime << USART_GTPR_GT_Pos ) );
        MODIFY_REG(
            hsmartcard->Instance->GTPR,
            (uint16_t) ( USART_GTPR_GT | USART_GTPR_PSC ),
            (uint16_t) tmpreg );
        ...
        case SMARTCARD_CLOCKSOURCE_PCLK1:
                ...
                tmpreg =
                    (uint32_t) ( ( ( pclk / SMARTCARDPrescTable[hsmartcard->Init.ClockPrescaler] ) +
        ...
        case SMARTCARD_CLOCKSOURCE_PLL2Q:
                ...
                tmpreg = (uint32_t) ( ( ( pll2_clocks.PLL2_Q_Frequency /
        ...
        case SMARTCARD_CLOCKSOURCE_PLL3Q:
                ...
                tmpreg = (uint32_t) ( ( ( pll3_clocks.PLL3_Q_Frequency /
                                          SMARTCARDPrescTable[hsmartcard->Init.ClockPrescaler] ) +
                                        ( hsmartcard->Init.BaudRate / 2U ) ) /
                                      hsmartcard->Init.BaudRate );
                break;
...
static void SMARTCARD_AdvFeatureConfig( SMARTCARD_HandleTypeDef* hsmartcard )
{
        ...
        if ( HAL_IS_BIT_SET(
                 hsmartcard->AdvancedInit.AdvFeatureInit, SMARTCARD_ADVFEATURE_TXINVERT_INIT ) ) {
                assert_param(
                    IS_SMARTCARD_ADVFEATURE_TXINV( hsmartcard->AdvancedInit.TxPinLevelInvert ) );
                MODIFY_REG(
                    hsmartcard->Instance->CR2,
                    USART_CR2_TXINV,
        ...
        if ( HAL_IS_BIT_SET(
                 hsmartcard->AdvancedInit.AdvFeatureInit, SMARTCARD_ADVFEATURE_RXINVERT_INIT ) ) {
                assert_param(
        ...
        if ( HAL_IS_BIT_SET(
                 hsmartcard->AdvancedInit.AdvFeatureInit, SMARTCARD_ADVFEATURE_DATAINVERT_INIT ) ) {
                assert_param(
        ...
        if ( HAL_IS_BIT_SET(
                 hsmartcard->AdvancedInit.AdvFeatureInit, SMARTCARD_ADVFEATURE_SWAP_INIT ) ) {
                assert_param( IS_SMARTCARD_ADVFEATURE_SWAP( hsmartcard->AdvancedInit.Swap ) );
                MODIFY_REG(
                    hsmartcard->Instance->CR2, USART_CR2_SWAP, hsmartcard->AdvancedInit.Swap );
        }
        ...
        if ( HAL_IS_BIT_SET(
                 hsmartcard->AdvancedInit.AdvFeatureInit, SMARTCARD_ADVFEATURE_MSBFIRST_INIT ) ) {
                assert_param(
        ...
        if ( hsmartcard->gState == HAL_SMARTCARD_STATE_BUSY_TX ) {
                for ( nb_tx_data = hsmartcard->NbTxDataToProcess; nb_tx_data > 0U; nb_tx_data-- ) {
                        if ( hsmartcard->TxXferCount == 0U ) {
                            ...
                            READ_BIT( hsmartcard->Instance->ISR, USART_ISR_TXE_TXFNF ) != 0U ) {
