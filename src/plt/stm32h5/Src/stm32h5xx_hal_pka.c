    ...
    *** Low level operation ***
    ...
    [..]
      ...
      (++) HAL_PKA_Arithmetic_GetResult() to retrieve the result of the operation.
            The resulting size can be the input parameter or the input parameter size + 1
  (overflow).
 ...
 * @{
 */
uint32_t PKA_GetMode( PKA_HandleTypeDef const* hpka );
HAL_StatusTypeDef
PKA_PollEndOfOperation( PKA_HandleTypeDef const* hpka, uint32_t Timeout, uint32_t Tickstart );
uint32_t          PKA_CheckError( PKA_HandleTypeDef const* hpka, uint32_t mode );
...
void              PKA_Memcpy_u8_to_u32( __IO uint32_t dst[], uint8_t const src[], size_t n );
...
void PKA_MontgomeryParam_Set( PKA_HandleTypeDef* hpka, uint32_t const size, uint8_t const* pOp1 );
void PKA_ARI_Set(
...
HAL_StatusTypeDef PKA_WaitOnFlagUntilTimeout(
    PKA_HandleTypeDef* hpka,
    uint32_t           Flag,
    FlagStatus         Status,
    uint32_t           Tickstart,
    uint32_t           Timeout );
uint32_t PKA_Result_GetSize( PKA_HandleTypeDef const* hpka, uint32_t Startindex, uint32_t Maxsize );
...
HAL_StatusTypeDef HAL_PKA_Init( PKA_HandleTypeDef* hpka )
{
        ...
        if ( hpka != NULL ) {
                ...
                if ( PKA_WaitOnFlagUntilTimeout( hpka, PKA_SR_INITOK, RESET, tickstart, 5000 ) !=
                     HAL_OK ) {
                        return HAL_TIMEOUT;
                }
                ...
                SET_BIT(
                    hpka->Instance->CLRFR,
                    PKA_CLRFR_PROCENDFC | PKA_CLRFR_RAMERRFC | PKA_CLRFR_ADDRERRFC |
                        PKA_CLRFR_OPERRFC );
...
HAL_StatusTypeDef HAL_PKA_DeInit( PKA_HandleTypeDef* hpka )
{
        ...
        if ( hpka != NULL ) {
                ...
                /* This abort any operation in progress (PKA RAM content is not guaranteed in this
                 * case) */
                hpka->Instance->CR = 0;
...
__weak void HAL_PKA_MspDeInit( PKA_HandleTypeDef* hpka )
{
        ...
        /* Wait the INITOK flag Setting */
        if ( PKA_WaitOnFlagUntilTimeout(
                 hpka, PKA_SR_INITOK, RESET, tickstart, PKA_RAM_ERASE_TIMEOUT ) != HAL_OK ) {
                /* update the state */
                hpka->State = HAL_PKA_STATE_ERROR;
        }
 ...
 */
HAL_StatusTypeDef
HAL_PKA_UnRegisterCallback( PKA_HandleTypeDef* hpka, HAL_PKA_CallbackIDTypeDef CallbackID )
{
        HAL_StatusTypeDef status = HAL_OK;
 ...
 ===============================================================================
    ...
    (#) There are two modes of operation:
       ...
       (++) No-Blocking mode : The operation is performed using Interrupts.
            ...
            The end of the operation is indicated by HAL_PKA_OperationCpltCallback in case of
success. To stop any operation in interrupt mode, use HAL_PKA_Abort().
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ModExp( PKA_HandleTypeDef* hpka, PKA_ModExpInTypeDef* in, uint32_t Timeout )
{
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ModExpFastMode( PKA_HandleTypeDef* hpka, PKA_ModExpFastModeInTypeDef* in, uint32_t Timeout )
{
        /* Set input parameter in PKA RAM */
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ModExpFastMode_IT( PKA_HandleTypeDef* hpka, PKA_ModExpFastModeInTypeDef* in )
{
        /* Set input parameter in PKA RAM */
        PKA_ModExpFastMode_Set( hpka, in );
        hpka->opsize = in->OpSize;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_PKA_ModExpProtectMode(
    PKA_HandleTypeDef*              hpka,
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ModExpProtectMode_IT( PKA_HandleTypeDef* hpka, PKA_ModExpProtectModeInTypeDef* in )
{
        /* Set input parameter in PKA RAM */
        PKA_ModExpProtectMode_Set( hpka, in );
        hpka->opsize = in->OpSize;
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ECDSASign( PKA_HandleTypeDef* hpka, PKA_ECDSASignInTypeDef* in, uint32_t Timeout )
{
        /* Set input parameter in PKA RAM */
...
/**
 * @brief  Sign a message using elliptic curves over prime fields in non-blocking mode with
 * Interrupt.
 * @param  hpka PKA handle
 * @param  in Input information
 * @retval HAL status
 ...
 */
void HAL_PKA_ECDSASign_GetResult(
    PKA_HandleTypeDef*               hpka,
    ...
    PKA_ECDSASignOutExtParamTypeDef* outExt )
{
        ...
        if ( outExt != NULL ) {
                ...
                PKA_Memcpy_u32_to_u8(
                    outExt->ptX, &hpka->Instance->RAM[PKA_ECDSA_SIGN_OUT_FINAL_POINT_X], size );
                PKA_Memcpy_u32_to_u8(
                    outExt->ptY, &hpka->Instance->RAM[PKA_ECDSA_SIGN_OUT_FINAL_POINT_Y], size );
        }
}

/**
 * @brief  Verify the validity of a signature using elliptic curves over prime fields in blocking
 * mode.
 * @param  hpka PKA handle
 * @param  in Input information
 * @param  Timeout Timeout duration
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ECDSAVerif( PKA_HandleTypeDef* hpka, PKA_ECDSAVerifInTypeDef* in, uint32_t Timeout )
{
        /* Set input parameter in PKA RAM */
 ...
 */
HAL_StatusTypeDef
HAL_PKA_RSACRTExp( PKA_HandleTypeDef* hpka, PKA_RSACRTExpInTypeDef* in, uint32_t Timeout )
{
        /* Set input parameter in PKA RAM */
 ...
 */
HAL_StatusTypeDef
HAL_PKA_PointCheck( PKA_HandleTypeDef* hpka, PKA_PointCheckInTypeDef* in, uint32_t Timeout )
{
...
#define PKA_POINT_IS_ON_CURVE 0xD60DUL
        ...
        return ( hpka->Instance->RAM[PKA_POINT_CHECK_OUT_ERROR] == PKA_POINT_IS_ON_CURVE ) ? 1UL :
                                                                                             0UL;
}
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ECCMul( PKA_HandleTypeDef* hpka, PKA_ECCMulInTypeDef* in, uint32_t Timeout )
{
        /* Set input parameter in PKA RAM */
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ECCMulEx( PKA_HandleTypeDef* hpka, PKA_ECCMulExInTypeDef* in, uint32_t Timeout )
{
        /* Set input parameter in PKA RAM */
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ModAdd( PKA_HandleTypeDef* hpka, PKA_ModAddInTypeDef* in, uint32_t Timeout )
{
        /* Set input parameter in PKA RAM */
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ModInv( PKA_HandleTypeDef* hpka, PKA_ModInvInTypeDef* in, uint32_t Timeout )
{
        /* Set input parameter in PKA RAM */
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ModSub( PKA_HandleTypeDef* hpka, PKA_ModSubInTypeDef* in, uint32_t Timeout )
{
        /* Set input parameter in PKA RAM */
        PKA_ARI_Set( hpka, in->size, in->pOp1, in->pOp2, in->pOp3 );
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ModRed( PKA_HandleTypeDef* hpka, PKA_ModRedInTypeDef* in, uint32_t Timeout )
{
        /* Set input parameter in PKA RAM */
 ...
 */
HAL_StatusTypeDef
HAL_PKA_MontgomeryMul( PKA_HandleTypeDef* hpka, PKA_MontgomeryMulInTypeDef* in, uint32_t Timeout )
{
        /* Set input parameter in PKA RAM */
        PKA_ARI_Set( hpka, in->size, in->pOp1, in->pOp2, in->pOp3 );
 ...
 */
HAL_StatusTypeDef
HAL_PKA_MontgomeryMul_IT( PKA_HandleTypeDef* hpka, PKA_MontgomeryMulInTypeDef* in )
{
        /* Set input parameter in PKA RAM */
        PKA_ARI_Set( hpka, in->size, in->pOp1, in->pOp2, in->pOp3 );
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_PKA_MontgomeryParam(
    PKA_HandleTypeDef*            hpka,
 ...
 */
HAL_StatusTypeDef
HAL_PKA_MontgomeryParam_IT( PKA_HandleTypeDef* hpka, PKA_MontgomeryParamInTypeDef* in )
{
        /* Set input parameter in PKA RAM */
        PKA_MontgomeryParam_Set( hpka, in->size, in->pOp1 );
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_PKA_ECCDoubleBaseLadder(
    PKA_HandleTypeDef*                hpka,
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ECCDoubleBaseLadder_IT( PKA_HandleTypeDef* hpka, PKA_ECCDoubleBaseLadderInTypeDef* in )
{
        /* Set input parameter in PKA RAM */
        PKA_ECCDoubleBaseLadder_Set( hpka, in );
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ECCProjective2Affine_IT( PKA_HandleTypeDef* hpka, PKA_ECCProjective2AffineInTypeDef* in )
{
        /* Set input parameter in PKA RAM */
        PKA_ECCProjective2Affine_Set( hpka, in );
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_PKA_ECCCompleteAddition(
    PKA_HandleTypeDef*                hpka,
 ...
 */
HAL_StatusTypeDef
HAL_PKA_ECCCompleteAddition_IT( PKA_HandleTypeDef* hpka, PKA_ECCCompleteAdditionInTypeDef* in )
{
        /* Set input parameter in PKA RAM */
        PKA_ECCCompleteAddition_Set( hpka, in );
 ...
 * @retval HAL status
 */
HAL_PKA_StateTypeDef HAL_PKA_GetState( PKA_HandleTypeDef const* hpka )
{
 ...
 */
uint32_t HAL_PKA_GetError( PKA_HandleTypeDef const* hpka )
{
...
#define EDCSA_SIGN_NOERROR PKA_NO_ERROR
                /* If error output result is different from no error, ecsa sign operation need to be
                 * repeated */
        ...
        if ( mode == PKA_MODE_DOUBLE_BASE_LADDER ) {
                /* If error output result is different from no error, PKA operation need to be
                 * repeated */
                if ( hpka->Instance->RAM[PKA_ECC_DOUBLE_LADDER_OUT_ERROR] != PKA_NO_ERROR )
                        err |= HAL_PKA_ERROR_OPERATION;
        }
...
uint32_t PKA_GetBitSize_u32( uint32_t wordNumber )
{
        /* Convert from number of uint32_t in an array to the associated number of bits in this
         * array */
        ...
        if ( dst != NULL ) {
                if ( src != NULL ) {
                        ...
                        for ( ; index_uint32_t < ( n / 4UL ); index_uint32_t++ ) {
                                /* Avoid casting from uint8_t* to uint32_t* by copying 4 uint8_t in
                                 * a row */
...
void PKA_ModExp_Set( PKA_HandleTypeDef* hpka, PKA_ModExpInTypeDef* in )
{
        ...
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_MODULAR_EXP_IN_EXPONENT_BASE], in->pOp1, in->OpSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_MODULAR_EXP_IN_EXPONENT_BASE + ( ( in->OpSize + 3UL ) / 4UL ) );

        /* Move the exponent to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_MODULAR_EXP_IN_EXPONENT], in->pExp, in->expSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_MODULAR_EXP_IN_EXPONENT + ( ( in->expSize + 3UL ) / 4UL ) );

        /* Move the modulus to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_MODULAR_EXP_IN_MODULUS], in->pMod, in->OpSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_MODULAR_EXP_IN_MODULUS + ( ( in->OpSize + 3UL ) / 4UL ) );
}
...
void PKA_ModExpFastMode_Set( PKA_HandleTypeDef* hpka, PKA_ModExpFastModeInTypeDef* in )
{
        ...
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_MODULAR_EXP_IN_EXPONENT_BASE], in->pOp1, in->OpSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_MODULAR_EXP_IN_EXPONENT_BASE + ( in->OpSize / 4UL ) );

        /* Move the exponent to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_MODULAR_EXP_IN_EXPONENT], in->pExp, in->expSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_MODULAR_EXP_IN_EXPONENT + ( in->expSize / 4UL ) );

        /* Move the modulus to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_MODULAR_EXP_IN_MODULUS], in->pMod, in->OpSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_MODULAR_EXP_IN_MODULUS + ( in->OpSize / 4UL ) );
        ...
        PKA_Memcpy_u32_to_u32(
            ...
            in->OpSize / 4UL );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_MODULAR_EXP_IN_MONTGOMERY_PARAM + ( in->OpSize / 4UL ) );
}
...
void PKA_ModExpProtectMode_Set( PKA_HandleTypeDef* hpka, PKA_ModExpProtectModeInTypeDef* in )
{
        ...
        /* Move the input parameters pOp1 to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_MODULAR_EXP_PROTECT_IN_EXPONENT_BASE], in->pOp1, in->OpSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_MODULAR_EXP_PROTECT_IN_EXPONENT_BASE + ( in->OpSize / 4UL ) );

        /* Move the exponent to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_MODULAR_EXP_PROTECT_IN_EXPONENT], in->pExp, in->expSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_MODULAR_EXP_PROTECT_IN_EXPONENT + ( in->expSize / 4UL ) );

        /* Move the modulus to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_MODULAR_EXP_PROTECT_IN_MODULUS], in->pMod, in->OpSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_MODULAR_EXP_PROTECT_IN_MODULUS + ( in->OpSize / 4UL ) );

        /* Move Phi value to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_MODULAR_EXP_PROTECT_IN_PHI], in->pPhi, in->OpSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_MODULAR_EXP_PROTECT_IN_PHI + ( in->OpSize / 4UL ) );
}
 ...
 * @param  hpka PKA handle
 * @param  in Input information
 * @note   If the modulus size is bigger than the hash size (with a curve SECP521R1 when using a
 * SHA256 hash for example)the hash value should be written at the end of the buffer with zeros
...
void PKA_ECDSASign_Set( PKA_HandleTypeDef* hpka, PKA_ECDSASignInTypeDef* in )
{
        ...
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_SIGN_IN_A_COEFF], in->coef, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECDSA_SIGN_IN_A_COEFF + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters coefficient B to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_SIGN_IN_B_COEFF], in->coefB, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECDSA_SIGN_IN_B_COEFF + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters modulus value p to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_SIGN_IN_MOD_GF], in->modulus, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECDSA_SIGN_IN_MOD_GF + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters integer k to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_SIGN_IN_K], in->integer, in->primeOrderSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECDSA_SIGN_IN_K + ( ( in->primeOrderSize + 3UL ) / 4UL ) );

        /* Move the input parameters base point G coordinate x to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_SIGN_IN_INITIAL_POINT_X],
            in->basePointX,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECDSA_SIGN_IN_INITIAL_POINT_X + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters base point G coordinate y to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_SIGN_IN_INITIAL_POINT_Y],
            in->basePointY,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECDSA_SIGN_IN_INITIAL_POINT_Y + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters hash of message z to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_SIGN_IN_HASH_E], in->hash, in->primeOrderSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECDSA_SIGN_IN_HASH_E + ( ( in->primeOrderSize + 3UL ) / 4UL ) );

        /* Move the input parameters private key d to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_SIGN_IN_PRIVATE_KEY_D],
            in->privateKey,
            in->primeOrderSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECDSA_SIGN_IN_PRIVATE_KEY_D + ( ( in->primeOrderSize + 3UL ) / 4UL ) );

        /* Move the input parameters prime order n to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_SIGN_IN_ORDER_N], in->primeOrder, in->primeOrderSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECDSA_SIGN_IN_ORDER_N + ( ( in->primeOrderSize + 3UL ) / 4UL ) );
}
...
void PKA_ECDSAVerif_Set( PKA_HandleTypeDef* hpka, PKA_ECDSAVerifInTypeDef* in )
{
        ...
        /* Move the input parameters coefficient |a| to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_VERIF_IN_A_COEFF], in->coef, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECDSA_VERIF_IN_A_COEFF + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters modulus value p to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_VERIF_IN_MOD_GF], in->modulus, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECDSA_VERIF_IN_MOD_GF + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters base point G coordinate x to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_VERIF_IN_INITIAL_POINT_X],
            in->basePointX,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECDSA_VERIF_IN_INITIAL_POINT_X + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters base point G coordinate y to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_VERIF_IN_INITIAL_POINT_Y],
            in->basePointY,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECDSA_VERIF_IN_INITIAL_POINT_Y + ( ( in->modulusSize + 3UL ) / 4UL ) );
        ...
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_VERIF_IN_SIGNATURE_R], in->RSign, in->primeOrderSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECDSA_VERIF_IN_SIGNATURE_R + ( ( in->primeOrderSize + 3UL ) / 4UL ) );

        /* Move the input parameters signature part s to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_VERIF_IN_SIGNATURE_S], in->SSign, in->primeOrderSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECDSA_VERIF_IN_SIGNATURE_S + ( ( in->primeOrderSize + 3UL ) / 4UL ) );

        /* Move the input parameters hash of message z to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_VERIF_IN_HASH_E], in->hash, in->primeOrderSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECDSA_VERIF_IN_HASH_E + ( ( in->primeOrderSize + 3UL ) / 4UL ) );

        /* Move the input parameters curve prime order n to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECDSA_VERIF_IN_ORDER_N], in->primeOrder, in->primeOrderSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECDSA_VERIF_IN_ORDER_N + ( ( in->primeOrderSize + 3UL ) / 4UL ) );
}
...
void PKA_RSACRTExp_Set( PKA_HandleTypeDef* hpka, PKA_RSACRTExpInTypeDef* in )
{
        ...
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_RSA_CRT_EXP_IN_DP_CRT], in->pOpDp, in->size / 2UL );
        ...
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_RSA_CRT_EXP_IN_DQ_CRT], in->pOpDq, in->size / 2UL );
        ...
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_RSA_CRT_EXP_IN_QINV_CRT], in->pOpQinv, in->size / 2UL );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_RSA_CRT_EXP_IN_QINV_CRT + ( in->size / 8UL ) );

        /* Move the input parameters prime p to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_RSA_CRT_EXP_IN_PRIME_P], in->pPrimeP, in->size / 2UL );
        __PKA_RAM_PARAM_END( hpka->Instance->RAM, PKA_RSA_CRT_EXP_IN_PRIME_P + ( in->size / 8UL ) );

        /* Move the input parameters prime q to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_RSA_CRT_EXP_IN_PRIME_Q], in->pPrimeQ, in->size / 2UL );
        ...
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_RSA_CRT_EXP_IN_EXPONENT_BASE], in->popA, in->size );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_RSA_CRT_EXP_IN_EXPONENT_BASE + ( in->size / 4UL ) );
}
...
void PKA_PointCheck_Set( PKA_HandleTypeDef* hpka, PKA_PointCheckInTypeDef* in )
{
        ...
        /* Move the input parameters coefficient |a| to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_POINT_CHECK_IN_A_COEFF], in->coefA, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_POINT_CHECK_IN_A_COEFF + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters coefficient b to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_POINT_CHECK_IN_B_COEFF], in->coefB, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_POINT_CHECK_IN_B_COEFF + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters modulus value p to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_POINT_CHECK_IN_MOD_GF], in->modulus, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_POINT_CHECK_IN_MOD_GF + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters Point P coordinate x to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_POINT_CHECK_IN_INITIAL_POINT_X], in->pointX, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_POINT_CHECK_IN_INITIAL_POINT_X + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters Point P coordinate y to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_POINT_CHECK_IN_INITIAL_POINT_Y], in->pointY, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_POINT_CHECK_IN_INITIAL_POINT_Y + ( ( in->modulusSize + 3UL ) / 4UL ) );
        ...
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_POINT_CHECK_IN_MONTGOMERY_PARAM + ( ( in->modulusSize + 3UL ) / 4UL ) );
}
...
void PKA_ECCMul_Set( PKA_HandleTypeDef* hpka, PKA_ECCMulInTypeDef* in )
{
        ...
        /* Move the input parameters coefficient |a| to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_SCALAR_MUL_IN_A_COEFF], in->coefA, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECC_SCALAR_MUL_IN_A_COEFF + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters coefficient b to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_SCALAR_MUL_IN_B_COEFF], in->coefB, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECC_SCALAR_MUL_IN_B_COEFF + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters modulus value p to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_SCALAR_MUL_IN_MOD_GF], in->modulus, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECC_SCALAR_MUL_IN_MOD_GF + ( ( in->modulusSize + 3UL ) / 4UL ) );

        /* Move the input parameters scalar multiplier k to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_SCALAR_MUL_IN_K], in->scalarMul, in->scalarMulSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_SCALAR_MUL_IN_K + ( ( in->scalarMulSize + 3UL ) / 4UL ) );

        /* Move the input parameters Point P coordinate x to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_SCALAR_MUL_IN_INITIAL_POINT_X],
            in->pointX,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
        ...
        /* Move the input parameters Point P coordinate y to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_SCALAR_MUL_IN_INITIAL_POINT_Y],
            in->pointY,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
        ...
        /* Move the input parameters curve prime order N to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_SCALAR_MUL_IN_N_PRIME_ORDER],
            in->primeOrder,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM,
            PKA_ECC_SCALAR_MUL_IN_N_PRIME_ORDER + ( ( in->modulusSize + 3UL ) / 4UL ) );
}
...
void PKA_ModInv_Set( PKA_HandleTypeDef* hpka, PKA_ModInvInTypeDef* in )
{
        ...
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_MODULAR_INV_IN_OP2_MOD], in->pMod, in->size * 4UL );
...
void PKA_ModRed_Set( PKA_HandleTypeDef* hpka, PKA_ModRedInTypeDef* in )
{
        ...
        /* Move the input parameters operand A to PKA RAM */
        PKA_Memcpy_u32_to_u32(
            &hpka->Instance->RAM[PKA_MODULAR_REDUC_IN_OPERAND], in->pOp1, in->OpSize );
        __PKA_RAM_PARAM_END( hpka->Instance->RAM, PKA_MODULAR_REDUC_IN_OPERAND + in->OpSize );

        /* Move the input parameters modulus value n to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_MODULAR_REDUC_IN_MODULUS], in->pMod, in->modSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_MODULAR_REDUC_IN_MODULUS + ( ( in->modSize + 3UL ) / 4UL ) );
}
...
void PKA_MontgomeryParam_Set( PKA_HandleTypeDef* hpka, uint32_t const size, uint8_t const* pOp1 )
{
        ...
        if ( pOp1 != NULL ) {
                ...
                /* Move the input parameters pOp1 to PKA RAM */
                PKA_Memcpy_u8_to_u32(
                    &hpka->Instance->RAM[PKA_MONTGOMERY_PARAM_IN_MODULUS], pOp1, size );
                __PKA_RAM_PARAM_END(
                    hpka->Instance->RAM,
...
void PKA_ECCDoubleBaseLadder_Set( PKA_HandleTypeDef* hpka, PKA_ECCDoubleBaseLadderInTypeDef* in )
{
        ...
        /* Move the input parameters coefficient |a| to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_DOUBLE_LADDER_IN_A_COEFF], in->coefA, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_DOUBLE_LADDER_IN_A_COEFF + ( in->modulusSize / 4UL ) );

        /* Move the input parameters modulus value p to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_DOUBLE_LADDER_IN_MOD_P], in->modulus, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_DOUBLE_LADDER_IN_MOD_P + ( in->modulusSize / 4UL ) );

        /* Move the input parameters integer k to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_DOUBLE_LADDER_IN_K_INTEGER],
            in->integerK,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_DOUBLE_LADDER_IN_K_INTEGER + ( in->modulusSize / 4UL ) );

        /* Move the input parameters integer m to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_DOUBLE_LADDER_IN_M_INTEGER],
            in->integerM,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_DOUBLE_LADDER_IN_M_INTEGER + ( in->modulusSize / 4UL ) );

        /* Move the input parameters first point coordinate x to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_DOUBLE_LADDER_IN_POINT1_X],
            in->basePointX1,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_DOUBLE_LADDER_IN_POINT1_X + ( in->modulusSize / 4UL ) );

        /* Move the input parameters first point  coordinate y to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_DOUBLE_LADDER_IN_POINT1_Y],
            in->basePointY1,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_DOUBLE_LADDER_IN_POINT1_Y + ( in->modulusSize / 4UL ) );

        /* Move the input parameters first point  coordinate z to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_DOUBLE_LADDER_IN_POINT1_Z],
            in->basePointZ1,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_DOUBLE_LADDER_IN_POINT1_Z + ( in->modulusSize / 4UL ) );

        /* Move the input parameters second point coordinate x to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_DOUBLE_LADDER_IN_POINT2_X],
            in->basePointX2,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_DOUBLE_LADDER_IN_POINT2_X + ( in->modulusSize / 4UL ) );

        /* Move the input parameters second point  coordinate y to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_DOUBLE_LADDER_IN_POINT2_Y],
            in->basePointY2,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_DOUBLE_LADDER_IN_POINT2_Y + ( in->modulusSize / 4UL ) );

        /* Move the input parameters second point  coordinate z to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_DOUBLE_LADDER_IN_POINT2_Z],
            in->basePointZ2,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_DOUBLE_LADDER_IN_POINT2_Z + ( in->modulusSize / 4UL ) );
}
    ...
    PKA_ECCDoubleBaseLadderOutTypeDef* out )
{
        ...
        if ( out != NULL ) {
                PKA_Memcpy_u32_to_u8(
                    out->ptX, &hpka->Instance->RAM[PKA_ECC_DOUBLE_LADDER_OUT_RESULT_X], size );
                PKA_Memcpy_u32_to_u8(
                    out->ptY, &hpka->Instance->RAM[PKA_ECC_DOUBLE_LADDER_OUT_RESULT_Y], size );
        }
}
...
void PKA_ECCProjective2Affine_Set( PKA_HandleTypeDef* hpka, PKA_ECCProjective2AffineInTypeDef* in )
{
        ...
        /* Move the input parameters modulus value p to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_PROJECTIVE_AFF_IN_MOD_P], in->modulus, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_PROJECTIVE_AFF_IN_MOD_P + ( in->modulusSize / 4UL ) );

        /* Move the input parameters point coordinate x to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_PROJECTIVE_AFF_IN_POINT_X],
            in->basePointX,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_PROJECTIVE_AFF_IN_POINT_X + ( in->modulusSize / 4UL ) );

        /* Move the input parameters point coordinate y to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_PROJECTIVE_AFF_IN_POINT_Y],
            in->basePointY,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_PROJECTIVE_AFF_IN_POINT_Y + ( in->modulusSize / 4UL ) );

        /* Move the input parameters point coordinate z to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_PROJECTIVE_AFF_IN_POINT_Z],
            in->basePointZ,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_PROJECTIVE_AFF_IN_POINT_Z + ( in->modulusSize / 4UL ) );
    ...
    PKA_ECCProjective2AffineOutTypeDef* out )
{
        ...
        if ( out != NULL ) {
                PKA_Memcpy_u32_to_u8(
                    out->ptX, &hpka->Instance->RAM[PKA_ECC_PROJECTIVE_AFF_OUT_RESULT_X], size );
                PKA_Memcpy_u32_to_u8(
                    out->ptY, &hpka->Instance->RAM[PKA_ECC_PROJECTIVE_AFF_OUT_RESULT_Y], size );
        }
}
...
void PKA_ECCCompleteAddition_Set( PKA_HandleTypeDef* hpka, PKA_ECCCompleteAdditionInTypeDef* in )
{
        ...
        /* Move the input parameters modulus value p to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_COMPLETE_ADD_IN_MOD_P], in->modulus, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_COMPLETE_ADD_IN_MOD_P + ( in->modulusSize / 4UL ) );

        /* Move the input parameters coefA value to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_COMPLETE_ADD_IN_A_COEFF], in->coefA, in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_COMPLETE_ADD_IN_A_COEFF + ( in->modulusSize / 4UL ) );

        /* Move the input parameters first point x value  to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_COMPLETE_ADD_IN_POINT1_X],
            in->basePointX1,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_COMPLETE_ADD_IN_POINT1_X + ( in->modulusSize / 4UL ) );

        /* Move the input parameters first point y value  to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_COMPLETE_ADD_IN_POINT1_Y],
            in->basePointY1,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_COMPLETE_ADD_IN_POINT1_Y + ( in->modulusSize / 4UL ) );

        /* Move the input parameters first point z value  to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_COMPLETE_ADD_IN_POINT1_Z],
            in->basePointZ1,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_COMPLETE_ADD_IN_POINT1_Z + ( in->modulusSize / 4UL ) );

        /* Move the input parameters second point x value  to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_COMPLETE_ADD_IN_POINT2_X],
            in->basePointX2,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_COMPLETE_ADD_IN_POINT2_X + ( in->modulusSize / 4UL ) );

        /* Move the input parameters second point y value  to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_COMPLETE_ADD_IN_POINT2_Y],
            in->basePointY2,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_COMPLETE_ADD_IN_POINT2_Y + ( in->modulusSize / 4UL ) );

        /* Move the input parameters second point z value  to PKA RAM */
        PKA_Memcpy_u8_to_u32(
            &hpka->Instance->RAM[PKA_ECC_COMPLETE_ADD_IN_POINT2_Z],
            in->basePointZ2,
            in->modulusSize );
        __PKA_RAM_PARAM_END(
            hpka->Instance->RAM, PKA_ECC_COMPLETE_ADD_IN_POINT2_Z + ( in->modulusSize / 4UL ) );
}
...
void HAL_PKA_ECCCompleteAddition_GetResult(
    PKA_HandleTypeDef*                 hpka,
    PKA_ECCCompleteAdditionOutTypeDef* out )
{
        uint32_t size;
        ...
        if ( out != NULL ) {
                PKA_Memcpy_u32_to_u8(
                    out->ptX, &hpka->Instance->RAM[PKA_ECC_COMPLETE_ADD_OUT_RESULT_X], size );
                PKA_Memcpy_u32_to_u8(
                    out->ptY, &hpka->Instance->RAM[PKA_ECC_COMPLETE_ADD_OUT_RESULT_Y], size );
                PKA_Memcpy_u32_to_u8(
                    out->ptZ, &hpka->Instance->RAM[PKA_ECC_COMPLETE_ADD_OUT_RESULT_Z], size );
        }
}
    ...
    uint8_t const*     pOp3 )
{
        ...
        if ( pOp1 != NULL ) {
                /* Move the input parameters pOp1 to PKA RAM */
                PKA_Memcpy_u32_to_u32(
                    &hpka->Instance->RAM[PKA_ARITHMETIC_ALL_OPS_IN_OP1], pOp1, size );
        ...
        if ( pOp2 != NULL ) {
                ...
                PKA_Memcpy_u32_to_u32(
                    &hpka->Instance->RAM[PKA_ARITHMETIC_ALL_OPS_IN_OP2], pOp2, size );
        ...
        if ( pOp3 != NULL ) {
                ...
                PKA_Memcpy_u8_to_u32(
                    &hpka->Instance->RAM[PKA_ARITHMETIC_ALL_OPS_IN_OP3], pOp3, size * 4UL );
