  ...
  ******************************************************************************
  ...
  __HAL_RCC_AES_CLK_ENABLE
      ...
      (#)Initialize the CRYP according to the specified parameters :
         ...
         (##) The HeaderSize used to give size of header buffer in word or bytes, depending upon
  HeaderWidthUnit field.
         ...
         (##) The KeyMode used to special key operation modes (for SAES : wrapped key, shared key
  with AES peripheral).
  ...
  HAL_CRYP_Decrypt_IT(), HAL_CRYP_Encrypt_DMA() or HAL_CRYP_Decrypt_DMA().
    ...
    [..] If the key is defined as a 128-bit long array key[127..0] = {b127 ... b0} where
     ...
     [..] Hereafter, another illustration when considering a 128-bit long key made of 16 bytes
  {B15..B0}. The 4 32-bit words that make the key must be stored as follows in MCU memory:
    ...
    [..]  This section describes the AES Galois/counter mode (GCM) supported by the peripherals:
      ...
      (#)  Four phases are performed in GCM :
         ...
         (##) Header phase: peripheral processes the Additional Authenticated Data (AAD), with hash
          computation only.
         (##) Payload phase: peripheral processes the plaintext (P) with hash computation +
  keystream encryption + data XORing. It works in a similar way for ciphertext (C).
         (##) Final phase: peripheral generates the authenticated tag (T) using the last block of
  data. HAL_CRYPEx_AESGCM_GenerateAuthTAG API used in this phase to generate 4 words which
  correspond to the Tag. user should consider only part of this 4 words, if Tag length is less than
  128 bits.
    ...
    [..]  A detailed description of the CCM message structure is available below.
      ...
      (#)    CCM in peripheral:
         ...
         (##) For authentication two phases are performed :
          - Header phase: peripheral processes the Additional Authenticated Data (AAD) first, then
  the cleartext message only cleartext payload (not the ciphertext payload) is used and no output.
         (##) Final phase: peripheral generates the authenticated tag (T) using the last block of
  data. HAL_CRYPEx_AESCCM_GenerateAuthTAG API used in this phase to generate 4 words which
  correspond to the Tag. user should consider only part of this 4 words, if Tag length is less than
  128 bits
...
#define CRYP_OPERATINGMODE_KEYDERIVATION                                                          \
        AES_CR_MODE_0 /*!< Key derivation mode  only used when performing ECB and CBC decryptions \
                         (Mode 2) */
...
static void CRYP_SetDMAConfig(
    CRYP_HandleTypeDef* hcryp,
    uint32_t            inputaddr,
    uint16_t            Size,
    uint32_t            outputaddr );
static HAL_StatusTypeDef
            CRYP_SetHeaderDMAConfig( CRYP_HandleTypeDef* hcryp, uint32_t inputaddr, uint16_t Size );
...
static HAL_StatusTypeDef
CRYP_WaitFLAG( CRYP_HandleTypeDef* hcryp, uint32_t flag, FlagStatus Status, uint32_t Timeout );
...
#if ( USE_HAL_CRYP_SUSPEND_RESUME == 1U )
static void CRYP_Read_IVRegisters( CRYP_HandleTypeDef* hcryp, uint32_t const* Output );
static void CRYP_Write_IVRegisters( CRYP_HandleTypeDef* hcryp, uint32_t const* Input );
static void CRYP_Read_SuspendRegisters( CRYP_HandleTypeDef* hcryp, uint32_t const* Output );
static void CRYP_Write_SuspendRegisters( CRYP_HandleTypeDef* hcryp, uint32_t const* Input );
static void
CRYP_Read_KeyRegisters( CRYP_HandleTypeDef* hcryp, uint32_t const* Output, uint32_t KeySize );
static void
CRYP_Write_KeyRegisters( CRYP_HandleTypeDef* hcryp, uint32_t const* Input, uint32_t KeySize );
...
CRYP_HandleTypeDef
      (+) For interleave mode, API HAL_CRYP_SaveContext and HAL_CRYP_RestoreContext to be used to
save then Restore CRYP configuration and parameters. CRYP_IVCONFIG_ONCE should be selected for
KeyIVConfigSkip parameter. Only polling mode is supported, interleave mode should be used with
...
HAL_StatusTypeDef HAL_CRYP_Init( CRYP_HandleTypeDef* hcryp )
{
        ...
        if ( hcryp->Instance == AES ) {
                ...
                MODIFY_REG(
                    hcryp->Instance->CR,
                    AES_CR_KMOD | AES_CR_DATATYPE | AES_CR_KEYSIZE | AES_CR_CHMOD | AES_CR_KEYSEL |
                        AES_CR_KEYPROT,
                    cr_value );
        }
                ...
                if ( hcryp->Instance == AES ) {
                        ...
                        if ( ( hcryp->Init.KeySelect != CRYP_KEYSEL_NORMAL ) &&
                             ( hcryp->Init.KeyMode == CRYP_KEYMODE_WRAPPED ) ) {
                                ...
                                /* Check the busy flag before writing CR register */
                                if ( CRYP_WaitFLAG(
                                         hcryp, AES_SR_BUSY, SET, CRYP_GENERAL_TIMEOUT ) !=
                                     HAL_OK ) {
                                        hcryp->State = HAL_CRYP_STATE_READY;
                                        __HAL_UNLOCK( hcryp );
                                        return HAL_ERROR;
                                ...
                                MODIFY_REG(
                                    hcryp->Instance->CR, AES_CR_KMOD, CRYP_KEYMODE_WRAPPED );
                        }
                        /* Check the busy flag before writing CR register */
                        if ( CRYP_WaitFLAG( hcryp, AES_SR_BUSY, SET, CRYP_GENERAL_TIMEOUT ) !=
                             HAL_OK ) {
                        ...
                        MODIFY_REG(
                            hcryp->Instance->CR,
                            AES_CR_DATATYPE | AES_CR_KEYSIZE | AES_CR_CHMOD | AES_CR_KEYSEL |
                                AES_CR_KEYPROT | AES_CR_KMOD,
                            hcryp->Init.DataType | hcryp->Init.KeySize | hcryp->Init.Algorithm |
                                hcryp->Init.KeySelect | hcryp->Init.KeyProtection |
                                hcryp->Init.KeyMode );
 ...
 * @retval status
 */
HAL_StatusTypeDef HAL_CRYP_RegisterCallback(
    CRYP_HandleTypeDef*        hcryp,
...
HAL_CRYP_UnRegisterCallback( CRYP_HandleTypeDef* hcryp, HAL_CRYP_CallbackIDTypeDef CallbackID )
{
        ...
        if ( hcryp->State == HAL_CRYP_STATE_READY ) {
                ...
                default:
                        /* Update the error code */
                        hcryp->ErrorCode |= HAL_CRYP_ERROR_INVALID_CALLBACK; /*!< Legacy weak ERROR
                                                                                INVALID CALLBACK  */
                        /* Return error status */
                        status = HAL_ERROR;
                        break;
...
HAL_StatusTypeDef HAL_CRYP_Resume( CRYP_HandleTypeDef* hcryp )
{
        ...
        if ( hcryp->State_saved != HAL_CRYP_STATE_SUSPENDED ) {
                ...
                } else /* Authentication algorithms case */
                {
                        ...
                        /* At the same time, set handle state back to READY to be able to resume the
                        AES calculations without the processing APIs returning HAL_BUSY when called.
                      */
                        hcryp->State = HAL_CRYP_STATE_READY;
                }
                ...
                if ( READ_BIT( hcryp->CR_saved, AES_CR_MODE ) == CRYP_OPERATINGMODE_ENCRYPT ) {
                        if ( HAL_CRYP_Encrypt_IT(
                                 hcryp,
                                 hcryp->pCrypInBuffPtr_saved,
                                 hcryp->Size_saved,
...
HAL_StatusTypeDef HAL_CRYP_SaveContext( CRYP_HandleTypeDef* hcryp, CRYP_ContextTypeDef* pcont )
{
        ...
        if ( hcryp->State == HAL_CRYP_STATE_READY ) {
                ...
                pcont->IER_Reg =
                    READ_BIT( hcryp->Instance->IER, CRYP_IT_CCFIE | CRYP_IT_RWEIE | CRYP_IT_KEIE );
 ...
 * @param  Size  Length of the plaintext buffer either in word or in byte, according to
 * DataWidthUnit
    ...
    uint32_t            Timeout )
{
        ...
        if ( hcryp->State == HAL_CRYP_STATE_READY ) {
                ...
                /* Reset CrypInCount, CrypOutCount and Initialize pCrypInBuffPtr and pCrypOutBuffPtr
                 * parameters */
 ...
 */
HAL_StatusTypeDef
HAL_CRYP_Encrypt_IT( CRYP_HandleTypeDef* hcryp, uint32_t* pInput, uint16_t Size, uint32_t* pOutput )
{
        HAL_StatusTypeDef status;
        uint32_t          algo;
#ifdef USE_FULL_ASSERT
 ...
 */
HAL_StatusTypeDef
HAL_CRYP_Decrypt_IT( CRYP_HandleTypeDef* hcryp, uint32_t* pInput, uint16_t Size, uint32_t* pOutput )
{
        HAL_StatusTypeDef status;
        uint32_t          algo;
#ifdef USE_FULL_ASSERT
                        ...
                        if ( ( dokeyivconfig == 1U ) &&
                             ( hcryp->Init.KeyIVConfigSkip != CRYP_KEYNOCONFIG ) ) {
                                if ( hcryp->Instance == AES ) {
                                        ...
                                        else /* After sharing the key, AES should set KMOD[1:0] to
                                                00.*/
                                                hcryp->Instance->CR &= ~CRYP_KEYMODE_SHARED;
                                } else {
                                        /* We should re-write Key, in the case where we change key
                                        ...
                                        } while ( HAL_IS_BIT_CLR(
                                            hcryp->Instance->SR, CRYP_FLAG_KEYVALID ) );
                                }
        ...
        if ( ( itflagisr & CRYP_FLAG_CCF ) != 0U ) {
                if ( ( itsource & CRYP_IT_CCFIE ) != 0U ) {
                        ...
                        if ( ( hcryp->Init.Algorithm == CRYP_AES_GCM_GMAC ) ||
                             ( hcryp->Init.Algorithm == CRYP_AES_CCM ) ) {
                                /* if header phase */
                                if ( ( hcryp->Instance->CR & CRYP_PHASE_HEADER ) ==
                                     CRYP_PHASE_HEADER ) {
 ...
 */
uint32_t HAL_CRYP_GetError( CRYP_HandleTypeDef const* hcryp )
{
 ...
 * @retval HAL state
 */
HAL_CRYP_STATETypeDef HAL_CRYP_GetState( CRYP_HandleTypeDef const* hcryp )
{
        return hcryp->State;
}
                ...
                if ( ( hcryp->Init.KeyIVConfigSkip == CRYP_KEYIVCONFIG_ONCE ) ||
                     ( hcryp->Init.KeyIVConfigSkip == CRYP_KEYIVCONFIG_ALWAYS ) ) {
                        if ( hcryp->Instance == AES ) {
                                ...
                                /* We should re-write Key, in the case where we change key after
                                 * first operation */
                                if ( ( hcryp->Init.KeySelect == CRYP_KEYSEL_NORMAL ) &&
                                     ( hcryp->Init.KeyMode == CRYP_KEYMODE_NORMAL ) ) {
                                        /* Set the Key */
                                ...
                                while (
                                    HAL_IS_BIT_CLR( hcryp->Instance->SR, CRYP_FLAG_KEYVALID ) ) {
                                        /* Check for the Timeout */
        ...
        if ( dokeyivconfig == 1U ) {
                if ( hcryp->Instance == AES ) {
                        ...
                        if ( hcryp->Init.Algorithm != CRYP_AES_CTR ) /*ECB or CBC*/
                        {
                                ...
                                MODIFY_REG(
                                    hcryp->Instance->CR, AES_CR_MODE, CRYP_OPERATINGMODE_DECRYPT );
                        } else /*Algorithm CTR */
                        {
                                /* Set the Key */
                                if ( hcryp->Init.KeyIVConfigSkip != CRYP_KEYNOCONFIG ) {
                ...
                if ( hcryp->Init.KeyIVConfigSkip == CRYP_IVCONFIG_ONCE ) {
                        if ( hcryp->Instance == AES ) {
                                ...
                                if ( hcryp->Init.Algorithm != CRYP_AES_CTR ) /*ECB or CBC*/
                                {
                                        /* key preparation for decryption, operating mode 2*/
                                        MODIFY_REG(
                                            hcryp->Instance->CR, AES_CR_KMOD, CRYP_KEYMODE_NORMAL );
                                        MODIFY_REG(
                                            hcryp->Instance->CR,
                                            AES_CR_MODE,
                                            CRYP_OPERATINGMODE_KEYDERIVATION );
...
static void CRYP_DMAInCplt( DMA_HandleTypeDef* hdma )
{
        ...
        uint32_t            tmp;
        uint32_t const      mask[12] = {
            0x0U,
            0xFF000000U,
            0xFFFF0000U,
            0xFFFFFF00U, /* 32-bit data type */
            0x0U,
            0x0000FF00U,
            0x0000FFFFU,
            0xFF00FFFFU, /* 16-bit data type */
            0x0U,
            0x000000FFU,
            0x0000FFFFU,
            0x00FFFFFFU }; /*  8-bit data type */
        uint32_t algo;
        ...
        if ( hcryp->Phase == CRYP_PHASE_HEADER_DMA_FEED ) {
                /* DMA is disabled, CCF is meaningful. Wait for computation completion before moving
                 * forward */
                ...
                if ( ( headersize_in_bytes % 16U ) != 0U ) {
                        ...
                        for ( loopcounter = 0U;
                              ( loopcounter < ( ( headersize_in_bytes / 4U ) % 4U ) );
                              loopcounter++ ) {
                                hcryp->Instance->DINR =
                        ...
                        if ( ( headersize_in_bytes % 4U ) == 0U ) {
                                ...
                                tmp &= mask
                                    [( hcryp->Init.DataType * 2U ) + ( headersize_in_bytes % 4U )];
...
static void CRYP_DMAOutCplt( DMA_HandleTypeDef* hdma )
{
        ...
        if ( ( ( hcryp->Size ) % 16U ) != 0U ) {
                ...
                count = 0U;
                while ( ( hcryp->CrypOutCount < ( ( hcryp->Size + 3U ) / 4U ) ) &&
                        ( count < 4U ) ) {
    ...
    uint32_t            outputaddr )
{
        ...
        if ( ( hcryp->hdmain->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                ...
                status = HAL_DMA_Start_IT(
                    hcryp->hdmain, inputaddr, (uint32_t) &hcryp->Instance->DINR, Size );
        }
        ...
        if ( ( hcryp->hdmaout->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                ...
                status = HAL_DMA_Start_IT(
                    hcryp->hdmaout, (uint32_t) &hcryp->Instance->DOUTR, outputaddr, Size );
        }
        ...
        if ( hcryp->State == HAL_CRYP_STATE_BUSY ) {
                ...
                if ( hcryp->CrypOutCount == ( hcryp->Size / 4U ) ) {
                        ...
                        __HAL_CRYP_DISABLE_IT(
                            hcryp, CRYP_IT_CCFIE | CRYP_IT_RWEIE | CRYP_IT_KEIE );
...
static HAL_StatusTypeDef CRYP_AESGCM_Process_IT( CRYP_HandleTypeDef* hcryp )
{
        ...
        if ( dokeyivconfig == 1U ) {
                ...
                if ( hcryp->Init.HeaderSize == 0U ) /*header phase is  skipped*/
                {
                        ...
                        {
                                /* Size should be %4  otherwise Tag will  be incorrectly generated
                                for GCM Encryption: Workaround is implemented in polling mode, so if
                                last block of payload <128bit do not use CRYP_Encrypt_IT otherwise
                                TAG is incorrectly generated for GCM Encryption. */

                                /* Compute the number of padding bytes in last block of payload */
                                npblb = 16U - ( (uint32_t) hcryp->Size );

                                if ( ( hcryp->Instance->CR & AES_CR_MODE ) ==
                                     CRYP_OPERATINGMODE_ENCRYPT ) {
                                        /* Set to 0 the number of non-valid bytes using NPBLB
                                         * register*/
                                        MODIFY_REG(
                                            hcryp->Instance->CR, AES_CR_NPBLB, npblb << 20U );
                                }
                                ...
                                /*  last block optionally pad the data with zeros*/
                                for ( loopcounter = 0U; loopcounter < lastwordsize;
                                      loopcounter++ ) {
                                        hcryp->Instance->DINR =
                                            *(uint32_t*) ( hcryp->pCrypInBuffPtr +
                                                           hcryp->CrypInCount );
                ...
                /* Enter header data */
                /* Cher first whether header length is small enough to enter the full header in one
                   shot */
        ...
        if ( dokeyivconfig == 1U ) {
                ...
                if ( headersize_in_bytes == 0U ) /* Header phase is  skipped */
                {
                        ...
                        if ( hcryp->Init.Algorithm == CRYP_AES_CCM ) {
                                /* Increment CrypHeaderCount to pass in
                        ...
                        } else /* Size < 4 words  : first block is the last block*/
                        {
                                ...
                                if ( ( ( mode == CRYP_OPERATINGMODE_ENCRYPT ) &&
                                     ...
                                     ( ( mode == CRYP_OPERATINGMODE_DECRYPT ) &&
                                       ( hcryp->Init.Algorithm == CRYP_AES_CCM ) ) ) {
                                        /* Specify the number of non-valid bytes using NPBLB
                                         * register*/
                                        MODIFY_REG(
                                            hcryp->Instance->CR, AES_CR_NPBLB, npblb << 20U );
                                }
                ...
                /* Enter header data */
                /* Check first whether header length is small enough to enter the full header in one
                   shot */
                else if ( headersize_in_bytes <= 16U ) {
                        /*  Last block optionally pad the data with zeros*/
                        for ( loopcounter = 0U; ( loopcounter < ( headersize_in_bytes / 4U ) );
...
static HAL_StatusTypeDef CRYP_GCMCCM_SetPayloadPhase_DMA( CRYP_HandleTypeDef* hcryp )
{
        ...
        } else /* length of input data is < 16 */
        {
                ...
                /* Set Npblb in case of AES GCM payload encryption or AES CCM payload decryption to
                 * get right tag*/
...
static HAL_StatusTypeDef CRYP_GCMCCM_SetHeaderPhase_DMA( CRYP_HandleTypeDef* hcryp )
{
        ...
        if ( headersize_in_bytes >= 16U ) {
                ...
                if ( headersize_in_bytes != 0U ) {
                        ...
                        if ( CRYP_WaitOnCCFlag( hcryp, CRYP_TIMEOUT_GCMCCMHEADERPHASE ) !=
                             HAL_OK ) {
                                /* Disable the CRYP peripheral clock */
                                __HAL_CRYP_DISABLE( hcryp );
                ...
                /* Move to payload phase if header length is null or
                   if the header length was less than 16 and header written by software instead of
                   DMA */
...
/**
 * @brief  In case of message GCM/GMAC (CCM/CMAC when applicable) processing resumption, rewrite the
 * Key Registers in the AES_KEYRx registers.
 * @param  hcryp pointer to a CRYP_HandleTypeDef structure that contains
 *         the configuration information for CRYP module.
