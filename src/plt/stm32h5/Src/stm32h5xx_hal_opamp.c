        ...
        if ( hopamp == NULL ) {
                ...
                if ( ( hopamp->Init.UserTrimming ) == OPAMP_TRIMMING_USER ) {
                        if ( hopamp->Init.PowerMode == OPAMP_POWERMODE_NORMAL ) {
                                assert_param(
                                    IS_OPAMP_TRIMMINGVALUE( hopamp->Init.TrimmingValueP ) );
                                assert_param(
                                    IS_OPAMP_TRIMMINGVALUE( hopamp->Init.TrimmingValueN ) );
                        } else {
                                assert_param( IS_OPAMP_TRIMMINGVALUE(
                                    hopamp->Init.TrimmingValuePHighSpeed ) );
                                assert_param( IS_OPAMP_TRIMMINGVALUE(
                ...
                if ( hopamp->Init.Mode == OPAMP_PGA_MODE ) {
                        MODIFY_REG(
                            ...
                            hopamp->Init.PowerMode | hopamp->Init.Mode | hopamp->Init.PgaGain |
                                hopamp->Init.PgaConnect | hopamp->Init.NonInvertingInput |
...
HAL_StatusTypeDef HAL_OPAMP_SelfCalibrate( OPAMP_HandleTypeDef* hopamp )
{
        ...
        if ( hopamp == NULL ) {
                ...
                if ( hopamp->State == HAL_OPAMP_STATE_READY ) {
                        ...
                        MODIFY_REG(
                            *tmp_opamp_reg_trimming, OPAMP_OTR_TRIMOFFSETN, trimmingvaluen );
...
HAL_OPAMP_GetTrimOffset( OPAMP_HandleTypeDef const* hopamp, uint32_t trimmingoffset )
{
        ...
        if ( hopamp->State == HAL_OPAMP_STATE_READY ) {
                ...
                if ( READ_BIT( hopamp->Instance->CSR, OPAMP_CSR_USERTRIM ) != 0U ) {
                        ...
                        if ( trimmingoffset == OPAMP_FACTORYTRIMMING_P ) {
                                ...
                                trimmingvalue =
                                    ( ( *tmp_opamp_reg_trimming ) & OPAMP_OTR_TRIMOFFSETP ) >>
                                    OPAMP_INPUT_NONINVERTING;
                        } else {
 ...
 * @retval HAL state
 */
HAL_OPAMP_StateTypeDef HAL_OPAMP_GetState( OPAMP_HandleTypeDef const* hopamp )
{
        /* Check the OPAMP handle allocation */
        if ( hopamp == NULL )
 ...
 * @note  The HAL_OPAMP_RegisterCallback() may be called before HAL_OPAMP_Init() in
 * HAL_OPAMP_STATE_RESET to register callbacks for HAL_OPAMP_MSPINIT_CB_ID and
 ...
 * @note  The HAL_OPAMP_UnRegisterCallback() may be called before HAL_OPAMP_Init() in
 * HAL_OPAMP_STATE_RESET to un-register callbacks for HAL_OPAMP_MSPINIT_CB_ID and
 * HAL_OPAMP_MSPDEINIT_CB_ID
 * @param hopamp  OPAMP handle
