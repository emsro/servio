...
static HAL_StatusTypeDef I2S_WaitFlagStateUntilTimeout(
...
HAL_StatusTypeDef HAL_I2S_Init( I2S_HandleTypeDef* hi2s )
{
        ...
        if ( IS_I2S_MASTER( hi2s->Init.Mode ) ) {
                ...
                /* Force i2smod to 1 just to be sure that (2xi2sdiv + i2sodd) is always higher than
                 * 0 */
                if ( i2sdiv == 0UL )
                        i2sodd = 1UL;

                MODIFY_REG(
                    hi2s->Instance->I2SCFGR,
                    ( SPI_I2SCFGR_I2SDIV | SPI_I2SCFGR_ODD ),
                    ( ( i2sdiv << SPI_I2SCFGR_I2SDIV_Pos ) | ( i2sodd << SPI_I2SCFGR_ODD_Pos ) ) );
        }

        /*-------------------------- I2Sx I2SCFGR Configuration --------------------*/
        /* Configure I2SMOD, I2SCFG, I2SSTD, PCMSYNC, DATLEN ,CHLEN ,CKPOL, WSINV, DATAFMT, I2SDIV,
         * ODD and MCKOE bits bits */
        /* And configure the I2S with the I2S_InitStruct values */
        MODIFY_REG(
            hi2s->Instance->I2SCFGR,
            ( SPI_I2SCFGR_I2SMOD | SPI_I2SCFGR_I2SCFG | SPI_I2SCFGR_I2SSTD | SPI_I2SCFGR_PCMSYNC |
              SPI_I2SCFGR_DATLEN | SPI_I2SCFGR_CHLEN | SPI_I2SCFGR_CKPOL | SPI_I2SCFGR_WSINV |
              SPI_I2SCFGR_DATFMT | SPI_I2SCFGR_MCKOE ),
            ( SPI_I2SCFGR_I2SMOD | hi2s->Init.Mode | hi2s->Init.Standard | hi2s->Init.DataFormat |
              hi2s->Init.CPOL | hi2s->Init.WSInversion | hi2s->Init.Data24BitAlignment |
        ...
        if ( IS_I2S_MASTER( hi2s->Init.Mode ) ) {
                ...
                MODIFY_REG(
                    hi2s->Instance->CFG2, SPI_CFG2_AFCNTR, ( hi2s->Init.MasterKeepIOState ) );
        }
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_I2S_RegisterCallback(
    I2S_HandleTypeDef*        hi2s,
 ...
 * @note   The HAL_I2S_UnRegisterCallback() may be called before HAL_I2S_Init() in
 * HAL_I2S_STATE_RESET to un-register callbacks for HAL_I2S_MSPINIT_CB_ID and
 ...
 */
HAL_StatusTypeDef
HAL_I2S_Transmit( I2S_HandleTypeDef* hi2s, uint16_t const* pData, uint16_t Size, uint32_t Timeout )
{
#if defined( __GNUC__ )
        ...
        hi2s->ErrorCode   = HAL_I2S_ERROR_NONE;
        hi2s->pTxBuffPtr  = (uint16_t const*) pData;
        ...
        if ( I2S_WaitFlagStateUntilTimeout( hi2s, I2S_FLAG_TXP, SET, tickstart, Timeout ) !=
             HAL_OK ) {
                /* Set the error code */
        ...
        while ( hi2s->TxXferCount > 0UL ) {
                if ( ( hi2s->Init.DataFormat == I2S_DATAFORMAT_24B ) ||
                     ( hi2s->Init.DataFormat == I2S_DATAFORMAT_32B ) ) {
                        ...
                        hi2s->Instance->TXDR = *( (uint32_t const*) hi2s->pTxBuffPtr );
                        hi2s->pTxBuffPtr += 2;
                        hi2s->TxXferCount--;
                } else {
                        /* Transmit data in 16 Bit mode */
#if defined( __GNUC__ )
                        *ptxdr_16bits = *( (uint16_t const*) hi2s->pTxBuffPtr );
#else
                        *( (__IO uint16_t*) &hi2s->Instance->TXDR ) =
                            *( (uint16_t const*) hi2s->pTxBuffPtr );
#endif /* __GNUC__ */
 ...
 * @note   The I2S is kept enabled at the end of transaction to avoid the clock de-synchronization
 *         between Master and Slave(example: audio streaming).
 * @note   In I2S Master Receiver mode, just after enabling the peripheral the clock will be
 * generate in continuous way and as the I2S is not disabled at the end of the I2S transaction.
 * @retval HAL status
 */
HAL_StatusTypeDef
HAL_I2S_Receive( I2S_HandleTypeDef* hi2s, uint16_t* pData, uint16_t Size, uint32_t Timeout )
{
        ...
        while ( hi2s->RxXferCount > 0UL ) {
                ...
                if ( I2S_WaitFlagStateUntilTimeout( hi2s, I2S_FLAG_RXP, SET, tickstart, Timeout ) !=
                     HAL_OK ) {
                        /* Set the error code */
                        SET_BIT( hi2s->ErrorCode, HAL_I2S_ERROR_TIMEOUT );
                        hi2s->State = HAL_I2S_STATE_READY;
    ...
    uint32_t           Timeout )
{
        ...
        hi2s->TxXferSize  = Size;
        hi2s->TxXferCount = Size;
        hi2s->pTxBuffPtr  = (uint16_t const*) pTxData;
        hi2s->RxXferSize  = Size;
        hi2s->RxXferCount = Size;
 ...
 */
HAL_StatusTypeDef
HAL_I2S_Transmit_IT( I2S_HandleTypeDef* hi2s, uint16_t const* pData, uint16_t Size )
{
        if ( ( pData == NULL ) || ( Size == 0UL ) )
                return HAL_ERROR;
 ...
 */
HAL_StatusTypeDef
HAL_I2S_Transmit_DMA( I2S_HandleTypeDef* hi2s, uint16_t const* pData, uint16_t Size )
{
        ...
        if ( ( hi2s->hdmatx->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                if ( hi2s->hdmatx->LinkedListQueue != NULL ) {
                        ...
                        hi2s->hdmatx->LinkedListQueue->Head
                            ->LinkRegisters[NODE_CSAR_DEFAULT_OFFSET] = (uint32_t) hi2s->pTxBuffPtr;
...
HAL_StatusTypeDef HAL_I2S_Receive_DMA( I2S_HandleTypeDef* hi2s, uint16_t* pData, uint16_t Size )
{
        ...
        if ( ( hi2s->hdmarx->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                if ( hi2s->hdmarx->LinkedListQueue != NULL ) {
                        ...
                        /* Set DMA destination address */
                        hi2s->hdmarx->LinkedListQueue->Head
                            ->LinkRegisters[NODE_CDAR_DEFAULT_OFFSET] = (uint32_t) hi2s->pRxBuffPtr;
...
HAL_StatusTypeDef HAL_I2S_DMAStop( I2S_HandleTypeDef* hi2s )
{
        ...
        /* The Lock is not implemented on this API to allow the user application
           ...
           HAL_I2S_RxCpltCallback() when calling HAL_DMA_Abort() API the DMA TX or RX Transfer
           complete interrupt is generated and the correspond call back is executed
           HAL_I2S_TxCpltCallback() or HAL_I2S_RxCpltCallback()
           */
 ...
 */
uint32_t HAL_I2S_IsEnabledIOSwap( I2S_HandleTypeDef const* hi2s )
{
        /* Check the I2S handle allocation */
        if ( hi2s == NULL )
                return 0;
        ...
        return (
            ( READ_BIT( hi2s->Instance->CFG2, SPI_CFG2_IOSWP ) == ( SPI_CFG2_IOSWP ) ) ? 1UL :
                                                                                         0UL );
}
 ...
 * @retval HAL state
 */
HAL_I2S_StateTypeDef HAL_I2S_GetState( I2S_HandleTypeDef const* hi2s )
{
        return hi2s->State;
}
 ...
 */
uint32_t HAL_I2S_GetError( I2S_HandleTypeDef const* hi2s )
{
...
static void I2S_Transmit_16Bit_IT( I2S_HandleTypeDef* hi2s )
{
        ...
        *ptxdr_16bits = *( (uint16_t const*) hi2s->pTxBuffPtr );
#else
        *( (__IO uint16_t*) &hi2s->Instance->TXDR ) = *( (uint16_t const*) hi2s->pTxBuffPtr );
#endif /* __GNUC__ */
        hi2s->pTxBuffPtr++;
        hi2s->TxXferCount--;
        ...
        if ( hi2s->RxXferCount == 0UL ) {
                if ( IS_I2S_FULLDUPLEX( hi2s->Init.Mode ) ) {
                        ...
                        __HAL_I2S_DISABLE_IT(
                            hi2s, ( I2S_IT_TXP | I2S_IT_RXP | I2S_IT_DXP | I2S_IT_ERR ) );
                } else {
                        /* Disable RXP and ERR interrupt */
                        __HAL_I2S_DISABLE_IT( hi2s, ( I2S_IT_RXP | I2S_IT_ERR ) );
                }
