/**
  ...
  **********************************************************************************************************************
  ...
  [..]
      The STM32h5xx device family integrate one DTS sensor interface that converts the temperature
  into a square wave which frequency is proportional to the temperature.

       (+) Temperature window comparator feature:
           The DTS allows defining the high and low threshold that will be used for temperature
  comparison. If the temperature data is equal or higher than the high threshold, or equal or lower
  than the low threshold, an interrupt is generated and the corresponding flag.

       (+) Synchronous interrupts:
           A global interrupt output line is available on the DTS block. The interrupt can be
  generated at the end of measurement and/or when the measurement result is equal/higher or
  equal/lower than a predefined threshold.

       (+) Asynchronous wakeup:
           The DTS block provides an asynchronous interrupt line. It is used only when the LSE is
  selected as reference clock. This line can generate a signal that wakes up the system from Sleep
  and stop mode at the end of measurement and/or when the measurement result is equal/higher or
  equal/lower than a predefined threshold.
       ...
       (+) Operating Modes:
           ...
           (++) PCLK only :    The temperature sensor registers can be accessed. The interface can
  consequently be reconfigured and the measurement sequence is performed using PCLK clock.

           (++) PCLK and LSE : The temperature sensor registers can be accessed. The interface can
  consequently be reconfigured and the measurement sequence is performed using the LSE clock.

           (++) LSE only :     The registers cannot be accessed. The measurement can be performed
  using the LSE clock This mode is used to exit from Sleep and stop mode by using hardware triggers
                               and the asynchronous interrupt line.
       (+) Calibration:
           The temperature sensor must run the calibration prior to any frequency measurement. The
  calibration is performed automatically when the temperature measurement is triggered except for
  ...
  ======================================================================================================================
  [..]
      This driver provides functions to configure and program the DTS instances of STM32H5xx
  devices.
      ...
      (+)  Initialize the DTS low level resources by implementing the HAL_DTS_MspInit():
          ...
          (++) If required enable the DTS interrupt by configuring and enabling EXTI line in
  Interrupt mode After that enable the DTS interrupt vector using HAL_NVIC_EnableIRQ() function.
    ...
    *** Callback and interrupts ***
    =============================================
    [..]
     When the compilation flag USE_HAL_DTS_REGISTER_CALLBACKS is set to 0 or not defined, the
  callback registration feature is not available and all callbacks are set to the corresponding weak
  functions.

      (+) Implement the weak function HAL_DTS_EndCallback() to get a callback at the end of the
  temperature measurement.
      ...
      (+) Implement the weak function HAL_DTS_AsyncHighCallback() to get a callback when the
  temperature exceed the high threshold in an asynchronous measurement.
      (+) Implement the weak function HAL_DTS_AsyncLowCallback() to get a callback when the
  temperature go down the low threshold in an asynchronous measurement.

    [..]
     The compilation flag USE_HAL_DTS_REGISTER_CALLBACKS, when set to 1, allows the user to
 ...
 *           @arg @ref HAL_DTS_ASYNC_LOW_THRESHOLD_CB_ID asynchronous low threshold detection
 * callback ID.
 *           @arg @ref HAL_DTS_HIGH_THRESHOLD_CB_ID high threshold detection callback ID.
 *           @arg @ref HAL_DTS_ASYNC_HIGH_THRESHOLD_CB_ID asynchronous high threshold detection
 * callback ID.
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_DTS_RegisterCallback(
    ...
    pDTS_CallbackTypeDef      pCallback )
{
        ...
        if ( pCallback == NULL ) {
                /* Update status */
                status = HAL_ERROR;
        } else if ( hdts->State == HAL_DTS_STATE_READY ) {
                switch ( CallbackID ) {
 ...
 */
HAL_StatusTypeDef
HAL_DTS_UnRegisterCallback( DTS_HandleTypeDef* hdts, HAL_DTS_CallbackIDTypeDef CallbackID )
{
        HAL_StatusTypeDef status = HAL_OK;
...
HAL_StatusTypeDef HAL_DTS_Start_IT( DTS_HandleTypeDef* hdts )
{
        ...
        if ( hdts->State == HAL_DTS_STATE_READY ) {
                ...
                if ( hdts->Init.RefClock == DTS_REFCLKSEL_LSE ) {
                        __HAL_DTS_ENABLE_IT(
                            hdts, DTS_IT_TS1_AITE | DTS_IT_TS1_AITL | DTS_IT_TS1_AITH );
                } else {
                        /* Enable the IT(s) */
                        __HAL_DTS_ENABLE_IT(
                            hdts, DTS_IT_TS1_ITE | DTS_IT_TS1_ITL | DTS_IT_TS1_ITH );
                }
 ...
 * @retval HAL state
 */
HAL_DTS_StateTypeDef HAL_DTS_GetState( DTS_HandleTypeDef const* hdts )
{
        /* Check the DTS handle allocation */
        if ( hdts == NULL )
