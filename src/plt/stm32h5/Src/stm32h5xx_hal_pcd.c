...
static uint16_t
HAL_PCD_EP_DB_Receive( PCD_HandleTypeDef* hpcd, PCD_EPTypeDef* ep, uint16_t wEPVal );
        ...
        if ( USB_GetMode( hpcd->Instance ) == USB_OTG_MODE_DEVICE ) {
                ...
                if ( __HAL_PCD_GET_FLAG( hpcd, USB_OTG_GINTSTS_RXFLVL ) ) {
                        ...
                        if ( ( ( RegVal & USB_OTG_GRXSTSP_PKTSTS ) >> 17 ) == STS_DATA_UPDT ) {
                            ...
                            ( ( RegVal & USB_OTG_GRXSTSP_PKTSTS ) >> 17 ) == STS_SETUP_UPDT ) {
                                ...
                                if ( ( ep_intr & 0x1U ) != 0U ) {
                                        ...
                                        if ( ( epint & USB_OTG_DOEPINT_STUP ) ==
                                             USB_OTG_DOEPINT_STUP ) {
                                                ...
                                                /* Class B setup phase done for previous decoded
                                                 * setup */
                ...
                if ( __HAL_PCD_GET_FLAG( hpcd, USB_OTG_GINTSTS_IEPINT ) ) {
                        ...
                        while ( ep_intr != 0U ) {
                                ...
                                {
                                        epint = USB_ReadDevInEPInterrupt(
                                            hpcd->Instance, (uint8_t) epnum );
                ...
                if ( __HAL_PCD_GET_FLAG( hpcd, USB_OTG_GINTSTS_ENUMDNE ) ) {
                        ...
                        (void) USB_SetTurnaroundTime(
                            hpcd->Instance, HAL_RCC_GetHCLKFreq(), (uint8_t) hpcd->Init.speed );
                                ...
                                if ( ( hpcd->IN_ep[epnum].type == EP_TYPE_ISOC ) &&
                                     ( ( RegVal & USB_OTG_DIEPCTL_EPENA ) ==
                                       USB_OTG_DIEPCTL_EPENA ) ) {
                                        ...
                                        /* Abort current transaction and disable the EP */
                                        (void) HAL_PCD_EP_Abort(
                                            hpcd, (uint8_t) ( epnum | 0x80U ) );
                                }
                        }
...
HAL_StatusTypeDef HAL_PCD_EP_SetStall( PCD_HandleTypeDef* hpcd, uint8_t ep_addr )
{
        ...
        if ( ( ep_addr & EP_ADDR_MSK ) == 0U ) {
                (void) USB_EP0_OutStart(
                    hpcd->Instance, (uint8_t) hpcd->Init.dma_enable, (uint8_t*) hpcd->Setup );
        }
#endif /* defined (USB_OTG_FS) || defined (USB_OTG_HS) */
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_PCD_SetTestMode( PCD_HandleTypeDef const* hpcd, uint8_t testmode )
{
...
static HAL_StatusTypeDef PCD_EP_OutXfrComplete_int( PCD_HandleTypeDef* hpcd, uint32_t epnum )
{
        ...
        if ( hpcd->Init.dma_enable == 1U ) {
                if ( ( DoepintReg & USB_OTG_DOEPINT_STUP ) == USB_OTG_DOEPINT_STUP ) /* Class C */
                {
                    ...
                    ( DoepintReg & USB_OTG_DOEPINT_OTEPSPR ) == USB_OTG_DOEPINT_OTEPSPR ) /* Class E
                                                                                           */
                {
                        CLEAR_OUT_EP_INTR( epnum, USB_OTG_DOEPINT_OTEPSPR );
                } else if (
                    ( DoepintReg & ( USB_OTG_DOEPINT_STUP | USB_OTG_DOEPINT_OTEPSPR ) ) == 0U ) {
                        /* StupPktRcvd = 1 this is a setup packet */
                        if ( ( gSNPSiD > USB_OTG_CORE_ID_300A ) &&
                             ( ( DoepintReg & USB_OTG_DOEPINT_STPKTRX ) ==
                               USB_OTG_DOEPINT_STPKTRX ) ) {
                                ...
                                if ( epnum == 0U ) {
                                        if ( ep->xfer_len == 0U ) {
                                                ...
                                                (void) USB_EP0_OutStart(
                                                    hpcd->Instance, 1U, (uint8_t*) hpcd->Setup );
                                        } else {
                ...
                if ( epindex == 0U ) {
                        ...
                        if ( ( wIstr & USB_ISTR_DIR ) == 0U ) {
                                ...
                                if ( ( wEPVal & USB_EP_SETUP ) != 0U ) {
                                        /* Get SETUP Packet */
                                        ep->xfer_count =
                                            PCD_GET_EP_RX_CNT( hpcd->Instance, ep->num );

                                        if ( ep->xfer_count != 8U ) {
                                                /* Set Stall condition for EP0 IN/OUT */
                                                PCD_SET_EP_RX_STATUS(
                                                    hpcd->Instance, PCD_ENDP0, USB_EP_RX_STALL );
                                                PCD_SET_EP_TX_STATUS(
                                                    hpcd->Instance, PCD_ENDP0, USB_EP_TX_STALL );
                                        ...
                                        if ( ep->xfer_count == 0U ) {
                                                ...
                                                PCD_SET_EP_RX_STATUS(
                                                    hpcd->Instance, PCD_ENDP0, USB_EP_RX_VALID );
                                        } else if ( ep->xfer_buff != 0U ) {
                                                USB_ReadPMA(
                                                    hpcd->Instance,
                                                    ep->xfer_buff,
                                                    ep->pmaadress,
        ...
        if ( ( wEPVal & USB_EP_DTOG_TX ) != 0U ) {
                ...
                } else /* Transfer is not yet Done */
                {
                        ...
                        if ( ep->xfer_fill_db == 1U ) {
                                ...
                                USB_WritePMA(
                                    hpcd->Instance, ep->xfer_buff, ep->pmaaddr0, (uint16_t) len );
                        }
                }
        } else /* Data Buffer1 ACK received */
        {
                ...
                } else /* Transfer is not yet Done */
                {
                        ...
                        if ( ep->xfer_fill_db == 1U ) {
                                ...
                                USB_WritePMA(
                                    hpcd->Instance, ep->xfer_buff, ep->pmaaddr1, (uint16_t) len );
                        }
                }
        }
