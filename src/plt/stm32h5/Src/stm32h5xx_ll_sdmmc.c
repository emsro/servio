...
uint32_t SDMMC_GetCmdResp5( SDMMC_TypeDef* SDMMCx, uint8_t SDIO_CMD, uint8_t* pData )
{
        ...
        if ( ( response_r5 & SDMMC_SDIO_R5_ERRORBITS ) == SDMMC_ALLZERO ) {
                /* we only want 8 bit read or write data and the 8 bit response flags are masked in
                 * the data pointer */
                if ( pData != NULL )
            ...
            ( response_r5 & SDMMC_SDIO_R5_INVALID_FUNCTION_NUMBER ) ==
            ...
            ( response_r5 & SDMMC_SDIO_R5_COM_CRC_FAILED ) == SDMMC_SDIO_R5_COM_CRC_FAILED ) {
                return SDMMC_ERROR_COM_CRC_FAILED;
        } else {
                return SDMMC_ERROR_GENERAL_UNKNOWN_ERR;
        }
...
uint32_t SDMMC_DMALinkedList_EnableCircularMode( SDMMC_DMALinkedListTypeDef* pLinkedList )
{
        ...
        MODIFY_REG(
            pLinkedList->pTailNode->IDMALAR,
            SDMMC_IDMALAR_ULA | SDMMC_IDMALAR_IDMALA,
            SDMMC_IDMALAR_ULA );
