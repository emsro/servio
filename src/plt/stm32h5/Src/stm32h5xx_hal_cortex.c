...
function.
    ...
    *** How to configure MPU regions using CORTEX HAL driver ***
    ...
    (#) Configure the necessary MPU regions using HAL_MPU_ConfigRegion() ennsuring that the MPU
region configuration link to the right MPU attributes number.
    (#) Enable the MPU using HAL_MPU_Enable() function.

     -@- The memory management fault exception is enabled in HAL_MPU_Enable() function and the
system will enter the memory management fault handler MemManage_Handler() when an illegal memory
access is performed.
     -@- If the MPU has previously been programmed, disable the unused regions to prevent any
previous region configuration from affecting the new MPU configuration.
...
========================================================================================================================
 NVIC_PRIORITYGROUP_0 |                0                  |            0-15            | 0 bit for
pre-emption priority |                                   |                            | 4 bits for
subpriority
------------------------------------------------------------------------------------------------------------------------
 NVIC_PRIORITYGROUP_1 |                0-1                |            0-7             | 1 bit for
pre-emption priority |                                   |                            | 3 bits for
subpriority
------------------------------------------------------------------------------------------------------------------------
 NVIC_PRIORITYGROUP_2 |                0-3                |            0-3             | 2 bits for
pre-emption priority |                                   |                            | 2 bits for
subpriority
------------------------------------------------------------------------------------------------------------------------
 NVIC_PRIORITYGROUP_3 |                0-7                |            0-1             | 3 bits for
pre-emption priority |                                   |                            | 1 bit for
subpriority
------------------------------------------------------------------------------------------------------------------------
 NVIC_PRIORITYGROUP_4 |                0-15               |            0               | 4 bits for
pre-emption priority |                                   |                            | 0 bit for
subpriority
========================================================================================================================
  */
 ...
 * @{
 */
static void MPU_ConfigRegion( MPU_Type* MPUx, MPU_Region_InitTypeDef const* const pMPU_RegionInit );
static void MPU_ConfigMemoryAttributes(
    MPU_Type*                               MPUx,
    MPU_Attributes_InitTypeDef const* const pMPU_AttributesInit );
...
{
        __DMB(); /* Data Memory Barrier operation to force any outstanding writes to memory before
                    enabling the MPU */
         ...
         */
        __DSB(); /* Ensure that the subsequent instruction is executed only after the write to
                    memory */
 ...
 * @retval None
 */
void HAL_MPU_ConfigRegion( MPU_Region_InitTypeDef const* const pMPU_RegionInit )
{
 ...
 * @retval None
 */
void HAL_MPU_ConfigRegion_NS( MPU_Region_InitTypeDef const* const pMPU_RegionInit )
{
 ...
 * @retval None
 */
void HAL_MPU_ConfigMemoryAttributes( MPU_Attributes_InitTypeDef const* const pMPU_AttributesInit )
{
    ...
    MPU_Attributes_InitTypeDef const* const pMPU_AttributesInit )
{
        ...
        *( p_mair ) =
            attr_values | ( (uint32_t) pMPU_AttributesInit->Attributes << ( attr_number * 8U ) );
}
