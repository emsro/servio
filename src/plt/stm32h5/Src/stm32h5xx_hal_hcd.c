    ...
    uint16_t           mps )
{
        ...
        status = USB_HC_Init(
            hhcd->Instance, ch_num, epnum, dev_address, speed, ep_type, (uint16_t) HCcharMps );
    ...
    uint8_t            do_ping )
{
        ...
        case EP_TYPE_CTRL:
                if ( token == 1U ) /* send data */
                {
                        ...
                        } else if ( hhcd->hc[ch_num].do_ssplit == 1U ) {
        ...
        return USB_HC_StartXfer(
            hhcd->Instance, &hhcd->hc[ch_num], (uint8_t) hhcd->Init.dma_enable );
}
 ...
 */
HAL_StatusTypeDef
HAL_HCD_UnRegisterCallback( HCD_HandleTypeDef* hhcd, HAL_HCD_CallbackIDTypeDef CallbackID )
{
        HAL_StatusTypeDef status = HAL_OK;
 ...
 */
HAL_StatusTypeDef
HAL_HCD_HC_SetHubInfo( HCD_HandleTypeDef* hhcd, uint8_t ch_num, uint8_t addr, uint8_t PortNbr )
{
...
static void HCD_HC_IN_IRQHandler( HCD_HandleTypeDef* hhcd, uint8_t chnum )
{
        ...
        if ( __HAL_HCD_GET_CH_FLAG( hhcd, chnum, USB_OTG_HCINT_FRMOR ) ) {
                ...
                if ( ( hhcd->hc[chnum].ep_type == EP_TYPE_CTRL ) ||
                    ...
                    ( hhcd->hc[chnum].ep_type == EP_TYPE_ISOC ) ) {
                        ...
                        HAL_HCD_HC_NotifyURBChange_Callback(
                            hhcd, chnum, hhcd->hc[chnum].urb_state );
#endif /* USE_HAL_HCD_REGISTER_CALLBACKS */
                } else {
                        /* ... */
                }
                ...
                if ( hhcd->hc[chnum].ep_type == EP_TYPE_INTR ) {
                    ...
                    ( hhcd->hc[chnum].ep_type == EP_TYPE_CTRL ) ||
                    ( hhcd->hc[chnum].ep_type == EP_TYPE_BULK ) ) {
                        hhcd->hc[chnum].ErrCnt = 0U;
        ...
        case GRXSTS_PKTSTS_IN:
                ...
                if ( ( pktcnt > 0U ) && ( hhcd->hc[chnum].xfer_buff != (void*) 0 ) ) {
                        if ( ( hhcd->hc[chnum].xfer_count + pktcnt ) <= hhcd->hc[chnum].xfer_len ) {
                                (void) USB_ReadPacket(
                                    hhcd->Instance, hhcd->hc[chnum].xfer_buff, (uint16_t) pktcnt );
...
static uint8_t
HAL_HCD_GetLogical_Channel( HCD_HandleTypeDef const* hhcd, uint8_t phy_chnum, uint8_t dir );
static uint8_t HAL_HCD_Check_usedChannel( HCD_HandleTypeDef const* hhcd, uint8_t ch_num );
static uint8_t HAL_HCD_Get_FreePhyChannel(
    HCD_HandleTypeDef* hhcd,
    uint8_t            ch_num,
    uint8_t            epnum,
    uint8_t            ep_type );
...
static void
HCD_HC_IN_BulkDb( HCD_HandleTypeDef* hhcd, uint8_t ch_num, uint8_t phy_chnum, uint32_t regvalue );
static void
HCD_HC_OUT_BulkDb( HCD_HandleTypeDef* hhcd, uint8_t ch_num, uint8_t phy_chnum, uint32_t regvalue );
#endif /* (USE_USB_DOUBLE_BUFFER == 1U) */

static uint16_t HAL_HCD_GetFreePMA( HCD_HandleTypeDef* hhcd, uint16_t mps );
static HAL_StatusTypeDef
HAL_HCD_PMAFree( HCD_HandleTypeDef* hhcd, uint32_t pma_base, uint16_t mps );
static void inline HCD_HC_IN_ISO(
    HCD_HandleTypeDef* hhcd,
    uint8_t            ch_num,
    uint8_t            phy_chnum,
    uint32_t           regvalue );
    ...
    uint16_t           mps )
{
        ...
        if ( used_channel == 0U ) {
                ...
                } else if ( hhcd->hc[ch_num & 0xFU].ep_num != 0U ) {
                ...
                } else if ( ( ( hhcd->ep0_PmaAllocState & 0xF00U ) >> 8 ) == 1U ) {
                        ep0_virtual_channel = (uint8_t) ( hhcd->ep0_PmaAllocState & 0xFU );
...
HAL_StatusTypeDef HAL_HCD_HC_Halt( HCD_HandleTypeDef* hhcd, uint8_t ch_num )
{
        ...
        if ( hhcd->hc[ch_num & 0xFU].ch_dir == CH_IN_DIR ) {
                (void) USB_HC_IN_Halt(
                    hhcd->Instance, (uint8_t) hhcd->hc[ch_num & 0xFU].phy_ch_num );
        } else {
                (void) USB_HC_OUT_Halt(
                    hhcd->Instance, (uint8_t) hhcd->hc[ch_num & 0xFU].phy_ch_num );
        }
        __HAL_UNLOCK( hhcd );
...
HCD_HC_OUT_BulkDb( HCD_HandleTypeDef* hhcd, uint8_t ch_num, uint8_t phy_chnum, uint32_t regvalue )
{
        ...
        if ( ( regvalue & USB_CH_DTOG_TX ) != 0U ) {
                ...
                /* Transfer no yet finished only one packet of mps is transferred and ACKed from
                 * device */
                if ( hhcd->hc[ch_num & 0xFU].xfer_len != 0U ) {
                        ...
                        /* hhcd->hc[ch_num&0xFU].xfer_len_db==0 ==> when all data are written in the
                         * PMA to yet transferred */
                        if ( hhcd->hc[ch_num & 0xFU].xfer_len_db > 0U ) /* Still data to fill in the
                                                                           buffer */
                        {
                                ...
                                if ( hhcd->hc[ch_num & 0xFU].xfer_len_db >
                                     hhcd->hc[ch_num & 0xFU].max_packet ) {
                                        ...
                                        hhcd->hc[ch_num & 0xFU].xfer_len_db =
                                            0U; /* end of fill buffer */
                                }

                                /* Write remaining data to Buffer0 */
                                HCD_SET_CH_DBUF0_CNT(
                                    hhcd->Instance, phy_chnum, 1U, (uint16_t) len );
                                USB_WritePMA(
                                    hhcd->Instance,
                                    hhcd->hc[ch_num & 0xFU].xfer_buff,
                                    hhcd->hc[ch_num & 0xFU].pmaaddr0,
                                    (uint16_t) len );
                        }
...
static void HCD_HC_IN_IRQHandler( HCD_HandleTypeDef* hhcd, uint8_t chnum )
{
        ...
        /* Take a Flag snapshot from the CHEP register, due to STRX bits are used for both control
         * and status */
        ...
        if ( ( ch_reg & USB_CH_ERRRX ) == 0U ) {
                ...
                if ( ( ch_reg & USB_CH_UTYPE ) == USB_EP_ISOCHRONOUS ) {
                        ...
                        if ( ( (ch_reg) &USB_CH_RX_STRX ) == USB_CH_RX_ACK_SBUF ) {
                                ...
                                received_bytes =
                                    (uint16_t) HCD_GET_CH_RX_CNT( hhcd->Instance, phy_chnum );
                                ...
                                if ( ( hhcd->hc[ch_num & 0xFU].xfer_len == 0U ) ||
                                     ( ( received_bytes < hhcd->hc[ch_num & 0xFU].max_packet ) ) ) {
                                        ...
                                        /* Reactivate the Channel to Submit another URB since the
                                         * Transfer is not yet completed */
                                        HCD_SET_CH_RX_STATUS(
                                            hhcd->Instance, phy_chnum, USB_CH_RX_STRX );
                                }
                        ...
                        /* Manage NACK Response */
                            ...
                            ( ( ch_reg & USB_CH_RX_STRX ) == USB_CH_RX_NAK ) &&
                            ( hhcd->hc[ch_num & 0xFU].urb_state != URB_DONE ) ) {
                                ...
                                if ( hhcd->hc[ch_num & 0xFU].ep_type == EP_TYPE_INTR ) {
                                        /* Close the channel */
                                        HCD_SET_CH_RX_STATUS(
                                            hhcd->Instance, phy_chnum, USB_CH_RX_DIS );
                                }
                        }
                        ...
                        /* Double Buffer Management in case of Bulk Transaction */
                            ...
                            ( ( ch_reg & USB_CH_RX_STRX ) == USB_CH_RX_ACK_DBUF ) &&
...
#if ( USE_HAL_HCD_REGISTER_CALLBACKS == 1U )
                        hhcd->HC_NotifyURBChangeCallback(
                            hhcd, ch_num, hhcd->hc[ch_num & 0xFU].urb_state );
#else
                        HAL_HCD_HC_NotifyURBChange_Callback(
                            hhcd, ch_num, hhcd->hc[ch_num & 0xFU].urb_state );
#endif /* USE_HAL_HCD_REGISTER_CALLBACKS */
        ...
        if ( ( ch_reg & USB_CH_ERRTX ) == 0U ) {
                ...
                if ( ( ch_reg & USB_CH_UTYPE ) == USB_EP_ISOCHRONOUS ) {
                        ...
                        if ( ( hhcd->Instance->ISTR & USB_ISTR_ERR ) == 0U ) {
                                ...
                                else /* Double buffer isochronous out */
                                {
                                        ...
                                        USB_DRD_SET_CHEP_TX_STATUS(
                                            hhcd->Instance, phy_chnum, USB_CH_TX_DIS );
                                }
#endif /* (USE_USB_DOUBLE_BUFFER == 1U) */
                ...
                } else /* Manage all Non Isochronous Transaction */
                {
                        ...
                        if ( ( ch_reg & USB_CH_TX_STTX ) == USB_CH_TX_ACK_SBUF ) {
                                data_xfr = (uint16_t) ( ( ( USB_DRD_PMA_BUFF + phy_chnum )->TXBD &
                                                          0x03FF0000U ) >>
                                                        16U );
                                ...
                                /* Transfer no yet finished only one packet of mps is transferred
                                 * and ACKed from device */
                                if ( hhcd->hc[ch_num & 0xFU].xfer_len != 0U ) {
                                        /* Manage multiple Xfer */
                                        hhcd->hc[ch_num & 0xFU].xfer_buff += data_xfr;
                                        hhcd->hc[ch_num & 0xFU].xfer_count += data_xfr;
                                        ...
                                        /* Start a new transfer */
                                        (void) USB_HC_StartXfer(
                                            hhcd->Instance, &hhcd->hc[ch_num & 0xFU] );
                                } else {
                                        /* Transfer complete */
                                        hhcd->hc[ch_num & 0xFU].xfer_count += data_xfr;
                                        hhcd->hc[ch_num & 0xFU].state     = HC_XFRC;
                        ...
                        /* Check NACK Response */
                            ...
                            ( ( ch_reg & USB_CHEP_NAK ) == USB_CHEP_NAK ) ||
                            ( ( ch_reg & USB_CH_TX_STTX ) == USB_CH_TX_NAK ) ) {
                                ...
                                WregCh =
                                    *(__IO uint32_t*) ( &( hhcd->Instance->CHEP0R ) + phy_chnum );
                        ...
                        /* Check double buffer ACK in case of bulk transaction */
                                ...
                                (void) HCD_HC_OUT_BulkDb(
                                    hhcd, ch_num, (uint8_t) phy_chnum, ch_reg );
                        }
#endif /* (USE_USB_DOUBLE_BUFFER == 1U) */
                        else {
                                /*...*/
...
static void HCD_Port_IRQHandler( HCD_HandleTypeDef* hhcd )
{
        ...
        if ( ( hhcd->HostState == HCD_HCD_STATE_DISCONNECTED ) != 0U ) {
                ...
                /* J-state or K-state detected & lastState=Connected: a Missed disconnection is
                 * detected */
                if ( ( ( FnrReg & USB_FNR_RXDP ) != 0U ) ||
                     ( ( IstrReg & USB_ISTR_LS_DCONN ) != 0U ) ) {
                        /* Host Port State */
...
HAL_HCD_GetLogical_Channel( HCD_HandleTypeDef const* hhcd, uint8_t phy_chnum, uint8_t dir )
{
        ...
        if ( dir == 0U ) {
                if ( ( ( hhcd->phy_chout_state[phy_chnum & 0x7U] & 0x00F0U ) >> 4U ) != 0U ) {
                        return (
                            (uint8_t) ( ( hhcd->phy_chout_state[phy_chnum & 0x7U] & 0x00F0U ) >>
                                        4U ) -
                            1U );
                } else {
        ...
        else if ( ( ( hhcd->phy_chin_state[phy_chnum & 0x7U] & 0x00F0U ) >> 4U ) != 0U ) {
                return (
                    (uint8_t) ( ( hhcd->phy_chin_state[phy_chnum & 0x7U] & 0x00F0U ) >> 4U ) - 1U );
        } else {
                /* Channel not registered Error */
                return HCD_LOGICAL_CH_NOT_OPENED;
        }
        ...
        if ( ( epnum & 0x7FU ) == 0U ) {
                ...
                if ( ch_num == 0U ) {
                        if ( hhcd->phy_chin_state[idx] == 0U ) {
                                /* chin_state to store the ep_type to be used for the same channel
                                 * in OUT direction adding + 1 to ep_type avoid starting with a 0
                        ...
                        if ( hhcd->phy_chout_state[idx] == 0U ) {
                                /* chout_state will store the ep_type to be used for the same
                                 * channel in IN direction adding + 1 to ep_type avoid starting with
                                 * a 0 value. ep_type take by default (0/1/2/3) */
                    ...
                    ( ( hhcd->phy_chout_state[idx] & 0xF0U ) >> 4U ) !=
                    ( (uint16_t) ch_num + 1U ) ) {
                        /* chout_state will store the ep_type to be used for the same channel in IN
                         * direction adding + 1 to ep_type avoid starting with a 0 value. ep_type
                         * take by default (0/1/2/3) */
                        hhcd->phy_chout_state[idx] = ( ( (uint16_t) ch_num + 1U ) << 4U ) |
                                                     ( (uint16_t) ep_type + 1U ) |
                                                     ( ( (uint16_t) epnum & 0x0FU ) << 8U );
                }
 ...
 */
HAL_StatusTypeDef
HAL_HCD_PMAlloc( HCD_HandleTypeDef* hhcd, uint8_t ch_num, uint16_t ch_kind, uint16_t mps )
{
        uint16_t pma_addr0;
#if ( USE_USB_DOUBLE_BUFFER == 1U )
