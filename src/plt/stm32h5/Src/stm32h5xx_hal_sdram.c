/**
  ...
  ******************************************************************************
  ...
  HAL_SDRAM_WriteOperation_Enable()/ HAL_SDRAM_WriteOperation_Disable() to respectively
  enable/disable the SDRAM write operation or the function HAL_SDRAM_SendCommand() to send a
  specified command to the SDRAM device. The command to be sent must be configured with the
    ...
    uint32_t             BufferSize )
{
        ...
        if ( state == HAL_SDRAM_STATE_BUSY ) {
            ...
            ( state == HAL_SDRAM_STATE_READY ) || ( state == HAL_SDRAM_STATE_WRITE_PROTECTED ) ) {
                /* Process Locked */
                __HAL_LOCK( hsdram );
            ...
            ( state == HAL_SDRAM_STATE_READY ) || ( state == HAL_SDRAM_STATE_WRITE_PROTECTED ) ) {
                ...
                if ( ( hsdram->hdma->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                        ...
                        status = HAL_DMA_Start_IT(
                            hsdram->hdma, (uint32_t) pAddress, (uint32_t) pDstBuffer, size );
                }
    ...
    uint32_t             BufferSize )
{
        ...
        if ( hsdram->State == HAL_SDRAM_STATE_BUSY ) {
                ...
                if ( ( hsdram->hdma->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                        ...
                        /* Enable the DMA Stream */
                        status = HAL_DMA_Start_IT(
                            hsdram->hdma, (uint32_t) pSrcBuffer, (uint32_t) pAddress, size );
                }
 ...
 * @retval HAL state
 */
HAL_SDRAM_StateTypeDef HAL_SDRAM_GetState( SDRAM_HandleTypeDef const* hsdram )
{
        return hsdram->State;
}
