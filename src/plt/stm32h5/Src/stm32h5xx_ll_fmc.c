...
#define BTR_CLEAR_MASK                                                                          \
        ( (uint32_t) ( FMC_BTRx_ADDSET | FMC_BTRx_ADDHLD | FMC_BTRx_DATAST | FMC_BTRx_BUSTURN | \
                       FMC_BTRx_CLKDIV | FMC_BTRx_DATLAT | FMC_BTRx_ACCMOD | FMC_BTRx_DATAHLD ) )
...
#define PMEM_CLEAR_MASK \
        ( (uint32_t) ( FMC_PMEM_MEMSET | FMC_PMEM_MEMWAIT | FMC_PMEM_MEMHOLD | FMC_PMEM_MEMHIZ ) )

/* --- PATT Register ---*/
/* PATT register clear mask */
#define PATT_CLEAR_MASK \
        ( (uint32_t) ( FMC_PATT_ATTSET | FMC_PATT_ATTWAIT | FMC_PATT_ATTHOLD | FMC_PATT_ATTHIZ ) )
...
#define SDCR_CLEAR_MASK                                                                    \
        ( (uint32_t) ( FMC_SDCRx_NC | FMC_SDCRx_NR | FMC_SDCRx_MWID | FMC_SDCRx_NB |       \
                       FMC_SDCRx_CAS | FMC_SDCRx_WP | FMC_SDCRx_SDCLK | FMC_SDCRx_RBURST | \
                       FMC_SDCRx_RPIPE ) )

/* --- SDTR Register ---*/
/* SDTR register clear mask */
#define SDTR_CLEAR_MASK                                                                   \
        ( (uint32_t) ( FMC_SDTRx_TMRD | FMC_SDTRx_TXSR | FMC_SDTRx_TRAS | FMC_SDTRx_TRC | \
                       FMC_SDTRx_TWR | FMC_SDTRx_TRP | FMC_SDTRx_TRCD ) )
#endif /* FMC_Bank5_6_R */
 ...
 */
HAL_StatusTypeDef
FMC_NORSRAM_Init( FMC_NORSRAM_TypeDef* Device, FMC_NORSRAM_InitTypeDef const* Init )
{
        ...
        mask =
            ( FMC_BCRx_MBKEN | FMC_BCRx_MUXEN | FMC_BCRx_MTYP | FMC_BCRx_MWID | FMC_BCRx_FACCEN |
              FMC_BCRx_BURSTEN | FMC_BCRx_WAITPOL | FMC_BCRx_WAITCFG | FMC_BCRx_WREN |
              FMC_BCRx_WAITEN | FMC_BCRx_EXTMOD | FMC_BCRx_ASYNCWAIT | FMC_BCRx_CBURSTRW );
        ...
        if ( ( Init->ContinuousClock == FMC_CONTINUOUS_CLOCK_SYNC_ASYNC ) &&
             ( Init->NSBank != FMC_NORSRAM_BANK1 ) ) {
                MODIFY_REG(
                    Device->BTCR[FMC_NORSRAM_BANK1], FMC_BCR1_CCLKEN, Init->ContinuousClock );
        }
...
HAL_StatusTypeDef FMC_NORSRAM_Timing_Init(
    FMC_NORSRAM_TypeDef*             Device,
    FMC_NORSRAM_TimingTypeDef const* Timing,
    uint32_t                         Bank )
{
        uint32_t tmpr;
        ...
        Device->BTCR[Bank + 1U] = ( Timing->AddressSetupTime << FMC_BTRx_ADDSET_Pos ) |
...
HAL_StatusTypeDef FMC_NORSRAM_Extended_Timing_Init(
    FMC_NORSRAM_EXTENDED_TypeDef*    Device,
    FMC_NORSRAM_TimingTypeDef const* Timing,
    uint32_t                         Bank,
        ...
        if ( ExtendedMode == FMC_EXTENDED_MODE_ENABLE ) {
                ...
                MODIFY_REG(
                    ...
                    ( Timing->AddressSetupTime |
                      ...
                      ( ( Timing->DataHoldTime ) << FMC_BWTRx_DATAHLD_Pos ) | Timing->AccessMode |
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef FMC_NAND_Init( FMC_NAND_TypeDef* Device, FMC_NAND_InitTypeDef const* Init )
{
        /* Check the parameters */
        ...
        MODIFY_REG(
            Device->PCR,
            PCR_CLEAR_MASK,
            ( Init->Waitfeature | FMC_PCR_MEMORY_TYPE_NAND | Init->MemoryDataWidth |
              Init->EccComputation | Init->ECCPageSize |
...
HAL_StatusTypeDef FMC_NAND_CommonSpace_Timing_Init(
    FMC_NAND_TypeDef*                 Device,
    FMC_NAND_PCC_TimingTypeDef const* Timing,
    uint32_t                          Bank )
{
        /* Check the parameters */
        assert_param( IS_FMC_NAND_DEVICE( Device ) );
 ...
 */
HAL_StatusTypeDef
FMC_NAND_GetECC( FMC_NAND_TypeDef const* Device, uint32_t* ECCval, uint32_t Bank, uint32_t Timeout )
{
        uint32_t tickstart;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef FMC_SDRAM_Init( FMC_SDRAM_TypeDef* Device, FMC_SDRAM_InitTypeDef const* Init )
{
        /* Check the parameters */
        ...
        if ( Init->SDBank == FMC_SDRAM_BANK1 ) {
                MODIFY_REG(
                    Device->SDCR[FMC_SDRAM_BANK1],
                    SDCR_CLEAR_MASK,
                    ( Init->ColumnBitsNumber | Init->RowBitsNumber | Init->MemoryDataWidth |
                      Init->InternalBankNumber | Init->CASLatency | Init->WriteProtection |
...
HAL_StatusTypeDef FMC_SDRAM_Timing_Init(
    FMC_SDRAM_TypeDef*             Device,
    FMC_SDRAM_TimingTypeDef const* Timing,
    uint32_t                       Bank )
{
        /* Check the parameters */
        assert_param( IS_FMC_SDRAM_DEVICE( Device ) );
        ...
        } else /* FMC_Bank2_SDRAM */
        {
                MODIFY_REG(
                    Device->SDTR[FMC_SDRAM_BANK1],
                    FMC_SDTRx_TRC | FMC_SDTRx_TRP,
                    ( ( ( Timing->RowCycleDelay ) - 1U ) << FMC_SDTRx_TRC_Pos ) |
                        ( ( ( Timing->RPDelay ) - 1U ) << FMC_SDTRx_TRP_Pos ) );

                MODIFY_REG(
                    Device->SDTR[FMC_SDRAM_BANK2],
                    SDTR_CLEAR_MASK,
                    ( ( ( Timing->LoadToActiveDelay ) - 1U ) |
                      ( ( ( Timing->ExitSelfRefreshDelay ) - 1U ) << FMC_SDTRx_TXSR_Pos ) |
                      ( ( ( Timing->SelfRefreshTime ) - 1U ) << FMC_SDTRx_TRAS_Pos ) |
...
HAL_StatusTypeDef FMC_SDRAM_SendCommand(
    FMC_SDRAM_TypeDef*              Device,
    FMC_SDRAM_CommandTypeDef const* Command,
    uint32_t                        Timeout )
{
        /* Check the parameters */
        assert_param( IS_FMC_SDRAM_DEVICE( Device ) );
        ...
        MODIFY_REG(
            Device->SDCMR,
            ( FMC_SDCMR_MODE | FMC_SDCMR_CTB2 | FMC_SDCMR_CTB1 | FMC_SDCMR_NRFS | FMC_SDCMR_MRD ),
...
FMC_SDRAM_SetAutoRefreshNumber( FMC_SDRAM_TypeDef* Device, uint32_t AutoRefreshNumber )
{
        ...
        MODIFY_REG(
            Device->SDCMR, FMC_SDCMR_NRFS, ( ( AutoRefreshNumber - 1U ) << FMC_SDCMR_NRFS_Pos ) );
 ...
 */
uint32_t FMC_SDRAM_GetModeStatus( FMC_SDRAM_TypeDef const* Device, uint32_t Bank )
{
        uint32_t tmpreg;
