   ...
   (#) Configure and enable the MPU to override default config if needed, please refers
    ...
    [..]
        ...
        (+) Use HAL_DCACHE_CleanByAddr() to clean cache content for a specific range:
            ...
            (++) Used when buffer is updated by CPU before usage by a peripheral (typically DMA
 transfer)
            (++) Blocking call until operation is done.
        (+) Use HAL_DCACHE_CleanInvalidateByAddr() to clean and invalidate cache content for a
 specific range:
            (++) Cache content for specific range is written back to memory, and reloaded when
 needed.
     ...
     *** DCACHE HAL driver macros list ***
     ...
     [..]
      ...
      (+) __HAL_DCACHE_GET_IT_SOURCE: Check whether the specified DCACHE interrupt source is enabled
 or not.
...
static HAL_StatusTypeDef DCACHE_CommandByAddr(
    DCACHE_HandleTypeDef* hdcache,
    uint32_t              Command,
    uint32_t const* const pAddr,
    uint32_t              dSize,
    uint32_t              mode );
...
HAL_StatusTypeDef HAL_DCACHE_DeInit( DCACHE_HandleTypeDef* hdcache )
{
        ...
        WRITE_REG(
            hdcache->Instance->FCR, DCACHE_FCR_CCMDENDF | DCACHE_FCR_CERRF | DCACHE_FCR_CBSYENDF );
...
HAL_StatusTypeDef HAL_DCACHE_Disable( DCACHE_HandleTypeDef* hdcache )
{
        ...
        if ( HAL_DCACHE_IsEnabled( hdcache ) != 0U ) {
                ...
                while (
                    READ_BIT( hdcache->Instance->SR, ( DCACHE_SR_BUSYF | DCACHE_SR_BUSYCMDF ) ) !=
                    0U ) {
 ...
 */
uint32_t HAL_DCACHE_IsEnabled( DCACHE_HandleTypeDef const* hdcache )
{
 ...
 */
HAL_StatusTypeDef
HAL_DCACHE_SetReadBurstType( DCACHE_HandleTypeDef* hdcache, uint32_t ReadBurstType )
{
        HAL_StatusTypeDef status = HAL_OK;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_DCACHE_InvalidateByAddr(
    DCACHE_HandleTypeDef* hdcache,
    uint32_t const* const pAddr,
    uint32_t              dSize )
{
        ...
        status = DCACHE_CommandByAddr(
            hdcache, DCACHE_COMMAND_INVALIDATE, pAddr, dSize, DCACHE_POLLING_MODE );
 ...
 */
HAL_StatusTypeDef
HAL_DCACHE_CleanByAddr( DCACHE_HandleTypeDef* hdcache, uint32_t const* const pAddr, uint32_t dSize )
{
        HAL_StatusTypeDef status;
        ...
        status = DCACHE_CommandByAddr(
            hdcache, DCACHE_COMMAND_CLEAN, pAddr, dSize, DCACHE_POLLING_MODE );
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_DCACHE_CleanInvalidByAddr(
    DCACHE_HandleTypeDef* hdcache,
    uint32_t const* const pAddr,
    uint32_t              dSize )
{
        ...
        status = DCACHE_CommandByAddr(
            hdcache, DCACHE_COMMAND_CLEAN_INVALIDATE, pAddr, dSize, DCACHE_POLLING_MODE );
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_DCACHE_InvalidateByAddr_IT(
    DCACHE_HandleTypeDef* hdcache,
    uint32_t const* const pAddr,
    uint32_t              dSize )
{
        ...
        status = DCACHE_CommandByAddr(
            hdcache, DCACHE_COMMAND_INVALIDATE, pAddr, dSize, DCACHE_IT_MODE );
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_DCACHE_CleanByAddr_IT(
    DCACHE_HandleTypeDef* hdcache,
    uint32_t const* const pAddr,
    uint32_t              dSize )
{
        ...
        status =
            DCACHE_CommandByAddr( hdcache, DCACHE_COMMAND_CLEAN, pAddr, dSize, DCACHE_IT_MODE );
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_DCACHE_CleanInvalidByAddr_IT(
    DCACHE_HandleTypeDef* hdcache,
    uint32_t const* const pAddr,
    uint32_t              dSize )
{
        ...
        status = DCACHE_CommandByAddr(
            hdcache, DCACHE_COMMAND_CLEAN_INVALIDATE, pAddr, dSize, DCACHE_IT_MODE );
 ...
 */
uint32_t HAL_DCACHE_Monitor_GetReadHitValue( DCACHE_HandleTypeDef const* hdcache )
{
        /* Check the parameters */
        assert_param( IS_DCACHE_ALL_INSTANCE( hdcache->Instance ) );
 ...
 */
uint32_t HAL_DCACHE_Monitor_GetReadMissValue( DCACHE_HandleTypeDef const* hdcache )
{
        /* Check the parameters */
        assert_param( IS_DCACHE_ALL_INSTANCE( hdcache->Instance ) );
 ...
 */
uint32_t HAL_DCACHE_Monitor_GetWriteHitValue( DCACHE_HandleTypeDef const* hdcache )
{
        /* Check the parameters */
        assert_param( IS_DCACHE_ALL_INSTANCE( hdcache->Instance ) );
 ...
 */
uint32_t HAL_DCACHE_Monitor_GetWriteMissValue( DCACHE_HandleTypeDef const* hdcache )
{
        /* Check the parameters */
        assert_param( IS_DCACHE_ALL_INSTANCE( hdcache->Instance ) );
...
void HAL_DCACHE_IRQHandler( DCACHE_HandleTypeDef* hdcache )
{
        ...
        /* Check for end of clean and invalidate by address operation */
            ...
            READ_BIT( hdcache->Instance->CR, DCACHE_COMMAND_CLEAN_INVALIDATE ) ==
 ...
 *          @arg @ref HAL_DCACHE_CLEAN_AND_INVALIDATE_BY_ADDRESS_CB_ID Clean and Invalidate By Addr
 * callback ID
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_DCACHE_RegisterCallback(
    DCACHE_HandleTypeDef*        hdcache,
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_DCACHE_UnRegisterCallback(
    DCACHE_HandleTypeDef*        hdcache,
    HAL_DCACHE_CallbackIDTypeDef CallbackID )
{
        HAL_StatusTypeDef status = HAL_OK;
...
__weak void HAL_DCACHE_CleanAndInvalidateByAddrCallback( DCACHE_HandleTypeDef* hdcache )
{
        ...
        /* NOTE : This function should not be modified, when the callback is needed,
                  the HAL_DCACHE_CleanAndInvalidateByAddrCallback() should be implemented in the
           user file
         */
}
 ...
 * @retval HAL state
 */
HAL_DCACHE_StateTypeDef HAL_DCACHE_GetState( DCACHE_HandleTypeDef const* hdcache )
{
 ...
 */
uint32_t HAL_DCACHE_GetError( DCACHE_HandleTypeDef const* hdcache )
{
 ...
 * @param  dSize Size of the region to be Cleaned, Invalidated or Cleaned and Invalidated (in
 * bytes).
                ...
                if ( mode == DCACHE_IT_MODE ) {
                        ...
                        while ( READ_BIT( hdcache->Instance->SR, DCACHE_SR_CMDENDF ) == 0U ) {
                                if ( ( HAL_GetTick() - tickstart ) >
                                     DCACHE_COMMAND_TIMEOUT_VALUE ) {
                                        if ( READ_BIT( hdcache->Instance->SR, DCACHE_SR_CMDENDF ) ==
                                             0U ) {
                                                /* Update error code */
                                                hdcache->ErrorCode = HAL_DCACHE_ERROR_TIMEOUT;
