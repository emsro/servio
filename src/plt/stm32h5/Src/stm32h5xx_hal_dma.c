/**
  ...
  * @brief   This file provides firmware functions to manage the following functionalities of the
  ...
  **********************************************************************************************************************
  ...
  ======================================================================================================================
    ...
    [..]
      Normal transfers mode is initialized via the standard module and linked-list mode is
  configured via the extended module.

    [..]
      Additionally to linked-list capability, all advanced DMA features are managed and configured
  via the extended module as extensions to normal mode. Advanced features are :
    ...
    *** Initialization and De-Initialization ***
    ============================================
    [..]
      For a given channel, enable and configure the peripheral to be connected to the DMA Channel
  (except for internal SRAM/FLASH memories: no initialization is necessary) please refer to
  Reference manual for connection between peripherals and DMA requests.

    [..]
      For a given channel, use HAL_DMA_Init function to program the required configuration for
  normal transfer through the following parameters:
          ...
          (+) SrcBurstLength        : Specifies the source burst length (number of beats) for the
  DMA channel
              (++) can be a value of between 1 and 64

          (+) DestBurstLength       : Specifies the destination burst length (number of beats) for
  the DMA channel
              (++) can be a value of between 1 and 64
    ...
    *** Polling mode IO operation ***
    =================================
    [..]
          (+) Use HAL_DMA_Start() to start a DMA normal transfer after the configuration of source
  address, destination address and the size of data to be transferred.

          (+) Use HAL_DMA_PollForTransfer() to poll for selected transfer level. In this case a
  fixed Timeout can be configured by User depending on his application. Transfer level can be :
              (++) HAL_DMA_HALF_TRANSFER
              (++) HAL_DMA_FULL_TRANSFER
              For circular transfer, this API returns an HAL_ERROR with HAL_DMA_ERROR_NOT_SUPPORTED
  error code.

          (+) Use HAL_DMA_Abort() function to abort any ongoing DMA transfer in blocking mode.
              This API returns HAL_ERROR when there is no ongoing transfer or timeout is reached
  when disabling the DMA channel. (This API should not be called from an interrupt service routine)
    ...
    *** Interrupt mode IO operation ***
    ...
    [..]
          ...
          (+) Use HAL_DMA_RegisterCallback() function to register user callbacks from the following
  list :
          ...
          (+) Use HAL_DMA_Start_IT() to start the DMA transfer after the enable of DMA interrupts
  and the configuration of source address,destination address and the size of data to be
  transferred.

          (+) Use HAL_DMA_IRQHandler() called under DMA_IRQHandler() interrupt subroutine to handle
  any DMA interrupt.

          (+) Use HAL_DMA_Abort_IT() function to abort any on-going DMA transfer in non-blocking
  mode. This API will suspend immediately the DMA channel execution. When the transfer is
  effectively suspended, an interrupt is generated and HAL_DMA_IRQHandler() will reset the channel
  and execute the callback XferAbortCallback. (This API could be called from an interrupt service
  routine)
    ...
    *** Security and privilege attributes ***
    =========================================
    [..]
          (+) Use HAL_DMA_ConfigChannelAttributes() function to configure DMA channel security and
  privilege attributes.
          ...
          (+) Use HAL_DMA_LockChannelAttributes() function to lock the DMA channel security and
  privilege attributes configuration. This API can be called once after each system boot. If called
  again, HAL_DMA_ConfigChannelAttributes() API has no effect. Unlock is done either by a system boot
  or a by an RCC reset.
    ...
    *** DMA HAL driver macros list ***
    ...
    [..]
      Below the list of most used macros in DMA HAL driver.
          ...
          (+) __HAL_DMA_GET_IT_SOURCE : Check whether the specified DMA Channel interrupt has
  occurred or not.
...
static void DMA_SetConfig(
    DMA_HandleTypeDef const* const hdma,
    uint32_t                       SrcAddress,
    uint32_t                       DstAddress,
...
normal mode.
    ...
    [..]
      (+) The HAL_DMA_Init() function follows the DMA channel configuration procedures as described
in reference manual.
...
/**
 * @brief  Initialize the DMA channel in normal mode according to the specified parameters in the
...
@verbatim
  ...
  ======================================================================================================================
    [..]
      ...
      (+) Configure the source, destination address and data size and Start DMA transfer in normal
mode
      (+) Abort DMA transfer
    ...
    [..]
      (+) The HAL_DMA_Start() function allows to start the DMA channel transfer in normal mode
(Blocking mode).
      (+) The HAL_DMA_Start_IT() function allows to start the DMA channel transfer in normal mode
(Non-blocking mode).
      ...
      (+) The HAL_DMA_PollForTransfer() function allows to poll on half transfer and transfer
complete (Blocking mode). This API cannot be used for circular transfers.
      (+) The HAL_DMA_IRQHandler() function allows to handle any DMA channel interrupt (Non-blocking
mode).
      (+) The HAL_DMA_RegisterCallback() and HAL_DMA_UnRegisterCallback() functions allow
respectively to register and unregister user customized callbacks. User callbacks are called under
HAL_DMA_IRQHandler().
...
/**
 * @brief  Starts the DMA channel transfer in normal mode with interrupts enabled (Non-blocking
 * mode).
 * @param  hdma         : Pointer to a DMA_HandleTypeDef structure that contains the configuration
 * information for the specified DMA Channel.
 * @param  SrcAddress   : The source data address.
 ...
 * @param  hdma : Pointer to a DMA_HandleTypeDef structure that contains the configuration
 * information for the specified DMA Channel.
 * @note   After suspending a DMA channel, a wait until the DMA channel is effectively stopped is
 * added. If a channel is suspended while a data transfer is on-going, the current data will be
 * transferred and the channel will be effectively suspended only after the transfer of any on-going
 * data is finished.
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_DMA_Abort( DMA_HandleTypeDef* const hdma )
{
        ...
        if ( hdma->State != HAL_DMA_STATE_BUSY ) {
                ...
                if ( ( hdma->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                        ...
                        /* Clear remaining data size to ensure loading linked-list from memory next
                         * start */
                        hdma->Instance->CBR1 = 0U;
                }
...
{
        DMA_TypeDef const* p_dma_instance = GET_DMA_INSTANCE( hdma );
                ...
                if ( __HAL_DMA_GET_IT_SOURCE( hdma, DMA_IT_SUSP ) != 0U ) {
                        ...
                        if ( hdma->State == HAL_DMA_STATE_ABORT ) {
                                ...
                                if ( ( hdma->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                                        ...
                                        /* Clear remaining data size to ensure loading linked-list
                                         * from memory next start */
                                        hdma->Instance->CBR1 = 0U;
                                }
 ...
 * @param  hdma       : Pointer to a DMA_HandleTypeDef structure that contains the configuration
 * information for the specified DMA Channel.
 * @param  CallbackID : User Callback identifier which could be a value of HAL_DMA_CallbackIDTypeDef
 * enumeration.
 ...
 * @note   The HAL_DMA_UnRegisterCallback() may be called before HAL_DMA_Init() in
 * HAL_DMA_STATE_RESET to un-register callbacks for HAL_DMA_MSPINIT_CB_ID and
 ...
 */
HAL_StatusTypeDef
HAL_DMA_UnRegisterCallback( DMA_HandleTypeDef* const hdma, HAL_DMA_CallbackIDTypeDef CallbackID )
{
        HAL_StatusTypeDef status = HAL_OK;
...
@verbatim
  ...
  ======================================================================================================================
    ...
    [..]
      (+) The HAL_DMA_ConfigChannelAttributes() function allows to configure DMA channel security
and privilege attributes.
      (+) The HAL_DMA_GetConfigChannelAttributes() function allows to get DMA channel security and
privilege attributes configuration.
      (+) The HAL_DMA_LockChannelAttributes() function allows to lock the DMA channel security and
privilege attributes.
      (+) The HAL_DMA_GetLockChannelAttributes() function allows to get the DMA channel security and
privilege attributes lock status.
 ...
 */
HAL_StatusTypeDef
HAL_DMA_ConfigChannelAttributes( DMA_HandleTypeDef* const hdma, uint32_t ChannelAttributes )
{
        DMA_TypeDef* p_dma_instance;
...
{
        DMA_TypeDef const* p_dma_instance;
        uint32_t           attributes;
        uint32_t           channel_idx;
        ...
        attributes = ( ( p_dma_instance->PRIVCFGR & channel_idx ) == 0U ) ? DMA_CHANNEL_NPRIV :
                                                                            DMA_CHANNEL_PRIV;

#if defined( DMA_SECCFGR_SEC0 )
        /* Get DMA channel security attribute */
        attributes |= ( ( p_dma_instance->SECCFGR & channel_idx ) == 0U ) ? DMA_CHANNEL_NSEC :
                                                                            DMA_CHANNEL_SEC;

        /* Get DMA channel source security attribute */
        attributes |= ( ( hdma->Instance->CTR1 & DMA_CTR1_SSEC ) == 0U ) ? DMA_CHANNEL_SRC_NSEC :
                                                                           DMA_CHANNEL_SRC_SEC;

        /* Get DMA channel destination security attribute */
        attributes |= ( ( hdma->Instance->CTR1 & DMA_CTR1_DSEC ) == 0U ) ? DMA_CHANNEL_DEST_NSEC :
 ...
 * @param  hdma       : Pointer to a DMA_HandleTypeDef structure that contains the configuration
 * information for the specified DMA Channel.
 * @param  pLockState : Pointer to lock state (returned value can be DMA_CHANNEL_ATTRIBUTE_UNLOCKED
 * or DMA_CHANNEL_ATTRIBUTE_LOCKED).
 * @retval HAL status.
 */
HAL_StatusTypeDef
HAL_DMA_GetLockChannelAttributes( DMA_HandleTypeDef const* const hdma, uint32_t* const pLockState )
{
        DMA_TypeDef const* p_dma_instance;
        uint32_t           channel_idx;
    ...
    uint32_t                       SrcDataSize )
{
        ...
        __HAL_DMA_CLEAR_FLAG(
            hdma,
            DMA_FLAG_TC | DMA_FLAG_HT | DMA_FLAG_DTE | DMA_FLAG_ULE | DMA_FLAG_USE | DMA_FLAG_SUSP |
                DMA_FLAG_TO );
...
static void DMA_Init( DMA_HandleTypeDef const* const hdma )
{
        ...
        MODIFY_REG(
            hdma->Instance->CTR2,
            ( DMA_CTR2_TCEM | DMA_CTR2_TRIGPOL | DMA_CTR2_TRIGSEL | DMA_CTR2_TRIGM |
              DMA_CTR2_PFREQ | DMA_CTR2_BREQ | DMA_CTR2_DREQ | DMA_CTR2_SWREQ | DMA_CTR2_REQSEL ),
            tmpreg );
