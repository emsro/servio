  ...
  **********************************************************************************************************************
  ...
  [..]
      ...
      (#) The comparators have interrupt capability through direct line to NVIC (featuring
          ...
          Caution: Specific behavior for comparator of this STM32 series: comparator output triggers
  interruption on high level
          - triggering on level (instead of edge) implies to disable interrupt in comparator IRQ
  handler. In case of further operation needed in interrupt mode, comparator interruption must be
  rearmed.
  ...
  [..]
      ...
      (++) If needed, configure the GPIO connected to comparator output in alternate function mode
           using HAL_GPIO_Init().
      (++) If required enable the COMP interrupt by configuring and enabling EXTI line in Interrupt
  mode and selecting the desired sensitivity level using HAL_GPIO_Init() function. After that enable
  the comparator interrupt vector using HAL_NVIC_EnableIRQ() function.
      ...
      (#) Enable the comparator using HAL_COMP_Start(), HAL_COMP_Start_IT_OneShot() or
  HAL_COMP_Start_IT_AutoRearm() Note: Using HAL_COMP_Start_IT_OneShot() or
  HAL_COMP_Start_IT_AutoRearm(), these functions can change comparator output polarity to match
  initial comparator output level constraint. Note: Using HAL_COMP_Start_IT_OneShot(), after each
  interruption triggered the interruption is disabled in IRQ handler. If needed, comparartor
  interruption can be rearmed by calling again start function. Note: In case of comparator and
  interruption used to exit from low power mode, user most ensure of stable comparator input voltage
  (risk would be that comparator trigs early and IT disabled in IRQ handler before device entering
  in low power mode, inducing no further system wake up possible). Most appropriate function is
  HAL_COMP_Start_IT_AutoRearm() because comparartor triggers remains enable, ensuring system wake up
  capability.
...
HAL_StatusTypeDef HAL_COMP_Init( COMP_HandleTypeDef* hcomp )
{
        uint32_t tmp_csr;
        uint32_t exti_line;
        uint32_t comp_voltage_scaler_initialized; /* Value "0" if comparator voltage scaler is not
                                                     initialized */
        ...
        if ( hcomp == NULL ) {
                ...
                /* Set COMP parameters */
                tmp_csr =
                    ( hcomp->Init.InputMinus | hcomp->Init.InputPlus | hcomp->Init.BlankingSrce |
                      hcomp->Init.Hysteresis | hcomp->Init.OutputPol | hcomp->Init.Mode );

                /* Set parameters in COMP register */
                /* Note: Update all bits except read-only, lock and enable bits */
                MODIFY_REG(
                    hcomp->Instance->CFGR1,
                ...
                if ( hcomp->Init.InputPlus == COMP_INPUT_PLUS_IO2 ) {
                        MODIFY_REG(
                            hcomp->Instance->CFGR2, COMP_CFGR2_INPSEL0, COMP_CFGR2_INPSEL0 );
                }
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_COMP_RegisterCallback(
 ...
 */
HAL_StatusTypeDef
HAL_COMP_UnRegisterCallback( COMP_HandleTypeDef* hcomp, HAL_COMP_CallbackIDTypeDef CallbackID )
{
        HAL_StatusTypeDef status = HAL_OK;
...
HAL_StatusTypeDef HAL_COMP_Start( COMP_HandleTypeDef* hcomp )
{
        ...
        if ( hcomp == NULL ) {
                ...
                } else if ( hcomp->State == HAL_COMP_STATE_READY ) {
...
/**
  * @brief  Start the comparator with interruption low latency, interruption disabled at first
  trigger occurrence.
  * @note   Interruption low latency is achieved through direct line to NVIC (instead of going
  through EXTI).
  * @note   If needed, comparartor interruption can be rearmed by calling again this function.
  * @note   Specific to comparator of this STM32 series: comparator output triggers interruption on
  high level. This function can change output polarity depending on initial output level.
  * @param  hcomp COMP handle
  * @retval HAL status
  */
        ...
        if ( hcomp == NULL ) {
                ...
                if ( hcomp->State == HAL_COMP_STATE_READY ) {
                        ...
                        if ( polarity_toggle == 1U ) {
                                /* Toggle poarity */
                                if ( READ_BIT( hcomp->Instance->CFGR1, COMP_CFGR1_POLARITY ) ==
                                     0UL ) {
...
/**
  * @brief  Start the comparator with interruption low latency, interruption rearmed at each trigger
  occurrence.
  * @note   Interruption low latency is achieved through direct line to NVIC (instead of going
  through EXTI).
  * @note   If needed, comparartor interruption can be rearmed by calling again this function.
...
void HAL_COMP_IRQHandler( COMP_HandleTypeDef* hcomp )
{
        ...
        /* Note: Specific to comparator of this STM32 series: comparator output triggers
         * interruption on high level. */
 ...
 *
 */
uint32_t HAL_COMP_GetOutputLevel( COMP_HandleTypeDef const* hcomp )
{
        /* Check the parameter */
        assert_param( IS_COMP_ALL_INSTANCE( hcomp->Instance ) );
 ...
 * @retval HAL state
 */
HAL_COMP_StateTypeDef HAL_COMP_GetState( COMP_HandleTypeDef const* hcomp )
{
 ...
 */
uint32_t HAL_COMP_GetError( COMP_HandleTypeDef const* hcomp )
{
        /* Check the parameters */
        assert_param( IS_COMP_ALL_INSTANCE( hcomp->Instance ) );
