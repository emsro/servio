/**
  ...
  ******************************************************************************
  ...
  ==============================================================================
    ...
    (#) Initialize the PSSI registers by calling the @ref HAL_PSSI_Init(), configure also the low
  level Hardware (GPIO, CLOCK, NVIC...etc) by calling the customized @ref HAL_PSSI_MspInit(&hpssi)
  API.
    ...
    *** DMA mode IO operation ***
     ...
     *** Callback registration ***
     ...
     reset to the legacy weak functions in the @ref HAL_PSSI_Init()/ @ref HAL_PSSI_DeInit() only
  when these callbacks are null (not registered beforehand). If MspInit or MspDeInit are not null,
  the @ref HAL_PSSI_Init()/ @ref HAL_PSSI_DeInit() keep and use the user MspInit/MspDeInit callbacks
...
static HAL_StatusTypeDef PSSI_WaitOnStatusUntilTimeout(
    PSSI_HandleTypeDef* hpssi,
    uint32_t            Flag,
    FlagStatus          Status,
    uint32_t            Timeout,
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_PSSI_RegisterCallback(
    PSSI_HandleTypeDef*        hpssi,
 ...
 */
HAL_StatusTypeDef
HAL_PSSI_UnRegisterCallback( PSSI_HandleTypeDef* hpssi, HAL_PSSI_CallbackIDTypeDef CallbackID )
{
        HAL_StatusTypeDef status = HAL_OK;
 ...
 */
HAL_StatusTypeDef
HAL_PSSI_Transmit( PSSI_HandleTypeDef* hpssi, uint8_t* pData, uint32_t Size, uint32_t Timeout )
{
        uint32_t tickstart;
        ...
        if ( hpssi->State == HAL_PSSI_STATE_READY ) {
                ...
                MODIFY_REG(
                    hpssi->Instance->CR,
                    ( PSSI_CR_OUTEN | PSSI_CR_CKPOL ),
                    ( PSSI_CR_OUTEN_OUTPUT |
                      ( ( hpssi->Init.ClockPolarity == HAL_PSSI_RISING_EDGE ) ? 0U :
                                                                                PSSI_CR_CKPOL ) ) );
                ...
                if ( hpssi->Init.DataWidth == HAL_PSSI_8BITS ) {
                        ...
                        while ( transfer_size > 0U ) {
                                ...
                                if ( PSSI_WaitOnStatusUntilTimeout(
                                         hpssi, PSSI_FLAG_RTT1B, RESET, Timeout, tickstart ) !=
                                     HAL_OK ) {
                        ...
                        while ( transfer_size > 0U ) {
                                ...
                                if ( PSSI_WaitOnStatusUntilTimeout(
                                         hpssi, PSSI_FLAG_RTT4B, RESET, Timeout, tickstart ) !=
                                     HAL_OK ) {
                                        hpssi->ErrorCode = HAL_PSSI_ERROR_TIMEOUT;
                                        hpssi->State     = HAL_PSSI_STATE_READY;
                                        /* Process Unlocked */
 ...
 */
HAL_StatusTypeDef
HAL_PSSI_Receive( PSSI_HandleTypeDef* hpssi, uint8_t* pData, uint32_t Size, uint32_t Timeout )
{
        uint32_t tickstart;
        uint32_t transfer_size = Size;
        ...
        if ( hpssi->State == HAL_PSSI_STATE_READY ) {
                ...
                if ( hpssi->XferSize > 0U ) {
                        if ( hpssi->hdmatx != NULL ) {
                                ...
                                /* Configure BusWidth */
                                if ( hpssi->hdmatx->Init.DestDataWidth ==
                                ...
                                if ( ( hpssi->hdmatx->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                                        if ( hpssi->hdmatx->LinkedListQueue != NULL ) {
                                                ...
                                                dmaxferstatus =
                                                    HAL_DMAEx_List_Start_IT( hpssi->hdmatx );
                                        } else {
                                                /* Return error status */
                                                return HAL_ERROR;
                                        }
                        ...
                        if ( dmaxferstatus == HAL_OK ) {
                                ...
                                /* Note : The PSSI interrupts must be enabled after unlocking
                                   current process to avoid the risk of PSSI interrupt handle
                                   execution before current process unlock */
                        ...
                        if ( hpssi->hdmarx != NULL ) {
                                ...
                                if ( hpssi->hdmarx->Init.SrcDataWidth == DMA_SRC_DATAWIDTH_BYTE ) {
                                        MODIFY_REG(
                                            hpssi->Instance->CR,
                                            PSSI_CR_DMAEN | PSSI_CR_OUTEN | PSSI_CR_CKPOL,
                                            PSSI_CR_DMA_ENABLE | ( ( hpssi->Init.ClockPolarity ==
                                                                     HAL_PSSI_RISING_EDGE ) ?
                                                                       PSSI_CR_CKPOL :
                                                                       0U ) );
                                } else {
                                ...
                                if ( ( hpssi->hdmarx->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                                        if ( hpssi->hdmarx->LinkedListQueue != NULL ) {
                                                ...
                                                dmaxferstatus =
                                                    HAL_DMAEx_List_Start_IT( hpssi->hdmarx );
                                        } else {
                                                /* Return error status */
                                                return HAL_ERROR;
                                        }
                ...
                /* Abort DMA RX transfer if any */
                        ...
                        if ( hpssi->hdmarx != NULL ) {
                                ...
                                if ( HAL_DMA_Abort_IT( hpssi->hdmarx ) != HAL_OK ) {
                                        /* Call Directly hpssi->hdma->XferAbortCallback function in
                                         * case of error */
        ...
        if ( HAL_PSSI_GET_FLAG( hpssi, PSSI_FLAG_OVR_MIS ) != 0U ) {
                ...
                if ( ( hpssi->Instance->CR & PSSI_CR_DMAEN ) == PSSI_CR_DMAEN ) {
                        if ( hpssi->State == HAL_PSSI_STATE_BUSY_TX ) {
                                ...
                                if ( hpssi->hdmatx != NULL ) {
                                        /* Set the PSSI DMA Abort callback :
                                        will lead to call HAL_PSSI_ErrorCallback() at end of DMA
                                        abort procedure */
                                        hpssi->hdmatx->XferAbortCallback = PSSI_DMAAbort;
                        ...
                        /* Abort DMA RX transfer if any */
                                ...
                                if ( hpssi->hdmarx != NULL ) {
                                        ...
                                        /* Abort DMA RX */
                                        if ( HAL_DMA_Abort_IT( hpssi->hdmarx ) != HAL_OK ) {
                                                /* Call Directly hpssi->hdma->XferAbortCallback
...
#if ( USE_HAL_PSSI_REGISTER_CALLBACKS == 1 )
                                /* Call the corresponding callback to inform upper layer of the
                                 * error */
                                hpssi->ErrorCallback( hpssi );
#else
                                HAL_PSSI_ErrorCallback( hpssi );
 ...
 * @retval HAL state
 */
HAL_PSSI_StateTypeDef HAL_PSSI_GetState( PSSI_HandleTypeDef const* hpssi )
{
 ...
 */
uint32_t HAL_PSSI_GetError( PSSI_HandleTypeDef const* hpssi )
{
