...
HAL_StatusTypeDef USB_SetTurnaroundTime( USB_OTG_GlobalTypeDef* USBx, uint32_t hclk, uint8_t speed )
{
        ...
        response time to IN tokens, the USB turnaround time, so to compensate for the longer AHB
        read access latency to the Data FIFO */
 ...
 * @retval  Hal status
 */
HAL_StatusTypeDef USB_SetDevSpeed( USB_OTG_GlobalTypeDef const* USBx, uint8_t speed )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
 ...
 */
uint8_t USB_GetDevSpeed( USB_OTG_GlobalTypeDef const* USBx )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
        uint8_t  speed;
        ...
        if ( DevEnumSpeed == DSTS_ENUMSPD_HS_PHY_30MHZ_OR_60MHZ ) {
            ...
            ( DevEnumSpeed == DSTS_ENUMSPD_FS_PHY_30MHZ_OR_60MHZ ) ||
 ...
 */
HAL_StatusTypeDef
USB_ActivateEndpoint( USB_OTG_GlobalTypeDef const* USBx, USB_OTG_EPTypeDef const* ep )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
 ...
 */
HAL_StatusTypeDef
USB_ActivateDedicatedEndpoint( USB_OTG_GlobalTypeDef const* USBx, USB_OTG_EPTypeDef const* ep )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
        uint32_t epnum     = (uint32_t) ep->num;
 ...
 */
HAL_StatusTypeDef
USB_DeactivateEndpoint( USB_OTG_GlobalTypeDef const* USBx, USB_OTG_EPTypeDef const* ep )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
        uint32_t epnum     = (uint32_t) ep->num;
 ...
 */
HAL_StatusTypeDef
USB_DeactivateDedicatedEndpoint( USB_OTG_GlobalTypeDef const* USBx, USB_OTG_EPTypeDef const* ep )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
        uint32_t epnum     = (uint32_t) ep->num;
        ...
        if ( ep->is_in == 1U ) {
                ...
                if ( dma == 1U ) {
                        ...
                        if ( ep->type != EP_TYPE_ISOC ) {
                                ...
                                (void) USB_WritePacket(
                                    USBx, ep->xfer_buff, ep->num, (uint16_t) ep->xfer_len, dma );
                        }
                }
        } else /* OUT endpoint */
        {
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef USB_EPStopXfer( USB_OTG_GlobalTypeDef const* USBx, USB_OTG_EPTypeDef* ep )
{
        __IO uint32_t     count     = 0U;
        HAL_StatusTypeDef ret       = HAL_OK;
        uint32_t          USBx_BASE = (uint32_t) USBx;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef USB_WritePacket(
    USB_OTG_GlobalTypeDef const* USBx,
    uint8_t*                     src,
    uint8_t                      ch_ep_num,
    uint16_t                     len,
    uint8_t                      dma )
 ...
 */
void* USB_ReadPacket( USB_OTG_GlobalTypeDef const* USBx, uint8_t* dest, uint16_t len )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
        uint8_t* pDest     = dest;
        uint32_t pData;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef USB_EPSetStall( USB_OTG_GlobalTypeDef const* USBx, USB_OTG_EPTypeDef const* ep )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
        uint32_t epnum     = (uint32_t) ep->num;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef USB_EPClearStall( USB_OTG_GlobalTypeDef const* USBx, USB_OTG_EPTypeDef const* ep )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
        uint32_t epnum     = (uint32_t) ep->num;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef USB_SetDevAddress( USB_OTG_GlobalTypeDef const* USBx, uint8_t address )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef USB_DevConnect( USB_OTG_GlobalTypeDef const* USBx )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef USB_DevDisconnect( USB_OTG_GlobalTypeDef const* USBx )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
 ...
 */
uint32_t USB_ReadChInterrupts( USB_OTG_GlobalTypeDef const* USBx, uint8_t chnum )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
        uint32_t tmpreg;
 ...
 */
uint32_t USB_ReadDevAllOutEpInterrupt( USB_OTG_GlobalTypeDef const* USBx )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
        uint32_t tmpreg;
 ...
 */
uint32_t USB_ReadDevAllInEpInterrupt( USB_OTG_GlobalTypeDef const* USBx )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
        uint32_t tmpreg;
 ...
 */
uint32_t USB_ReadDevOutEPInterrupt( USB_OTG_GlobalTypeDef const* USBx, uint8_t epnum )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
        uint32_t tmpreg;
 ...
 */
uint32_t USB_ReadDevInEPInterrupt( USB_OTG_GlobalTypeDef const* USBx, uint8_t epnum )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
        uint32_t tmpreg;
        uint32_t msk;
 ...
 *           0 : Device
 */
uint32_t USB_GetMode( USB_OTG_GlobalTypeDef const* USBx )
{
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef USB_ActivateSetup( USB_OTG_GlobalTypeDef const* USBx )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
 ...
 */
HAL_StatusTypeDef
USB_EP0_OutStart( USB_OTG_GlobalTypeDef const* USBx, uint8_t dma, uint8_t const* psetup )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef USB_InitFSLSPClkSel( USB_OTG_GlobalTypeDef const* USBx, uint8_t freq )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
 ...
 */
HAL_StatusTypeDef USB_ResetPort( USB_OTG_GlobalTypeDef const* USBx )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef USB_DriveVbus( USB_OTG_GlobalTypeDef const* USBx, uint8_t state )
{
        uint32_t      USBx_BASE = (uint32_t) USBx;
        __IO uint32_t hprt0     = 0U;
    ...
    uint16_t               mps )
{
        ...
        case EP_TYPE_INTR:
                USBx_HC( (uint32_t) ch_num )->HCINTMSK =
                    USB_OTG_HCINTMSK_XFRCM | USB_OTG_HCINTMSK_STALLM | USB_OTG_HCINTMSK_TXERRM |
                    USB_OTG_HCINTMSK_DTERRM | USB_OTG_HCINTMSK_NAKM | USB_OTG_HCINTMSK_AHBERR |
        ...
        case EP_TYPE_ISOC:
                USBx_HC( (uint32_t) ch_num )->HCINTMSK =
                    USB_OTG_HCINTMSK_XFRCM | USB_OTG_HCINTMSK_ACKM | USB_OTG_HCINTMSK_AHBERR |
                    USB_OTG_HCINTMSK_FRMORM;
 ...
 */
HAL_StatusTypeDef
USB_HC_StartXfer( USB_OTG_GlobalTypeDef* USBx, USB_OTG_HCTypeDef* hc, uint8_t dma )
{
        uint32_t      USBx_BASE = (uint32_t) USBx;
        ...
        if ( USBx == USB_OTG_HS ) {
                ...
                } else if ( ( hc->speed == USBH_HS_SPEED ) && ( hc->do_ping == 1U ) ) {
        ...
        if ( ( hc->ep_is_in == 0U ) && ( hc->xfer_len > 0U ) && ( hc->do_csplit == 0U ) ) {
                ...
                (void) USB_WritePacket(
                    USBx, hc->xfer_buff, hc->ch_num, (uint16_t) hc->xfer_len, 0 );
        }
 ...
 * @retval HAL state
 */
uint32_t USB_HC_ReadInterrupt( USB_OTG_GlobalTypeDef const* USBx )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
 ...
 * @retval HAL state
 */
HAL_StatusTypeDef USB_HC_Halt( USB_OTG_GlobalTypeDef const* USBx, uint8_t hc_num )
{
        uint32_t      USBx_BASE  = (uint32_t) USBx;
        ...
        /* In buffer DMA, Channel disable must not be programmed for non-split periodic channels.
           At the end of the next uframe/frame (in the worst case), the core generates a channel
 ...
 * @retval HAL state
 */
HAL_StatusTypeDef USB_DoPing( USB_OTG_GlobalTypeDef const* USBx, uint8_t ch_num )
{
        uint32_t USBx_BASE   = (uint32_t) USBx;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef USB_ActivateRemoteWakeup( USB_OTG_GlobalTypeDef const* USBx )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef USB_DeActivateRemoteWakeup( USB_OTG_GlobalTypeDef const* USBx )
{
        uint32_t USBx_BASE = (uint32_t) USBx;
...
static HAL_StatusTypeDef USB_HC_BULK_DB_StartXfer(
...
USB_HC_ISO_DB_StartXfer( USB_DRD_TypeDef* USBx, USB_DRD_HCTypeDef* hc, uint32_t len );
                        ...
                        if ( ep->type == EP_TYPE_BULK ) {
                                ...
                                /* auto Switch to single buffer mode when transfer <Mps no need to
                                   manage in double buffer */
                                        ...
                                        /* Write the user buffer to USB PMA */
                                        USB_WritePMA(
                                            USBx, ep->xfer_buff, pmabuffer, (uint16_t) len );
                                }
...
HAL_StatusTypeDef USB_HostInit( USB_DRD_TypeDef* USBx, USB_DRD_CfgTypeDef cfg )
{
        ...
        /* Disable all interrupts */
        USBx->CNTR &=
            ~( USB_CNTR_CTRM | USB_CNTR_PMAOVRM | USB_CNTR_ERRM | USB_CNTR_WKUPM | USB_CNTR_SUSPM |
    ...
    uint16_t         mps )
{
        ...
        wChRegVal &=
            ~( USB_CHEP_DEVADDR | USB_CHEP_ADDR | USB_CHEP_LSEP | USB_CHEP_NAK | USB_CHEP_KIND |
               USB_CHEP_ERRTX | USB_CHEP_ERRRX | ( 0xFU << 27 ) );

        /* Set device address and Endpoint number associated to the channel */
        wChRegVal |=
            ( ( (uint32_t) dev_address << USB_CHEP_DEVADDR_Pos ) | ( (uint32_t) epnum & 0x0FU ) );
...
HAL_StatusTypeDef USB_HC_StartXfer( USB_DRD_TypeDef* USBx, USB_DRD_HCTypeDef* hc )
{
        ...
        if ( hc->ch_dir == CH_IN_DIR ) /* In Channel */
        {
                ...
                if ( hc->doublebuffer == 0U ) {
                        ...
                        if ( ( hc->xfer_len > hc->max_packet ) ) {
                                (void) USB_HC_DoubleBuffer(
                                    USBx, (uint8_t) phy_ch_num, USB_DRD_BULK_DBUFF_ENBALE );
                        ...
                        {
                                (void) USB_HC_DoubleBuffer(
                                    USBx, (uint8_t) phy_ch_num, USB_DRD_BULK_DBUFF_DISABLE );
...
{
        USBx->ISTR &=
            ~( USB_ISTR_DIR | USB_ISTR_L1REQ | USB_ISTR_ESOF | USB_ISTR_SOF | USB_ISTR_RESET |
               USB_ISTR_DCON | USB_ISTR_SUSP | USB_ISTR_WKUP | USB_ISTR_ERR | USB_ISTR_PMAOVR |
               USB_ISTR_CTR );
