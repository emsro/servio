  ...
  [..]
    ...
    (#) Initialize the IRDA low level resources by implementing the HAL_IRDA_MspInit() API
        ...
        (++) USARTx/UARTx pins configuration:
            ...
            (+++) Configure these USARTx/UARTx pins (TX as alternate function pull-up, RX as
  alternate function Input).
        (++) NVIC configuration if you need to use interrupt process (HAL_IRDA_Transmit_IT()
            ...
            (+++) The specific IRDA interrupts (Transmission complete interrupt,
                  ...
                  __HAL_IRDA_ENABLE_IT() and __HAL_IRDA_DISABLE_IT() inside the transmit and receive
  process.
...
#define IRDA_CR1_FIELDS                                                                    \
        ( (uint32_t) ( USART_CR1_M | USART_CR1_PCE | USART_CR1_PS | USART_CR1_TE |         \
                       USART_CR1_RE ) ) /*!< UART or USART CR1 fields of parameters set by \
                                           IRDA_SetConfig API */
...
static HAL_StatusTypeDef IRDA_WaitOnFlagUntilTimeout(
 ...
 * @note   The HAL_IRDA_RegisterCallback() may be called before HAL_IRDA_Init() in
 * HAL_IRDA_STATE_RESET to register callbacks for HAL_IRDA_MSPINIT_CB_ID and
 ...
 * @note   The HAL_IRDA_UnRegisterCallback() may be called before HAL_IRDA_Init() in
  ...
  [..]
    (#) There are two modes of transfer:
        ...
        (++) Non-Blocking mode: the communication is performed using Interrupts
             ...
             The HAL_IRDA_ErrorCallback() user callback will be executed when a communication error
is detected
    ...
    (#) For Abort services based on interrupts (HAL_IRDA_Abortxxx_IT), a set of Abort Complete
...
error severity is to be evaluated by user : this concerns Frame Error, Parity Error or Noise Error
             ...
             Received character is then retrieved and stored in Rx buffer, Error code is set to
allow user to identify error type, and HAL_IRDA_ErrorCallback() user callback is executed. Transfer
is kept ongoing on IRDA side. If user wants to abort it, Abort services should be called by user.
...
code is set to allow user to identify error type, and HAL_IRDA_ErrorCallback() user callback is
executed.
 ...
 * = 01), the sent data is handled as a set of u16. In this case, Size must reflect the number of
 * u16 available through pData.
 * @param hirda Pointer to a IRDA_HandleTypeDef structure that contains
 *               the configuration information for the specified IRDA module.
...
HAL_StatusTypeDef HAL_IRDA_Transmit(
    ...
    uint32_t            Timeout )
{
        uint8_t const*  pdata8bits;
        uint16_t const* pdata16bits;
        uint32_t        tickstart;
        ...
        if ( hirda->gState == HAL_IRDA_STATE_READY ) {
                ...
                if ( ( hirda->Init.WordLength == IRDA_WORDLENGTH_9B ) &&
                     ( hirda->Init.Parity == IRDA_PARITY_NONE ) ) {
                        pdata8bits  = NULL;
                        pdata16bits = (uint16_t const*) pData; /* Derogation R.11.3 */
                } else {
                        pdata8bits  = pData;
                        pdata16bits = NULL;
                }
                ...
                while ( hirda->TxXferCount > 0U ) {
                        ...
                        if ( IRDA_WaitOnFlagUntilTimeout(
                                 hirda, IRDA_FLAG_TXE, RESET, tickstart, Timeout ) != HAL_OK ) {
                                return HAL_TIMEOUT;
                        }
                        if ( pdata8bits == NULL ) {
                ...
                if ( IRDA_WaitOnFlagUntilTimeout(
                         hirda, IRDA_FLAG_TC, RESET, tickstart, Timeout ) != HAL_OK ) {
                        return HAL_TIMEOUT;
                }
 ...
 * @note  When UART parity is not enabled (PCE = 0), and Word Length is configured to 9 bits (M1-M0
 * = 01), the received data is handled as a set of u16. In this case, Size must reflect the number
 ...
 */
HAL_StatusTypeDef
HAL_IRDA_Receive( IRDA_HandleTypeDef* hirda, uint8_t* pData, uint16_t Size, uint32_t Timeout )
{
        uint8_t*  pdata8bits;
        ...
        if ( hirda->RxState == HAL_IRDA_STATE_READY ) {
                ...
                while ( hirda->RxXferCount > 0U ) {
                        ...
                        if ( IRDA_WaitOnFlagUntilTimeout(
                                 hirda, IRDA_FLAG_RXNE, RESET, tickstart, Timeout ) != HAL_OK ) {
                                return HAL_TIMEOUT;
                        }
                        if ( pdata8bits == NULL ) {
 ...
 */
HAL_StatusTypeDef
HAL_IRDA_Transmit_IT( IRDA_HandleTypeDef* hirda, uint8_t const* pData, uint16_t Size )
{
        /* Check that a Tx process is not already ongoing */
        if ( hirda->gState == HAL_IRDA_STATE_READY ) {
                if ( ( pData == NULL ) || ( Size == 0U ) )
 ...
 */
HAL_StatusTypeDef
HAL_IRDA_Transmit_DMA( IRDA_HandleTypeDef* hirda, uint8_t const* pData, uint16_t Size )
{
        ...
        if ( hirda->gState == HAL_IRDA_STATE_READY ) {
                ...
                /* In case of 9bits/No Parity transfer, pData buffer provided as input parameter
                   should be aligned on a u16 frontier, so nbByte should be equal to Size multiplied
                   by 2 */
                if ( ( hirda->Init.WordLength == IRDA_WORDLENGTH_9B ) &&
                     ( hirda->Init.Parity == IRDA_PARITY_NONE ) ) {
                        nbByte = Size * 2U;
...
HAL_StatusTypeDef HAL_IRDA_DMAStop( IRDA_HandleTypeDef* hirda )
{
        /* The Lock is not implemented on this API to allow the user application
           ...
           HAL_IRDA_RxCpltCallback() / HAL_IRDA_TxHalfCpltCallback / HAL_IRDA_RxHalfCpltCallback:
                ...
                if ( HAL_IS_BIT_SET( hirda->Instance->CR3, USART_CR3_DMAT ) ) {
                        ...
                        if ( hirda->hdmatx != NULL ) {
                                if ( HAL_DMA_Abort( hirda->hdmatx ) != HAL_OK ) {
                                        if ( HAL_DMA_GetError( hirda->hdmatx ) ==
                ...
                if ( HAL_IS_BIT_SET( hirda->Instance->CR3, USART_CR3_DMAR ) ) {
                        ...
                        if ( hirda->hdmarx != NULL ) {
                                if ( HAL_DMA_Abort( hirda->hdmarx ) != HAL_OK ) {
                                        if ( HAL_DMA_GetError( hirda->hdmarx ) ==
                                             HAL_DMA_ERROR_TIMEOUT ) {
                                                /* Set error code to DMA */
                                                hirda->ErrorCode = HAL_IRDA_ERROR_DMA;
...
HAL_StatusTypeDef HAL_IRDA_Abort( IRDA_HandleTypeDef* hirda )
{
        ...
        __HAL_IRDA_CLEAR_FLAG(
            hirda, IRDA_CLEAR_OREF | IRDA_CLEAR_NEF | IRDA_CLEAR_PEF | IRDA_CLEAR_FEF );
...
#if defined( HAL_DMA_MODULE_ENABLED )
        /* If DMA Tx and/or DMA Rx Handles are associated to IRDA Handle, DMA Abort complete
...
HAL_StatusTypeDef HAL_IRDA_AbortTransmit_IT( IRDA_HandleTypeDef* hirda )
{
        ...
        if ( HAL_IS_BIT_SET( hirda->Instance->CR3, USART_CR3_DMAT ) ) {
                ...
                if ( hirda->hdmatx != NULL ) {
                        ...
                        /* Abort DMA TX */
                        if ( HAL_DMA_Abort_IT( hirda->hdmatx ) != HAL_OK ) {
                                /* Call Directly hirda->hdmatx->XferAbortCallback function in case
                                 * of error */
...
HAL_StatusTypeDef HAL_IRDA_AbortReceive_IT( IRDA_HandleTypeDef* hirda )
{
        ...
        if ( HAL_IS_BIT_SET( hirda->Instance->CR3, USART_CR3_DMAR ) ) {
                ...
                if ( hirda->hdmarx != NULL ) {
                        ...
                        __HAL_IRDA_CLEAR_FLAG(
                            hirda,
                            IRDA_CLEAR_OREF | IRDA_CLEAR_NEF | IRDA_CLEAR_PEF | IRDA_CLEAR_FEF );
                        ...
                        if ( ( HAL_IS_BIT_SET( hirda->Instance->CR3, USART_CR3_DMAR ) ) ||
                             ( ( errorcode & HAL_IRDA_ERROR_ORE ) != 0U ) ) {
                                ...
                                if ( HAL_IS_BIT_SET( hirda->Instance->CR3, USART_CR3_DMAR ) ) {
                                        ...
                                        if ( hirda->hdmarx != NULL ) {
                                                /* Set the IRDA DMA Abort callback :
                                                   will lead to call HAL_IRDA_ErrorCallback() at end
                                                   of DMA abort procedure */
                                                hirda->hdmarx->XferAbortCallback =
                                                    IRDA_DMAAbortOnError;

                                                /* Abort DMA RX */
                                                if ( HAL_DMA_Abort_IT( hirda->hdmarx ) != HAL_OK ) {
                                                        /* Call Directly
                                                         * hirda->hdmarx->XferAbortCallback function
                                                         * in case of error */
                                                        hirda->hdmarx->XferAbortCallback(
                                                            hirda->hdmarx );
                                                }
 ...
 * @retval HAL state
 */
HAL_IRDA_StateTypeDef HAL_IRDA_GetState( IRDA_HandleTypeDef const* hirda )
{
 ...
 */
uint32_t HAL_IRDA_GetError( IRDA_HandleTypeDef const* hirda )
{
...
static HAL_StatusTypeDef IRDA_SetConfig( IRDA_HandleTypeDef* hirda )
{
        ...
        static uint16_t const   IRDAPrescTable[12] = {
        ...
        MODIFY_REG(
            hirda->Instance->GTPR, (uint16_t) USART_GTPR_PSC, (uint16_t) hirda->Init.Prescaler );
        ...
        case IRDA_CLOCKSOURCE_CSI:
                tmpreg = (uint32_t) ( IRDA_DIV_SAMPLING16(
                    CSI_VALUE, hirda->Init.BaudRate, hirda->Init.ClockPrescaler ) );
                break;
        ...
        case IRDA_CLOCKSOURCE_HSI:
                tmpreg = (uint32_t) ( IRDA_DIV_SAMPLING16(
                    HSI_VALUE, hirda->Init.BaudRate, hirda->Init.ClockPrescaler ) );
                break;
...
static HAL_StatusTypeDef IRDA_CheckIdleState( IRDA_HandleTypeDef* hirda )
{
        ...
        if ( ( hirda->Instance->CR1 & USART_CR1_TE ) == USART_CR1_TE ) {
                ...
                if ( IRDA_WaitOnFlagUntilTimeout(
                         hirda, USART_ISR_TEACK, RESET, tickstart, IRDA_TEACK_REACK_TIMEOUT ) !=
                     HAL_OK ) {
                        /* Timeout occurred */
                        return HAL_TIMEOUT;
                }
        ...
        if ( ( hirda->Instance->CR1 & USART_CR1_RE ) == USART_CR1_RE ) {
                ...
                if ( IRDA_WaitOnFlagUntilTimeout(
                         hirda, USART_ISR_REACK, RESET, tickstart, IRDA_TEACK_REACK_TIMEOUT ) !=
                     HAL_OK ) {
                        /* Timeout occurred */
                        return HAL_TIMEOUT;
                }
...
#if defined( HAL_DMA_MODULE_ENABLED )
/**
 * @brief  End ongoing Tx transfer on IRDA peripheral (following error detection or Transmit
 * completion).
 * @param  hirda Pointer to a IRDA_HandleTypeDef structure that contains
 *               the configuration information for the specified IRDA module.
 * @retval None
...
static void IRDA_Transmit_IT( IRDA_HandleTypeDef* hirda )
{
        ...
        if ( hirda->gState == HAL_IRDA_STATE_BUSY_TX ) {
                if ( hirda->TxXferCount == 0U ) {
                        ...
                        if ( ( hirda->Init.WordLength == IRDA_WORDLENGTH_9B ) &&
                             ( hirda->Init.Parity == IRDA_PARITY_NONE ) ) {
                                tmp = (uint16_t const*) hirda->pTxBuffPtr; /* Derogation R.11.3 */
...
static void IRDA_Receive_IT( IRDA_HandleTypeDef* hirda )
{
        ...
        if ( hirda->RxState == HAL_IRDA_STATE_BUSY_RX ) {
                ...
                if ( hirda->RxXferCount == 0U ) {
                        ...
                        CLEAR_BIT(
                            hirda->Instance->CR1, ( USART_CR1_RXNEIE_RXFNEIE | USART_CR1_PEIE ) );
