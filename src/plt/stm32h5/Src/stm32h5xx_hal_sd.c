...
void HAL_SD_IRQHandler( SD_HandleTypeDef* hsd )
{
        ...
        if ( ( __HAL_SD_GET_FLAG( hsd, SDMMC_FLAG_RXFIFOHF ) != RESET ) &&
            ...
            ( __HAL_SD_GET_FLAG( hsd, SDMMC_FLAG_TXFIFOHE ) != RESET ) &&
            ( ( context & SD_CONTEXT_IT ) != 0U ) ) {
                SD_Write_IT( hsd );
        }
 ...
 *          @arg @ref HAL_SD_READ_DMA_LNKLST_BUF_CPLT_CB_ID  SD DMA Rx Linked List Node buffer
 * Callback ID
 *          @arg @ref HAL_SD_WRITE_DMA_LNKLST_BUF_CPLT_CB_ID SD DMA Tx Linked List Node buffer
 * Callback ID
...
HAL_StatusTypeDef HAL_SD_GetCardCID( SD_HandleTypeDef const* hsd, HAL_SD_CardCIDTypeDef* pCID )
{
        ...
        pCID->ProdSN =
            ( ( ( hsd->CID[2] & 0x00FFFFFFU ) << 8U ) | ( ( hsd->CID[3] & 0xFF000000U ) >> 24U ) );
 ...
 */
HAL_StatusTypeDef
HAL_SD_GetCardInfo( SD_HandleTypeDef const* hsd, HAL_SD_CardInfoTypeDef* pCardInfo )
{
                        ...
                        if ( hsd->Init.ClockDiv >= ( sdmmc_clk / ( 2U * SD_NORMAL_SPEED_FREQ ) ) ) {
                                ...
                                if ( hsd->Init.ClockDiv == 0U ) {
                                        if ( sdmmc_clk > SD_HIGH_SPEED_FREQ ) {
                                                Init.ClockDiv =
                                                    sdmmc_clk / ( 2U * SD_HIGH_SPEED_FREQ );
                                        } else {
                                                Init.ClockDiv = hsd->Init.ClockDiv;
                                        }
                                } else if (
                                    ( sdmmc_clk / ( 2U * hsd->Init.ClockDiv ) ) >
                                    SD_HIGH_SPEED_FREQ ) {
                                ...
                                if ( hsd->Init.ClockDiv == 0U ) {
                                        if ( sdmmc_clk > SD_NORMAL_SPEED_FREQ ) {
                                                Init.ClockDiv =
                                                    sdmmc_clk / ( 2U * SD_NORMAL_SPEED_FREQ );
                                        } else {
                                                Init.ClockDiv = hsd->Init.ClockDiv;
                                        }
                                } else if (
...
HAL_StatusTypeDef HAL_SD_Abort( SD_HandleTypeDef* hsd )
{
        ...
        if ( hsd->State == HAL_SD_STATE_BUSY ) {
                ...
                if ( ( hsd->Instance->DCTRL & SDMMC_DCTRL_DTDIR ) == SDMMC_TRANSFER_DIR_TO_CARD ) {
                        if ( hsd->ErrorCode == HAL_SD_ERROR_NONE ) {
                                while ( !__HAL_SD_GET_FLAG(
                                    hsd, SDMMC_FLAG_DABORT | SDMMC_FLAG_BUSYD0END ) ) {
                    ...
                    ( hsd->Instance->DCTRL & SDMMC_DCTRL_DTDIR ) == SDMMC_TRANSFER_DIR_TO_SDMMC ) {
                        while (
...
static uint32_t SD_UltraHighSpeed( SD_HandleTypeDef* hsd, uint32_t UltraHighSpeedMode )
{
        ...
        if ( hsd->SdCard.CardSpeed == CARD_ULTRA_HIGH_SPEED ) {
                ...
                if ( ( ( (uint8_t*) SD_hs )[13] & 2U ) != 2U ) {
                         ...
                         */
                        MODIFY_REG(
                            hsd->Instance->CLKCR, SDMMC_CLKCR_SELCLKRX, SDMMC_CLKCR_SELCLKRX_1 );
...
static uint32_t SD_DDR_Mode( SD_HandleTypeDef* hsd )
{
        ...
        if ( hsd->SdCard.CardSpeed == CARD_ULTRA_HIGH_SPEED ) {
                ...
                if ( ( ( (uint8_t*) SD_hs )[13] & 2U ) != 2U ) {
                        ...
                        MODIFY_REG(
                            hsd->Instance->CLKCR, SDMMC_CLKCR_SELCLKRX, SDMMC_CLKCR_SELCLKRX_0 );
                        LL_DLYB_Enable( SD_GET_DLYB_INSTANCE( hsd->Instance ) );
#endif /* (DLYB_SDMMC1) || (DLYB_SDMMC2) */
                }
        }
