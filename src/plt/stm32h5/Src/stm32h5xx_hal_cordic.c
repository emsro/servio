 ...
 */
static void
CORDIC_WriteInDataIncrementPtr( CORDIC_HandleTypeDef const* hcordic, int32_t const** ppInBuff );
static void
CORDIC_ReadOutDataIncrementPtr( CORDIC_HandleTypeDef const* hcordic, int32_t** ppOutBuff );
    ...
    uint32_t              Timeout )
{
        uint32_t       tickstart;
        uint32_t       index;
        int32_t const* p_tmp_in_buff  = pInBuff;
...
HAL_StatusTypeDef HAL_CORDIC_Calculate_IT(
    ...
    uint32_t              NbCalc )
{
        int32_t const* tmp_pInBuff = pInBuff;
