    ...
    FMC_NAND_PCC_TimingTypeDef* AttSpace_Timing )
{
        ...
        (void) FMC_NAND_CommonSpace_Timing_Init(
            hnand->Instance, ComSpace_Timing, hnand->Init.NandBank );

        /* Initialize NAND attribute space timing Interface */
        (void) FMC_NAND_AttributeSpace_Timing_Init(
            hnand->Instance, AttSpace_Timing, hnand->Init.NandBank );
                ...
                while ( ( nbpages != 0U ) && ( nandaddress < ( ( hnand->Config.BlockSize ) *
                        ...
                        *(__IO uint8_t*) ( (uint32_t) ( deviceaddress | CMD_AREA ) ) =
                        ...
                        if ( ( hnand->Config.PageSize ) <= 512U ) {
                                if ( ( ( hnand->Config.BlockSize ) * ( hnand->Config.BlockNbr ) ) <=
                                     65535U ) {
