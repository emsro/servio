...
/**
 * @brief  This function configures the Cortex-M SysTick source to have 1ms time base with LSE as
...
/**
 * @brief  This function configures the Cortex-M SysTick source to have 1ms time base with LSI as
 * SysTick clock source.
 * @note   When a RTOS is used, it is recommended to avoid changing the Systick
 *         configuration by calling this function, for a delay use rather osDelay RTOS service.
 * @retval None
...
/** @addtogroup UTILS_EF_SYSTEM
 ...
 voltage range 3   |
             (++) |                 |    1.30 - 1.40V   |   1.15 - 1.26V   |   1.05 - 1.15V   | 0,95
 - 1,05V    |
             (++)
 |-----------------|-------------------|------------------|------------------|-------------------|
             (++) |0WS(1 CPU cycles)|   0 < HCLK <= 42  |  0 < HCLK <= 34  |  0 < HCLK <= 30  | 0 <
 HCLK <= 20    |
             (++)
 |-----------------|-------------------|------------------|------------------|-------------------|
             (++) |1WS(2 CPU cycles)|  42 < HCLK <= 84  | 34 < HCLK <= 68  | 30 < HCLK <= 60  | 20 <
 HCLK <= 40   |
             (++)
 |-----------------|-------------------|------------------|------------------|-------------------|
             (++) |2WS(3 CPU cycles)|  84 < HCLK <= 126 | 68 < HCLK <= 102 | 60 < HCLK <= 90  | 40 <
 HCLK <= 60   |
             (++)
 |-----------------|-------------------|------------------|------------------|-------------------|
             (++) |3WS(4 CPU cycles)| 126 < HCLK <= 168 | 102 < HCLK <= 136| 90 < HCLK <= 120 | 60 <
 HCLK <= 80   |
             (++)
 |-----------------|-------------------|------------------|------------------|-------------------|
             (++) |4WS(5 CPU cycles)|  168 < HCLK <= 210| 136 < HCLK <= 170| 120 < HCLK <= 150| 80 <
 HCLK <= 100  |
             (++)
 |-----------------|-------------------|------------------|------------------|-------------------|
...
/**
 * @brief  This function configures system clock at maximum frequency with CSI as clock source of
 * the PLL1
 ...
 *         - PLL1M: ensure that the VCO input frequency ranges from 1 to 16 MHz (PLL1VCO_input = CSI
 * frequency / PLL1M)
 ...
 */
ErrorStatus LL_PLL_ConfigSystemClock_CSI(
    ...
    LL_UTILS_ClkInitTypeDef* UTILS_ClkInitStruct )
{
        ...
        assert_param( IS_LL_UTILS_CHECK_VCO_RANGES(
            UTILS_PLLInitStruct->VCO_Input, UTILS_PLLInitStruct->VCO_Output ) );
        ...
        vcooutput_freq = LL_RCC_CalcPLLClockFreq(
            ...
            UTILS_PLLInitStruct->FRACN,
            1U );
        /* PLL1P Set to 1 to check the assert param (VCO_output)*/
#endif /* USE_FULL_ASSERT */
        assert_param(
            IS_LL_UTILS_PLLVCO_OUTPUT( vcooutput_freq, UTILS_PLLInitStruct->VCO_Output ) );
...
/**
 * @brief  This function configures system clock at maximum frequency with HSI as clock source of
 * the PLL1
 * @note   The application need to ensure that all PLLs are disabled.
 * @note   Function is based on the following formula:
 *         - PLL1 output frequency = (((HSI frequency / PLL1M) * PLL1N) / PLL1P)
 *         - PLL1M: ensure that the VCO input frequency ranges from 1 to 16 MHz (PLL1VCO_input = HSI
 * frequency / PLL1M)
 ...
 *         - PLL1M: ensure that the VCO input frequency ranges from 1 to 16 MHz (PLL1VCO_input = HSE
 * frequency / PLL1M)
 *         - PLL1N: ensure that the VCO output frequency is between 192 and 836 MHz
 *          (PLL1VCO_output = PLL1VCO_input * PLL1N)
 *         - PLL1P: ensure that max frequency at 250 MHz is reached (PLL1VCO_output / PLL1P)
...
ErrorStatus LL_SetFlashLatency( uint32_t HCLK_Frequency )
{
        ...
        if ( HCLK_Frequency == 0U ) {
                status = ERROR;
        } else if ( LL_PWR_GetRegulVoltageScaling() == LL_PWR_REGU_VOLTAGE_SCALE0 ) {
    ...
    LL_UTILS_PLLInitTypeDef const* UTILS_PLLInitStruct )
{
        ...
        /*  - PLLP: ensure that PLL1P output frequency does not exceed the corresponding maximum
         * voltage scale frequency.  */
    ...
    LL_UTILS_ClkInitTypeDef* UTILS_ClkInitStruct )
{
        ...
        hclk_frequency =
            __LL_RCC_CALC_HCLK_FREQ( SYSCLK_Frequency, UTILS_ClkInitStruct->SYSCLKDivider );
