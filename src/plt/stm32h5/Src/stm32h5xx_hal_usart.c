...
#define USART_CR1_FIELDS                                                                          \
        ( (uint32_t) ( USART_CR1_M | USART_CR1_PCE | USART_CR1_PS | USART_CR1_TE | USART_CR1_RE | \
                       USART_CR1_OVER8 | USART_CR1_FIFOEN ) ) /*!< USART CR1 fields of parameters \
...
#define USART_CR3_FIELDS                                                                           \
        ( (uint32_t) ( USART_CR3_TXFTCFG | USART_CR3_RXFTCFG ) ) /*!< USART or USART CR3 fields of \
                                                                    parameters set by              \
                                                                    USART_SetConfig API */
...
HAL_StatusTypeDef HAL_USART_Transmit(
    ...
    uint32_t             Timeout )
{
        uint8_t const*  ptxdata8bits;
        uint16_t const* ptxdata16bits;
        uint32_t        tickstart;
        ...
        if ( husart->State == HAL_USART_STATE_READY ) {
                ...
                if ( ( husart->Init.WordLength == USART_WORDLENGTH_9B ) &&
                     ( husart->Init.Parity == USART_PARITY_NONE ) ) {
                        ptxdata8bits  = NULL;
                        ptxdata16bits = (uint16_t const*) pTxData;
                } else {
                ...
                while ( husart->TxXferCount > 0U ) {
                        if ( USART_WaitOnFlagUntilTimeout(
                                 husart, USART_FLAG_TXE, RESET, tickstart, Timeout ) != HAL_OK ) {
                                return HAL_TIMEOUT;
                        }
                ...
                if ( USART_WaitOnFlagUntilTimeout(
                         husart, USART_FLAG_TC, RESET, tickstart, Timeout ) != HAL_OK ) {
                        return HAL_TIMEOUT;
                }
        ...
        if ( husart->State == HAL_USART_STATE_READY ) {
                ...
                while ( husart->RxXferCount > 0U ) {
                        if ( husart->SlaveMode == USART_SLAVEMODE_DISABLE ) {
                                 ...
                                 * for all the cases. */
                                if ( USART_WaitOnFlagUntilTimeout(
                                         husart, USART_FLAG_TXE, RESET, tickstart, Timeout ) !=
                                     HAL_OK ) {
                                        return HAL_TIMEOUT;
                                }
                        ...
                        /* Wait for RXNE Flag */
                        if ( USART_WaitOnFlagUntilTimeout(
                                 husart, USART_FLAG_RXNE, RESET, tickstart, Timeout ) != HAL_OK ) {
                                return HAL_TIMEOUT;
                        }
        ...
        if ( husart->State == HAL_USART_STATE_READY ) {
                ...
                while ( ( husart->TxXferCount > 0U ) || ( rxdatacount > 0U ) ) {
                        ...
                        if ( husart->RxXferCount > 0U ) {
                                /* Wait for RXNE Flag */
                                if ( USART_WaitOnFlagUntilTimeout(
                                         husart, USART_FLAG_RXNE, RESET, tickstart, Timeout ) !=
                                     HAL_OK ) {
                                        return HAL_TIMEOUT;
                                }
                                ...
                                if ( prxdata8bits == NULL ) {
                                        *prxdata16bits =
                                            (uint16_t) ( husart->Instance->RDR & uhMask );
                                        prxdata16bits++;
                                } else {
                                        *prxdata8bits = (uint8_t) ( husart->Instance->RDR &
                                                                    (uint8_t) ( uhMask & 0xFFU ) );
 ...
 */
HAL_StatusTypeDef
HAL_USART_Transmit_IT( USART_HandleTypeDef* husart, uint8_t const* pTxData, uint16_t Size )
{
        if ( husart->State == HAL_USART_STATE_READY ) {
                if ( ( pTxData == NULL ) || ( Size == 0U ) )
                        return HAL_ERROR;
 ...
 */
HAL_StatusTypeDef
HAL_USART_Transmit_DMA( USART_HandleTypeDef* husart, uint8_t const* pTxData, uint16_t Size )
{
        HAL_StatusTypeDef status = HAL_OK;
        uint32_t const*   tmp;
...
{
        HAL_USART_StateTypeDef const state = husart->State;
        ...
        if ( ( HAL_IS_BIT_SET( husart->Instance->CR3, USART_CR3_DMAT ) ) &&
            ...
            ( state == HAL_USART_STATE_BUSY_RX ) || ( state == HAL_USART_STATE_BUSY_TX_RX ) ) {
...
HAL_StatusTypeDef HAL_USART_Abort( USART_HandleTypeDef* husart )
{
        ...
        __HAL_USART_CLEAR_FLAG(
            husart, USART_CLEAR_OREF | USART_CLEAR_NEF | USART_CLEAR_PEF | USART_CLEAR_FEF );
                             ...
                             ( errorcode != 0U ) ) {
                                ...
                                if ( HAL_IS_BIT_SET( husart->Instance->CR3, USART_CR3_DMAR ) ) {
                                        ...
                                        if ( husart->hdmarx != NULL ) {
                                                ...
                                                /* Abort DMA RX */
                                                if ( HAL_DMA_Abort_IT( husart->hdmarx ) !=
                                                     HAL_OK ) {
                                                        /* Call Directly
                                                         * husart->hdmarx->XferAbortCallback
 ...
 */
HAL_USART_StateTypeDef HAL_USART_GetState( USART_HandleTypeDef const* husart )
{
        return husart->State;
}
 ...
 */
uint32_t HAL_USART_GetError( USART_HandleTypeDef const* husart )
{
...
static void USART_DMATransmitCplt( DMA_HandleTypeDef* hdma )
{
        ...
        /* DMA Circular mode */
        else if ( husart->State == HAL_USART_STATE_BUSY_TX ) {
#if ( USE_HAL_USART_REGISTER_CALLBACKS == 1 )
...
static HAL_StatusTypeDef USART_SetConfig( USART_HandleTypeDef* husart )
{
         ...
         * set LBCL bit according to husart->Init.CLKLastBit value (used in USART Synchronous SPI
         * master mode only) set STOP[13:12] bits according to husart->Init.StopBits value */
        ...
        case USART_CLOCKSOURCE_HSI:
                usartdiv = (uint32_t) ( USART_DIV_SAMPLING8(
                    HSI_VALUE, husart->Init.BaudRate, husart->Init.ClockPrescaler ) );
                break;
        case USART_CLOCKSOURCE_CSI:
                usartdiv = (uint32_t) ( USART_DIV_SAMPLING8(
                    CSI_VALUE, husart->Init.BaudRate, husart->Init.ClockPrescaler ) );
                break;
        case USART_CLOCKSOURCE_LSE:
                usartdiv = (uint32_t) ( USART_DIV_SAMPLING8(
                    LSE_VALUE, husart->Init.BaudRate, husart->Init.ClockPrescaler ) );
                break;
        default:
                ret = HAL_ERROR;
                break;
...
static HAL_StatusTypeDef USART_CheckIdleState( USART_HandleTypeDef* husart )
{
        ...
        if ( ( husart->Instance->CR1 & USART_CR1_TE ) == USART_CR1_TE ) {
                ...
                if ( USART_WaitOnFlagUntilTimeout(
                         husart, USART_ISR_TEACK, RESET, tickstart, USART_TEACK_REACK_TIMEOUT ) !=
                     HAL_OK ) {
                        /* Timeout occurred */
                        return HAL_TIMEOUT;
                }
        ...
        if ( ( husart->Instance->CR1 & USART_CR1_RE ) == USART_CR1_RE ) {
                ...
                if ( USART_WaitOnFlagUntilTimeout(
                         husart, USART_ISR_REACK, RESET, tickstart, USART_TEACK_REACK_TIMEOUT ) !=
                     HAL_OK ) {
                        /* Timeout occurred */
                        return HAL_TIMEOUT;
                }
...
static void USART_TxISR_16BIT( USART_HandleTypeDef* husart )
{
        ...
        if ( ( state == HAL_USART_STATE_BUSY_TX ) || ( state == HAL_USART_STATE_BUSY_TX_RX ) ) {
                if ( husart->TxXferCount == 0U ) {
                        ...
                        tmp                   = (uint16_t const*) husart->pTxBuffPtr;
        ...
        if ( ( state == HAL_USART_STATE_BUSY_RX ) || ( state == HAL_USART_STATE_BUSY_TX_RX ) ) {
                *husart->pRxBuffPtr = (uint8_t) ( husart->Instance->RDR & (uint8_t) uhMask );
                ...
                if ( husart->RxXferCount == 0U ) {
                        ...
                        CLEAR_BIT(
                            husart->Instance->CR1, ( USART_CR1_RXNEIE_RXFNEIE | USART_CR1_PEIE ) );
                        ...
                        if ( state == HAL_USART_STATE_BUSY_RX ) {
                            ...
                            ( READ_BIT( husart->Instance->CR1, USART_CR1_TCIE ) !=
                              USART_CR1_TCIE ) &&
                        ...
                        if ( __HAL_USART_GET_FLAG( husart, USART_FLAG_RXFNE ) == SET ) {
                                *husart->pRxBuffPtr = (uint8_t) ( husart->Instance->RDR &
                                ...
                                if ( husart->RxXferCount == 0U ) {
                                        ...
                                        txftie =
                                            READ_BIT( husart->Instance->CR3, USART_CR3_TXFTIE );
                                        txdatacount = husart->TxXferCount;
