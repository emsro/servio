/**
  ...
  **********************************************************************************************************************
  ...
  ======================================================================================================================
    ...
    (#)Initialize the I3C low level resources by implementing the HAL_I3C_MspInit() API:
        ...
        (##) DMA Configuration if you need to use DMA process
            ...
            (+++) Associate the initialized DMA handle to the hi3c DMA CCC, Tx, Rx or Status handle
  as necessary
    ...
    (#) Configure the Target Communication Bus characterics for Target mode.
        This mean, configure the parameter BusAvailableDuration in the LL_I3C_TgtBusConfTypeDef
  structure through h3c Init structure.
    ...
    (#) Initialize the I3C registers by calling the HAL_I3C_Init(), configures also the low level
  Hardware (GPIO, CLOCK, NVIC...etc) by calling the customized HAL_I3C_MspInit(&hi3c) API.

    (#) Configure the different FIFO parameters in I3C_FifoConfTypeDef structure as RxFifoThreshold,
  TxFifoThreshold for Controller or Target mode. And enable/disable the Control or Status FIFO only
  for Controller Mode. Use HAL_I3C_SetConfigFifo() function to finalize the configuration, and
  HAL_I3C_GetConfigFifo() to retrieve FIFO configuration. Possibility to clear the FIFO
  configuration by using HAL_I3C_ClearConfigFifo() which reset the configuration FIFO to their
  default hardware value

    (#) Configure the different additional Controller configuration in I3C_CtrlConfTypeDef structure
  as DynamicAddr, StallTime, HotJoinAllowed, ACKStallState, CCCStallState, TxStallState,
  RxStallState, HighKeeperSDA. Use HAL_I3C_Ctrl_Config() function to finalize the Controller
  configuration.

    (#) Configure the different additional Target configuration in I3C_TgtConfTypeDef structure as
  Identifier, MIPIIdentifier, CtrlRoleRequest, HotJoinRequest, IBIRequest, IBIPayload,
  IBIPayloadSize, MaxReadDataSize, MaxWriteDataSize, CtrlCapability, GroupAddrCapability,
  DataTurnAroundDuration, MaxReadTurnAround, MaxDataSpeed, MaxSpeedLimitation, HandOffActivityState,
  HandOffDelay, PendingReadMDB. Use HAL_I3C_Tgt_Config() function to finalize the Target
  configuration.
    ...
    (#) Before initiate any IO operation, the application must launch an assignment of the different
        ...
        For the initiation of ENTDAA procedure from the controller, each target connected and
  powered on the I3C bus must repond to this particular Command Common Code by sending its proper
  Payload (a amount of 48bits which contain the target characteristics) Each time a target responds
  to ENTDAA sequence, the application is informed through HAL_I3C_TgtReqDynamicAddrCallback() of the
  reception of the target paylaod. And then application must send a associated dynamic address
  through HAL_I3C_Ctrl_SetDynAddr(). This procedure in loop automatically in hardware side until a
  target respond to repeated ENTDAA sequence. The application is informed of the end of the
  procedure at reception of HAL_I3C_CtrlDAACpltCallback(). Then application can easily retrieve
  ENTDAA payload information through HAL_I3C_Get_ENTDAA_Payload_Info() function. At the end of
  procedure, the function HAL_I3C_Ctrl_ConfigBusDevices() must be called to store in hardware
        register part the target capabilities as Dynamic address, IBI support with or without
  additional data byte, Controller role request support, Controller stop transfer after IBI through
  I3C_DeviceConfTypeDef structure.

    (#) Other action to be done, before initiate any IO operation, the application must prepare the
  different frame descriptor with its associated buffer allocation in their side. Configure the
  different information related to CCC transfer through I3C_CCCTypeDef structure Configure the
  different information related to Private or I2C transfer through I3C_PrivateTypeDef structure
        Configure the different buffer pointers and associated size needed for the driver
  communication through I3C_XferTypeDef structure The I3C_XferTypeDef structure contains different
  parameters about Control, Status buffer, and Transmit and Receive buffer. Use
  HAL_I3C_AddDescToFrame() function each time application add a descriptor in the frame before call
    ...
    (#) To check if I3C target device is ready for communication, use the function
  HAL_I3C_Ctrl_IsDeviceI3C_Ready()

    (#) To check if I2C target device is ready for communication, use the function
  HAL_I3C_Ctrl_IsDeviceI2C_Ready()

    (#) To send a message header {S + 0x7E + W + STOP}, use the function
  HAL_I3C_Ctrl_GenerateArbitration().
    (#) To insert a target reset pattern before the STOP of a transmitted frame containing a RSTACT
  CCC command, the application must enable the reset pattern configuration using
  HAL_I3C_Ctrl_SetConfigResetPattern() before calling HAL_I3C_Ctrl_TransmitCCC() or
  HAL_I3C_Ctrl_ReceiveCCC() interfaces.

        To have a standard STOP emitted at the end of a frame containing a RSTACT CCC command, the
  application must disable the reset pattern configuration using
  HAL_I3C_Ctrl_SetConfigResetPattern() before calling HAL_I3C_Ctrl_TransmitCCC() or
  HAL_I3C_Ctrl_ReceiveCCC() interfaces.

        Use HAL_I3C_Ctrl_SetConfigResetPattern() function to configure the insertion of the reset
  pattern at the end of a Frame, and HAL_I3C_Ctrl_GetConfigResetPattern() to retrieve reset pattern
  configuration.
    ...
    *** Polling mode IO operation ***
    =================================
    [..]
      (+) Activate asynchronous event in controller or target mode a Common Command Code in a
  broadcast or a direct communication in blocking mode using HAL_I3C_Ctrl_TransmitCCC()
      (+) Transmit in controller mode a Common Command Code in a broadcast or a direct communication
  in blocking mode using HAL_I3C_Ctrl_TransmitCCC()
      (+) Receive in controller mode a Common Command Code in a direct communication in blocking
  mode using HAL_I3C_Ctrl_ReceiveCCC()
      (+) Transmit in controller mode an amount of private data in an I3C or an I2C communication in
  blocking mode using HAL_I3C_Ctrl_Transmit()
      (+) Receive in controller mode an amount of private data in an I3C or an I2C communication in
  blocking mode using HAL_I3C_Ctrl_Receive()
      ...
      (+) At the end of a transfer, the different FIFO can be flushed if necessary by using
  HAL_I3C_FlushAllFifo() for flush all the FIFO, or flush individually y using
  HAL_I3C_FlushTxFifo(), HAL_I3C_FlushRxFifo(), HAL_I3C_FlushControlFifo(),
  HAL_I3C_FlushStatusFifo().
  ...
  HAL_I3C_Tgt_ControlRoleReq()
    ...
    *** DMA and Interrupt mode IO operation ***
    ...
    [..]
      (+) Transmit in controller mode a Common Command Code in a broadcast or a direct communication
  in non-blocking mode using HAL_I3C_Ctrl_TransmitCCC_IT() or HAL_I3C_Ctrl_TransmitCCC_DMA()
      ...
      (+) Transmit in controller mode an amount of private data in an I3C or an I2C communication in
  non-blocking mode using HAL_I3C_Ctrl_Transmit_IT() or HAL_I3C_Ctrl_Transmit_DMA()
      (+) At transmission end of transfer, HAL_I3C_CtrlTxCpltCallback() is executed and users can
           add their own code by customization of function pointer HAL_I3C_CtrlTxCpltCallback()
      (+) Receive in controller mode an amount of private data in an I3C or an I2C communication in
  non-blocking mode using HAL_I3C_Ctrl_Receive_IT() or HAL_I3C_Ctrl_Receive_DMA()
      (+) At reception end of transfer, HAL_I3C_CtrlRxCpltCallback() is executed and users can
           add their own code by customization of function pointer HAL_I3C_CtrlRxCpltCallback()
      (+) Transfer in multiple direction (transmit/receive) in controller mode a Common Command Code
  in a direct communication or an amount of private data in an I2C or I3C communication in
  non-blocking mode using HAL_I3C_Ctrl_MultipleTransfer_IT() or HAL_I3C_Ctrl_MultipleTransfer_DMA()
  ...
  HAL_I3C_CtrlMultipleXferCpltCallback()
      (+) Transmit in target mode an amount of private data in an I3C communication in non-blocking
  mode using HAL_I3C_Tgt_Transmit_IT() or HAL_I3C_Tgt_Transmit_DMA()
      ...
      (+) Receive in target mode an amount of private data in an I3C communication in non-blocking
  mode using HAL_I3C_Tgt_Receive_IT() or HAL_I3C_Tgt_Receive_DMA()
  ...
  HAL_I3C_DeactivateNotification() function is used for enable or disable one or more notification
  related to specific asynchronous event. Each time one or more event detected by hardware the
  associated HAL_I3C_NotifyCallback() is executed and users can add their own code by customization
  of function pointer HAL_I3C_NotifyCallback(). Then application can easily retrieve some specific
  ...
  HAL_I3C_DeactivateNotification() function is used for enable or disable Wake Up interrupt. At
  wakeup detection the associated HAL_I3C_NotifyCallback() is executed.
    ...
    [..]
     Function HAL_I3C_RegisterCallback() allows to register following callbacks:
       (+) CtrlTxCpltCallback   : callback for Controller transmission CCC, I3C private or I2C end
  of transfer.
       (+) CtrlRxCpltCallback   : callback for Controller reception CCC, I3C private or I2C end of
  transfer.
       (+) CtrlMultipleXferCpltCallback : callback for Controller multiple Direct CCC, I3C private
  or I2C end of transfer.
       (+) CtrlDAACpltCallback  : callback for Controller Enter Dynamic Address Assignment end of
  transfer.
  ...
  transfer.
       ...
       (+) TgtReqDynamicAddrCallback  : callback for controller application
            when a target sent its payload to the controller during Dynamic Address Assignment
  process.
 ...
 * @param  __PAYLOAD__ specifies the Device Characteristics capabilities retrieve during ENTDAA
 * procedure. This parameter must be a number between Min_Data=0x00(uint64_t) and
 ...
 *         This parameter must be a number between Min_Data=0x00(uint64_t) and
...
#define I3C_GET_MIPIMID( __PID__ ) \
        ( (uint16_t) ( (uint64_t) ( __PID__ ) >> I3C_MIPIMID_PID_SHIFT ) & I3C_MIPIMID_PID_MASK )
...
#define I3C_GET_PART_ID( __PID__ ) \
        ( (uint16_t) ( (uint64_t) ( __PID__ ) >> I3C_PART_ID_PID_SHIFT ) & I3C_PART_ID_PID_MASK )
...
#define I3C_GET_ADVANCED_CAPABLE( __BCR__ ) \
        ( ( ( ( ( __BCR__ ) & I3C_BCR_BCR5_Msk ) >> I3C_BCR_BCR5_Pos ) == 1U ) ? ENABLE : DISABLE )
...
#define I3C_GET_VIRTUAL_TGT( __BCR__ ) \
        ( ( ( ( ( __BCR__ ) & I3C_BCR_BCR4_Msk ) >> I3C_BCR_BCR4_Pos ) == 1U ) ? ENABLE : DISABLE )
...
#define I3C_GET_OFFLINE_CAPABLE( __BCR__ ) \
        ( ( ( ( ( __BCR__ ) & I3C_BCR_BCR3_Msk ) >> I3C_BCR_BCR3_Pos ) == 1U ) ? ENABLE : DISABLE )
...
#define I3C_GET_MAX_DATA_SPEED_LIMIT( __BCR__ ) \
        ( ( ( ( ( __BCR__ ) & I3C_BCR_BCR0_Msk ) >> I3C_BCR_BCR0_Pos ) == 1U ) ? ENABLE : DISABLE )
 ...
 * @{
 */
/* Structure containing address device and message type used for the private function
...
static HAL_StatusTypeDef
I3C_Ctrl_Tx_Listen_Event_ISR( struct __I3C_HandleTypeDef* hi3c, uint32_t itMasks );
static HAL_StatusTypeDef
I3C_Ctrl_Rx_Listen_Event_ISR( struct __I3C_HandleTypeDef* hi3c, uint32_t itMasks );
static HAL_StatusTypeDef
I3C_Ctrl_Multiple_Xfer_Listen_Event_ISR( struct __I3C_HandleTypeDef* hi3c, uint32_t itMasks );
#if defined( HAL_DMA_MODULE_ENABLED )
...
static HAL_StatusTypeDef
I3C_Ctrl_Multiple_Xfer_DMA_ISR( struct __I3C_HandleTypeDef* hi3c, uint32_t itMasks );
#endif /* HAL_DMA_MODULE_ENABLED */
...
static HAL_StatusTypeDef I3C_WaitOnFlagUntilTimeout(
    I3C_HandleTypeDef* hi3c,
    uint32_t           flag,
...
static HAL_StatusTypeDef
I3C_Xfer_PriorPreparation( I3C_HandleTypeDef* hi3c, uint8_t counter, uint32_t option );
static uint32_t I3C_FillTxBuffer_CCC(
    I3C_HandleTypeDef* hi3c,
    uint32_t           indexDesc,
    uint32_t           txSize,
    uint32_t           txCurrentIndex );
static uint32_t I3C_FillTxBuffer_Private(
    I3C_HandleTypeDef* hi3c,
    uint32_t           indexDesc,
    uint32_t           txSize,
    uint32_t           txCurrentIndex );
static HAL_StatusTypeDef
I3C_ControlBuffer_PriorPreparation( I3C_HandleTypeDef* hi3c, uint8_t counter, uint32_t option );
static HAL_StatusTypeDef I3C_Ctrl_IsDevice_Ready(
    I3C_HandleTypeDef*       hi3c,
    I3C_DeviceTypeDef const* pDevice,
    uint32_t                 trials,
    uint32_t                 timeout );
...
I3Cx peripheral:
         ...
         (+) Users must implement HAL_I3C_MspInit() function in which they configure all related
peripherals resources (APB and Kernel CLOCK, GPIO, DMA, IT and NVIC).

         (+) Call the function HAL_I3C_Init() to configure the bus characteristic depends on the
device mode with the selected configuration below:
         ...
         (+) Call the function HAL_I3C_DeInit() to restore the default configuration of the selected
I3Cx peripheral.
 ...
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
...
@verbatim
 ...
 =======================================================================================================================
    [..]  This subsection provides a set of functions allowing to manage callbacks and interrupts
request:

         (+) Register/Unregister callback function:
             (++) Call the function HAL_I3C_RegisterCallback() to register an I3C user callback.
             (++) Call the function HAL_I3C_RegisterNotifyCallback() to register an I3C user
notification callback.
             (++) Call the function HAL_I3C_RegisterDynamicAddrCallback() to register an I3C user
address callback.
             (++) Call the function HAL_I3C_RegisterHotJoinCallback() to register an I3C user hot
join callback.
             (++) Call the function HAL_I3C_UnRegisterCallback() to unregister an I3C user callback.

         (+) Notification management function:
             (++) Call the function HAL_I3C_ActivateNotification() to activate the I3C
notifications.
             (++) Call the function HAL_I3C_DeactivateNotification() to deactivate the I3C
notifications.

         (+) Controller callback functions:
             (++) Users must implement HAL_I3C_CtrlTxCpltCallback() function when the transmission
of private data or Tx CCC transfer is completed.
             (++) Users must implement HAL_I3C_CtrlRxCpltCallback() function when the reception of
private data or Rx CCC transfer is completed.
             (++) Users must implement HAL_I3C_CtrlMultipleXferCpltCallback() function when the
multiple transfer of CCC, I3C private or I2C transfer is completed.
             (++) Users must implement HAL_I3C_CtrlDAACpltCallback() function when Dynamic Address
Assignment procedure is completed.
             (++) Users must implement HAL_I3C_TgtReqDynamicAddrCallback() function in the
controller application when a target sent its payload to the controller during Dynamic Address
Assignment procedure.

         (+) Target callback functions:
             (++) Users must implement HAL_I3C_TgtTxCpltCallback() function when the transmission of
private data is completed.
             (++) Users must implement HAL_I3C_TgtRxCpltCallback() function when the reception of
private data is completed.
             (++) Users must implement HAL_I3C_TgtHotJoinCallback() function when a target hot join
process is completed.
         ...
         (+) Common callback functions:
             ...
             (++) Users must implement HAL_I3C_AbortCpltCallback() function when an abort process is
completed.
             (++) Users must implement HAL_I3C_ErrorCallback() function when an error is occurred.

         (+) Interrupt and event function:
             (++) Call the function HAL_I3C_ER_IRQHandler() in the ISR file to handle I3C error
interrupts request.
             (++) Call the function HAL_I3C_EV_IRQHandler() in the ISR file to handle I3C event
interrupts request.
@endverbatim
  * @{
  */
 ...
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_RegisterCallback(
    I3C_HandleTypeDef*        hi3c,
 ...
 * @retval HAL Status      :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_RegisterNotifyCallback(
    I3C_HandleTypeDef*         hi3c,
...
/**
 * @brief  Register a User I3C dynamic address Callback to be used instead of the weak predefined
 * callback.
 * @param  hi3c                : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 ...
 * @note   The HAL_I3C_UnRegisterCallback() may be called before HAL_I3C_Init() in
 * HAL_I3C_STATE_RESET to un-register callbacks for HAL_I3C_MSPINIT_CB_ID and
 ...
 */
HAL_StatusTypeDef
HAL_I3C_UnRegisterCallback( I3C_HandleTypeDef* hi3c, HAL_I3C_CallbackIDTypeDef callbackID )
{
        HAL_StatusTypeDef status = HAL_OK;
 ...
 * @param  hi3c          : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  pXferData     : [IN/OUT] Pointer to an I3C_XferTypeDef structure that contains the
 * reception buffer to retrieve data during broadcast CCC DEFTGTS and DEFGRPA when Target mode only.
 * @param  interruptMask : [IN]  Parameter indicates which interrupts will be enabled.
 *                               This parameter can be any combination of @arg I3C_TARGET_INTERRUPT
 * when the I3C is in target mode or a combination of @arg I3C_CONTROLLER_INTERRUPT when it is in
 * controller mode.
 * @retval HAL Status    :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_ActivateNotification(
    I3C_HandleTypeDef* hi3c,
        ...
        if ( hi3c == NULL ) {
                ...
                /* Check the I3C mode */
                    ...
                    ( pXferData == NULL ) ) {
                        ...
                        if ( hi3c->Mode == HAL_I3C_MODE_TARGET ) {
                                if ( ( interruptMask & ( HAL_I3C_IT_DEFIE | HAL_I3C_IT_GRPIE ) ) !=
                                     0U ) {
                                        ...
                                        /* Check on the Rx threshold to know the Rx treatment
                                         * process : byte or word */
                        ...
                        to avoid the risk of I3C interrupt handle execution before current process
                        unlock */
...
@verbatim
 ...
 =======================================================================================================================
    [..]  This subsection provides a set of functions allowing to configure the I3C instances.
         ...
         (+) Call the function HAL_I3C_Ctrl_BusCharacteristicConfig() to modify the controller Bus
Characteristics after initialize the bus through HAL_I3C_Init.

         (+) Call the function HAL_I3C_Tgt_BusCharacteristicConfig() to modify the target Bus
Characteristics after initialize the bus through HAL_I3C_Init.

         (+) Call the function HAL_I3C_SetConfigFifo() to set FIFOs configuration (enabled FIFOs and
             threshold level) with the selected parameters in the configuration structure
I3C_FifoConfTypeDef.

         (+) Call the function HAL_I3C_Ctrl_Config() to configure the I3C Controller instances with
the selected parameters in the configuration structure I3C_CtrlConfTypeDef. This function is called
only when mode is Controller.

         (+) Call the function HAL_I3C_Tgt_Config() to configure the I3C Target instances with the
selected parameters in the configuration structure I3C_TgtConfTypeDef. This function is called only
when mode is Target.

         (+) Call the function HAL_I3C_Ctrl_ConfigBusDevices() to configure Hardware device
characteristics register with Devices capabilities present on the Bus. All different characteristics
must be fill through structure I3C_DeviceConfTypeDef. This function is called only when mode is
Controller.

         (+) Call the function HAL_I3C_AddDescToFrame() to prepare the full transfer usecase in a
Controller transfer descriptor which contained different buffer pointers and their associated size
through I3C_XferTypeDef. This function must be called before initiate any communication transfer.
         (+) Call the function HAL_I3C_Ctrl_SetConfigResetPattern() to configure the insertion of
the reset pattern at the end of a Frame.
         (+) Call the function HAL_I3C_Ctrl_GetConfigResetPattern() to get the current reset pattern
configuration
 ...
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  pConfig    : [IN]  Pointer to an LL_I3C_CtrlBusConfTypeDef structure contains controller
 * bus configuration.
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_BusCharacteristicConfig(
    I3C_HandleTypeDef*               hi3c,
    LL_I3C_CtrlBusConfTypeDef const* pConfig )
{
        HAL_StatusTypeDef status = HAL_OK;
        uint32_t          waveform_value;
        uint32_t          timing_value;
 ...
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  pConfig    : [IN]  Pointer to an LL_I3C_TgtBusConfTypeDef structure contains target bus
 * configuration.
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Tgt_BusCharacteristicConfig(
    I3C_HandleTypeDef*              hi3c,
    LL_I3C_TgtBusConfTypeDef const* pConfig )
{
        HAL_StatusTypeDef status = HAL_OK;
                ...
                if ( pConfig == NULL ) {
                        ...
                        if ( ( hi3c->State != HAL_I3C_STATE_READY ) ||
                             ( hi3c->Mode != HAL_I3C_MODE_TARGET ) ) {
                                 ...
                                 * condition time */
                                LL_I3C_SetAvalTiming(
                                    hi3c->Instance, pConfig->BusAvailableDuration );
 ...
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  pConfig    : [IN]  Pointer to an I3C_FifoConfTypeDef structure contains FIFOs
 * configuration.
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef
HAL_I3C_SetConfigFifo( I3C_HandleTypeDef* hi3c, I3C_FifoConfTypeDef const* pConfig )
{
        HAL_StatusTypeDef status = HAL_OK;
        uint32_t          cfgr_value;
        uint32_t          cfgr_mask;
 ...
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  pConfig    : [IN]  Pointer to an I3C_CtrlConfTypeDef structure that contains controller
 * configuration.
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_Config( I3C_HandleTypeDef* hi3c, I3C_CtrlConfTypeDef const* pConfig )
{
        HAL_StatusTypeDef status = HAL_OK;
        ...
        if ( hi3c == NULL ) {
                ...
                /* Check the I3C state and mode */
                    ...
                    ( hi3c->Mode != HAL_I3C_MODE_CONTROLLER ) ) {
                        ...
                        MODIFY_REG(
                            hi3c->Instance->CFGR, I3C_CFGR_HKSDAEN | I3C_CFGR_HJACK, cfgr_value );
 ...
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  pConfig    : [IN]  Pointer to an I3C_TgtConfTypeDef structure that contains target
 * configuration.
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Tgt_Config( I3C_HandleTypeDef* hi3c, I3C_TgtConfTypeDef const* pConfig )
{
        HAL_StatusTypeDef status = HAL_OK;
        ...
        if ( hi3c == NULL ) {
                ...
                /* Check the I3C state and mode */
                    ...
                    ( hi3c->Mode != HAL_I3C_MODE_TARGET ) ) {
                        ...
                        bcr_value =
                            ( ( (uint32_t) pConfig->MaxSpeedLimitation << I3C_BCR_BCR0_Pos ) |
 ...
 * @note   This function is called only when the I3C instance is initialized as controller.
 *         This function can be called by the controller application to help the automatic treatment
 * when target have capability of IBI and/or Control-Role.
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  pDesc      : [IN]  Pointer to an I3C_DeviceConfTypeDef descriptor that contains the bus
 ...
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_ConfigBusDevices(
    I3C_HandleTypeDef*           hi3c,
    I3C_DeviceConfTypeDef const* pDesc,
    uint8_t                      nbDevice )
{
        HAL_StatusTypeDef status = HAL_OK;
                ...
                /* Check the I3C state and mode */
                    ...
                    ( hi3c->Mode != HAL_I3C_MODE_CONTROLLER ) ) {
                        ...
                        for ( uint32_t index = 0U; index < nbDevice; index++ ) {
                                ...
                                write_value =
                                    ( ( (uint32_t) pDesc[index].TargetDynamicAddr
                                        << I3C_DEVRX_DA_Pos ) |
 ...
 * @note   This function must be called before initiate initiate any controller communication
 * transfer. This function help the preparation of the full transfer usecase in a transfer
 * descriptor which contained different buffer pointers and their associated size through
 * I3C_XferTypeDef.
 * @note   The Tx FIFO threshold @ref HAL_I3C_TXFIFO_THRESHOLD_4_4 is not allowed when the transfer
 * descriptor contains multiple transmission frames.
 * @param  hi3c          : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  pCCCDesc      : [IN]  Pointer to an I3C_CCCTypeDef structure that contains the CCC
 * descriptor information.
 * @param  pPrivateDesc  : [IN]  Pointer to an I3C_PrivateTypeDef structure that contains the
 * transfer descriptor.
 * @param  pXferData     : [IN/OUT] Pointer to an I3C_XferTypeDef structure that contains required
 * transmission buffers (control buffer, data buffer and status buffer).
 * @param  nbFrame       : [IN]  The number of CCC commands or the number of device to treat.
 * @param  option        : [IN]  Value indicates the transfer option. It can be one value of @ref
 * I3C_OPTION_DEFINITION
 * @retval HAL Status    :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_AddDescToFrame(
    I3C_HandleTypeDef*        hi3c,
    I3C_CCCTypeDef const*     pCCCDesc,
    I3C_PrivateTypeDef const* pPrivateDesc,
    I3C_XferTypeDef*          pXferData,
    uint8_t                   nbFrame,
    uint32_t                  option )
{
 ...
 * @note  When the transfer descriptor contains multiple frames with RESTART option, the reset
 * pattern at the end of RSTACT CCC frame is not possible.
 ...
 */
HAL_StatusTypeDef
HAL_I3C_Ctrl_SetConfigResetPattern( I3C_HandleTypeDef* hi3c, uint32_t resetPattern )
{
        HAL_StatusTypeDef    status = HAL_OK;
        HAL_I3C_StateTypeDef handle_state;
 ...
 */
HAL_StatusTypeDef
HAL_I3C_Ctrl_GetConfigResetPattern( I3C_HandleTypeDef* hi3c, uint32_t* pResetPattern )
{
        HAL_StatusTypeDef status = HAL_OK;
...
@verbatim
 ...
 =======================================================================================================================
    [..]  This subsection provides a set of functions allowing to manage I3C FIFOs.
         ...
         (+) Call the function HAL_I3C_FlushAllFifo() to flush the content of all used FIFOs
(Control, Status, Tx and Rx FIFO).
         ...
         (+) Call the function HAL_I3C_FlushStatusFifo() to flush only the content of Status FIFO.
             This function is called only when mode is controller.
         (+) Call the function HAL_I3C_ClearConfigFifo() to clear the FIFOs configuration and set it
to default values.
         (+) Call the function HAL_I3C_GetConfigFifo() to get the current FIFOs configuration
(enabled FIFOs and threshold level).

         (+) Users must not call all above functions before I3C initialization.

         (+) Users should call Flush APIs after an end of process, before starting a transfer or in
case of bus failure and error detection.
 ...
 * @param  hi3c       : [IN]     Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  pConfig    : [IN/OUT] Pointer to an I3C_FifoConfTypeDef structure that returns current
 * FIFOs configuration.
 * @retval HAL Status :          Value from HAL_StatusTypeDef enumeration.
 */
...
@verbatim
 ...
 =======================================================================================================================
    [..]  This subsection provides a set of functions allowing to manage controller I3C operation.
         ...
         (+) Call the function HAL_I3C_Ctrl_TransmitCCC_DMA() to transmit direct write or a
broadcast CCC command in DMA mode.
         (+) Call the function HAL_I3C_Ctrl_ReceiveCCC() to transmit direct read CCC command in
polling mode.
         (+) Call the function HAL_I3C_Ctrl_ReceiveCCC_IT() to transmit direct read CCC command in
interrupt mode.
         (+) Call the function HAL_I3C_Ctrl_ReceiveCCC_DMA() to transmit direct read CCC command in
DMA mode.
         (+) Call the function HAL_I3C_Ctrl_Transmit() to transmit private data in polling mode.
         (+) Call the function HAL_I3C_Ctrl_Transmit_IT() to transmit private data in interrupt
mode.
         ...
         (+) Call the function HAL_I3C_Ctrl_MultipleTransfer_IT() to transfer I3C or I2C private
data or CCC command in multiple direction in interrupt mode.
         (+) Call the function HAL_I3C_Ctrl_MultipleTransfer_DMA() to transfer I3C or I2C private
data or CCC command in multiple direction in DMA mode.
         ...
         (+) Call the function HAL_I3C_Ctrl_SetDynAddr() to set, asscociate the target dynamic
address during the Dynamic Address Assignment processus.
         (+) Call the function HAL_I3C_Ctrl_IsDeviceI3C_Ready() to check if I3C target device is
ready.
         (+) Call the function HAL_I3C_Ctrl_IsDeviceI2C_Ready() to check if I2C target device is
ready.
 ...
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  pXferData  : [IN]  Pointer to an I3C_XferTypeDef structure that contains required
 * transmission buffers (control buffer, data buffer and status buffer). This value contain transfer
 * data after called @ref HAL_I3C_AddDescToFrame().
 * @param  timeout    : [IN]  Timeout duration in millisecond.
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef
HAL_I3C_Ctrl_TransmitCCC( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData, uint32_t timeout )
{
        uint32_t             tickstart;
 ...
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_TransmitCCC_IT( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData )
{
        HAL_I3C_StateTypeDef handle_state;
        HAL_StatusTypeDef    status = HAL_OK;
 ...
 */
HAL_StatusTypeDef
HAL_I3C_Ctrl_TransmitCCC_DMA( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData )
{
                ...
                /* check on the State */
                    ...
                    ( handle_state != HAL_I3C_STATE_LISTEN ) ) {
                        ...
                        if ( hi3c->TxXferCount != 0U ) {
                                ...
                                /* Check on the Tx threshold to know the Tx treatment process : byte
                                 * or word */
                                if ( LL_I3C_GetTxFIFOThreshold( hi3c->Instance ) ==
                                     LL_I3C_TXFIFO_THRESHOLD_1_4 ) {
                                        /* assert that DMA source and destination width are
                                         * configured in byte */
                                        assert_param( IS_I3C_DMASOURCEBYTE_VALUE(
                                            hi3c->hdmatx->Init.SrcDataWidth ) );
                                        ...
                                        /* assert that DMA source and destination width are
                                         * configured in word */
                                        assert_param( IS_I3C_DMASOURCEWORD_VALUE(
                                            hi3c->hdmatx->Init.SrcDataWidth ) );
 ...
 */
HAL_StatusTypeDef
HAL_I3C_Ctrl_ReceiveCCC( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData, uint32_t timeout )
{
        uint32_t             tickstart;
        uint32_t             exit_condition;
        HAL_StatusTypeDef    status = HAL_OK;
 ...
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_ReceiveCCC_IT( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData )
{
        HAL_I3C_StateTypeDef handle_state;
        HAL_StatusTypeDef    status = HAL_OK;
 ...
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_ReceiveCCC_DMA( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData )
{
        HAL_I3C_StateTypeDef handle_state;
        HAL_StatusTypeDef    status = HAL_OK;
        HAL_StatusTypeDef    control_dma_status;
 ...
 */
HAL_StatusTypeDef
HAL_I3C_Ctrl_Transmit( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData, uint32_t timeout )
{
        uint32_t             tickstart;
        uint32_t             exit_condition;
        HAL_StatusTypeDef    status = HAL_OK;
 ...
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_Transmit_IT( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData )
{
        HAL_I3C_StateTypeDef handle_state;
        HAL_StatusTypeDef    status = HAL_OK;
 ...
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_Transmit_DMA( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData )
{
        HAL_StatusTypeDef    control_dma_status;
        HAL_StatusTypeDef    tx_dma_status = HAL_OK;
        HAL_I3C_StateTypeDef handle_state;
 ...
 * configuration information for the specified I3C.
 * @param  pXferData  : [IN]  Pointer to an I3C_XferTypeDef structure that contains required
 * reception buffers (control buffer, data buffer and status buffer). This value contain transfer
 * data after called @ref HAL_I3C_AddDescToFrame().
 * @param  timeout    : [IN]  Timeout duration in millisecond.
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef
HAL_I3C_Ctrl_Receive( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData, uint32_t timeout )
{
        uint32_t             tickstart;
        uint32_t             exit_condition;
        HAL_StatusTypeDef    status = HAL_OK;
 ...
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_Receive_IT( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData )
{
        HAL_I3C_StateTypeDef handle_state;
        HAL_StatusTypeDef    status = HAL_OK;
 ...
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_Receive_DMA( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData )
{
        HAL_StatusTypeDef    control_dma_status;
        HAL_StatusTypeDef    rx_dma_status = HAL_OK;
        HAL_I3C_StateTypeDef handle_state;
 ...
 * @note   The Tx FIFO threshold @ref HAL_I3C_TXFIFO_THRESHOLD_4_4 is not allowed when the transfer
 * descriptor contains multiple transmission frames.
 * @note   This function must be called to transfer read/write I3C or I2C private data or a direct
 * read/write CCC.
 * @note   The TxBuf.Size must be equal to the sum of all TxBuf.Size exist in the descriptor.
 * @note   The RxBuf.Size must be equal to the sum of all RxBuf.Size exist in the descriptor.
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  pXferData  : [IN]  Pointer to an I3C_XferTypeDef structure that contains required
 * transmit and receive buffers (control buffer, data buffers and status buffer). This value contain
 * transfer data after called @ref HAL_I3C_AddDescToFrame().
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef
HAL_I3C_Ctrl_MultipleTransfer_IT( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData )
{
        HAL_I3C_StateTypeDef handle_state;
        HAL_StatusTypeDef    status = HAL_OK;
        ...
        if ( hi3c == NULL ) {
                ...
                /* check on the State */
                    ...
                    ( handle_state != HAL_I3C_STATE_LISTEN ) ) {
                        ...
                        I3C_Enable_IRQ(
                            hi3c, ( I3C_XFER_CONTROLLER_TX_IT | I3C_XFER_CONTROLLER_RX_IT ) );
 ...
 * @note   The TxBuf.Size must be equal to the sum of all TxBuf.Size exist in the descriptor.
 * @note   The RxBuf.Size must be equal to the sum of all RxBuf.Size exist in the descriptor.
 * @note   This function must be called to transfer read/write private data or a direct read/write
 * CCC command.
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  pXferData  : [IN]  Pointer to an I3C_XferTypeDef structure that contains required
 * transmit and receive buffers(control buffer, data buffer and status buffer). This value contain
 * transfer data after called @ref HAL_I3C_AddDescToFrame().
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef
HAL_I3C_Ctrl_MultipleTransfer_DMA( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData )
{
        HAL_StatusTypeDef    control_dma_status;
        HAL_StatusTypeDef    tx_dma_status = HAL_OK;
        HAL_StatusTypeDef    rx_dma_status = HAL_OK;
        ...
        if ( hi3c == NULL ) {
                ...
                /* Check on hdmatx, hdmarx and hdmacr handle */
                    ...
                    ( hi3c->hdmatx == NULL ) || ( hi3c->hdmacr == NULL ) ||
                    ( hi3c->hdmarx == NULL ) ) {
                        hi3c->ErrorCode = HAL_I3C_ERROR_DMA_PARAM;
                        status          = HAL_ERROR;
                }
 ...
 *                                   It can be one value of @ref
 * I3C_DYNAMIC_ADDRESS_OPTION_DEFINITION.
 * @param  timeout        : [IN]     Timeout duration in millisecond.
 * @retval HAL Status     :          Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_DynAddrAssign(
    I3C_HandleTypeDef* hi3c,
                             ...
                             ( ( handle_state == HAL_I3C_STATE_READY ) ||
                               ( handle_state == HAL_I3C_STATE_LISTEN ) ) ) {
                                ...
                                LL_I3C_ControllerHandleCCC(
                                    hi3c->Instance,
                                    I3C_BROADCAST_RSTDAA,
                                    0U,
                                    LL_I3C_GENERATE_STOP );

                                /* Wait Frame completion flag */
                                status = I3C_WaitOnFlagUntilTimeout(
                                    hi3c, HAL_I3C_FLAG_FCF, RESET, timeout, tickstart );
                        ...
                        if ( status == HAL_OK ) {
                                ...
                                if ( ( handle_state == HAL_I3C_STATE_READY ) ||
                                     ...
                                     ( handle_state == HAL_I3C_STATE_BUSY_DAA ) ) {
                                        ...
                                        /* Wait frame complete flag or TX FIFO not full flag until
                                         * timeout */
                                        status =
                                            I3C_WaitOnDAAUntilTimeout( hi3c, timeout, tickstart );

                                        /* Check TX FIFO not full flag */
                                        if ( __HAL_I3C_GET_FLAG( hi3c, HAL_I3C_FLAG_TXFNFF ) ==
                                             SET ) {
                                                /* Check on the Rx FIFO threshold to know the Rx
                                        ...
                                        /* Check on frame complete flag */
                                                ...
                                                /* Clear frame complete flag */
                                                if ( __HAL_I3C_GET_FLAG( hi3c, HAL_I3C_FLAG_FCF ) ==
                                                     SET ) {
                                                        /* Clear frame complete flag */
                                                        LL_I3C_ClearFlag_FC( hi3c->Instance );
                                                }
...
/**
 * @brief  Controller assign dynamic address (send a broadcast ENTDAA CCC command) in interrupt
 * mode.
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  dynOption  : [IN]  Parameter indicates the Dynamic address assignment option.
 ...
 * @retval HAL Status :      Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_IsDeviceI3C_Ready(
    I3C_HandleTypeDef* hi3c,
    uint8_t            devAddress,
    uint32_t           trials,
    uint32_t           timeout )
 ...
 * @retval HAL Status :      Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_IsDeviceI2C_Ready(
    I3C_HandleTypeDef* hi3c,
    uint8_t            devAddress,
    uint32_t           trials,
    uint32_t           timeout )
...
@verbatim
 ...
 =======================================================================================================================
    [..]  This subsection provides a set of functions allowing to manage target I3C operation.
         ...
         (+) Call the function HAL_I3C_Tgt_ControlRoleReq() to send a control-role request in
polling mode.
         (+) Call the function HAL_I3C_Tgt_ControlRoleReq_IT() to send a control-role request in
interrupt mode.
         (+) Call the function HAL_I3C_Tgt_HotJoinReq() to send a Hot-Join request in polling mode.
         (+) Call the function HAL_I3C_Tgt_HotJoinReq_IT() to send a Hot-Join request in interrupt
mode.
 ...
 * configuration information for the specified I3C.
 * @param  pXferData  : [IN]  Pointer to an I3C_XferTypeDef structure that contains required
 * transmission buffers information (Pointer to the Tx buffer (TxBuf.pBuffer) and size of data to
 * transmit in bytes (TxBuf.Size)).
 * @param  timeout    : [IN]  Timeout duration in millisecond.
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef
HAL_I3C_Tgt_Transmit( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData, uint32_t timeout )
{
        uint32_t             tickstart;
        HAL_StatusTypeDef    status = HAL_OK;
        HAL_I3C_StateTypeDef handle_state;
        ...
        if ( hi3c == NULL ) {
                ...
                /* check if DEF or GRP CCC notifications are enabled */
                    ...
                    ( I3C_CHECK_IT_SOURCE( it_source, HAL_I3C_IT_DEFIE ) != RESET ) ||
                ...
                /* Verify the dynamic address validity */
                        ...
                        LL_I3C_ConfigTxPreload(
                            hi3c->Instance, (uint16_t) hi3c->pXferData->TxBuf.Size );
...
HAL_StatusTypeDef HAL_I3C_Tgt_Transmit_DMA( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData )
{
        ...
        if ( hi3c == NULL ) {
                ...
                /* Verify the dynamic address validity */
                        ...
                        if ( LL_I3C_GetTxFIFOThreshold( hi3c->Instance ) ==
                             LL_I3C_TXFIFO_THRESHOLD_1_4 ) {
                                /* assert that DMA source and destination width are configured in
                                 * byte */
                                assert_param(
 ...
 * configuration information for the specified I3C.
 * @param  pXferData  : [IN]  Pointer to an I3C_XferTypeDef structure that contains required
 * reception buffers information (Pointer to the Rx buffer (RxBuf.pBuffer) and size of data to be
 * received in bytes (RxBuf.Size)).
 * @param  timeout    : [IN]  Timeout duration in millisecond.
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef
HAL_I3C_Tgt_Receive( I3C_HandleTypeDef* hi3c, I3C_XferTypeDef* pXferData, uint32_t timeout )
{
        uint32_t             tickstart;
        HAL_StatusTypeDef    status = HAL_OK;
        HAL_I3C_StateTypeDef handle_state;
        ...
        if ( hi3c == NULL ) {
                ...
                if ( status == HAL_OK ) {
                        ...
                        LL_I3C_TargetHandleMessage(
                            hi3c->Instance, LL_I3C_TARGET_MTYPE_CONTROLLER_ROLE_REQ, 0U );

                        /* Wait Controllership completion confirmation flag */
                        status = I3C_WaitOnFlagUntilTimeout(
                            hi3c, HAL_I3C_FLAG_CRUPDF, RESET, timeout, tickstart );
 ...
 * @param  hi3c       : [IN]     Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  pAddress   : [IN/OUT] Pointer to the target own dynamic address assigned by the
 * controller.
 * @param  timeout    : [IN]     Timeout duration in millisecond.
 * @retval HAL Status :          Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef
HAL_I3C_Tgt_HotJoinReq( I3C_HandleTypeDef* hi3c, uint8_t* pAddress, uint32_t timeout )
{
        uint32_t             tickstart;
        HAL_I3C_StateTypeDef handle_state;
        ...
        if ( hi3c == NULL ) {
                ...
                if ( status == HAL_OK ) {
                        ...
                        /* Request hot join */
                        LL_I3C_TargetHandleMessage(
                            hi3c->Instance, LL_I3C_TARGET_MTYPE_HOT_JOIN, 0U );

                        /* Wait hot join completion confirmation flag */
                        status = I3C_WaitOnFlagUntilTimeout(
                            hi3c, HAL_I3C_FLAG_DAUPDF, RESET, timeout, tickstart );
 ...
 * @retval HAL Status  :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Tgt_IBIReq(
    I3C_HandleTypeDef* hi3c,
        ...
        if ( hi3c == NULL ) {
                ...
                if ( status == HAL_OK ) {
                        ...
                        if ( status == HAL_OK ) {
                                ...
                                /* Request IBI */
                                LL_I3C_TargetHandleMessage(
                                    hi3c->Instance, LL_I3C_TARGET_MTYPE_IBI, payloadSize );

                                /* Wait IBI completion confirmation flag */
                                status = I3C_WaitOnFlagUntilTimeout(
                                    hi3c, HAL_I3C_FLAG_IBIENDF, RESET, timeout, tickstart );
 ...
 */
HAL_StatusTypeDef
HAL_I3C_Tgt_IBIReq_IT( I3C_HandleTypeDef* hi3c, uint8_t const* pPayload, uint8_t payloadSize )
{
        uint32_t             payload_value = 0U;
        HAL_I3C_StateTypeDef handle_state;
        HAL_StatusTypeDef    status = HAL_OK;
...
@verbatim
 ...
 =======================================================================================================================
    [..]  This subsection provides a set of functions allowing to Abort transfer or to get in
run-time the status of the peripheral.
         ...
         (+) Call the function HAL_I3C_Get_ENTDAA_Payload_Info() to get BCR, DCR and PID information
after ENTDAA.
...
HAL_StatusTypeDef HAL_I3C_Abort_IT( I3C_HandleTypeDef* hi3c )
{
        ...
        if ( hi3c == NULL ) {
                status = HAL_ERROR;
        } else if ( hi3c->State != HAL_I3C_STATE_ABORT ) {
                ...
                /* Flush the different Fifos to generate an automatic stop mode link to underflow or
                 * overflow detection timeout */
                /* Flush the content of Tx Fifo */
                LL_I3C_RequestTxFIFOFlush( hi3c->Instance );
                ...
                /* Check on the I3C mode: Control and status FIFOs available only with controller
                 * mode */
                if ( hi3c->Mode == HAL_I3C_MODE_CONTROLLER ) {
                        /* Flush the content of Control Fifo */
                        LL_I3C_RequestControlFIFOFlush( hi3c->Instance );
 ...
 */
HAL_I3C_StateTypeDef HAL_I3C_GetState( I3C_HandleTypeDef const* hi3c )
{
        return hi3c->State;
}
 ...
 */
HAL_I3C_ModeTypeDef HAL_I3C_GetMode( I3C_HandleTypeDef const* hi3c )
{
        return hi3c->Mode;
}
 ...
 */
uint32_t HAL_I3C_GetError( I3C_HandleTypeDef const* hi3c )
{
 ...
 * @param  pCCCInfo : [IN/OUT] Pointer to an I3C_CCCInfoTypeDef structure that contains the CCC
 * information updated after CCC event.
 * @retval HAL Status : Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef
HAL_I3C_GetCCCInfo( I3C_HandleTypeDef* hi3c, uint32_t notifyId, I3C_CCCInfoTypeDef* pCCCInfo )
{
        HAL_StatusTypeDef status = HAL_OK;
        ...
        if ( hi3c == NULL ) {
                ...
                /* Check the I3C state */
                        ...
                        if ( ( notifyId & EVENT_ID_DAU ) == EVENT_ID_DAU ) {
                                ...
                                pCCCInfo->DynamicAddr =
                                    LL_I3C_GetOwnDynamicAddress( hi3c->Instance );
                        }

                        /* Retrieve Maximum Write Data Length (target) */
                        if ( ( notifyId & EVENT_ID_SETMWL ) == EVENT_ID_SETMWL ) {
                                pCCCInfo->MaxWriteLength =
                                    LL_I3C_GetMaxWriteLength( hi3c->Instance );
                        }
                        ...
                        if ( ( notifyId & EVENT_ID_ENEC_DISEC ) == EVENT_ID_ENEC_DISEC ) {
                                pCCCInfo->HotJoinAllowed =
                                    LL_I3C_IsEnabledHotJoin( hi3c->Instance );
                        ...
                        if ( ( notifyId & EVENT_ID_IBI ) == EVENT_ID_IBI ) {
                                ...
                                pCCCInfo->IBITgtNbPayload =
                                    LL_I3C_GetNbIBIAddData( hi3c->Instance );
 ...
 * @param  pENTDAA_payload :[OUT] Pointer to an I3C_ENTDAAPayloadTypeDef structure that contains the
 * BCR, DCR and PID information.
 * @retval HAL Status : Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Get_ENTDAA_Payload_Info(
    I3C_HandleTypeDef*        hi3c,
...
static HAL_StatusTypeDef I3C_Tgt_Event_ISR( struct __I3C_HandleTypeDef* hi3c, uint32_t itMasks )
{
        ...
        /* I3C target complete controller-role hand-off procedure (direct GETACCR CCC) event
         * management --------------------*/
        ...
        /* I3C target receive activity state update (direct or broadcast ENTASx) CCC event
         * management ----------------------*/
                ...
                if ( I3C_CHECK_FLAG( itMasks, I3C_EVR_FCF ) != RESET ) {
                        ...
                        if ( LL_I3C_GetXferDataCount( hi3c->Instance ) ==
                             hi3c->pXferData->TxBuf.Size ) {
                                ...
                                /* Call the transmit complete callback to inform upper layer of End
                                 * of Transfer */
#if ( USE_HAL_I3C_REGISTER_CALLBACKS == 1U )
                ...
                if ( I3C_CHECK_FLAG( itMasks, I3C_EVR_FCF ) != RESET ) {
                        ...
                        if ( LL_I3C_GetXferDataCount( hi3c->Instance ) ==
                             hi3c->pXferData->RxBuf.Size ) {
                                ...
                                /* Call the receive complete callback to inform upper layer of End
                                 * of Transfer */
#if ( USE_HAL_I3C_REGISTER_CALLBACKS == 1U )
        ...
        if ( hi3c->State == HAL_I3C_STATE_BUSY_TX ) {
                ...
                if ( I3C_CHECK_FLAG( itMasks, I3C_EVR_FCF ) != RESET ) {
                        ...
                        if ( I3C_GET_DMA_REMAIN_DATA( hi3c->hdmatx ) == 0U ) {
                                ...
                                /* Call target transmit complete callback to inform upper layer of
                                 * End of Transfer */
#if ( USE_HAL_I3C_REGISTER_CALLBACKS == 1U )
                                hi3c->TgtTxCpltCallback( hi3c );
#else
        ...
        if ( hi3c->State == HAL_I3C_STATE_BUSY_RX ) {
                ...
                if ( I3C_CHECK_FLAG( itMasks, I3C_EVR_FCF ) != RESET ) {
                        ...
                        if ( I3C_GET_DMA_REMAIN_DATA( hi3c->hdmarx ) == 0U ) {
                                ...
                                /* Call target receive complete callback to inform upper layer of
                                 * End of Transfer */
#if ( USE_HAL_I3C_REGISTER_CALLBACKS == 1U )
                                hi3c->TgtRxCpltCallback( hi3c );
#else
...
/**
 * @brief  Interrupt Sub-Routine which handles controller multiple transmission/reception in
 * interrupt mode.
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  itMasks    : [IN]  Flag Interrupt Masks flags to handle.
        ...
        if ( hi3c->State == HAL_I3C_STATE_BUSY_TX_RX ) {
                ...
                if ( I3C_CHECK_FLAG( itMasks, I3C_EVR_FCF ) != RESET ) {
                        ...
                        if ( hi3c->ControlXferCount == 0U ) {
                                ...
                                /* Call the transmit, receive complete callback to inform upper
                                 * layer of End of Transfer */
#if ( USE_HAL_I3C_REGISTER_CALLBACKS == 1U )
...
/**
 * @brief  Interrupt Sub-Routine which handles controller transmission and Controller received
...
/**
 * @brief  Interrupt Sub-Routine which handles controller reception and Controller received events
 * in interrupt mode.
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  itMasks    : [IN]  Flag Interrupt Masks flags to handle.
...
/**
 * @brief  Interrupt Sub-Routine which handles controller CCC Dynamic Address Assignment command in
 * interrupt mode.
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  itMasks    : [IN]  Flag Interrupt Masks flags to handle.
...
static HAL_StatusTypeDef I3C_Ctrl_DAA_ISR( struct __I3C_HandleTypeDef* hi3c, uint32_t itMasks )
{
        ...
        if ( hi3c->State == HAL_I3C_STATE_BUSY_DAA ) {
                ...
                if ( I3C_CHECK_FLAG( itMasks, I3C_EVR_CFNFF ) != RESET ) {
                        /* Write ENTDAA CCC information in the control register */
                        LL_I3C_ControllerHandleCCC(
                            hi3c->Instance, I3C_BROADCAST_ENTDAA, 0U, LL_I3C_GENERATE_STOP );
                }
                ...
                if ( I3C_CHECK_FLAG( itMasks, I3C_EVR_FCF ) != RESET ) {
                        ...
                        if ( I3C_GET_DMA_REMAIN_DATA( hi3c->hdmacr ) == 0U ) {
                                ...
                                if ( I3C_GET_DMA_REMAIN_DATA( hi3c->hdmatx ) == 0U ) {
                                        ...
                                        /* Call controller transmit complete callback to inform
...
/**
 * @brief  Interrupt Sub-Routine which handles controller multiple receive and transmit data in DMA
 * mode.
 * @param  hi3c       : [IN]  Pointer to an I3C_HandleTypeDef structure that contains the
 * configuration information for the specified I3C.
 * @param  itMasks    : [IN]  Flag Interrupt Masks flags to handle.
                        ...
                        if ( I3C_GET_DMA_REMAIN_DATA( hi3c->hdmacr ) == 0U ) {
                                ...
                                if ( I3C_GET_DMA_REMAIN_DATA( hi3c->hdmarx ) == 0U ) {
                                        if ( I3C_GET_DMA_REMAIN_DATA( hi3c->hdmatx ) == 0U ) {
                                                ...
                                                /* Call controller transmit, receive complete
                                                 * callback to inform upper layer of End of Transfer
                                                 */
#if ( USE_HAL_I3C_REGISTER_CALLBACKS == 1U )
                                                hi3c->CtrlMultipleXferCpltCallback( hi3c );
#else
...
static HAL_StatusTypeDef I3C_Abort_ISR( struct __I3C_HandleTypeDef* hi3c, uint32_t itMasks )
{
        ...
        if ( hi3c->State == HAL_I3C_STATE_ABORT ) {
                ...
                /* Evenif abort is called, the Frame completion can arrive if abort is requested at
                 * the end of the processus */
 ...
 * @param  hdma : [IN] Pointer to a DMA_HandleTypeDef structure that contains the configuration
...
I3C_WaitOnDAAUntilTimeout( I3C_HandleTypeDef* hi3c, uint32_t timeout, uint32_t tickstart )
{
        ...
        while ( ( ( active_flags & ( HAL_I3C_FLAG_FCF | HAL_I3C_FLAG_TXFNFF ) ) == 0U ) &&
                ...
                if ( timeout != HAL_MAX_DELAY ) {
                        if ( ( ( HAL_GetTick() - tickstart ) > timeout ) || ( timeout == 0U ) ) {
                                if ( ( active_flags &
                                       ( HAL_I3C_FLAG_FCF | HAL_I3C_FLAG_TXFNFF ) ) == 0U ) {
                                        hi3c->ErrorCode |= HAL_I3C_ERROR_TIMEOUT;
                                        status = HAL_TIMEOUT;
                                }
                        }
...
static void I3C_TransmitWordTreatment( I3C_HandleTypeDef* hi3c )
{
        ...
        while ( __HAL_I3C_GET_FLAG( hi3c, HAL_I3C_FLAG_TXFNFF ) == SET ) {
                ...
                LL_I3C_TransmitData32(
                    hi3c->Instance, *( (uint32_t*) hi3c->pXferData->TxBuf.pBuffer ) );
                             ...
                             ( direction == HAL_I3C_DIRECTION_READ ) ) {
                                ...
                                if ( ( global_tx_size > hi3c->pXferData->TxBuf.Size ) ||
                                     ...
                                     ( hi3c->pXferData->TxBuf.pBuffer == NULL ) ) {
                                        ...
                                        /* Fill global Tx buffer with data and update the current
                                         * index of the Tx buffer */
                                        current_tx_index = I3C_FillTxBuffer_CCC(
                                            hi3c, descr_index, 1U, current_tx_index );
                                }
                                ...
                                for ( index = 0U; index < ( (uint32_t) counter - 1U ); index++ ) {
                                        ...
                                        if ( hi3c->pPrivateDesc[index].Direction ==
                                             HAL_I3C_DIRECTION_READ ) {
                                                nb_data_bytes =
                                                    hi3c->pPrivateDesc[index].RxBuf.Size;
                                        } else {
                                                nb_data_bytes =
                                                    hi3c->pPrivateDesc[index].TxBuf.Size;
                                        }
    ...
    uint32_t                 timeout )
{
        ...
        /* check on the State */
            ...
            ( handle_state != HAL_I3C_STATE_READY ) && ( handle_state != HAL_I3C_STATE_LISTEN ) ) {
                ...
                CR_tmp =
                    ( HAL_I3C_DIRECTION_WRITE | ( (uint32_t) pDevice->Address << I3C_CR_ADD_Pos ) |
...
static void I3C_Enable_IRQ( I3C_HandleTypeDef* hi3c, uint32_t InterruptRequest )
{
        ...
        if ( ( InterruptRequest & I3C_XFER_CONTROLLER_TX_IT ) != 0U ) {
                /* Enable frame complete, control FIFO not full, transmit FIFO not full and error
                 * interrupts */
                tmpisr |=
                    ( HAL_I3C_IT_FCIE | HAL_I3C_IT_CFNFIE | HAL_I3C_IT_TXFNFIE | HAL_I3C_IT_ERRIE );
        }
        ...
        if ( ( InterruptRequest & I3C_XFER_CONTROLLER_RX_IT ) != 0U ) {
                /* Enable frame complete, control FIFO not full, receive FIFO not empty and error
                 * interrupts */
                tmpisr |=
                    ( HAL_I3C_IT_FCIE | HAL_I3C_IT_CFNFIE | HAL_I3C_IT_RXFNEIE | HAL_I3C_IT_ERRIE );
        }

        /* Check if requested interrupts are related to controller transmit read or a broadcast CCC
         * in IT mode */
        if ( ( InterruptRequest & I3C_XFER_CONTROLLER_RX_CCC_IT ) != 0U ) {
                ...
                tmpisr |=
                    ( HAL_I3C_IT_FCIE | HAL_I3C_IT_TXFNFIE | HAL_I3C_IT_CFNFIE |
                      HAL_I3C_IT_RXFNEIE | HAL_I3C_IT_ERRIE );
        }

        /* Check if requested interrupts are related to controller transmit broadcast ENTDAA CCC in
         * IT mode */
...
static void I3C_Disable_IRQ( I3C_HandleTypeDef* hi3c, uint32_t InterruptRequest )
{
        ...
        /* Check if requested interrupts are related to controller transmit in IT mode */
        if ( ( InterruptRequest & I3C_XFER_CONTROLLER_TX_IT ) != 0U ) {
                /* Disable frame complete, control FIFO not full, transmit FIFO not full and error
                 * interrupts */
                tmpisr |=
                    ( HAL_I3C_IT_FCIE | HAL_I3C_IT_CFNFIE | HAL_I3C_IT_TXFNFIE | HAL_I3C_IT_ERRIE );
        }
...
static void I3C_ErrorTreatment( I3C_HandleTypeDef* hi3c )
{
        ...
        if ( tmpstate == HAL_I3C_STATE_BUSY ) {
                ...
                I3C_Disable_IRQ(
                    hi3c,
                    ( I3C_XFER_TARGET_IBI | I3C_XFER_TARGET_HOTJOIN | I3C_XFER_TARGET_CTRLROLE ) );
        } else {
