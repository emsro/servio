...
HAL_GTZC_TZSC_ConfigPeriphAttributes( uint32_t PeriphId, uint32_t PeriphAttributes )
{
        ...
        if ( ( PeriphId & GTZC_PERIPH_ALL ) != 0U ) {
                ...
                if ( ( PeriphAttributes & GTZC_TZSC_PERIPH_PRIV ) == GTZC_TZSC_PERIPH_PRIV ) {
                    ...
                    ( PeriphAttributes & GTZC_TZSC_PERIPH_NPRIV ) == GTZC_TZSC_PERIPH_NPRIV ) {
                ...
                if ( ( PeriphAttributes & GTZC_TZSC_PERIPH_SEC ) == GTZC_TZSC_PERIPH_SEC ) {
                        SET_BIT(
                            *(__IO uint32_t*) register_address,
 ...
 */
HAL_StatusTypeDef
HAL_GTZC_TZSC_GetConfigPeriphAttributes( uint32_t PeriphId, uint32_t* PeriphAttributes )
{
        uint32_t i;
        uint32_t reg_value;
        uint32_t register_address;
...
HAL_StatusTypeDef HAL_GTZC_TZSC_MPCWM_ConfigMemAttributes(
    ...
    MPCWM_ConfigTypeDef const* pMPCWM_Desc )
{
        uint32_t register_address;
        uint32_t reg_value;
        uint32_t size;
        /* granularity value depends on selected memory */
        uint32_t granularity = ( MemBaseAddress == BKPSRAM_BASE ) ? GTZC_TZSC_MPCWM_GRANULARITY_2 :
        ...
        reg_value = ( ( pMPCWM_Desc->Offset / granularity ) << GTZC_TZSC_MPCWMR_SUBZ_START_Pos ) &
                    GTZC_TZSC_MPCWMR_SUBZ_START_Msk;
        reg_value |= ( ( pMPCWM_Desc->Length / granularity ) << GTZC_TZSC_MPCWMR_SUBZ_LENGTH_Pos ) &
        ...
        reg_value = ( pMPCWM_Desc->Attribute << GTZC_TZSC_MPCWM_CFGR_SEC_Pos ) | pMPCWM_Desc->Lock |
                    pMPCWM_Desc->AreaStatus;
        MODIFY_REG(
            *(__IO uint32_t*) ( register_address - 4U ),
            ( GTZC_TZSC_MPCWM_CFGR_PRIV | GTZC_TZSC_MPCWM_CFGR_SEC | GTZC_TZSC_MPCWM_CFGR_SRLOCK |
        ...
        MODIFY_REG(
            *(__IO uint32_t*) ( register_address - 4U ),
            ( GTZC_TZSC_MPCWM_CFGR_PRIV | GTZC_TZSC_MPCWM_CFGR_SRLOCK | GTZC_TZSC_MPCWM_CFGR_SREN ),
            reg_value );
 ...
 *         When the WaterMark memory supports two sub-regions A and B. pMPCWM_Desc argument must
 * point to an array of two MPCWM_ConfigTypeDef structures. The structure description is available
 * in @ref GTZC_Exported_Types.
 * @retval HAL status.
 */
    ...
    MPCWM_ConfigTypeDef* pMPCWM_Desc )
{
        ...
        pMPCWM_Desc[0].Offset =
            ( ( reg_value & GTZC_TZSC_MPCWMR_SUBZ_START_Msk ) >> GTZC_TZSC_MPCWMR_SUBZ_START_Pos ) *
            granularity;
...
#else
        pMPCWM_Desc[0].Attribute =
            ( reg_value & GTZC_TZSC_MPCWM_CFGR_PRIV ) >> ( GTZC_TZSC_MPCWM_CFGR_PRIV_Pos - 1U );
#endif /* defined(__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U) */
 ...
 */
uint32_t HAL_GTZC_TZSC_GetLock( GTZC_TZSC_TypeDef const* TZSC_Instance )
{
 ...
 */
HAL_StatusTypeDef
HAL_GTZC_MPCBB_ConfigMem( uint32_t MemBaseAddress, MPCBB_ConfigTypeDef const* pMPCBB_desc )
{
        ...
        for ( i = 0U; i < size_in_superblocks; i++ ) {
                WRITE_REG(
                    mpcbb_ptr->SECCFGR[i], pMPCBB_desc->AttributeConfig.MPCBB_SecConfig_array[i] );
        }
        ...
        MODIFY_REG(
            mpcbb_ptr->CR, GTZC_MPCBB_CR_INVSECSTATE_Msk | GTZC_MPCBB_CR_SRWILADIS_Msk, reg_value );
 ...
 */
HAL_StatusTypeDef
HAL_GTZC_MPCBB_GetConfigMem( uint32_t MemBaseAddress, MPCBB_ConfigTypeDef* pMPCBB_desc )
{
        GTZC_MPCBB_TypeDef* mpcbb_ptr;
        uint32_t            mem_size;
        uint32_t            size_in_superblocks;
    ...
    uint32_t const* pMemAttributes )
{
        ...
        if ( ( ( IS_ADDRESS_IN_NS( SRAM1, MemAddress ) ) &&
            ...
            ( ( IS_ADDRESS_IN_S( SRAM1, MemAddress ) ) &&
            ...
            ( ( IS_ADDRESS_IN_NS( SRAM2, MemAddress ) ) &&
            ...
            ( ( IS_ADDRESS_IN_S( SRAM2, MemAddress ) ) &&
            ...
            ( ( IS_ADDRESS_IN_NS( SRAM3, MemAddress ) ) &&
              ( IS_ADDRESS_IN_NS( SRAM3, end_address ) ) ) != 0U ) {
                mpcbb_ptr    = GTZC_MPCBB3;
                base_address = SRAM3_BASE_NS;
        } else if (
            ( ( IS_ADDRESS_IN_S( SRAM3, MemAddress ) ) &&
        ...
        for ( i = 0U; i < NbBlocks; i++ ) {
                ...
                if ( ( pMemAttributes[i] & GTZC_MPCBB_BLOCK_SEC ) == GTZC_MPCBB_BLOCK_SEC ) {
                        SET_BIT(
                            mpcbb_ptr->SECCFGR[offset_reg_start],
                            1UL << ( offset_bit_start % 32U ) );
                        do_attr_change = 1U;
                } else if (
                    ( pMemAttributes[i] & GTZC_MPCBB_BLOCK_NSEC ) == GTZC_MPCBB_BLOCK_NSEC ) {
                        CLEAR_BIT(
                            mpcbb_ptr->SECCFGR[offset_reg_start],
                            1UL << ( offset_bit_start % 32U ) );
                        do_attr_change = 1U;
                ...
                if ( ( pMemAttributes[i] & GTZC_MPCBB_BLOCK_PRIV ) == GTZC_MPCBB_BLOCK_PRIV ) {
                        SET_BIT(
                            mpcbb_ptr->PRIVCFGR[offset_reg_start],
                            1UL << ( offset_bit_start % 32U ) );
                } else if (
                    ( pMemAttributes[i] & GTZC_MPCBB_BLOCK_NPRIV ) == GTZC_MPCBB_BLOCK_NPRIV ) {
                        CLEAR_BIT(
                            mpcbb_ptr->PRIVCFGR[offset_reg_start],
                            1UL << ( offset_bit_start % 32U ) );
                } else {
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_GTZC_MPCBB_GetConfigMemAttributes(
    uint32_t  MemAddress,
    uint32_t  NbBlocks,
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_GTZC_MPCBB_LockConfig(
    uint32_t        MemAddress,
    uint32_t        NbSuperBlocks,
    uint32_t const* pLockAttributes )
{
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_GTZC_MPCBB_GetLockConfig(
    uint32_t  MemAddress,
    uint32_t  NbSuperBlocks,
...
HAL_StatusTypeDef HAL_GTZC_TZIC_DisableIT( uint32_t PeriphId )
{
        ...
        if ( ( PeriphId & GTZC_PERIPH_ALL ) != 0U ) {
                ...
                CLEAR_BIT(
                    *(__IO uint32_t*) register_address, 1UL << GTZC_GET_PERIPH_POS( PeriphId ) );
        }
