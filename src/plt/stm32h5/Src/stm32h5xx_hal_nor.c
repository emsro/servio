/**
  ...
  ******************************************************************************
  ...
  HAL_NOR_WriteOperation_Enable()/ HAL_NOR_WriteOperation_Disable() to respectively enable/disable
  the NOR write operation
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NOR_Init(
    ...
    FMC_NORSRAM_TimingTypeDef* ExtTiming )
{
        ...
        if ( hnor->Init.WriteOperation == FMC_WRITE_OPERATION_DISABLE ) {
                ...
                if ( uwNORMemoryDataWidth == NOR_MEMORY_8B ) {
                        ...
                        NOR_WRITE(
                            NOR_ADDR_SHIFT(
                                deviceaddress, uwNORMemoryDataWidth, NOR_CMD_ADDRESS_FIRST_CFI ),
                            NOR_CMD_DATA_CFI );
                }
    ...
    uint32_t           uwBufferSize )
{
        ...
        if ( hnor->State == HAL_NOR_STATE_BUSY ) {
                ...
                if ( hnor->CommandSet == NOR_AMD_FUJITSU_COMMAND_SET ) {
                        ...
                        NOR_WRITE(
                            ( deviceaddress + uwAddress ), (uint16_t) ( uwBufferSize - 1U ) );
                } else if ( hnor->CommandSet == NOR_INTEL_SHARP_EXT_COMMAND_SET ) {
                        /* Write Buffer Load Command */
...
HAL_NOR_Erase_Block( NOR_HandleTypeDef* hnor, uint32_t BlockAddress, uint32_t Address )
{
        ...
        if ( hnor->State == HAL_NOR_STATE_BUSY ) {
                ...
                if ( hnor->CommandSet == NOR_AMD_FUJITSU_COMMAND_SET ) {
                        ...
                        NOR_WRITE(
                            (uint32_t) ( BlockAddress + Address ), NOR_CMD_DATA_BLOCK_ERASE );
                } else if ( hnor->CommandSet == NOR_INTEL_SHARP_EXT_COMMAND_SET ) {
 ...
 * @retval status
 */
HAL_StatusTypeDef HAL_NOR_RegisterCallback(
    NOR_HandleTypeDef*        hnor,
 ...
 */
HAL_NOR_StateTypeDef HAL_NOR_GetState( NOR_HandleTypeDef const* hnor )
{
        return hnor->State;
}
