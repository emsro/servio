...
LPTIM_OC1_SetConfig( LPTIM_HandleTypeDef* hlptim, LPTIM_OC_ConfigTypeDef const* sConfig );
static HAL_StatusTypeDef
LPTIM_OC2_SetConfig( LPTIM_HandleTypeDef* hlptim, LPTIM_OC_ConfigTypeDef const* sConfig );
static void
LPTIM_IC1_SetConfig( LPTIM_HandleTypeDef* hlptim, LPTIM_IC_ConfigTypeDef const* sConfig );
static void
LPTIM_IC2_SetConfig( LPTIM_HandleTypeDef* hlptim, LPTIM_IC_ConfigTypeDef const* sConfig );
#if ( USE_HAL_LPTIM_REGISTER_CALLBACKS == 1 )
static void LPTIM_ResetCallback( LPTIM_HandleTypeDef* lptim );
#endif /* USE_HAL_LPTIM_REGISTER_CALLBACKS */
static HAL_StatusTypeDef LPTIM_WaitForFlag( LPTIM_HandleTypeDef const* hlptim, uint32_t flag );
...
HAL_StatusTypeDef
LPTIM_DMA_Start_IT( DMA_HandleTypeDef* hdma, uint32_t src, uint32_t dst, uint32_t length );
...
HAL_StatusTypeDef HAL_LPTIM_Init( LPTIM_HandleTypeDef* hlptim )
{
        ...
        tmpcfgr &= (uint32_t) ( ~(
            LPTIM_CFGR_CKSEL | LPTIM_CFGR_TRIGEN | LPTIM_CFGR_PRELOAD | LPTIM_CFGR_PRESC |
            LPTIM_CFGR_COUNTMODE ) );

        /* Set initialization parameters */
        tmpcfgr |=
            ( hlptim->Init.Clock.Source | hlptim->Init.Clock.Prescaler | hlptim->Init.UpdateMode |
        ...
        if ( ( hlptim->Init.Clock.Source == LPTIM_CLOCKSOURCE_ULPTIM ) ||
             ( hlptim->Init.CounterSource == LPTIM_COUNTERSOURCE_EXTERNAL ) ) {
                tmpcfgr |=
                    ( hlptim->Init.UltraLowPowerClock.Polarity |
                      hlptim->Init.UltraLowPowerClock.SampleTime );
        }
        ...
        if ( hlptim->Init.Trigger.Source != LPTIM_TRIGSOURCE_SOFTWARE ) {
                ...
                tmpcfgr |=
                    ( hlptim->Init.Trigger.Source | hlptim->Init.Trigger.ActiveEdge |
                      hlptim->Init.Trigger.SampleTime );
        }
...
HAL_StatusTypeDef HAL_LPTIM_PWM_Start_IT( LPTIM_HandleTypeDef* hlptim, uint32_t Channel )
{
        ...
        case LPTIM_CHANNEL_1:
                ...
                __HAL_LPTIM_ENABLE_IT(
                    hlptim,
                    LPTIM_IT_CMP1OK | LPTIM_IT_CC1 | LPTIM_IT_ARROK | LPTIM_IT_ARRM |
        ...
        case LPTIM_CHANNEL_2:
                ...
                __HAL_LPTIM_ENABLE_IT(
                    hlptim,
                    LPTIM_IT_CMP2OK | LPTIM_IT_CC2 | LPTIM_IT_ARROK | LPTIM_IT_ARRM |
                        LPTIM_IT_REPOK | LPTIM_IT_UPDATE );
                break;
        default:
                break;
    ...
    uint32_t             Length )
{
        ...
        case LPTIM_CHANNEL_1:
                ...
                if ( LPTIM_DMA_Start_IT(
                         hdma, (uint32_t) pData, (uint32_t) &hlptim->Instance->CCR1, Length ) !=
                     HAL_OK ) {
                        /* Return error status */
                        return HAL_ERROR;
                }
        ...
        case LPTIM_CHANNEL_2:
                ...
                if ( LPTIM_DMA_Start_IT(
                         hdma, (uint32_t) pData, (uint32_t) &hlptim->Instance->CCR2, Length ) !=
                     HAL_OK ) {
                        /* Return error status */
                        return HAL_ERROR;
                }
...
HAL_StatusTypeDef HAL_LPTIM_Counter_Start_IT( LPTIM_HandleTypeDef* hlptim )
{
        ...
        /* Enable interrupt */
        __HAL_LPTIM_ENABLE_IT(
            hlptim, LPTIM_IT_ARROK | LPTIM_IT_ARRM | LPTIM_IT_REPOK | LPTIM_IT_UPDATE );
    ...
    uint32_t             Length )
{
        ...
        case LPTIM_CHANNEL_1:
                ...
                hdma = hlptim->hdma[LPTIM_DMA_ID_CC1];
                if ( LPTIM_DMA_Start_IT(
                         hdma, (uint32_t) &hlptim->Instance->CCR1, (uint32_t) pData, Length ) !=
                     HAL_OK ) {
                        /* Return error status */
                        return HAL_ERROR;
                }
        ...
        case LPTIM_CHANNEL_2:
                ...
                hdma = hlptim->hdma[LPTIM_DMA_ID_CC2];
                if ( LPTIM_DMA_Start_IT(
                         hdma, (uint32_t) &hlptim->Instance->CCR2, (uint32_t) pData, Length ) !=
                     HAL_OK ) {
                        /* Return error status */
                        return HAL_ERROR;
                }
...
/**
 * @brief Repetition counter underflowed (or contains zero) and LPTIM counter overflowed callback in
 * non-blocking mode.
 * @param  hlptim LPTIM handle
 * @retval None
 */
 ...
 *          @arg @ref HAL_LPTIM_REP_COUNTER_WRITE_CB_ID Repetition counter register write complete
 * Callback ID
 ...
 */
HAL_StatusTypeDef
HAL_LPTIM_UnRegisterCallback( LPTIM_HandleTypeDef* hlptim, HAL_LPTIM_CallbackIDTypeDef CallbackID )
{
        HAL_StatusTypeDef status = HAL_OK;
...
LPTIM_IC1_SetConfig( LPTIM_HandleTypeDef* hlptim, LPTIM_IC_ConfigTypeDef const* sConfig )
{
        ...
        tmpccmr1 |=
            sConfig->ICPrescaler | sConfig->ICPolarity | sConfig->ICFilter | LPTIM_CCMR1_CC1SEL;
...
LPTIM_IC2_SetConfig( LPTIM_HandleTypeDef* hlptim, LPTIM_IC_ConfigTypeDef const* sConfig )
{
        ...
        tmpccmr1 |=
            ( sConfig->ICPrescaler << ( LPTIM_CCMR1_IC2PSC_Pos - LPTIM_CCMR1_IC1PSC_Pos ) ) |
