 ...
 ==============================================================================
    [..]
      ...
      (#) FLASH Memory IO Programming functions:
           ...
           (++) Flash memory programming by 128 bits (user area, OBKeys) and 16 bits (OTP and Flash
...
HAL_FLASH_Program( uint32_t TypeProgram, uint32_t FlashAddress, uint32_t DataAddress )
{
        ...
        if ( status == HAL_OK ) {
                ...
                if ( ( TypeProgram & ( ~FLASH_NON_SECURE_MASK ) ) == FLASH_TYPEPROGRAM_QUADWORD ) {
                    ...
                    ( TypeProgram == FLASH_TYPEPROGRAM_QUADWORD_OBK ) ||
                    ...
                    ( TypeProgram & ( ~FLASH_NON_SECURE_MASK ) ) == FLASH_TYPEPROGRAM_WORD_EDATA ) {
                        /* Check the parameters */
                        assert_param( IS_FLASH_EDATA_ADDRESS( FlashAddress ) );
                ...
                CLEAR_BIT(
                    ( *reg_cr ),
                    ( TypeProgram & ~( FLASH_NON_SECURE_MASK | FLASH_OBK | FLASH_OTP |
                                       FLASH_OBKCFGR_ALT_SECT ) ) );
...
void HAL_FLASH_IRQHandler( void )
{
        ...
        if ( pFlash.ProcedureOnGoing == 0U ) {
                ...
                ( *reg_cr ) &= ~(
                    FLASH_IT_EOP | FLASH_IT_WRPERR | FLASH_IT_PGSERR | FLASH_IT_STRBERR |
                    FLASH_IT_INCERR | FLASH_IT_OBKERR | FLASH_IT_OBKWERR | FLASH_IT_OPTCHANGEERR );
#else
                ( *reg_cr ) &=
                    ~( FLASH_IT_EOP | FLASH_IT_WRPERR | FLASH_IT_PGSERR | FLASH_IT_STRBERR |
...
{
        /* Wait for the FLASH operation to complete by polling on BUSY, WBNE and DBNE flags to be
           reset. Even if the FLASH operation fails, the BUSY, WBNE and DBNE flags will be reset and
