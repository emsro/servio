...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_RNGEx_SetConfig( RNG_HandleTypeDef* hrng, RNG_ConfigTypeDef const* pConf )
{
        uint32_t          tickstart;
        uint32_t          cr_value;
        ...
        if ( hrng->State == HAL_RNG_STATE_READY ) {
                ...
                MODIFY_REG(
                    hrng->Instance->CR,
                    RNG_CR_NISTC | RNG_CR_CLKDIV | RNG_CR_RNG_CONFIG1 | RNG_CR_RNG_CONFIG2 |
                        RNG_CR_RNG_CONFIG3 | RNG_CR_ARDIS,
