...
#define CRYP_OPERATINGMODE_KEYDERIVATION_DECRYPT                                             \
        AES_CR_MODE /*!< Key derivation and decryption only used when performing ECB and CBC \
                       decryptions  */

#define CRYPEx_PHASE_PROCESS 0x02U /*!< CRYP peripheral is in processing phase */
#define CRYPEx_PHASE_FINAL                                                                        \
        0x03U /*!< CRYP peripheral is in final phase this is relevant only with CCM and GCM modes \
               */
...
CRYPEx_WaitFLAG( CRYP_HandleTypeDef* hcryp, uint32_t flag, FlagStatus Status, uint32_t Timeout );
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_CRYPEx_WrapKey(
    CRYP_HandleTypeDef* hcryp,
    uint32_t*           pInput,
    uint32_t*           pOutput,
    uint32_t            Timeout )
 ...
 */
HAL_StatusTypeDef
HAL_CRYPEx_UnwrapKey( CRYP_HandleTypeDef* hcryp, uint32_t* pInput, uint32_t Timeout )
{
        HAL_StatusTypeDef status;
        uint32_t          algo;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_CRYPEx_EncryptSharedKey(
    CRYP_HandleTypeDef* hcryp,
    uint32_t*           pKey,
    uint32_t*           pOutput,
    uint32_t            ID,
    uint32_t            Timeout )
{
        ...
        if ( hcryp->State == HAL_CRYP_STATE_READY ) {
                ...
                /* Set the operating mode */
                MODIFY_REG(
                    hcryp->Instance->CR, AES_CR_KMOD | AES_CR_KSHAREID, CRYP_KEYMODE_SHARED | ID );
                /* Check the busy flag before writing CR register */
                if ( CRYPEx_WaitFLAG( hcryp, AES_SR_BUSY, SET, CRYPEx_GENERAL_TIMEOUT ) !=
                     HAL_OK ) {
                        hcryp->State = HAL_CRYP_STATE_READY;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_CRYPEx_DecryptSharedKey(
    CRYP_HandleTypeDef* hcryp,
    uint32_t*           pKey,
    uint32_t            ID,
    uint32_t            Timeout )
