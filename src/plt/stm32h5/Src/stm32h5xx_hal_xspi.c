/**
 ...
 __HAL_RCC_XSPI_RELEASE_RESET().
    ...
    *** Auto-polling functional mode ***
    ...
    [..]
     ...
     (+) In interrupt mode, HAL_XSPI_StatusMatchCallback() will be called each time the status match
 is reached.
    ...
    *** Control functions ***
    ...
    [..]
     HAL_XSPI_SetFifoThreshold() function configures the threshold on the Fifo of the OctoSPI
 Peripheral.
    [..]
 ...
 clock.
     (+) The delay line length can be Configure to one period of the Input clock with
...
static HAL_StatusTypeDef XSPI_WaitFlagStateUntilTimeout(
...
XSPI_ConfigCmd( XSPI_HandleTypeDef* hxspi, XSPI_RegularCmdTypeDef* const pCmd );
...
HAL_StatusTypeDef HAL_XSPI_Init( XSPI_HandleTypeDef* hxspi )
{
        ...
        if ( hxspi == NULL ) {
                ...
                if ( hxspi->State == HAL_XSPI_STATE_RESET ) {
                        ...
                        MODIFY_REG(
                            hxspi->Instance->DCR1,
                            ( XSPI_DCR1_MTYP | XSPI_DCR1_DEVSIZE | XSPI_DCR1_CSHT | XSPI_DCR1_FRCK |
                              XSPI_DCR1_CKMODE ),
                            ( hxspi->Init.MemoryType |
                              ( ( hxspi->Init.MemorySize ) << XSPI_DCR1_DEVSIZE_Pos ) |
                              ( ( hxspi->Init.ChipSelectHighTimeCycle - 1U )
                                << XSPI_DCR1_CSHT_Pos ) |
                        ...
                        /* Configure wrap size */
                        MODIFY_REG(
                            hxspi->Instance->DCR2, XSPI_DCR2_WRAPSIZE, hxspi->Init.WrapSize );
                        ...
                        if ( status == HAL_OK ) {
                                ...
                                MODIFY_REG(
                                    hxspi->Instance->CR, XSPI_CR_DMM, hxspi->Init.MemoryMode );
                                ...
                                /* Enable free running clock if needed : must be done after XSPI
                                 * enable */
...
void HAL_XSPI_IRQHandler( XSPI_HandleTypeDef* hxspi )
{
        ...
        /* XSPI transfer complete interrupt occurred ----------------------------*/
            ...
            ( ( flag & HAL_XSPI_FLAG_TC ) != 0U ) && ( ( itsource & HAL_XSPI_IT_TC ) != 0U ) ) {
                if ( currentstate == HAL_XSPI_STATE_BUSY_RX ) {
                        if ( ( hxspi->XferCount > 0U ) && ( ( flag & XSPI_SR_FLEVEL ) != 0U ) ) {
                                ...
                                *hxspi->pBuffPtr = *( (__IO uint8_t*) data_reg );
                                ...
                                HAL_XSPI_DISABLE_IT(
                                    hxspi, HAL_XSPI_IT_TC | HAL_XSPI_IT_FT | HAL_XSPI_IT_TE );
        ...
        /* XSPI status match interrupt occurred ---------------------------------*/
            ...
            ( ( flag & HAL_XSPI_FLAG_SM ) != 0U ) && ( ( itsource & HAL_XSPI_IT_SM ) != 0U ) ) {
                /* Clear flag */
        ...
        /* XSPI transfer error interrupt occurred -------------------------------*/
            ...
            ( ( flag & HAL_XSPI_FLAG_TE ) != 0U ) && ( ( itsource & HAL_XSPI_IT_TE ) != 0U ) ) {
                /* Clear flag */
                ...
                HAL_XSPI_DISABLE_IT(
                    hxspi,
                    ( HAL_XSPI_IT_TO | HAL_XSPI_IT_SM | HAL_XSPI_IT_FT | HAL_XSPI_IT_TC |
                      HAL_XSPI_IT_TE ) );
        ...
        /* XSPI timeout interrupt occurred --------------------------------------*/
            ...
            ( ( flag & HAL_XSPI_FLAG_TO ) != 0U ) && ( ( itsource & HAL_XSPI_IT_TO ) != 0U ) ) {
                /* Clear flag */
        ...
        if ( ( ( state == HAL_XSPI_STATE_READY ) &&
             ...
             ( ( state == HAL_XSPI_STATE_WRITE_CMD_CFG ) &&
               ( ( pCmd->OperationType == HAL_XSPI_OPTYPE_READ_CFG ) ||
                ...
                /* Wait till busy flag is reset */
                status = XSPI_WaitFlagStateUntilTimeout(
                    hxspi, HAL_XSPI_FLAG_BUSY, RESET, tickstart, Timeout );
                ...
                if ( status == HAL_OK ) {
                        ...
                        if ( status == HAL_OK ) {
                                if ( pCmd->DataMode == HAL_XSPI_DATA_NONE ) {
                                        /* When there is no data phase, the transfer start as soon
                                           as the configuration is done so wait until BUSY flag is
                                           reset to go back in idle state. */
                                        status = XSPI_WaitFlagStateUntilTimeout(
                                            hxspi, HAL_XSPI_FLAG_BUSY, RESET, tickstart, Timeout );
        ...
        if ( pData == NULL ) {
                ...
                if ( hxspi->State == HAL_XSPI_STATE_CMD_CFG ) {
                        ...
                        do {
                                ...
                                status = XSPI_WaitFlagStateUntilTimeout(
                                    hxspi, HAL_XSPI_FLAG_FT, SET, tickstart, Timeout );
                        ...
                        if ( status == HAL_OK ) {
                                 ...
                                 */
                                status = XSPI_WaitFlagStateUntilTimeout(
                                    hxspi, HAL_XSPI_FLAG_TC, SET, tickstart, Timeout );
        ...
        if ( pData == NULL ) {
                ...
                if ( hxspi->State == HAL_XSPI_STATE_CMD_CFG ) {
                        ...
                        if ( hxspi->Init.MemoryType == HAL_XSPI_MEMTYPE_HYPERBUS ) {
                            ...
                            READ_BIT( hxspi->Instance->CCR, XSPI_CCR_ADMODE ) !=
                        ...
                        do {
                                /* Wait till fifo threshold or transfer complete flags are set to
                                 * read received data */
                                status = XSPI_WaitFlagStateUntilTimeout(
                                    hxspi,
                                    ( HAL_XSPI_FLAG_FT | HAL_XSPI_FLAG_TC ),
                                    SET,
                                    tickstart,
                                    Timeout );
                ...
                if ( hxspi->State == HAL_XSPI_STATE_CMD_CFG ) {
                        if ( ( hxspi->hdmatx->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                                ...
                                if ( ( p_queue != NULL ) && ( p_queue->Head != NULL ) ) {
                                        data_width =
                                            p_queue->Head->LinkRegisters[NODE_CTR1_DEFAULT_OFFSET] &
                                            DMA_CTR1_DDW_LOG2;
                                } else {
                        ...
                        if ( data_width == DMA_DEST_DATAWIDTH_BYTE ) {
                                ...
                                if ( ( ( data_size % 2U ) != 0U ) ||
                                     ( ( hxspi->Init.FifoThresholdByte % 2U ) != 0U ) ) {
                                        /* The number of data or the fifo threshold is not aligned
                                        on halfword
                                        => no transfer possible with DMA peripheral access
                                        configured as halfword */
                                        hxspi->ErrorCode = HAL_XSPI_ERROR_INVALID_PARAM;
                                        status           = HAL_ERROR;
                                } else {
                                        hxspi->XferCount = data_size;
                        ...
                        if ( status == HAL_OK ) {
                                ...
                                if ( ( hxspi->hdmatx->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                                        if ( hxspi->hdmatx->LinkedListQueue != NULL ) {
                                                ...
                                                MODIFY_REG(
                                                    p_queue->Head
                                                        ->LinkRegisters[NODE_CTR2_DEFAULT_OFFSET],
                                                    DMA_CTR2_DREQ,
                                    ...
                                    ( hxspi->hdmatx->Init.Direction == DMA_MEMORY_TO_PERIPH ) &&
                                    ...
                                    ( hxspi->hdmatx->Init.DestInc == DMA_DINC_FIXED ) ) {
                                        ...
                                        /* no transmit possible with DMA peripheral, invalid
                                         * configuration */
                                        hxspi->ErrorCode = HAL_XSPI_ERROR_INVALID_PARAM;
                                        status           = HAL_ERROR;
                                }
                ...
                if ( hxspi->State == HAL_XSPI_STATE_CMD_CFG ) {
                        ...
                        if ( status == HAL_OK ) {
                                ...
                                if ( ( hxspi->hdmarx->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                                    ...
                                    ( hxspi->hdmarx->Init.Direction == DMA_PERIPH_TO_MEMORY ) &&
                                    ...
                                    ( hxspi->hdmarx->Init.DestInc == DMA_DINC_INCREMENTED ) ) {
                                        ...
                                        /* no receive possible with DMA peripheral, invalid
                                         * configuration */
                                        hxspi->ErrorCode = HAL_XSPI_ERROR_INVALID_PARAM;
                                        status           = HAL_ERROR;
                                }
                                if ( status == HAL_OK ) {
                                        ...
                                        /* Trig the transfer by re-writing address or instruction
                                         * register */
                                        if ( hxspi->Init.MemoryType == HAL_XSPI_MEMTYPE_HYPERBUS ) {
                                                WRITE_REG( hxspi->Instance->AR, addr_reg );
                                        } else if (
        ...
        if ( ( hxspi->State == HAL_XSPI_STATE_CMD_CFG ) &&
             ( pCfg->AutomaticStop == HAL_XSPI_AUTOMATIC_STOP_ENABLE ) ) {
                ...
                if ( status == HAL_OK ) {
                        ...
                        status = XSPI_WaitFlagStateUntilTimeout(
                            hxspi, HAL_XSPI_FLAG_SM, SET, tickstart, Timeout );
...
HAL_StatusTypeDef HAL_XSPI_Abort( XSPI_HandleTypeDef* hxspi )
{
        ...
        if ( hxspi->State != HAL_XSPI_STATE_RESET ) {
                ...
                if ( HAL_XSPI_GET_FLAG( hxspi, HAL_XSPI_FLAG_BUSY ) != RESET ) {
                        ...
                        status = XSPI_WaitFlagStateUntilTimeout(
                            hxspi, HAL_XSPI_FLAG_TC, SET, tickstart, hxspi->Timeout );
...
HAL_StatusTypeDef HAL_XSPI_Abort_IT( XSPI_HandleTypeDef* hxspi )
{
        ...
        if ( hxspi->State != HAL_XSPI_STATE_RESET ) {
                ...
                } else if ( HAL_XSPI_GET_FLAG( hxspi, HAL_XSPI_FLAG_BUSY ) != RESET ) {
                        /* Clear transfer complete flag */
                        HAL_XSPI_CLEAR_FLAG( hxspi, HAL_XSPI_FLAG_TC );
 ...
 */
HAL_StatusTypeDef
HAL_XSPI_DLYB_GetClockPeriod( XSPI_HandleTypeDef* hxspi, HAL_XSPI_DLYB_CfgTypeDef* const pdlyb_cfg )
{
        HAL_StatusTypeDef status = HAL_ERROR;
...
static HAL_StatusTypeDef XSPI_ConfigCmd( XSPI_HandleTypeDef* hxspi, XSPI_RegularCmdTypeDef* pCmd )
{
        ...
        *ccr_reg = ( pCmd->DQSMode | pCmd->SIOOMode );
        ...
        /* Workaround for Erratasheet: Memory-mapped write error response when DQS output is
         * disabled */
        if ( pCmd->OperationType == HAL_XSPI_OPTYPE_WRITE_CFG ) {
                        ...
                        if ( pCmd->DataMode != HAL_XSPI_DATA_NONE ) {
                                ...
                                MODIFY_REG(
                                    ...
                                    ( pCmd->InstructionMode | pCmd->InstructionDTRMode |
                                      ...
                                      pCmd->AddressDTRMode | pCmd->AddressWidth | pCmd->DataMode |
                                      pCmd->DataDTRMode ) );
                        } else {
                                ...
                                if ( ( hxspi->Init.DelayHoldQuarterCycle ==
                                       HAL_XSPI_DHQC_ENABLE ) &&
                                     ( pCmd->InstructionDTRMode ==
                                       HAL_XSPI_INSTRUCTION_DTR_ENABLE ) ) {
                                        MODIFY_REG(
                                            ( *ccr_reg ), XSPI_CCR_DDTR, HAL_XSPI_DATA_DTR_ENABLE );
                                }
                        }
        ...
        } else if ( pCmd->AddressMode != HAL_XSPI_ADDRESS_NONE ) {
                if ( pCmd->DataMode != HAL_XSPI_DATA_NONE ) {
                        ...
                        MODIFY_REG(
                            ( *ccr_reg ),
                            ( XSPI_CCR_ADMODE | XSPI_CCR_ADDTR | XSPI_CCR_ADSIZE | XSPI_CCR_DMODE |
                              XSPI_CCR_DDTR ),
