...
HAL_StatusTypeDef HAL_IWDG_Init( IWDG_HandleTypeDef* hiwdg )
{
        ...
        /* Check Reload update flag, before performing any reload of the counter, else previous
        value will be taken. */
        ...
        if ( hiwdg->Init.EWI == IWDG_EWI_DISABLE ) {
                 ...
                 * acknowledge the early wakeup interrupt in any cases. it clears the EWIF flag in
                 * SR register Set Watchdog Early Wakeup Comparator to 0x00 */
                hiwdg->Instance->EWCR = IWDG_EWCR_EWIC;
        } else {
                /* EWI comparator value different from 0, enable the early wakeup interrupt,
                 * acknowledge the early wakeup interrupt in any cases. it clears the EWIF flag in
                 * SR register Set Watchdog Early Wakeup Comparator value */
