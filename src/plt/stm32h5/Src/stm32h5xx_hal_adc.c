...
#define ADC_CFGR_FIELDS_1                                                                          \
        ( ADC_CFGR_RES | ADC_CFGR_ALIGN | ADC_CFGR_CONT | ADC_CFGR_OVRMOD | ADC_CFGR_DISCEN |      \
          ADC_CFGR_DISCNUM | ADC_CFGR_EXTEN | ADC_CFGR_EXTSEL ) /*!< ADC_CFGR fields of parameters \
               that can be updated when no regular conversion is on-going */
...
HAL_StatusTypeDef HAL_ADC_Start( ADC_HandleTypeDef* hadc )
{
        ...
        ADC_TypeDef const* tmpADC_Master;
        ...
        if ( LL_ADC_REG_IsConversionOngoing( hadc->Instance ) == 0UL ) {
                ...
                if ( tmp_hal_status == HAL_OK ) {
                        ...
                        if ( HAL_IS_BIT_SET( hadc->State, HAL_ADC_STATE_INJ_BUSY ) ) {
                                 ...
                                 */
                                CLEAR_BIT(
                                    hadc->ErrorCode, ( HAL_ADC_ERROR_OVR | HAL_ADC_ERROR_DMA ) );
                        } else {
 ...
 * not present on all STM32 series)
        ...
        if ( LL_ADC_REG_IsConversionOngoing( hadc->Instance ) == 0UL ) {
                ...
                if ( tmp_hal_status == HAL_OK ) {
                        ...
                        if ( hadc->Init.Overrun == ADC_OVR_DATA_PRESERVED )
                                ...
                                /* Case of multimode enabled (when multimode feature is available):
                                 */
                                /*  - if ADC is slave and dual regular conversions are enabled, ADC
                                 * is  */
                                /*    enabled only (conversion is not started), */
                                /*  - if ADC is master, ADC is enabled and conversion is started. */
#if defined( ADC_MULTIMODE_SUPPORT )
        ...
        if ( LL_ADC_REG_IsConversionOngoing( hadc->Instance ) == 0UL ) {
...
#endif /* ADC_MULTIMODE_SUPPORT */
                {
                        ...
                        if ( tmp_hal_status == HAL_OK ) {
                                ...
                                if ( ( hadc->DMA_Handle->Mode & DMA_LINKEDLIST ) ==
                                     DMA_LINKEDLIST ) {
                                ...
                                if ( __HAL_ADC_GET_FLAG( hadc, ADC_FLAG_EOS ) ) {
                                        ...
                                        if ( LL_ADC_REG_IsConversionOngoing( hadc->Instance ) ==
                                             0UL ) {
                                                ...
                                                if ( ( hadc->State & HAL_ADC_STATE_INJ_BUSY ) ==
                                                     0UL ) {
                                                        SET_BIT( hadc->State, HAL_ADC_STATE_READY );
                                                }
                                        } else {
                ...
                if ( ( tmp_adc_is_conversion_on_going_regular == 0UL ) &&
                     ( tmp_adc_is_conversion_on_going_injected == 0UL ) ) {
                        ...
                        if ( pConfig->OffsetNumber != ADC_OFFSET_NONE ) {
                                 ...
                                 */
                                if ( __LL_ADC_CHANNEL_TO_DECIMAL_NB( LL_ADC_GetOffsetChannel(
                                         hadc->Instance, LL_ADC_OFFSET_1 ) ) ==
 ...
 */
HAL_StatusTypeDef
HAL_ADC_AnalogWDGConfig( ADC_HandleTypeDef* hadc, ADC_AnalogWDGConfTypeDef const* pAnalogWDGConfig )
{
        HAL_StatusTypeDef tmp_hal_status = HAL_OK;
...
{
        uint32_t       tickstart;
        uint32_t const tmp_adc_is_disable_on_going = LL_ADC_IsDisableOngoing( hadc->Instance );
