...
HAL_FMAC_AppendFilterData(). This function should only be called once the previous input data has
been handled (the preloaded input data isn't concerned).

       (#) In all modes except none, provide a new output buffer to be filled via
           HAL_FMAC_ConfigFilterOutputBuffer(). This function should only be called once the
previous user's output buffer has been filled.
...
static HAL_StatusTypeDef FMAC_FilterConfig(
    FMAC_HandleTypeDef*       hfmac,
    FMAC_FilterConfigTypeDef* pConfig,
    uint8_t                   PreloadAccess );
static HAL_StatusTypeDef FMAC_FilterPreload(
    FMAC_HandleTypeDef* hfmac,
    int16_t*            pInput,
    uint8_t             InputSize,
    int16_t*            pOutput,
    ...
    uint8_t             PreloadAccess );
static void
FMAC_WritePreloadDataIncrementPtr( FMAC_HandleTypeDef* hfmac, int16_t** ppData, uint8_t Size );
static HAL_StatusTypeDef
FMAC_WaitOnStartUntilTimeout( FMAC_HandleTypeDef* hfmac, uint32_t Tickstart, uint32_t Timeout );
static HAL_StatusTypeDef FMAC_AppendFilterDataUpdateState(
    FMAC_HandleTypeDef* hfmac,
    int16_t*            pInput,
    uint16_t*           pInputSize );
 ...
 * @note   The HAL_FMAC_RegisterCallback() may be called before HAL_FMAC_Init() in
 * HAL_FMAC_STATE_RESET to register callbacks for HAL_FMAC_MSPINIT_CB_ID and
 ...
 * @retval HAL_StatusTypeDef HAL status
 */
HAL_StatusTypeDef HAL_FMAC_RegisterCallback(
    FMAC_HandleTypeDef*        hfmac,
 ...
 * @note   The HAL_FMAC_UnRegisterCallback() may be called before HAL_FMAC_Init() in
 * HAL_FMAC_STATE_RESET to register callbacks for HAL_FMAC_MSPINIT_CB_ID and
 * HAL_FMAC_MSPDEINIT_CB_ID.
 * @param  hfmac pointer to a FMAC_HandleTypeDef structure that contains
 *         the configuration information for FMAC module
 ...
 */
HAL_StatusTypeDef
HAL_FMAC_UnRegisterCallback( FMAC_HandleTypeDef* hfmac, HAL_FMAC_CallbackIDTypeDef CallbackID )
{
        HAL_StatusTypeDef status = HAL_OK;
 ...
 */
HAL_StatusTypeDef
HAL_FMAC_FilterConfig( FMAC_HandleTypeDef* hfmac, FMAC_FilterConfigTypeDef* pConfig )
{
 ...
 */
HAL_StatusTypeDef
HAL_FMAC_FilterConfig_DMA( FMAC_HandleTypeDef* hfmac, FMAC_FilterConfigTypeDef* pConfig )
{
 ...
 * @note   The input and the output buffers can be filled by calling several times @ref
 * HAL_FMAC_FilterPreload (each call filling partly the buffers). In case of overflow (too much data
 * provided through all these calls), an error will be returned.
 * @retval HAL_StatusTypeDef HAL status
 */
HAL_StatusTypeDef HAL_FMAC_FilterPreload(
    FMAC_HandleTypeDef* hfmac,
    int16_t*            pInput,
    uint8_t             InputSize,
    int16_t*            pOutput,
    uint8_t             OutputSize )
{
        return ( FMAC_FilterPreload(
            hfmac, pInput, InputSize, pOutput, OutputSize, PRELOAD_ACCESS_POLLING ) );
}
...
{
        return ( FMAC_FilterPreload(
            hfmac, pInput, InputSize, pOutput, OutputSize, PRELOAD_ACCESS_DMA ) );
}
 ...
 * @param  pOutputSize pointer to the size of the output buffer. The number of read data will be
 * written here.
 * @retval HAL_StatusTypeDef HAL status
 */
HAL_StatusTypeDef
HAL_FMAC_FilterStart( FMAC_HandleTypeDef* hfmac, int16_t* pOutput, uint16_t* pOutputSize )
{
        uint32_t          tmpcr = 0U;
        HAL_StatusTypeDef status;
        ...
        if ( hfmac->State == HAL_FMAC_STATE_READY ) {
                ...
                if ( status == HAL_OK ) {
                        /* PARAM: Start the filter ( this can generate interrupts before the end of
                         * the HAL_FMAC_FilterStart ) */
 ...
 * @retval HAL_StatusTypeDef HAL status
 */
HAL_StatusTypeDef HAL_FMAC_ConfigFilterOutputBuffer(
    FMAC_HandleTypeDef* hfmac,
    int16_t*            pOutput,
    uint16_t*           pOutputSize )
{
...
HAL_StatusTypeDef HAL_FMAC_PollFilterData( FMAC_HandleTypeDef* hfmac, uint32_t Timeout )
{
        ...
        /* Get the input and output mode (if no buffer was previously provided, nothing will be
         * read/written) */
 ...
 */
HAL_FMAC_StateTypeDef HAL_FMAC_GetState( FMAC_HandleTypeDef const* hfmac )
{
        /* Return FMAC state */
        return hfmac->State;
}
 ...
 */
uint32_t HAL_FMAC_GetError( FMAC_HandleTypeDef const* hfmac )
{
    ...
    uint8_t                   PreloadAccess )
{
        ...
        /* FMAC_X1BUFCFG: Configure the input threshold if valid when compared to the configured X1
         * size */
        if ( pConfig->InputThreshold != FMAC_THRESHOLD_NO_VALUE ) {
                /* Check the parameter */
                assert_param( IS_FMAC_THRESHOLD_APPLICABLE(
                    FMAC_GET_X1_SIZE( hfmac ), pConfig->InputThreshold, pConfig->InputAccess ) );
        ...
        /* FMAC_YBUFCFG: Configure the output threshold if valid when compared to the configured Y
         * size */
        if ( pConfig->OutputThreshold != FMAC_THRESHOLD_NO_VALUE ) {
                /* Check the parameter */
                assert_param( IS_FMAC_THRESHOLD_APPLICABLE(
                    FMAC_GET_Y_SIZE( hfmac ), pConfig->OutputThreshold, pConfig->OutputAccess ) );
        ...
        /* Save the input/output accesses in order to configure RIEN, WIEN, DMAREN and DMAWEN during
         * filter start */
        ...
        if ( ( pConfig->pCoeffB != NULL ) && ( pConfig->CoeffBSize != 0U ) ) {
                ...
                assert_param(
                    ( (uint32_t) pConfig->CoeffASize + (uint32_t) pConfig->CoeffBSize ) <= x2size );
                ...
                assert_param(
                    ( ( pConfig->Filter == FMAC_FUNC_CONVO_FIR ) && ( pConfig->pCoeffA == NULL ) &&
                ...
                if ( PreloadAccess == PRELOAD_ACCESS_POLLING ) {
                        ...
                        if ( ( pConfig->pCoeffA != NULL ) && ( pConfig->CoeffASize != 0U ) ) {
                                /* Load the buffer into the internal memory */
                                FMAC_WritePreloadDataIncrementPtr(
                                    hfmac, &( pConfig->pCoeffA ), pConfig->CoeffASize );
                        }

                        /* Wait for the end of the writing */
                        if ( FMAC_WaitOnStartUntilTimeout(
                                 hfmac, tickstart, HAL_FMAC_TIMEOUT_VALUE ) != HAL_OK ) {
                                hfmac->ErrorCode |= HAL_FMAC_ERROR_TIMEOUT;
                                hfmac->State = HAL_FMAC_STATE_TIMEOUT;
                                return HAL_ERROR;
                        }
                                ...
                                if ( ( hfmac->hdmaPreload->LinkedListQueue != NULL ) &&
                                     ( hfmac->hdmaPreload->LinkedListQueue->Head != NULL ) ) {
                                        ...
                                        hfmac->hdmaPreload->LinkedListQueue->Head
                                            ...
                                            (uint32_t) ( 2UL * pConfig->CoeffBSize ); /* Set DMA
                                                                                         data size
                                                                                       */
                                        hfmac->hdmaPreload->LinkedListQueue->Head
                                        ...
                                        hfmac->hdmaPreload->LinkedListQueue->Head
                                            ...
                                            (uint32_t) &hfmac->Instance->WDATA; /* Set DMA
                                                                                   destination
                                                                                   address */
...
/**
 * @brief  Register the new input buffer, update DMA configuration if needed and change the FMAC
 * state.
 * @param  hfmac pointer to a FMAC_HandleTypeDef structure that contains
 *         the configuration information for FMAC module.
 * @param  pInput New input vector (additional input data).
 ...
 */
static HAL_StatusTypeDef
FMAC_AppendFilterDataUpdateState( FMAC_HandleTypeDef* hfmac, int16_t* pInput, uint16_t* pInputSize )
{
        HAL_StatusTypeDef status;
        /* Change the FMAC state */
...
/**
 * @brief  Register the new output buffer, update DMA configuration if needed and change the FMAC
 * state.
 * @param  hfmac pointer to a FMAC_HandleTypeDef structure that contains
 *         the configuration information for FMAC module.
 * @param  pOutput New output vector.
                ...
                if ( ( hfmac->hdmaPreload->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                        if ( ( hfmac->hdmaPreload->LinkedListQueue != NULL ) &&
                             ( hfmac->hdmaPreload->LinkedListQueue->Head != NULL ) ) {
                                ...
                                hfmac->hdmaPreload->LinkedListQueue->Head
                                    ->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] =
                                    (uint32_t) ( 2UL * hfmac->InputCurrentSize ); /* Set DMA data
                                                                                     size */
                                hfmac->hdmaPreload->LinkedListQueue->Head
                                    ->LinkRegisters[NODE_CSAR_DEFAULT_OFFSET] =
