 ...
 [..]
    ...
    *** Callback registration ***
    ...
    [..]
     ...
     reset to the legacy weak functions in the @ref HAL_OTFDEC_Init()/ @ref HAL_OTFDEC_DeInit() only
...
HAL_OTFDEC_RegionSetKey( OTFDEC_HandleTypeDef* hotfdec, uint32_t RegionIndex, uint32_t* pKey )
{
        ...
        if ( pKey == NULL ) {
                ...
                /* Compute theoretically expected CRC and compare it with that reported by the
                 * peripheral */
...
uint32_t HAL_OTFDEC_KeyCRCComputation( uint32_t const* pKey )
{
        ...
        uint32_t const  key_strobe[4] = { 0xAA55AA55U, 0x3U, 0x18U, 0xC0U };
        uint8_t         i;
        uint8_t         crc = 0;
        uint32_t        j;
        uint32_t        keyval;
        uint32_t        k;
        uint32_t const* temp = pKey;
...
/**
  ...
  * @note   output pointer points at a temporary area in RAM to store the ciphered data. It is up to
  the user code
...
{
        OTFDEC_Region_TypeDef const* region;
        uint32_t                     address;
        uint32_t                     keycrc;
 ...
 * @param  Config pointer on structure that will be filled up with the region configuration
 * parameters
 * @retval HAL state
 */
    ...
    OTFDEC_RegionConfigTypeDef* Config )
{
        ...
        if ( Config == NULL ) {
                ...
                /* Read Version */
                Config->Version =
                    (uint16_t) ( READ_REG( region->REG_CONFIGR ) & OTFDEC_REG_CONFIGR_VERSION ) >>
