 ...
 * @{
 */
static void TIM_OC1_SetConfig( TIM_TypeDef* TIMx, TIM_OC_InitTypeDef const* OC_Config );
static void TIM_OC3_SetConfig( TIM_TypeDef* TIMx, TIM_OC_InitTypeDef const* OC_Config );
static void TIM_OC4_SetConfig( TIM_TypeDef* TIMx, TIM_OC_InitTypeDef const* OC_Config );
static void TIM_OC5_SetConfig( TIM_TypeDef* TIMx, TIM_OC_InitTypeDef const* OC_Config );
static void TIM_OC6_SetConfig( TIM_TypeDef* TIMx, TIM_OC_InitTypeDef const* OC_Config );
static void
...
TIM_SlaveTimer_SetConfig( TIM_HandleTypeDef* htim, TIM_SlaveConfigTypeDef const* sSlaveConfig );
...
HAL_TIM_IC_Start_DMA( TIM_HandleTypeDef* htim, uint32_t Channel, uint32_t* pData, uint16_t Length )
{
        ...
        if ( ( channel_state == HAL_TIM_CHANNEL_STATE_BUSY ) ||
            ...
            ( channel_state == HAL_TIM_CHANNEL_STATE_READY ) &&
            ( complementary_channel_state == HAL_TIM_CHANNEL_STATE_READY ) ) {
                if ( ( pData == NULL ) || ( Length == 0U ) ) {
                        return HAL_ERROR;
                } else {
    ...
    uint16_t           Length )
{
        ...
        if ( Channel == TIM_CHANNEL_1 ) {
                ...
                if ( ( channel_2_state == HAL_TIM_CHANNEL_STATE_BUSY ) ||
                    ...
                    ( complementary_channel_2_state == HAL_TIM_CHANNEL_STATE_READY ) ) {
                        if ( ( pData2 == NULL ) || ( Length == 0U ) ) {
                                ...
                                TIM_CHANNEL_STATE_SET(
                                    htim, TIM_CHANNEL_2, HAL_TIM_CHANNEL_STATE_BUSY );
                                TIM_CHANNEL_N_STATE_SET(
                                    htim, TIM_CHANNEL_2, HAL_TIM_CHANNEL_STATE_BUSY );
                        }
                } else {
            ...
            ( channel_1_state == HAL_TIM_CHANNEL_STATE_BUSY ) ||
    ...
    uint32_t           BurstLength )
{
        ...
        uint32_t                 data_width;
        DMA_HandleTypeDef const* hdma = NULL;
    ...
    uint32_t                           Channel )
{
        ...
        case TIM_CLEARINPUTSOURCE_COMP2: {
                ...
                MODIFY_REG(
                    htim->Instance->AF2, TIMx_AF2_OCRSEL, sClearInputConfig->ClearInputSource );
                break;
        }
#endif /* COMP1 && COMP2 */
 ...
 */
HAL_StatusTypeDef
HAL_TIM_SlaveConfigSynchro_IT( TIM_HandleTypeDef* htim, TIM_SlaveConfigTypeDef const* sSlaveConfig )
{
        /* Check the parameters */
        assert_param( IS_TIM_SLAVE_INSTANCE( htim->Instance ) );
        assert_param( IS_TIM_SLAVE_MODE( sSlaveConfig->SlaveMode ) );
    ...
    uint32_t     ExtTRGFilter )
{
        ...
        tmpsmcr |=
            (uint32_t) ( TIM_ExtTRGPrescaler | ( TIM_ExtTRGPolarity | ( ExtTRGFilter << 8U ) ) );
...
void TIM_CCxChannelCmd( TIM_TypeDef* TIMx, uint32_t Channel, uint32_t ChannelState )
{
        ...
        TIMx->CCER |=
            (uint32_t) ( ChannelState << ( Channel & 0x1FU ) ); /* 0x1FU = 31 bits max shift */
}
