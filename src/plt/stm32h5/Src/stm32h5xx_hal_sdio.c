/**
  ...
  ******************************************************************************
  ...
  __HAL_RCC_GPIOx_CLK_ENABLE();
        ...
        (##) No general propose DMA Configuration is needed, an Internal DMA for SDMMC Peripheral
  are used.

    (#) At this stage, you can perform SDIO read/write/erase operations after SDIO card
  initialization.
  ...
  *** SDIO Card Initialization and configuration ***
  ...
  [..]
    ...
    SDMMC Peripheral(STM32 side) and the SDIO Card, and put it into StandBy State (Ready for data
  transfer). This function provide the following operations:
    ...
    (#) To custumize the SDIO Init card function for the enumeration card sequence, you can register
  a user callback function by calling the HAL_SDIO_RegisterIdentifyCardCallback before the
  HAL_SDIO_Init() function.
  ...
  *** SDIO card common control register (CCCR) ***
  ======================
  [..]
    (+) The SDIO CCCR allow for quick host checking and control of an IO card's enable and
  interrupts on a per card and per function basis. To get the Card common control registers field,
  you can use the API HAL_SDIO_GetCardCommonControlRegister().

  *** SDIO card Function basic register (FBR) ***
  ===========================
  [..]
    (+) The SDIO card function basic register are used to allow the host to quickly determine the
  abilities and requirements of each function.
  ...
  *** SDIO peripheral IO interrupt ***
  =============================================
  [..]
  (+) Below the list of most used SDIO function to check and control the IO card's enable and
  interrupts on a per functions basis.
...
static HAL_StatusTypeDef SDIO_ReadDirect(
    SDIO_HandleTypeDef* hsdio,
    uint32_t            addr,
    uint32_t            raw,
    uint32_t            function_nbr,
    uint8_t*            pData );
static HAL_StatusTypeDef SDIO_WriteDirect(
    SDIO_HandleTypeDef* hsdio,
    uint32_t            addr,
    uint32_t            raw,
    uint32_t            function_nbr,
    uint8_t const*      pData );
static HAL_StatusTypeDef SDIO_WriteExtended(
    SDIO_HandleTypeDef*           hsdio,
    HAL_SDIO_ExtendedCmd_TypeDef* cmd_arg,
    uint8_t*                      pData,
    uint16_t                      Size_byte );
static uint8_t SDIO_Convert_Block_Size( SDIO_HandleTypeDef const* hsdio, uint32_t block_size );
...
HAL_StatusTypeDef HAL_SDIO_Init( SDIO_HandleTypeDef* hsdio )
{
        ...
        /* wait 74 Cycles: required power up waiting time before starting the SDIO initialization
         * sequence */
        ...
        if ( SDIO_WriteDirect(
                 hsdio, SDMMC_SDIO_CCCR4_SD_BYTE3, HAL_SDIO_WRITE_ONLY, SDIO_FUNCTION_0, &data ) !=
             HAL_OK ) {
                return HAL_ERROR;
        }
 ...
 */
HAL_StatusTypeDef
HAL_SDIO_SetBlockSize( SDIO_HandleTypeDef* hsdio, uint8_t function_nbr, uint16_t BlockSize )
{
...
HAL_StatusTypeDef HAL_SDIO_CardReset( SDIO_HandleTypeDef* hsdio )
{
        ...
        /**  To reset the SDIO module by CMD52 with writing to RES in CCCR or send CMD0 the card
         * shall change the speed mode default speed mode. The reset cmd (cmd0) is only used for
         * memory. In order to reset an I/O card or the I/O portion of a combo card, Use CMD52 to
         * write 1 to the RES bit in the CCC(bit3 of register 6).
         */
        if ( SDIO_WriteDirect(
                 hsdio,
 ...
 */
HAL_StatusTypeDef
HAL_SDIO_GetCardCommonControlRegister( SDIO_HandleTypeDef* hsdio, HAL_SDIO_CCCR_TypeDef* pCccr )
{
...
@verbatim
  ...
  ==============================================================================
  [..]
    This subsection provides a set of functions allowing to manage the data transfer from/to SDIO
card.
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SDIO_ReadDirect(
    SDIO_HandleTypeDef*               hsdio,
    HAL_SDIO_DirectCmd_TypeDef const* Argument,
    uint8_t*                          pData )
{
        uint32_t cmd;
        uint32_t errorstate;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SDIO_WriteDirect(
    SDIO_HandleTypeDef*               hsdio,
    HAL_SDIO_DirectCmd_TypeDef const* Argument,
    uint8_t                           Data )
{
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SDIO_ReadExtended(
    SDIO_HandleTypeDef*                 hsdio,
    HAL_SDIO_ExtendedCmd_TypeDef const* Argument,
    uint8_t*                            pData,
    uint32_t                            Size_byte,
    uint32_t                            Timeout_Ms )
{
        ...
        if ( hsdio->State == HAL_SDIO_STATE_READY ) {
                ...
                while ( !__HAL_SDIO_GET_FLAG(
                    hsdio,
                    SDMMC_FLAG_RXOVERR | SDMMC_FLAG_DCRCFAIL | SDMMC_FLAG_DTIMEOUT |
                        SDMMC_FLAG_DATAEND ) ) {
                        if ( __HAL_SDIO_GET_FLAG( hsdio, SDMMC_FLAG_RXFIFOHF ) &&
                             ( dataremaining >= SDMMC_FIFO_SIZE ) ) {
                                /* Read data from SDMMC Rx FIFO */
                                for ( regCount = 0U; regCount < ( SDMMC_FIFO_SIZE / 4U );
                                      regCount++ ) {
    ...
    uint32_t                            Timeout_Ms )
{
        ...
        if ( hsdio->State == HAL_SDIO_STATE_READY ) {
                ...
                while ( !__HAL_SDIO_GET_FLAG(
                    hsdio,
                    SDMMC_FLAG_TXUNDERR | SDMMC_FLAG_DCRCFAIL | SDMMC_FLAG_DTIMEOUT |
                        SDMMC_FLAG_DATAEND ) ) {
                        ...
                        if ( __HAL_SDIO_GET_FLAG( hsdio, SDMMC_FLAG_TXFIFOHE ) &&
                            ...
                            ( __HAL_SDIO_GET_FLAG(
                                hsdio, SDMMC_FLAG_TXFIFOHE | SDMMC_FLAG_TXFIFOE ) ) &&
                            ( dataremaining < SDMMC_FIFO_SIZE ) ) {
                                uint8_t const* u8buff = (uint8_t*) u32tempbuff;
                                while ( dataremaining > 0U ) {
                                        data = 0U;
                                        for ( byteCount = 0U;
                                              ( byteCount < 4U ) && ( dataremaining > 0U );
                                              byteCount++ ) {
                                                data |=
                                                    ( (uint32_t) ( *u8buff )
                                                      << ( byteCount << 3U ) );
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SDIO_ReadExtended_DMA(
    SDIO_HandleTypeDef*                 hsdio,
    HAL_SDIO_ExtendedCmd_TypeDef const* Argument,
    uint8_t*                            pData,
    uint32_t                            Size_byte )
        ...
        if ( hsdio->State == HAL_SDIO_STATE_READY ) {
                ...
                if ( nbr_of_block != 0U ) {
                        hsdio->remaining_data =
                            ( Size_byte - ( hsdio->block_size * nbr_of_block ) );
                        hsdio->next_reg_addr =
                            ( Argument->Reg_Addr ) |
        ...
        if ( hsdio->State == HAL_SDIO_STATE_READY ) {
                ...
                if ( nbr_of_block != 0U ) {
                        ...
                        if ( hsdio->block_size <= 128U ) {
                                hsdio->next_reg_addr =
                                    ( Argument->Reg_Addr ) |
                                    ( ( ( ( nbr_of_block * hsdio->block_size ) >> 1U ) & 0x3FFFU )
                                      << 1U ) |
                                    ( ( hsdio->remaining_data <=
                                        HAL_SDIO_DATA_BLOCK_SIZE_512BYTE ) ?
                                          1U :
...
@verbatim
  ...
  ==============================================================================
  [..]
    This subsection provides a set callback functions allowing to manage the data transfer from/to
SDIO card.
...
void HAL_SDIO_IRQHandler( SDIO_HandleTypeDef* hsdio )
{
        ...
        if ( READ_BIT( flags, SDMMC_FLAG_DATAEND ) != 0U ) {
                ...
                if ( hsdio->remaining_data != 0U ) {
                    ...
                    ( ( ctx & SDIO_CONTEXT_READ_SINGLE_BLOCK ) != 0U ) ||
                    ( ( ctx & SDIO_CONTEXT_READ_MULTIPLE_BLOCK ) != 0U ) ) {
#if defined( USE_HAL_SDIO_REGISTER_CALLBACKS ) && ( USE_HAL_SDIO_REGISTER_CALLBACKS == 1U )
 ...
 * @retval status
 */
HAL_StatusTypeDef HAL_SDIO_RegisterCallback(
    SDIO_HandleTypeDef*        hsdio,
 ...
 */
HAL_StatusTypeDef
HAL_SDIO_UnRegisterCallback( SDIO_HandleTypeDef* hsdio, HAL_SDIO_CallbackIDTypeDef CallbackID )
{
        HAL_StatusTypeDef status = HAL_OK;
 ...
 * @param  hsdio : Pointer to a SDIO_HandleTypeDef structure that contains the configuration
 * information.
...
@verbatim
  ...
  ==============================================================================
  [..]
    This subsection provides a set functions allowing to enable/disable IO functions interrupt
features on the SDIO card.
 ...
 */
HAL_StatusTypeDef
HAL_SDIO_EnableIOFunctionInterrupt( SDIO_HandleTypeDef* hsdio, uint32_t IOFunction )
{
        uint8_t intEn = 0U;
        ...
        if ( SDIO_ReadDirect(
                 hsdio, SDMMC_SDIO_CCCR4, HAL_SDIO_WRITE_ONLY, SDIO_FUNCTION_0, &intEn ) !=
             HAL_OK ) {
                return HAL_ERROR;
        }
 ...
 */
HAL_StatusTypeDef
HAL_SDIO_DisableIOFunctionInterrupt( SDIO_HandleTypeDef* hsdio, uint32_t IOFunction )
{
        uint8_t intEn = 0U;
        ...
        if ( SDIO_WriteDirect(
                 hsdio, SDMMC_SDIO_CCCR4, HAL_SDIO_READ_AFTER_WRITE, SDIO_FUNCTION_0, &intEn ) !=
             HAL_OK ) {
                return HAL_ERROR;
        }
...
HAL_StatusTypeDef HAL_SDIO_EnableIOFunction( SDIO_HandleTypeDef* hsdio, uint32_t IOFunction )
{
        ...
        if ( SDIO_ReadDirect(
                 hsdio, SDMMC_SDIO_CCCR0_SD_BYTE2, HAL_SDIO_WRITE_ONLY, SDIO_FUNCTION_0, &ioEn ) !=
             HAL_OK ) {
                return HAL_ERROR;
        }
        ...
        if ( SDIO_WriteDirect(
                 hsdio,
                 SDMMC_SDIO_CCCR0_SD_BYTE2,
                 HAL_SDIO_READ_AFTER_WRITE,
                 SDIO_FUNCTION_0,
                 &ioEn ) != HAL_OK ) {
                return HAL_ERROR;
        }
...
static HAL_StatusTypeDef SDIO_InitCard( SDIO_HandleTypeDef* hsdio )
{
        ...
        if ( nbr_of_func > 0U ) {
                ...
                if ( SDMMC_CmdSendOperationcondition(
                         hsdio->Instance, ( SDIO_OCR_VDD_32_33 | SDIO_OCR_SDIO_S18R ), &Resp4 ) !=
                     HAL_SDIO_ERROR_NONE ) {
                        return HAL_ERROR;
                }
        ...
        /** Cmd3 is sent while response is SDMMC_ERROR_ILLEGAL_CMD, due to the partial init test
         * done before (sending cmd0 after the sequence cmd0/cmd3 is sent is considered illegal).
         */
        do {
        ...
        errorstate =
            SDMMC_CmdSelDesel( hsdio->Instance, (uint32_t) ( ( (uint32_t) sdio_rca ) << 16U ) );
        if ( errorstate != HAL_SDIO_ERROR_NONE )
                return HAL_ERROR;
    ...
    uint16_t                      Size_byte )
{
        ...
        /* Compute how many blocks are to be send for pData of length data_size to be send */
        nbr_of_block = ( ( (uint32_t) Size_byte & ~( (uint32_t) hsdio->block_size & 1U ) ) ) >>
                       __CLZ( __RBIT( hsdio->block_size ) );
...
/**
 * @brief  Allows to convert a block size in the according SDMMC value for configuring the SDMMC
 * when doing a CMD53
 * @param  hsdio: Pointer to the SDIO handle.
                        ...
                        for ( count = 1; count <= SDIO_MAX_IO_NUMBER; count++ ) {
                                if ( ( ( pendingInt & ( 1U << count ) ) != 0U ) &&
                                     ( ( ( 1U << count ) & hsdio->IOFunctionMask ) != 0U ) ) {
                                        if ( ( hsdio->SDIO_IOFunction_Callback[count - 1U] ) !=
                                             NULL ) {
                                                ( hsdio->SDIO_IOFunction_Callback[count - 1U] )(
                                                    hsdio, count );
                                        }
                                }
                        }
