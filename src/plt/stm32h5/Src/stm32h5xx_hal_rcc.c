...
/** @defgroup RCC_Exported_Functions_Group1 Initialization and de-initialization functions
 ...
 ===============================================================================
    [..]
      ...
      (HSE, HSI, LSE, CSI, LSI, PLL1, HSE CSS and MCOs) and the System busses clocks (SYSCLK, AHB,
 APB1, APB2 and APB3).
 ...
 source.
         ...
         (+) PLL1 (clocked by HSI, HSE or CSI) providing up to three independent output clocks:
           ...
           (++) The second output is used to generate the clock for the USB (48 MHz), the FDCAN1/2,
                the SPI1/2/3, the OCTOSPI, the RNG (<=48 MHz), the SDMMC1/2 and to generate an
 accurate clock to achieve high-quality audio performance on SAI1/2 interface.

         (+) PLL2 (clocked by HSI, HSE or CSI) providing up to three independent output clocks:
           (++) The first output is used to generate the clock for the LPTIMs, the SPI1/2/3 and to
 generate an accurate clock to achieve high-quality audio performance on SAI1/2 interface.
           (++) The second output is used to generate the clock for USARTs, the UARTs, the LPUART1,
                the FDCAN1/2, the SPI4/5/6 and the USB.
           (++) The third output is used to generate the clock the SDMMC1/2, the ADC/DAC, the
 I2C1/2, the I3C1/2 and the OCTOSPI.

         (+) PLL3 (clocked by HSI , HSE or CSI) providing up to three independent output clocks:
           (++) The first output is used to generate the clock for SPI1/2/3 and to generate an
 accurate clock to achieve high-quality audio performance on SAI1/2 interface.
           (++) The second  output is used to generate the clock for USARTs, the UARTs, the LPUART1,
                the SPI4/5/6 and the USB.
           (++) The third output is used to generate the clock for the I2Cs, the I3Cs and the
 LPTIMs.
    ...
    [..] System, AHB and APB busses clocks configuration
         (+) Several clock sources can be used to drive the System clock (SYSCLK): CSI, HSI, HSE and
 the main PLL. The AHB clock (HCLK) is derived from System clock through configurable prescaler and
 used to clock the CPU, memory and peripherals mapped on AHB bus (DMA, GPIO...). APB1 (PCLK1), APB2
 (PCLK2) and APB3 (PCLK3) clocks are derived from AHB clock through configurable prescalers and used
 to clock the peripherals mapped on these busses. You can use "HAL_RCC_GetSysClockFreq()" function
 to retrieve the frequencies of these clocks.
         ...
         -@- All the peripheral clocks are derived from the System clock (SYSCLK) except:
           ...
           (+@) SAI: the SAI clock can be derived either from specific PLL (PLL1, PLL2 or PLL3),
                the per_ck clock (HSE, HSI or CSI) or from an external clock mapped on the SAI_CKIN
 pin. You have to use HAL_RCCEx_PeriphCLKConfig() function to configure this clock.
           (+@) SPI/I2S: the SPI1/2/3 clock can be derived either from specific PLL (PLL1, PLL2 or
 PLL3), the per_ck clock (HSE, HSI or CSI) or from an external clock mapped on the SPI_CKIN pin. You
 have to use HAL_RCCEx_PeriphCLKConfig() function to configure this clock.
           (+@) RTC: the RTC clock can be derived either from the LSI, LSE or HSE clock
                divided by 2 to 31.
                You have to use __HAL_RCC_RTC_ENABLE() and HAL_RCCEx_PeriphCLKConfig() function
                to configure this clock.
           (+@) USB: USB requires a frequency equal to 48 MHz to work correctly. This clock is
 derived of the main PLL or PLL2 through PLLQ divider. You have to use HAL_RCCEx_PeriphCLKConfig()
 ...
 HAL_RCCEx_PeriphCLKConfig() function to configure this clock.
           (+@) RNG: the RNG clock can be derived either from PLL1Q, HSI48, LSE or LSI clock. You
 have to use HAL_RCCEx_PeriphCLKConfig() function to configure this clock.
           (+@) DAC: the DAC clock can be derived either from LSE or LSI clock. You have
                to use HAL_RCCEx_PeriphCLKConfig() function to configure this clock.
           (+@) FDCAN: the FDCAN1/2 clock can be derived either from HSE, PLL1Q or PLL2Q clock. You
 have to use HAL_RCCEx_PeriphCLKConfig() function to configure this clock.
           (+@) CEC: the CEC clock can be derived either from LSE, LSI or CSI (divided by 122)
 clock.You have to use HAL_RCCEx_PeriphCLKConfig() function to configure this clock.
 ...
 |-----------------------------------------------------------------------------| |                 |
 voltage range 0  |  voltage range 1 | voltage range 2  | voltage range 3   | | |    1.26 - 1.35V |
 1.15 - 1.26V   |   1.05 - 1.15V   |   0,95 - 1,05V    |
           |-----------------|-------------------|------------------|------------------|-------------------|
           |0WS(1 CPU cycles)|   0 < HCLK <= 38  |  0 < HCLK <= 32  |  0 < HCLK <= 26  | 0 < HCLK <=
 16    |
           |-----------------|-------------------|------------------|------------------|-------------------|
           |1WS(2 CPU cycles)|  38 < HCLK <= 76  | 32 < HCLK <= 64  | 26 < HCLK <= 50  | 16 < HCLK
 <= 32   |
           |-----------------|-------------------|------------------|------------------|-------------------|
           |2WS(3 CPU cycles)|  76 < HCLK <= 114 | 64 < HCLK <= 96  | 50 < HCLK <= 80  | 32 < HCLK
 <= 50   |
           |-----------------|-------------------|------------------|------------------|-------------------|
           |3WS(4 CPU cycles)| 114 < HCLK <= 152 | 96 < HCLK <= 128 | 80 < HCLK <= 106 | 50 < HCLK
 <= 65   |
           |-----------------|-------------------|------------------|------------------|-------------------|
           |4WS(5 CPU cycles)|  152 < HCLK <= 190| 128 < HCLK <= 160| 106 < HCLK <= 130| 65 < HCLK
 <= 80   |
           |-----------------|-------------------|------------------|------------------|-------------------|
...
HAL_StatusTypeDef HAL_RCC_DeInit( void )
{
        ...
        CLEAR_BIT(
            RCC->CR,
            RCC_CR_CSION | RCC_CR_CSIKERON | RCC_CR_HSECSSON | RCC_CR_HSIKERON | RCC_CR_HSI48ON |
                RCC_CR_HSEON );
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_RCC_OscConfig( RCC_OscInitTypeDef const* pOscInitStruct )
{
        uint32_t tickstart;
                     ...
                     ( ( temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_PLLCLK ) &&
                       ( temp_pllckselr == RCC_PLL1_SOURCE_CSI ) ) ) {
                        ...
                        /* Otherwise, just the calibration and CSI is allowed */
                                ...
                                /* Adjusts the Internal Low-power oscillator (CSI) calibration
                                 * value.*/
                     ...
                     ( ( temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_PLLCLK ) &&
                       ( temp_pllckselr == RCC_PLL1_SOURCE_HSI ) ) ) {
                                ...
                                if ( temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_HSI ) {
                                        if ( __HAL_RCC_GET_HSI_DIVIDER() !=
                                             ( pOscInitStruct->HSIDiv ) ) {
                                                ...
                                                __HAL_RCC_HSI_DIVIDER_CONFIG(
                                                    pOscInitStruct->HSIDiv );

                                                /* Update the SystemCoreClock global variable with
                                                 * new HSI value  */
                                                (void) HAL_RCC_GetHCLKFreq();

                                                /* Configure the source of time base considering new
                        ...
                        if ( pOscInitStruct->HSIState != RCC_HSI_OFF ) {
                                ...
                                /* Adjust the Internal High Speed oscillator (HSI) calibration
                                 * value.*/
                                __HAL_RCC_HSI_CALIBRATIONVALUE_ADJUST(
                                    pOscInitStruct->HSICalibrationValue );
                        } else {
        ...
        if ( ( pOscInitStruct->PLL.PLLState ) != RCC_PLL_NONE ) {
                ...
                if ( temp_sysclksrc != RCC_SYSCLKSOURCE_STATUS_PLLCLK ) {
                        if ( ( pOscInitStruct->PLL.PLLState ) == RCC_PLL_ON ) {
                                ...
                                /* Configure the PLL1 clock source, multiplication and division
                                 * factors. */
                                ...
                                /* Unselect PLL1 clock source and disable all PLL1 outputs to save
                                 * power */
                                RCC->PLL1CFGR &=
                                    ~( RCC_PLL1CFGR_PLL1SRC | RCC_PLL1CFGR_PLL1PEN |
                        ...
                        if ( ( READ_BIT( RCC->PLL1FRACR, RCC_PLL1FRACR_PLL1FRACN ) >>
                               RCC_PLL1FRACR_PLL1FRACN_Pos ) != ( pOscInitStruct->PLL.PLLFRACN ) ) {
                                ...
                                /* Wait at least 2 CK_REF (PLL input source divided by M) period to
                                   make sure next latched value will be taken into account. */
 ...
 * @retval HAL Status.
 */
HAL_StatusTypeDef HAL_RCC_ClockConfig( RCC_ClkInitTypeDef const* pClkInitStruct, uint32_t FLatency )
{
        ...
        if ( ( ( pClkInitStruct->ClockType ) & RCC_CLOCKTYPE_SYSCLK ) == RCC_CLOCKTYPE_SYSCLK ) {
                ...
                if ( pClkInitStruct->SYSCLKSource == RCC_SYSCLKSOURCE_PLLCLK ) {
                        while ( __HAL_RCC_GET_SYSCLK_SOURCE() != RCC_SYSCLKSOURCE_STATUS_PLLCLK )
                                if ( ( HAL_GetTick() - tickstart ) > RCC_CLOCKSWITCH_TIMEOUT_VALUE )
                                        return HAL_TIMEOUT;
                } else if ( pClkInitStruct->SYSCLKSource == RCC_SYSCLKSOURCE_HSE ) {
...
void HAL_RCC_MCOConfig( uint32_t RCC_MCOx, uint32_t RCC_MCOSource, uint32_t RCC_MCODiv )
{
        ...
        if ( RCC_MCOx == RCC_MCO1 ) {
                ...
                MODIFY_REG(
                    RCC->CFGR1,
                    ( RCC_CFGR1_MCO1SEL | RCC_CFGR1_MCO1PRE ),
                ...
                MODIFY_REG(
                    RCC->CFGR1,
                    ( RCC_CFGR1_MCO2SEL | RCC_CFGR1_MCO2PRE ),
...
uint32_t HAL_RCC_GetSysClockFreq( void )
{
        ...
        if ( __HAL_RCC_GET_SYSCLK_SOURCE() == RCC_SYSCLKSOURCE_STATUS_CSI ) {
                ...
                */
                ...
                pllfracen =
                    ( ( RCC->PLL1CFGR & RCC_PLL1CFGR_PLL1FRACEN ) >> RCC_PLL1CFGR_PLL1FRACEN_Pos );
                ...
                if ( pllm != 0U ) {
                        ...
                        case RCC_PLL1_SOURCE_HSI: /* HSI used as PLL1 clock source */
                                ...
                                if ( __HAL_RCC_GET_FLAG( RCC_FLAG_HSIDIVF ) != 0U ) {
                                        hsivalue =
                                            ( HSI_VALUE >> ( __HAL_RCC_GET_HSI_DIVIDER() >>
                                                             RCC_CR_HSIDIV_Pos ) );
...
uint32_t HAL_RCC_GetHCLKFreq( void )
{
        ...
        SystemCoreClock =
            ...
            ( AHBPrescTable[( RCC->CFGR2 & RCC_CFGR2_HPRE ) >> RCC_CFGR2_HPRE_Pos] & 0x1FU );
...
void HAL_RCC_GetOscConfig( RCC_OscInitTypeDef* pOscInitStruct )
{
        ...
        pOscInitStruct->OscillatorType = RCC_OSCILLATORTYPE_HSE | RCC_OSCILLATORTYPE_HSI |
        ...
        pOscInitStruct->HSICalibrationValue =
            (uint32_t) ( READ_BIT( RCC->HSICFGR, RCC_HSICFGR_HSITRIM ) >> RCC_HSICFGR_HSITRIM_Pos );
        ...
        pOscInitStruct->PLL.PLLN =
            (uint32_t) ( ( ( reg2val & RCC_PLL1DIVR_PLL1N ) >> RCC_PLL1DIVR_PLL1N_Pos ) + 1U );
        pOscInitStruct->PLL.PLLQ =
            (uint32_t) ( ( ( reg2val & RCC_PLL1DIVR_PLL1Q ) >> RCC_PLL1DIVR_PLL1Q_Pos ) + 1U );
        pOscInitStruct->PLL.PLLR =
            (uint32_t) ( ( ( reg2val & RCC_PLL1DIVR_PLL1R ) >> RCC_PLL1DIVR_PLL1R_Pos ) + 1U );
        pOscInitStruct->PLL.PLLP =
            (uint32_t) ( ( ( reg2val & RCC_PLL1DIVR_PLL1P ) >> RCC_PLL1DIVR_PLL1P_Pos ) + 1U );
        pOscInitStruct->PLL.PLLRGE = (uint32_t) ( ( reg1val & RCC_PLL1CFGR_PLL1RGE ) );
        pOscInitStruct->PLL.PLLVCOSEL =
            (uint32_t) ( ( reg1val & RCC_PLL1CFGR_PLL1VCOSEL ) >> RCC_PLL1CFGR_PLL1VCOSEL_Pos );
        pOscInitStruct->PLL.PLLFRACN = (uint32_t) ( (
 ...
 * (**)  : For stm32h503xx devices, this parameter is unused, it can take 0 or any other numerical
 * value.
 */
 ...
 * @retval HAL Status.
 *
 * (*)  : This parameter is unused for stm32h503xx devices, it can take 0 or any other numerical
 * value.
 */
HAL_StatusTypeDef HAL_RCC_GetConfigAttributes( uint32_t Item, uint32_t* pAttributes )
{
        ...
        if ( ( RCC->SECCFGR & Item ) == Item ) {
                ...
                attributes =
                    ( ( RCC->PRIVCFGR & RCC_PRIVCFGR_SPRIV ) == 0U ) ? RCC_SEC_NPRIV : RCC_SEC_PRIV;
        } else {
...
#else  /* __ARM_FEATURE_CMSE */
        attributes =
            ( ( RCC->PRIVCFGR & RCC_PRIVCFGR_NSPRIV ) == 0U ) ? RCC_NSEC_NPRIV : RCC_NSEC_PRIV;
#endif /* __ARM_FEATURE_CMSE */
