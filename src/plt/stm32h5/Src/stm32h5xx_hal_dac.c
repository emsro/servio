...
length of data to be transferred at each end of conversion First issued trigger will start the
conversion of the value previously set by HAL_DAC_SetValue().
...
HAL_StatusTypeDef HAL_DAC_Start( DAC_HandleTypeDef* hdac, uint32_t Channel )
{
        ...
        /* Note: Variable divided by 2 to compensate partially CPU processing cycles, scaling in us
         * split to not exceed 32 */
        /*       bits register capacity and handle low frequency. */
        wait_loop_index =
            ( ( DAC_DELAY_STARTUP_US / 10UL ) *
        ...
        if ( Channel == DAC_CHANNEL_1 ) {
                ...
                if ( ( hdac->DMA_Handle1->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                        ...
                        if ( hdac->DMA_Handle1->Init.SrcDataWidth == DMA_SRC_DATAWIDTH_WORD ) {
                            ...
                            hdac->DMA_Handle1->Init.SrcDataWidth == DMA_SRC_DATAWIDTH_HALFWORD ) {
                                /* Halfword -> Bytes */
                                LengthInBytes = Length * 2U;
                        } else /* Bytes */
                        {
                        ...
                        status = HAL_DMA_Start_IT(
                            hdac->DMA_Handle1, (uint32_t) pData, tmpreg, LengthInBytes );
                }
        }
                ...
                if ( ( hdac->DMA_Handle2->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                        ...
                        if ( hdac->DMA_Handle2->Init.SrcDataWidth == DMA_SRC_DATAWIDTH_WORD ) {
                            ...
                            hdac->DMA_Handle2->Init.SrcDataWidth == DMA_SRC_DATAWIDTH_HALFWORD ) {
                                /* Halfword -> Bytes */
                                LengthInBytes = Length * 2U;
                        } else /* Bytes */
                        {
                        ...
                        /* Enable the DMA channel */
                        status = HAL_DMA_Start_IT(
                            hdac->DMA_Handle2, (uint32_t) pData, tmpreg, LengthInBytes );
                }
        }
...
HAL_DAC_SetValue( DAC_HandleTypeDef* hdac, uint32_t Channel, uint32_t Alignment, uint32_t Data )
{
        ...
        assert_param( IS_DAC_CHANNEL( Channel ) );
        assert_param( IS_DAC_ALIGN( Alignment ) );
        /* In case DMA Double data mode is activated, DATA range is almost full uin32_t one: no
         * check */
 ...
 */
uint32_t HAL_DAC_GetValue( DAC_HandleTypeDef const* hdac, uint32_t Channel )
{
        uint32_t result;
...
HAL_StatusTypeDef HAL_DAC_ConfigChannel(
    DAC_HandleTypeDef*            hdac,
    DAC_ChannelConfTypeDef const* sConfig,
    uint32_t                      Channel )
{
        ...
        if ( ( sConfig->DAC_SampleAndHold ) == DAC_SAMPLEANDHOLD_ENABLE ) {
                ...
                /* In case of mode normal and buffer disabled, connection to both on chip periph and
                 * external pin is not possible */
...
#if !defined( TIM8 )
        /* Devices STM32H503xx */
        /* On STM32H503EB (package WLCSP25) DAC channel 1 connection to GPIO is not available and
           should not be configured. Package information is stored at the address PACKAGE_BASE,
           WLCSP25 correspond to the value 0xF (For more information, please refer to the Reference
           Manual) */
 ...
 * @retval HAL state
 */
HAL_DAC_StateTypeDef HAL_DAC_GetState( DAC_HandleTypeDef const* hdac )
{
 ...
 */
uint32_t HAL_DAC_GetError( DAC_HandleTypeDef const* hdac )
{
 ...
 * @note   The HAL_DAC_RegisterCallback() may be called before HAL_DAC_Init() in HAL_DAC_STATE_RESET
 * to register callbacks for HAL_DAC_MSPINIT_CB_ID and HAL_DAC_MSPDEINIT_CB_ID
 ...
 * @note   The HAL_DAC_UnRegisterCallback() may be called before HAL_DAC_Init() in
 * HAL_DAC_STATE_RESET to un-register callbacks for HAL_DAC_MSPINIT_CB_ID and
