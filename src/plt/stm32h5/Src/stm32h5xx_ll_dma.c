...
uint32_t LL_DMA_Init( DMA_TypeDef* DMAx, uint32_t Channel, LL_DMA_InitTypeDef* DMA_InitStruct )
{
        ...
        if ( IS_LL_DMA_2D_CHANNEL_INSTANCE( DMAx, Channel ) != 0U ) {
                ...
                assert_param(
                    IS_LL_DMA_BLKRPT_SRC_ADDR_UPDATE( DMA_InitStruct->BlkRptSrcAddrUpdateMode ) );
                assert_param(
                    IS_LL_DMA_BLKRPT_DEST_ADDR_UPDATE( DMA_InitStruct->BlkRptDestAddrUpdateMode ) );
                assert_param(
                    IS_LL_DMA_BLKRPT_ADDR_UPDATE_VALUE( DMA_InitStruct->BlkRptSrcAddrOffset ) );
                assert_param(
                    IS_LL_DMA_BLKRPT_ADDR_UPDATE_VALUE( DMA_InitStruct->BlkRptDestAddrOffset ) );
        }
        ...
        LL_DMA_ConfigControl(
            DMAx,
            Channel,
            DMA_InitStruct->Priority | DMA_InitStruct->LinkAllocatedPort |
        ...
        LL_DMA_ConfigChannelTransfer(
            ...
            DMA_InitStruct->TransferEventMode | DMA_InitStruct->TriggerPolarity |
                DMA_InitStruct->BlkHWRequest | DMA_InitStruct->Mode | DMA_InitStruct->Direction );
        ...
        LL_DMA_ConfigAddresses(
            DMAx, Channel, DMA_InitStruct->SrcAddress, DMA_InitStruct->DestAddress );
        ...
        if ( IS_LL_DMA_2D_CHANNEL_INSTANCE( DMAx, Channel ) != 0U ) {
                ...
                LL_DMA_ConfigAddrUpdateValue(
                    DMAx, Channel, DMA_InitStruct->SrcAddrOffset, DMA_InitStruct->DestAddrOffset );
    ...
    LL_DMA_InitLinkedListTypeDef* DMA_InitLinkedListStruct )
{
        ...
        assert_param(
            IS_LL_DMA_TRANSFER_EVENT_MODE( DMA_InitLinkedListStruct->TransferEventMode ) );
        /* Check DMA instance */
        if ( IS_LL_GPDMA_CHANNEL_INSTANCE( DMAx, Channel ) != 0U ) {
                assert_param(
                    IS_LL_DMA_LINK_ALLOCATED_PORT( DMA_InitLinkedListStruct->LinkAllocatedPort ) );
        }
    ...
    LL_DMA_LinkNodeTypeDef*       pNode )
{
...

        {
                ...
                assert_param(
                    IS_LL_DMA_DEST_HALFWORD_EXCHANGE( DMA_InitNodeStruct->DestHWordExchange ) );
                assert_param(
        ...
        if ( DMA_InitNodeStruct->NodeType == LL_DMA_GPDMA_2D_NODE ) {
                ...
                assert_param(
                    IS_LL_DMA_BURST_DEST_ADDR_UPDATE( DMA_InitNodeStruct->DestAddrUpdateMode ) );
                assert_param(
                ...
                assert_param(
                    IS_LL_DMA_BLKRPT_ADDR_UPDATE_VALUE( DMA_InitNodeStruct->BlkRptSrcAddrOffset ) );
        ...
        if ( ( DMA_InitNodeStruct->UpdateRegisters & LL_DMA_UPDATE_CTR1 ) == LL_DMA_UPDATE_CTR1 ) {
                ...
                pNode->LinkRegisters[reg_counter] |=
                    ( DMA_InitNodeStruct->DestAllocatedPort |
                      DMA_InitNodeStruct->DestHWordExchange | DMA_InitNodeStruct->DestByteExchange |
 ...
 * @param  pPrevLinkNode Pointer to previous linked list node to be connected to new Linked list
 * node.
