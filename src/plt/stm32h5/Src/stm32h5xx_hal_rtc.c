                ...
                if ( __HAL_RTC_IS_CALENDAR_INITIALIZED( hrtc ) == 0U ) {
                        ...
                        if ( __HAL_RTC_GET_BINARY_MODE( hrtc ) != RTC_BINARY_ONLY ) {
                                ...
                                if ( status == HAL_OK ) {
                                        ...
                                        CLEAR_BIT(
                                            RTC->CR,
                                            ( RTC_CR_FMT | RTC_CR_POL | RTC_CR_OSEL |
                                              RTC_CR_TAMPOE ) );

                                        /* Set RTC_CR register */
                                        SET_BIT(
                                            RTC->CR,
                                            ( hrtc->Init.HourFormat | hrtc->Init.OutPut |
                                ...
                                /* The calendar does not need to be initialized as the 'binary only'
                                 * mode is selected */
                                status = HAL_OK;
                        }
                } else {
...
HAL_StatusTypeDef HAL_RTC_DeInit( RTC_HandleTypeDef* hrtc )
{
        ...
        if ( status == HAL_OK ) {
                ...
                WRITE_REG(
                    RTC->SCR,
                    RTC_SCR_CITSF | RTC_SCR_CTSOVF | RTC_SCR_CTSF | RTC_SCR_CWUTF | RTC_SCR_CALRBF |
                        RTC_SCR_CALRAF );
 ...
 *          @arg @ref HAL_RTC_MSPINIT_CB_ID                    Msp Init callback ID
 *          @arg @ref HAL_RTC_MSPDEINIT_CB_ID                  Msp DeInit callback ID
 * @note  The HAL_RTC_UnRegisterCallback() may be called before HAL_RTC_Init() in
 * HAL_RTC_STATE_RESET to un-register callbacks for HAL_RTC_MSPINIT_CB_ID and
 ...
 *          if Binary mode is RTC_BINARY_ONLY, this parameter is not used and RTC_SSR will be
 * automatically reset to 0xFFFFFFFF else sTime->SubSeconds is not used and RTC_SSR will be
 * automatically reset to the A 7-bit async prescaler (RTC_PRER_PREDIV_A)
...
HAL_RTC_SetTime( RTC_HandleTypeDef* hrtc, RTC_TimeTypeDef* sTime, uint32_t Format )
{
        ...
        /* Check the parameters depending of the Binary mode with 32-bit free-running counter
         * configuration */
        ...
        if ( status == HAL_OK ) {
                ...
                if ( READ_BIT( RTC->ICSR, RTC_ICSR_BIN ) != RTC_BINARY_ONLY ) {
                        if ( Format == RTC_FORMAT_BIN ) {
                                ...
                                if ( READ_BIT( RTC->CR, RTC_CR_FMT ) != 0U ) {
                                        assert_param(
                                            IS_RTC_HOUR12( RTC_Bcd2ToByte( sTime->Hours ) ) );
                                        assert_param( IS_RTC_HOURFORMAT12( sTime->TimeFormat ) );
                                } else {
                                        sTime->TimeFormat = 0x00U;
                                        assert_param(
                                            IS_RTC_HOUR24( RTC_Bcd2ToByte( sTime->Hours ) ) );
                                }
                                assert_param( IS_RTC_MINUTES( RTC_Bcd2ToByte( sTime->Minutes ) ) );
                                assert_param( IS_RTC_SECONDS( RTC_Bcd2ToByte( sTime->Seconds ) ) );
                                tmpreg =
                                    ( ( (uint32_t) ( sTime->Hours ) << RTC_TR_HU_Pos ) |
...
HAL_RTC_SetDate( RTC_HandleTypeDef* hrtc, RTC_DateTypeDef* sDate, uint32_t Format )
{
        ...
        if ( Format == RTC_FORMAT_BIN ) {
                ...
                datetmpreg =
                    ( ( (uint32_t) RTC_ByteToBcd2( sDate->Year ) << RTC_DR_YU_Pos ) |
                ...
                datetmpreg =
                    ( ( ( (uint32_t) sDate->Year ) << RTC_DR_YU_Pos ) |
...
HAL_RTC_SetAlarm( RTC_HandleTypeDef* hrtc, RTC_AlarmTypeDef* sAlarm, uint32_t Format )
{
...
#ifdef USE_FULL_ASSERT
        /* Check the parameters depending of the Binary mode (32-bit free-running counter
         * configuration) */
        if ( READ_BIT( RTC->ICSR, RTC_ICSR_BIN ) == RTC_BINARY_NONE ) {
                assert_param( IS_RTC_FORMAT( Format ) );
        ...
        if ( binary_mode != RTC_BINARY_ONLY ) {
                ...
                } else /* format BCD */
                {
                        ...
                        assert_param(
                            IS_RTC_MINUTES( RTC_Bcd2ToByte( sAlarm->AlarmTime.Minutes ) ) );
                        assert_param(
                            IS_RTC_SECONDS( RTC_Bcd2ToByte( sAlarm->AlarmTime.Seconds ) ) );
        ...
        if ( sAlarm->Alarm == RTC_ALARM_A ) {
                ...
                if ( binary_mode == RTC_BINARY_ONLY ) {
                        WRITE_REG(
                            RTC->ALRMBSSR, sAlarm->AlarmSubSecondMask | sAlarm->BinaryAutoClr );
                } else {
