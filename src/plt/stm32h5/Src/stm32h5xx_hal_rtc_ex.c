 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_RTCEx_PollForTimeStampEvent( RTC_HandleTypeDef const* hrtc, uint32_t Timeout )
{
        /* Prevent unused argument(s) compilation warning */
        UNUSED( hrtc );
 ...
 * software execution after Wake Up.
 * @retval HAL status
 */
    ...
    uint32_t           WakeUpAutoClr )
{
        ...
        WRITE_REG(
            RTC->WUTR, (uint32_t) ( WakeUpCounter | ( WakeUpAutoClr << RTC_WUTR_WUTOCLR_Pos ) ) );
...
HAL_RTCEx_SetTamper( RTC_HandleTypeDef const* hrtc, RTC_TamperTypeDef const* sTamper )
{
        ...
        WRITE_REG(
            TAMP->FLTCR,
            sTamper->Filter | sTamper->SamplingFrequency | sTamper->PrechargeDuration |
...
HAL_StatusTypeDef HAL_RTCEx_DeactivateTamper( RTC_HandleTypeDef const* hrtc, uint32_t Tamper )
{
        ...
        CLEAR_BIT(
            TAMP->CR2,
            ( Tamper << TAMP_CR2_TAMP1TRG_Pos ) | ( Tamper << TAMP_CR2_TAMP1MSK_Pos ) |
 ...
 * @note   For interrupt mode, the application must ensure that the EXTI TAMP interrupt line is
 * enabled.
 * @param  hrtc RTC handle
...
HAL_RTCEx_SecureModeGet( RTC_HandleTypeDef const* hrtc, RTC_SecureStateTypeDef* secureState )
{
        ...
        secureState->rtcNonSecureFeatures =
            ~( READ_BIT( rtc_seccfgr, RTC_NONSECURE_FEATURE_ALL ) ) & RTC_NONSECURE_FEATURE_ALL;
...
HAL_RTCEx_SecureModeSet( RTC_HandleTypeDef const* hrtc, RTC_SecureStateTypeDef const* secureState )
{
        ...
        WRITE_REG(
            RTC->SECCFGR,
            secureState->rtcSecureFull |
                ( ~( secureState->rtcNonSecureFeatures ) & RTC_NONSECURE_FEATURE_ALL ) );
        ...
        WRITE_REG(
            ...
            secureState->tampSecureFull | secureState->MonotonicCounterSecure |
                ( TAMP_SECCFGR_BKPRWSEC &
                  ( secureState->backupRegisterStartZone2 << TAMP_SECCFGR_BKPRWSEC_Pos ) ) |
                ( TAMP_SECCFGR_BKPWSEC &
                  ( secureState->backupRegisterStartZone3 << TAMP_SECCFGR_BKPWSEC_Pos ) ) );
    ...
    RTC_PrivilegeStateTypeDef const* privilegeState )
{
        ...
        WRITE_REG(
            RTC->PRIVCFGR,
            privilegeState->rtcPrivilegeFull | privilegeState->rtcPrivilegeFeatures );

        /* TAMP, Monotonic counter and Backup registers privilege configuration
           Warning : privilegeState->backupRegisterPrivZone is only writable in secure mode or if
           trustzone is disabled. In non secure mode, a notification is generated through a
           flag/interrupt in the TZIC (TrustZone interrupt controller). The bits are not written. */
        WRITE_REG(
            TAMP->PRIVCFGR,
            privilegeState->tampPrivilegeFull | privilegeState->backupRegisterPrivZone |
                privilegeState->MonotonicCounterPrivilege );

        /* Backup register start zone
           Warning : This parameter is only writable in secure mode or if trustzone is disabled.
                     In non secure mode, a notification is generated through a flag/interrupt in the
           TZIC (TrustZone interrupt controller). The bits are not written. Warning : Backup
           register start zones are shared with secure configuration */
    ...
    RTC_PrivilegeStateTypeDef* privilegeState )
{
        ...
        /* Warning, rtcPrivilegeFeatures is only relevant if privilegeState->rtcPrivilegeFull ==
         * RTC_PRIVILEGE_FULL_NO */
