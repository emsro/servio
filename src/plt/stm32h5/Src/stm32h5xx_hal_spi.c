...
static uint32_t SPI_GetPacketSize( SPI_HandleTypeDef const* hspi );
...
HAL_StatusTypeDef HAL_SPI_Init( SPI_HandleTypeDef* hspi )
{
        ...
        WRITE_REG(
            hspi->Instance->CFG2,
            ( hspi->Init.NSSPMode | hspi->Init.TIMode | hspi->Init.NSSPolarity | hspi->Init.NSS |
              hspi->Init.CLKPolarity | hspi->Init.CLKPhase | hspi->Init.FirstBit | hspi->Init.Mode |
              hspi->Init.MasterInterDataIdleness | hspi->Init.Direction |
              hspi->Init.MasterSSIdleness | hspi->Init.IOSwap | hspi->Init.ReadyMasterManagement |
              hspi->Init.ReadyPolarity ) );
        ...
        if ( ( hspi->Init.Mode & SPI_MODE_MASTER ) == SPI_MODE_MASTER ) {
                ...
                MODIFY_REG(
                    hspi->Instance->CFG2, SPI_CFG2_AFCNTR, ( hspi->Init.MasterKeepIOState ) );
        }
 ...
 * @note   The HAL_SPI_UnRegisterCallback() may be called before HAL_SPI_Init() in
 * HAL_SPI_STATE_RESET to un-register callbacks for HAL_SPI_MSPINIT_CB_ID and
 ...
 */
HAL_StatusTypeDef
HAL_SPI_Transmit( SPI_HandleTypeDef* hspi, uint8_t const* pData, uint16_t Size, uint32_t Timeout )
{
#if defined( __GNUC__ )
        ...
        hspi->ErrorCode   = HAL_SPI_ERROR_NONE;
        hspi->pTxBuffPtr  = (uint8_t const*) pData;
                ...
                while ( hspi->TxXferCount > 0UL ) {
                        ...
                        if ( __HAL_SPI_GET_FLAG( hspi, SPI_FLAG_TXP ) ) {
                                *( (__IO uint32_t*) &hspi->Instance->TXDR ) =
                                    *( (uint32_t const*) hspi->pTxBuffPtr );
...
#if defined( __GNUC__ )
                                        *ptxdr_16bits = *( (uint16_t const*) hspi->pTxBuffPtr );
#else
                                        *( (__IO uint16_t*) &hspi->Instance->TXDR ) =
                                            *( (uint16_t const*) hspi->pTxBuffPtr );
#endif /* __GNUC__ */
...
HAL_SPI_Receive( SPI_HandleTypeDef* hspi, uint8_t* pData, uint16_t Size, uint32_t Timeout )
{
             ...
             ( IS_SPI_FULL_INSTANCE( hspi->Instance ) ) ) {
                ...
                while ( hspi->RxXferCount > 0UL ) {
                        ...
                        /* Check RXWNE flag if RXP cannot be reached */
                            ...
                            ( hspi->RxXferCount < init_max_data_in_fifo ) &&
        ...
        if ( hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE ) {
                ...
                if ( SPI_WaitOnFlagUntilTimeout( hspi, SPI_FLAG_EOT, RESET, Timeout, tickstart ) !=
                     HAL_OK ) {
                        SET_BIT( hspi->ErrorCode, HAL_SPI_ERROR_FLAG );
                }
        }
    ...
    uint32_t           Timeout )
{
        ...
        hspi->RxXferCount = Size;
        hspi->RxXferSize  = Size;
        hspi->pTxBuffPtr  = (uint8_t const*) pTxData;
        hspi->TxXferCount = Size;
        hspi->TxXferSize  = Size;
                ...
                while ( ( initial_TxXferCount > 0UL ) || ( initial_RxXferCount > 0UL ) ) {
                        ...
                        if ( initial_RxXferCount > 0UL ) {
                                ...
                                /* Check RXWNE flag if RXP cannot be reached */
                                    ...
                                    ( initial_RxXferCount < init_max_data_in_fifo ) &&
                                    ( ( temp_sr_reg & SPI_SR_RXWNE_Msk ) != 0UL ) ) {
                                        *( (uint32_t*) hspi->pRxBuffPtr ) =
                                            *( (__IO uint32_t*) &hspi->Instance->RXDR );
                                        hspi->pRxBuffPtr += sizeof( uint32_t );
 ...
 */
HAL_StatusTypeDef
HAL_SPI_Transmit_IT( SPI_HandleTypeDef* hspi, uint8_t const* pData, uint16_t Size )
{
        /* Check Direction parameter */
        assert_param( IS_SPI_DIRECTION_2LINES_OR_1LINE_2LINES_TXONLY( hspi->Init.Direction ) );
    ...
    uint16_t           Size )
{
        ...
        __HAL_SPI_ENABLE_IT(
            hspi,
            ( SPI_IT_EOT | SPI_IT_DXP | SPI_IT_UDR | SPI_IT_OVR | SPI_IT_FRE | SPI_IT_MODF ) );
 ...
 */
HAL_StatusTypeDef
HAL_SPI_Transmit_DMA( SPI_HandleTypeDef* hspi, uint8_t const* pData, uint16_t Size )
{
        ...
        if ( ( hspi->hdmatx->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                if ( hspi->hdmatx->LinkedListQueue != NULL ) {
                        ...
                        hspi->hdmatx->LinkedListQueue->Head
                            ->LinkRegisters[NODE_CSAR_DEFAULT_OFFSET] = (uint32_t) hspi->pTxBuffPtr;
...
HAL_StatusTypeDef HAL_SPI_Receive_DMA( SPI_HandleTypeDef* hspi, uint8_t* pData, uint16_t Size )
{
        ...
        if ( ( hspi->hdmarx->Mode & DMA_LINKEDLIST ) == DMA_LINKEDLIST ) {
                if ( hspi->hdmarx->LinkedListQueue != NULL ) {
                        ...
                        /* Set DMA destination address */
                        hspi->hdmarx->LinkedListQueue->Head
                            ->LinkRegisters[NODE_CDAR_DEFAULT_OFFSET] = (uint32_t) hspi->pRxBuffPtr;
        ...
        if ( ( trigger & ( SPI_FLAG_MODF | SPI_FLAG_OVR | SPI_FLAG_FRE | SPI_FLAG_UDR ) ) != 0UL ) {
                ...
                if ( hspi->ErrorCode != HAL_SPI_ERROR_NONE ) {
                        ...
                        __HAL_SPI_DISABLE_IT(
                            hspi,
                            ( SPI_IT_EOT | SPI_IT_RXP | SPI_IT_TXP | SPI_IT_MODF | SPI_IT_OVR |
                              SPI_IT_FRE | SPI_IT_UDR ) );
 ...
 * @retval None
 */
__weak void HAL_SPI_TxRxHalfCpltCallback( SPI_HandleTypeDef* hspi ) /* Derogation
 ...
 * @retval SPI state
 */
HAL_SPI_StateTypeDef HAL_SPI_GetState( SPI_HandleTypeDef const* hspi )
{
 ...
 */
uint32_t HAL_SPI_GetError( SPI_HandleTypeDef const* hspi )
{
...
{
        SPI_HandleTypeDef* hspi =
            (SPI_HandleTypeDef*) ( (DMA_HandleTypeDef*) hdma )->Parent; /* Derogation
                                                                           MISRAC2012-Rule-8.13 */
 ...
 * @retval None
 */
static void SPI_DMAHalfTransmitReceiveCplt( DMA_HandleTypeDef* hdma ) /* Derogation
                                                                         MISRAC2012-Rule-8.13 */
{
        SPI_HandleTypeDef* hspi =
            (SPI_HandleTypeDef*) ( (DMA_HandleTypeDef*) hdma )->Parent; /* Derogation
...
{
        /* Transmit data in 8 Bit mode */
        *(__IO uint8_t*) &hspi->Instance->TXDR = *( (uint8_t const*) hspi->pTxBuffPtr );
        hspi->pTxBuffPtr += sizeof( uint8_t );
        hspi->TxXferCount--;
...
static void SPI_TxISR_16BIT( SPI_HandleTypeDef* hspi )
{
        ...
        *ptxdr_16bits = *( (uint16_t const*) hspi->pTxBuffPtr );
#else
        *( (__IO uint16_t*) &hspi->Instance->TXDR ) = *( (uint16_t const*) hspi->pTxBuffPtr );
#endif /* __GNUC__ */
        hspi->pTxBuffPtr += sizeof( uint16_t );
        hspi->TxXferCount--;
...
{
        /* Transmit data in 32 Bit mode */
        *( (__IO uint32_t*) &hspi->Instance->TXDR ) = *( (uint32_t const*) hspi->pTxBuffPtr );
        hspi->pTxBuffPtr += sizeof( uint32_t );
        hspi->TxXferCount--;
...
static void SPI_AbortTransfer( SPI_HandleTypeDef* hspi )
{
        ...
        __HAL_SPI_DISABLE_IT(
            hspi,
            ( SPI_IT_EOT | SPI_IT_TXP | SPI_IT_RXP | SPI_IT_DXP | SPI_IT_UDR | SPI_IT_OVR |
