   ...
   [..]
    ...
    (#)Multi-buffer processing HASH and HMAC are possible in polling, interrupt and DMA modes.
        ...
        (##) In DMA mode, once initialization is done, MDMAT bit must be set through
             ...
             From that point, each buffer can be fed to the Peripheral through HAL_HASH_Start_DMA()
 API for HASH and HAL_HASH_HMAC_Start_DMA() API for HMAC . Before entering the last buffer, reset
 the MDMAT bit with __HAL_HASH_RESET_MDMAT() macro then wrap-up the HASH processing in feeding the
 last input buffer through the same API HAL_HASH_Start_DMA()for HASH and HAL_HASH_HMAC_Start_DMA()
 API for HMAC and retrieve as well the computed digest.

    (#)To use this driver (version 2.0.0) with application developed with old driver (version 1.0.0)
 user have to:
        (##) Add Algorithm as parameter like DataType or KeySize.
        (##) Use new API HAL_HASH_Start() for HASH and HAL_HASH_HMAC_Start() for HMAC processing
 instead of old API like HAL_HASH_SHA1_Start and HAL_HMAC_SHA1_Start.
 ...
 */
static void
HASH_GetDigest( HASH_HandleTypeDef const* hhash, uint8_t const* pMsgDigest, uint8_t Size );
static void HASH_WriteData( HASH_HandleTypeDef* hhash, uint8_t const* pInBuffer, uint32_t Size );
static HAL_StatusTypeDef HASH_WriteData_IT( HASH_HandleTypeDef* hhash );
static void              HASH_DMAXferCplt( DMA_HandleTypeDef* hdma );
static void              HASH_DMAError( DMA_HandleTypeDef* hdma );
static HAL_StatusTypeDef HASH_WaitOnFlagUntilTimeout(
...
@verbatim
 ...
 ===============================================================================
    [..]  This section provides functions allowing to:
      ...
      (+) Configure HASH (HAL_HASH_SetConfig) with the specified parameters in the
HASH_ConfigTypeDef Parameters which are configured in This section are :
          (+) Data Type : no swap, half word swap, bit swap or byte swap
          (+) Algorithm : MD5,SHA1 or SHA2
      (+) Get HASH configuration (HAL_HASH_GetConfig) from the specified parameters in the
HASH_HandleTypeDef
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_SetConfig( HASH_HandleTypeDef* hhash, HASH_ConfigTypeDef const* pConf )
{
        uint32_t cr_value;
        ...
        if ( hhash->State == HAL_HASH_STATE_READY ) {
                ...
                /* Set the key size, data type, algorithm and mode */
                MODIFY_REG(
                    hhash->Instance->CR, HASH_CR_DATATYPE | HASH_CR_ALGO | HASH_CR_INIT, cr_value );
 ...
 * @retval status
 */
HAL_StatusTypeDef HAL_HASH_RegisterCallback(
    HASH_HandleTypeDef*        hhash,
 ...
 */
HAL_StatusTypeDef
HAL_HASH_UnRegisterCallback( HASH_HandleTypeDef* hhash, HAL_HASH_CallbackIDTypeDef CallbackID )
{
        HAL_StatusTypeDef status = HAL_OK;
...
HAL_StatusTypeDef HAL_HASH_ProcessSuspend( HASH_HandleTypeDef* hhash )
{
        ...
        if ( __HAL_HASH_GET_FLAG( hhash, HASH_FLAG_DMAS ) != RESET ) {
                if ( hhash->State == HAL_HASH_STATE_READY ) {
                         ...
                         * register) */
                        if ( HASH_WaitOnFlagUntilTimeout(
                                 hhash, HASH_FLAG_DMAS, SET, HASH_TIMEOUTVALUE ) != HAL_OK ) {
                                return HAL_TIMEOUT;
                        }
                        ...
                        if ( remainingwords <= nbbytePartialHash ) {
                                /* No suspension attempted since almost to the end of the
                                 * transferred data. */
                                /* Best option for user code is to wrap up low priority message
                                 * hashing     */
                                return HAL_ERROR;
                        }
                        ...
                        */
                        ...
                        if ( HASH_WaitOnFlagUntilTimeout(
                                 hhash, HASH_FLAG_DINIS, RESET, HASH_TIMEOUTVALUE ) != HAL_OK ) {
                                return HAL_TIMEOUT;
                        }

                        /* Compute how many words were supposed to be transferred by DMA */
                        sizeinwords =
                            ( ( ( hhash->Size % 4U ) != 0U ) ? ( ( hhash->Size + 3U ) / 4U ) :
                                                               ( hhash->Size / 4U ) );
...
@verbatim
 ...
 ===============================================================================
    ...
    [..]  In case of multi-buffer HASH processing (a single digest is computed while
      ...
      (+) DMA mode : HAL_HASH_Start_DMA(), MDMAT bit must be set through __HAL_HASH_SET_MDMAT()
macro, before entering the last buffer, reset the MDMAT bit with __HAL_HASH_RESET_MDMAT() macro then
wrap-up the HASH processing in feeding the last input buffer through the same API
HAL_HASH_Start_DMA()
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_Start_DMA(
    HASH_HandleTypeDef*  hhash,
    uint8_t const* const pInBuffer,
    uint32_t             Size,
    uint8_t* const       pOutBuffer )
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_Accumulate(
    HASH_HandleTypeDef*  hhash,
    uint8_t const* const pInBuffer,
    uint32_t             Size,
    uint32_t             Timeout )
...
/**
 * @brief  End computation of a single HASH signature after several calls to HAL_HASH_Accumulate()
 * API.
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_AccumulateLast(
    HASH_HandleTypeDef*  hhash,
    uint8_t const* const pInBuffer,
    uint32_t             Size,
    uint8_t* const       pOutBuffer,
 ...
 */
HAL_StatusTypeDef
HAL_HASH_Accumulate_IT( HASH_HandleTypeDef* hhash, uint8_t const* const pInBuffer, uint32_t Size )
{
        HAL_StatusTypeDef status;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_AccumulateLast_IT(
    HASH_HandleTypeDef*  hhash,
    uint8_t const* const pInBuffer,
    uint32_t             Size,
    uint8_t* const       pOutBuffer )
...
@verbatim
 ...
 ===============================================================================
    ...
    [..]  In case of multi-buffer HMAC processing (a single digest is computed while
      ...
      (+) Interrupt mode : HAL_HASH_HMAC_Accumulate_IT() and wrap-up the digest computation by a
call to HAL_HASH_HMAC_AccumulateLast_IT()
      (+) DMA mode : HAL_HASH_HMAC_Start_DMA(),MDMAT bit must be set through __HAL_HASH_SET_MDMAT()
macro, before entering the last buffer, reset the MDMAT bit with __HAL_HASH_RESET_MDMAT() macro then
wrap-up the HMAC processing in feeding the last input buffer through the same API
HAL_HASH_HMAC_Start_DMA()
...
/**
 * @brief  HMAC in polling mode, HASH peripheral processes Key then pInBuffer then reads the
 * computed digest.
 * @param  hhash HASH handle.
 * @param  pInBuffer pointer to the input buffer (buffer to be hashed).
 * @param  Size length of the input buffer in bytes.
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_HMAC_Start(
    HASH_HandleTypeDef*  hhash,
    uint8_t const* const pInBuffer,
    uint32_t             Size,
    uint8_t* const       pOutBuffer,
    uint32_t             Timeout )
{
        ...
        if ( hhash->State == HAL_HASH_STATE_READY ) {
                ...
                /* Reset HashInCount and Initialize Size, pHashKeyBuffPtr, pHashInBuffPtr and
                ...
                /* Check if key size is larger than block size of the algorithm, accordingly set
                 * LKEY and the other setting bits */
                ...
                MODIFY_REG(
                    hhash->Instance->STR, HASH_STR_NBLW, 8U * ( ( hhash->Init.KeySize ) % 4U ) );
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_HMAC_Accumulate(
    HASH_HandleTypeDef*  hhash,
    uint8_t const* const pInBuffer,
    uint32_t             Size,
    uint32_t             Timeout )
{
        ...
        if ( hhash->State == HAL_HASH_STATE_READY ) {
                ...
                if ( hhash->Phase == HAL_HASH_PHASE_READY ) {
                         ...
                         * set LKEY and the other setting */
                        if ( ( hhash->Init.Algorithm == HASH_ALGOSELECTION_SHA1 ) ||
                             ( hhash->Init.Algorithm == HASH_ALGOSELECTION_SHA224 ) ||
                             ( hhash->Init.Algorithm == HASH_ALGOSELECTION_SHA256 ) ) {
                                blocksize = BLOCK_64B;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_HMAC_AccumulateLast(
    HASH_HandleTypeDef*  hhash,
    uint8_t const* const pInBuffer,
    uint32_t             Size,
    uint8_t* const       pOutBuffer,
...
/**
 * @brief  HMAC in interrupt mode, HASH peripheral process Key then pInBuffer then read the computed
 * digest.
 * @param  hhash HASH handle.
 * @param  pInBuffer pointer to the input buffer (buffer to be hashed).
 * @param  Size length of the input buffer in bytes.
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_HMAC_Start_IT(
    HASH_HandleTypeDef*  hhash,
    uint8_t const* const pInBuffer,
    uint32_t             Size,
    uint8_t* const       pOutBuffer )
...
/**
 * @brief  HMAC accumulate in interrupt mode, HASH peripheral processes Key then several input
 * buffers.
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_HMAC_Accumulate_IT(
    HASH_HandleTypeDef*  hhash,
    uint8_t const* const pInBuffer,
    uint32_t             Size )
{
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_HMAC_AccumulateLast_IT(
    HASH_HandleTypeDef*  hhash,
    uint8_t const* const pInBuffer,
    uint32_t             Size,
    uint8_t* const       pOutBuffer )
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_HMAC_Start_DMA(
    HASH_HandleTypeDef*  hhash,
    uint8_t const* const pInBuffer,
    uint32_t             Size,
    uint8_t* const       pOutBuffer )
{
        ...
        if ( hhash->State == HAL_HASH_STATE_READY ) {
                ...
                if ( ( hhash->Instance->CR & HASH_CR_MDMAT ) == 0U ) {
                        /* Configure the number of valid bits in last word of the message */
                        MODIFY_REG(
                            hhash->Instance->STR, HASH_STR_NBLW, 8U * ( ( hhash->Size ) % 4U ) );
                } else {
                        /* Configure the number of valid bits in last word of the message */
                        MODIFY_REG( hhash->Instance->STR, HASH_STR_NBLW, 0U );
                }
 ...
 */
HAL_HASH_StateTypeDef HAL_HASH_GetState( HASH_HandleTypeDef const* hhash )
{
        return hhash->State;
}
 ...
 */
uint32_t HAL_HASH_GetError( HASH_HandleTypeDef const* hhash )
{
...
static void HASH_DMAXferCplt( DMA_HandleTypeDef* hdma )
{
        ...
        if ( READ_BIT( hhash->Instance->CR, HASH_CR_MODE ) == 0U ) {
                if ( ( hhash->Instance->CR & HASH_CR_MDMAT ) == 0U ) {
                        ...
                        /* Read the message digest */
                        HASH_GetDigest(
                            hhash, hhash->pHashOutBuffPtr, HASH_DIGEST_LENGTH( hhash ) );
        ...
        if ( ( Size % 4U ) != 0U ) {
                    ...
                    ( hhash->Init.DataType == HASH_BIT_SWAP ) ) /* byte swap or bit swap or */
                {
                        ...
                        if ( ( Size % 4U ) == 3U ) {
                                ...
                                hhash->Instance->DIN = ( (uint32_t) tmp1 ) |
                                                       ( (uint32_t) tmp2 << 8U ) |
                                                       ( (uint32_t) tmp3 << 16U );
                        }
                } else {
...
static HAL_StatusTypeDef HASH_WriteData_IT( HASH_HandleTypeDef* hhash )
{
        ...
        uint32_t      nbbytePartialHash =
            ( ( ( hhash->Instance->SR ) >> 16U ) * 4U ); /* Nb byte  to enter in HASH fifo to trig
                        ...
                        if ( hhash->SuspendRequest == HAL_HASH_SUSPEND ) {
                                        ...
                                        do {
                                                ...
                                                if ( count == 0U ) {
                                                        ...
                                                        __HAL_HASH_DISABLE_IT(
                                                            hhash, HASH_IT_DINI | HASH_IT_DCI );
                                            ...
                                            hhash->Instance->SR, HASH_FLAG_DINIS ) );
                                } else {
                                ...
                                HAL_HASH_InCpltCallback( hhash );
                                        ...
                                        if ( hhash->Accumulation == 0U ) {
                                                if ( __HAL_HASH_GET_IT_SOURCE(
                                                         hhash, HASH_IT_DINI ) ) {
                                                        /* Start the message padding then the Digest
                                                         * calculation */
                                                        SET_BIT(
                                                        ...
                                                        do {
                                                            ...
                                                            hhash->Instance->SR, HASH_FLAG_DCIS ) );
                                                }
                                        } else {
                                                /* Reset multi buffers accumulation flag */
                                                hhash->Accumulation = 0U;
                        ...
                        if ( hhash->Phase == HAL_HASH_PHASE_HMAC_STEP_2 ) /* loading input*/
                        {
                                ...
                                if ( hhash->SuspendRequest == HAL_HASH_SUSPEND ) {
                                        ...
                                        if ( hhash->Accumulation == 1U ) {
                                                /* Configure the number of valid bits in last word
                                                 * of the message */
                                                MODIFY_REG(
                                                    hhash->Instance->STR, HASH_STR_NBLW, 0U );
                                        } else {
                                                /* Configure the number of valid bits in last word
                                                 * of the message */
                                                MODIFY_REG(
                                        ...
                                        if ( ( ( hhash->HashInCount ) + nbbytePartialHash ) <
                                             ( hhash->Size ) ) {
                                                for ( buffercounter = 0U;
                                                      buffercounter < nbbytePartialHash;
                                                      buffercounter += 4U ) {
                                                        /* Write input data 4 bytes at a time */
                                                        hhash->Instance->DIN =
                                                            *(uint32_t*) inputaddr;
                                        ...
                                        HAL_HASH_InCpltCallback( hhash );
                                                ...
                                                if ( hhash->Accumulation == 0U ) {
                                                        if ( __HAL_HASH_GET_IT_SOURCE(
                                                                 hhash, HASH_IT_DINI ) ) {
                                                                /* Start the message padding then
                                                                 * the Digest calculation */
                                                                SET_BIT(
                                                                    hhash->Instance->STR,
                                                                    HASH_STR_DCAL );

                                                                /* Wait for HASH_FLAG_BUSY flag to
                                                                 * be set */
                                                                count = HASH_TIMEOUTVALUE;
                                                                do {
                                                                        count--;
                                ...
                                if ( ( ( hhash->HashInCount ) + nbbytePartialHash ) <
                                     ( hhash->Init.KeySize ) ) {
                                        ...
                                        do {
                                            ...
                                            HAL_IS_BIT_CLR( hhash->Instance->SR, HASH_FLAG_DCIS ) );
                                }
                                ...
                                if ( ( ( hhash->HashInCount ) + nbbytePartialHash ) <
                                     ( hhash->Init.KeySize ) ) {
                                        ...
                                        do {
                                            ...
                                            HAL_IS_BIT_SET( hhash->Instance->SR, HASH_FLAG_BUSY ) );
