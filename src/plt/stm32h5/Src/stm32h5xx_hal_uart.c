...
#define USART_CR1_FIELDS                                                                          \
        ( (uint32_t) ( USART_CR1_M | USART_CR1_PCE | USART_CR1_PS | USART_CR1_TE | USART_CR1_RE | \
                       USART_CR1_OVER8 | USART_CR1_FIFOEN ) ) /*!< UART or USART CR1 fields of    \
 ...
 * @{
 */
uint16_t const UARTPrescTable[12] = { 1U, 2U, 4U, 6U, 8U, 10U, 12U, 16U, 32U, 64U, 128U, 256U };
        ...
        if ( huart->gState == HAL_UART_STATE_READY ) {
                ...
                if ( ( huart->Init.WordLength == UART_WORDLENGTH_9B ) &&
                     ( huart->Init.Parity == UART_PARITY_NONE ) ) {
                        pdata8bits  = NULL;
                        pdata16bits = (uint16_t const*) pData;
                } else {
                        pdata8bits  = pData;
                        pdata16bits = NULL;
                }
                ...
                while ( huart->TxXferCount > 0U ) {
                        if ( UART_WaitOnFlagUntilTimeout(
                                 huart, UART_FLAG_TXE, RESET, tickstart, Timeout ) != HAL_OK ) {
                ...
                if ( UART_WaitOnFlagUntilTimeout(
                         huart, UART_FLAG_TC, RESET, tickstart, Timeout ) != HAL_OK ) {
                        huart->gState = HAL_UART_STATE_READY;
...
HAL_UART_Receive( UART_HandleTypeDef* huart, uint8_t* pData, uint16_t Size, uint32_t Timeout )
{
        ...
        if ( huart->RxState == HAL_UART_STATE_READY ) {
                ...
                while ( huart->RxXferCount > 0U ) {
                        if ( UART_WaitOnFlagUntilTimeout(
                                 huart, UART_FLAG_RXNE, RESET, tickstart, Timeout ) != HAL_OK ) {
 ...
 */
HAL_StatusTypeDef
HAL_UART_Transmit_IT( UART_HandleTypeDef* huart, uint8_t const* pData, uint16_t Size )
{
        /* Check that a Tx process is not already ongoing */
        if ( huart->gState == HAL_UART_STATE_READY ) {
                if ( ( pData == NULL ) || ( Size == 0U ) )
 ...
 */
HAL_StatusTypeDef
HAL_UART_Transmit_DMA( UART_HandleTypeDef* huart, uint8_t const* pData, uint16_t Size )
{
...
{
        HAL_UART_StateTypeDef const gstate  = huart->gState;
        HAL_UART_StateTypeDef const rxstate = huart->RxState;
...
HAL_StatusTypeDef HAL_UART_Abort( UART_HandleTypeDef* huart )
{
        ...
        ATOMIC_CLEAR_BIT(
            ...
            ( USART_CR1_RXNEIE_RXFNEIE | USART_CR1_PEIE | USART_CR1_TXEIE_TXFNFIE |
              USART_CR1_TCIE ) );
        ATOMIC_CLEAR_BIT(
            huart->Instance->CR3, USART_CR3_EIE | USART_CR3_RXFTIE | USART_CR3_TXFTIE );
        ...
        __HAL_UART_CLEAR_FLAG(
            huart, UART_CLEAR_OREF | UART_CLEAR_NEF | UART_CLEAR_PEF | UART_CLEAR_FEF );
...
void HAL_UART_IRQHandler( UART_HandleTypeDef* huart )
{
        ...
        if ( ( errorflags != 0U ) &&
             ( ( ( ( cr3its & ( USART_CR3_RXFTIE | USART_CR3_EIE ) ) != 0U ) ||
                 ( ( cr1its & ( USART_CR1_RXNEIE_RXFNEIE | USART_CR1_PEIE | USART_CR1_RTOIE ) ) !=
                   0U ) ) ) ) {
             ...
             ( ( isrflags & USART_ISR_IDLE ) != 0U ) && ( ( cr1its & USART_ISR_IDLE ) != 0U ) ) {
                ...
                if ( HAL_IS_BIT_SET( huart->Instance->CR3, USART_CR3_DMAR ) ) {
                        ...
                        if ( ( nb_remaining_rx_data > 0U ) &&
                             ( nb_remaining_rx_data < huart->RxXferSize ) ) {
                                ...
                                if ( nb_remaining_rx_data == huart->RxXferSize ) {
                                        if ( huart->hdmarx->Mode == DMA_LINKEDLIST_CIRCULAR ) {
                                                /* Initialize type of RxEvent that correspond to
                                                   RxEvent callback execution; In this case, Rx
                                                   Event type is Idle Event */
                                                huart->RxEventType = HAL_UART_RXEVENT_IDLE;
                        ...
                        if ( ( huart->RxXferCount > 0U ) && ( nb_rx_data > 0U ) ) {
                                ...
                                ATOMIC_CLEAR_BIT(
                                    huart->Instance->CR3, ( USART_CR3_EIE | USART_CR3_RXFTIE ) );
 ...
 * @retval HAL state
 */
HAL_UART_StateTypeDef HAL_UART_GetState( UART_HandleTypeDef const* huart )
{
        uint32_t temp1;
        uint32_t temp2;
        temp1 = huart->gState;
 ...
 */
uint32_t HAL_UART_GetError( UART_HandleTypeDef const* huart )
{
...
void UART_AdvFeatureConfig( UART_HandleTypeDef* huart )
{
        ...
        if ( HAL_IS_BIT_SET( huart->AdvancedInit.AdvFeatureInit, UART_ADVFEATURE_TXINVERT_INIT ) ) {
                ...
                MODIFY_REG(
                    huart->Instance->CR2, USART_CR2_TXINV, huart->AdvancedInit.TxPinLevelInvert );
        }
        ...
        if ( HAL_IS_BIT_SET( huart->AdvancedInit.AdvFeatureInit, UART_ADVFEATURE_RXINVERT_INIT ) ) {
                ...
                MODIFY_REG(
                    huart->Instance->CR2, USART_CR2_RXINV, huart->AdvancedInit.RxPinLevelInvert );
        }
        ...
        if ( HAL_IS_BIT_SET(
                ...
                MODIFY_REG(
                    huart->Instance->CR2, USART_CR2_DATAINV, huart->AdvancedInit.DataInvert );
        }
        ...
        if ( HAL_IS_BIT_SET(
                ...
                MODIFY_REG(
                    huart->Instance->CR3, USART_CR3_OVRDIS, huart->AdvancedInit.OverrunDisable );
        }
        ...
        if ( HAL_IS_BIT_SET(
                ...
                MODIFY_REG(
                    huart->Instance->CR3, USART_CR3_DDRE, huart->AdvancedInit.DMADisableonRxError );
        }
#endif /* HAL_DMA_MODULE_ENABLED */
        ...
        if ( HAL_IS_BIT_SET(
                ...
                MODIFY_REG(
                    huart->Instance->CR2, USART_CR2_ABREN, huart->AdvancedInit.AutoBaudRateEnable );
...
HAL_StatusTypeDef UART_CheckIdleState( UART_HandleTypeDef* huart )
{
        ...
        if ( ( huart->Instance->CR1 & USART_CR1_TE ) == USART_CR1_TE ) {
                ...
                if ( UART_WaitOnFlagUntilTimeout(
                         huart, USART_ISR_TEACK, RESET, tickstart, HAL_UART_TIMEOUT_VALUE ) !=
                     HAL_OK ) {
        ...
        if ( ( huart->Instance->CR1 & USART_CR1_RE ) == USART_CR1_RE ) {
                ...
                if ( UART_WaitOnFlagUntilTimeout(
                         huart, USART_ISR_REACK, RESET, tickstart, HAL_UART_TIMEOUT_VALUE ) !=
                     HAL_OK ) {
                        /* Disable RXNE, PE and ERR (Frame error, noise error, overrun error)
                        interrupts for the interrupt process */
                        ATOMIC_CLEAR_BIT(
                            huart->Instance->CR1, ( USART_CR1_RXNEIE_RXFNEIE | USART_CR1_PEIE ) );
                        ATOMIC_CLEAR_BIT( huart->Instance->CR3, USART_CR3_EIE );
...
static void UART_TxISR_16BIT( UART_HandleTypeDef* huart )
{
        ...
        if ( huart->gState == HAL_UART_STATE_BUSY_TX ) {
                if ( huart->TxXferCount == 0U ) {
                        ...
                        tmp                  = (uint16_t const*) huart->pTxBuffPtr;
                ...
                while ( ( nb_rx_data > 0U ) && ( ( isrflags & USART_ISR_RXNE_RXFNE ) != 0U ) ) {
                        ...
                        *huart->pRxBuffPtr = (uint8_t) ( uhdata & (uint8_t) uhMask );
                        ...
                        if ( huart->RxXferCount == 0U ) {
                                ...
                                if ( !( IS_LPUART_INSTANCE( huart->Instance ) ) ) {
                                        /* Check that USART RTOEN bit is set */
                                        if ( READ_BIT( huart->Instance->CR2, USART_CR2_RTOEN ) !=
                                             0U ) {
                                                /* Enable the UART Receiver Timeout Interrupt */
                                                ATOMIC_CLEAR_BIT(
                                                    huart->Instance->CR1, USART_CR1_RTOIE );
                                        }
                                }
