...
#endif /* USE_HAL_MMC_REGISTER_CALLBACKS */
{
        ...
        uint8_t const* pTxBuffPtr; /*!< Pointer to MMC Tx transfer Buffer    */
...
#define HAL_MMC_ERROR_RPMB_GENERAL_FAILURE \
        0x0001U /*!< General failure                                               */
#define HAL_MMC_ERROR_RPMB_AUTHENTICATION_FAILURE \
        0x0002U /*!< Authentication failure                                        */
#define HAL_MMC_ERROR_RPMB_COUNTER_FAILURE \
        0x0003U /*!< Counter failure                                               */
#define HAL_MMC_ERROR_RPMB_ADDRESS_FAILURE \
        0x0004U /*!< Address failure                                               */
#define HAL_MMC_ERROR_RPMB_WRITE_FAILURE \
        0x0005U /*!< Write failure                                                 */
#define HAL_MMC_ERROR_RPMB_READ_FAILURE \
        0x0006U /*!< Read failure                                                  */
...
#define HAL_MMC_SECURE_ERASE                                                                    \
        0x80000000U /*!< Perform a secure purge according SRT on the erase groups identified by \
                       CMD35 & 36 */
#define HAL_MMC_SECURE_TRIM_STEP1 \
        0x80000001U /*!< Mark the write blocks identified by CMD35 & 36 for secure erase */
#define HAL_MMC_SECURE_TRIM_STEP2                                                            \
        0x80008000U /*!< Perform a secure purge according SRT on the write blocks previously \
                       identified    */

#define IS_MMC_ERASE_TYPE( TYPE )                                                    \
        ( ( ( TYPE ) == HAL_MMC_ERASE ) || ( ( TYPE ) == HAL_MMC_TRIM ) ||           \
          ( ( TYPE ) == HAL_MMC_DISCARD ) || ( ( TYPE ) == HAL_MMC_SECURE_ERASE ) || \
          ( ( TYPE ) == HAL_MMC_SECURE_TRIM_STEP1 ) || ( ( TYPE ) == HAL_MMC_SECURE_TRIM_STEP2 ) )
/**
 * @}
 */
...
#define HAL_MMC_SRT_WRITE_CHAR_ERASE                                                           \
        0x00000002U /*!< Information removed by an overwriting with a character followed by an \
                       erase                    */
#define HAL_MMC_SRT_WRITE_CHAR_COMPL_RANDOM                                                     \
        0x00000004U /*!< Information removed by an overwriting with a character, its complement \
...
#define IS_MMC_SRT_TYPE( TYPE )                                                                \
        ( ( ( TYPE ) == HAL_MMC_SRT_ERASE ) || ( ( TYPE ) == HAL_MMC_SRT_WRITE_CHAR_ERASE ) || \
...
uint32_t             HAL_MMC_GetRPMBError( MMC_HandleTypeDef const* hmmc );
/**
 * @}
 */
...
HAL_StatusTypeDef HAL_MMC_RPMB_ProgramAuthenticationKey(
    ...
    uint8_t const*     pKey,
    uint32_t           Timeout );
HAL_StatusTypeDef HAL_MMC_RPMB_ProgramAuthenticationKey_IT(
    MMC_HandleTypeDef* hmmc,
    uint8_t const*     pKey,
    uint32_t           Timeout );
...
HAL_StatusTypeDef HAL_MMC_RPMB_WriteBlocks(
    ...
    uint8_t const*     pMAC,
    uint32_t           Timeout );
HAL_StatusTypeDef HAL_MMC_RPMB_WriteBlocks_IT(
    MMC_HandleTypeDef* hmmc,
    uint8_t const*     pData,
    uint16_t           BlockAdd,
    uint16_t           NumberOfBlocks,
...
HAL_StatusTypeDef HAL_MMC_RPMB_ReadBlocks(
    ...
    uint8_t*           pMAC,
    uint32_t           Timeout );
HAL_StatusTypeDef HAL_MMC_RPMB_ReadBlocks_IT(
    MMC_HandleTypeDef* hmmc,
    uint8_t*           pData,
    uint16_t           BlockAdd,
    uint16_t           NumberOfBlocks,
