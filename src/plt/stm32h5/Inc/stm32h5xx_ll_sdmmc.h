...
typedef struct
{
        uint32_t ClockEdge; /*!< Specifies the SDMMC_CCK clock transition on which Data and Command
                               change. This parameter can be a value of @ref SDMMC_LL_Clock_Edge */
...
#define SDMMC_ERROR_BLOCK_LEN_ERR                                                               \
        ( (uint32_t) 0x00000080U ) /*!< Transferred block length is not allowed for the card or \
                                      the number of transferred bytes does not match the block  \
                                      length   */
...
#define SDMMC_ERROR_CARD_ECC_FAILED                                                             \
        ( (uint32_t) 0x00004000U ) /*!< Card internal ECC was applied but failed to correct the \
                                      data  */
...
#define SDMMC_SDIO_R5_ERRORBITS                                                         \
        ( SDMMC_SDIO_R5_COM_CRC_FAILED | SDMMC_SDIO_R5_ILLEGAL_CMD |                    \
          SDMMC_SDIO_R5_GENERAL_UNKNOWN_ERROR | SDMMC_SDIO_R5_INVALID_FUNCTION_NUMBER | \
...
#define SDMMC_CMD_SDMMC_SEN_OP_COND                                                              \
        5U /*!< Sends host capacity support information (HCS) and asks the accessed card to send \
...
#define SDMMC_CMD_SEL_DESEL_CARD                                                              \
        7U /*!< Selects the card by its own relative address and gets deselected by any other \
              address    */
...
#define SDMMC_CMD_SET_BLOCKLEN                                                                     \
        16U /*!< Sets the block length (in bytes for SDSC) for all following block commands (read, \
               write, lock). Default block length is fixed to 512 Bytes. Not effective        */
/*!< for SDHS and SDXC.                                                                       */
#define SDMMC_CMD_READ_SINGLE_BLOCK                                                               \
        17U /*!< Reads single block of size selected by SET_BLOCKLEN in case of SDSC, and a block \
               of fixed 512 bytes in case of SDHC and SDXC.                                    */
...
#define SDMMC_CMD_WRITE_SINGLE_BLOCK                                                               \
        24U /*!< Writes single block of size selected by SET_BLOCKLEN in case of SDSC, and a block \
               of fixed 512 bytes in case of SDHC and SDXC.                                   */
...
#define SDMMC_CMD_LOCK_UNLOCK                                                                    \
        42U /*!< Sets/resets the password or lock/unlock the card. The size of the data block is \
               set by the SET_BLOCK_LEN command.                                                */
#define SDMMC_CMD_APP_CMD                                                                       \
        55U /*!< Indicates to the card that the next command is an application specific command \
               rather than a standard command. */
#define SDMMC_CMD_GEN_CMD                                                                         \
        56U /*!< Used either to transfer a data block to the card or to get a data block from the \
               card for general purpose/application specific commands.                         */
...
#define SDMMC_CMD_APP_SD_SET_BUSWIDTH                                                             \
        6U /*!< (ACMD6) Defines the data bus width to be used for data transfer. The allowed data \
              bus widths are given in SCR register. */
#define SDMMC_CMD_SD_APP_STATUS 13U /*!< (ACMD13) Sends the SD status. */
#define SDMMC_CMD_SD_APP_SEND_NUM_WRITE_BLOCKS                                                    \
        22U /*!< (ACMD22) Sends the number of the written (without errors) write blocks. Responds \
               with 32bit+CRC data block. */
#define SDMMC_CMD_SD_APP_OP_COND                                                                   \
        41U /*!< (ACMD41) Sends host capacity support information (HCS) and asks the accessed card \
               to send its operating condition register (OCR) content in the response on the CMD   \
               line. */
#define SDMMC_CMD_SD_APP_SET_CLR_CARD_DETECT                                                    \
        42U /*!< (ACMD42) Connect/Disconnect the 50 KOhm pull-up resistor on CD/DAT3 (pin 1) of \
               the card  */
...
#define IS_SDMMC_SPEED_MODE( MODE )                                                            \
        ( ( ( MODE ) == SDMMC_SPEED_MODE_AUTO ) || ( ( MODE ) == SDMMC_SPEED_MODE_DEFAULT ) || \
...
#define IS_SDMMC_TRANSFER_MODE( MODE )                                                             \
        ( ( ( MODE ) == SDMMC_TRANSFER_MODE_BLOCK ) || ( ( MODE ) == SDMMC_TRANSFER_MODE_SDIO ) || \
...
uint32_t
SDMMC_SDIO_CmdReadWriteDirect( SDMMC_TypeDef* SDMMCx, uint32_t Argument, uint8_t* pResponse );
uint32_t SDMMC_SDIO_CmdReadWriteExtended( SDMMC_TypeDef* SDMMCx, uint32_t Argument );
uint32_t
SDMMC_CmdSendOperationcondition( SDMMC_TypeDef* SDMMCx, uint32_t Argument, uint32_t* pResp );
/**
 * @}
 */
 ...
 * @{
 */
uint32_t SDMMC_DMALinkedList_BuildNode(
    SDMMC_DMALinkNodeTypeDef*           pNode,
    SDMMC_DMALinkNodeConfTypeDef const* pNodeConf );
uint32_t SDMMC_DMALinkedList_InsertNode(
    SDMMC_DMALinkedListTypeDef* pLinkedList,
    SDMMC_DMALinkNodeTypeDef*   pPrevNode,
    SDMMC_DMALinkNodeTypeDef*   pNode );
uint32_t SDMMC_DMALinkedList_RemoveNode(
    SDMMC_DMALinkedListTypeDef* pLinkedList,
    SDMMC_DMALinkNodeTypeDef*   pNode );
