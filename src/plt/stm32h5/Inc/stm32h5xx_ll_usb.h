...
typedef struct
{
        ...
        uint8_t dma_enable; /*!< USB DMA state.
                                  If DMA is not supported this parameter shall be set by default to
                               zero */
        ...
        uint8_t iso_singlebuffer_enable; /*!< Enable or disable the Single buffer mode on
                                            Isochronous  EP          */
...
typedef struct
{
        ...
        uint16_t pmaadress; /*!< PMA Address
                                 This parameter can be any value between Min_addr = 0 and Max_addr =
                               1K   */
...
#define USB_DRD_CLEAR_CHEP_RX_ERR( USBx, bChNum )                                                 \
        do {                                                                                      \
                ...
                _wRegVal =                                                                        \
                    ( _wRegVal & USB_CHEP_REG_MASK & ( ~USB_CHEP_ERRRX ) & ( ~USB_CHEP_VTRX ) ) | \
...
#define USB_DRD_CLEAR_CHEP_TX_ERR( USBx, bChNum )                                                 \
        do {                                                                                      \
                ...
                _wRegVal = USB_DRD_GET_CHEP( ( USBx ), ( bChNum ) );                              \
                _wRegVal =                                                                        \
                    ( _wRegVal & USB_CHEP_REG_MASK & ( ~USB_CHEP_ERRTX ) & ( ~USB_CHEP_VTTX ) ) | \
...
#define USB_DRD_SET_CHEP_TX_STATUS( USBx, bEpChNum, wState )                                  \
        do {                                                                                  \
                ...
                USB_DRD_SET_CHEP(                                                             \
                    ( USBx ), ( bEpChNum ), ( _wRegVal | USB_CHEP_VTRX | USB_CHEP_VTTX ) );   \
...
#define USB_DRD_SET_CHEP_ADDRESS( USBx, bEpChNum, bAddr )                                       \
        do {                                                                                    \
                uint32_t _wRegVal;                                                              \
                                                                                                \
                /*Read the USB->CHEPx into _wRegVal, Reset(DTOGRX/STRX/DTOGTX/STTX) and set the \
                 * EpAddress*/                                                                  \
                _wRegVal = ( USB_DRD_GET_CHEP( ( USBx ), ( bEpChNum ) ) & USB_CHEP_REG_MASK ) | \
                           ( bAddr );                                                           \
                                                                                                \
...
HAL_StatusTypeDef
USB_SetTurnaroundTime( USB_OTG_GlobalTypeDef* USBx, uint32_t hclk, uint8_t speed );
...
HAL_StatusTypeDef
USB_EPStartXfer( USB_OTG_GlobalTypeDef* USBx, USB_OTG_EPTypeDef* ep, uint8_t dma );
...
HAL_StatusTypeDef
USB_EPClearStall( USB_OTG_GlobalTypeDef const* USBx, USB_OTG_EPTypeDef const* ep );
...
HAL_StatusTypeDef
USB_HC_DoubleBuffer( USB_DRD_TypeDef* USBx, uint8_t phy_ch_num, uint8_t db_state );
HAL_StatusTypeDef USB_HC_Init(
