...
#if ( USE_HAL_OTFDEC_REGISTER_CALLBACKS == 1 )
        void ( *ErrorCallback )( struct __OTFDEC_HandleTypeDef* hotfdec ); /*!< OTFDEC error
                                                                              callback */

        void ( *MspInitCallback )( struct __OTFDEC_HandleTypeDef* hotfdec ); /*!< OTFDEC Msp Init
                                                                                callback */

        void ( *MspDeInitCallback )( struct __OTFDEC_HandleTypeDef* hotfdec ); /*!< OTFDEC Msp
                                                                                  DeInit callback */
#endif /* USE_HAL_OTFDEC_REGISTER_CALLBACKS */
 ...
 */
typedef void ( *pOTFDEC_CallbackTypeDef )( OTFDEC_HandleTypeDef* hotfdec ); /*!< pointer to a OTFDEC
                                                                               callback function */
...
#define OTFDEC_SEC_KEY_ERROR_INT \
        ( OTFDEC_IER_SEIE | OTFDEC_IER_KEIE ) /*!< OTFDEC security and key errors interrupts */
...
#define OTFDEC_ALL_INT \
        ( OTFDEC_IER_SEIE | OTFDEC_IER_XONEIE | OTFDEC_IER_KEIE ) /*!< OTFDEC all interrupts */
/**
 * @}
 */
...
#define OTFDEC_REG_CONFIGR_REG_ENABLE \
        OTFDEC_REG_CONFIGR_REG_EN /*!< OTFDEC region encryption or on-the-fly decryption enable */
/**
 * @}
 */
...
#define OTFDEC_REG_MODE_INSTRUCTION_ACCESSES_ONLY_WITH_CIPHER                                 \
        OTFDEC_REG_CONFIGR_MODE /*!< Only instruction accesses are decrypted with proprietary \
                                   cipher activated */
/**
 * @}
 */
...
HAL_StatusTypeDef
HAL_OTFDEC_RegionSetKey( OTFDEC_HandleTypeDef* hotfdec, uint32_t RegionIndex, uint32_t* pKey );
HAL_StatusTypeDef
HAL_OTFDEC_RegionSetMode( OTFDEC_HandleTypeDef* hotfdec, uint32_t RegionIndex, uint32_t mode );
HAL_StatusTypeDef HAL_OTFDEC_RegionConfig(
    OTFDEC_HandleTypeDef*             hotfdec,
    uint32_t                          RegionIndex,
    OTFDEC_RegionConfigTypeDef const* Config,
    uint32_t                          lock );
uint32_t          HAL_OTFDEC_KeyCRCComputation( uint32_t const* pKey );
...
HAL_StatusTypeDef HAL_OTFDEC_Cipher(
    OTFDEC_HandleTypeDef* hotfdec,
    uint32_t              RegionIndex,
 ...
 * @{
 */
HAL_OTFDEC_StateTypeDef HAL_OTFDEC_GetState( OTFDEC_HandleTypeDef const* hotfdec );
HAL_StatusTypeDef
         HAL_OTFDEC_GetConfigAttributes( OTFDEC_HandleTypeDef* hotfdec, uint32_t* Attributes );
uint32_t HAL_OTFDEC_RegionGetKeyCRC( OTFDEC_HandleTypeDef const* hotfdec, uint32_t RegionIndex );
HAL_StatusTypeDef HAL_OTFDEC_RegionGetConfig(
    OTFDEC_HandleTypeDef*       hotfdec,
    uint32_t                    RegionIndex,
