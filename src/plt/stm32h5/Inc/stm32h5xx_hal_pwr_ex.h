...
#define PWR_AVD_MODE_NORMAL \
        ( 0x00000000U ) /*!< Basic mode is used                                        */
 ...
 *                         @arg @ref PWR_REGULATOR_VOLTAGE_SCALE0 : Regulator voltage output scale
 * 0. Provides a typical output voltage at 1.2 V. Used when system clock frequency is up to 160 MHz.
 *                         @arg @ref PWR_REGULATOR_VOLTAGE_SCALE1 : Regulator voltage output
 * scale 1. Provides a typical output voltage at 1.1 V. Used when system clock frequency is up to
 * 100 MHz.
 ...
 * MHz.
 *                         @arg @ref PWR_REGULATOR_VOLTAGE_SCALE3 : Regulator voltage output
 * scale 3. Provides a typical output voltage at 0.9 V. Used when system clock frequency is up to 24
 * MHz.
 * @retval None.
 */
...
#define IS_PWR_MEMORY_BLOCK( BLOCK )                                                              \
        ( ( ( BLOCK ) == PWR_ETHERNET_MEMORY_BLOCK ) || ( ( BLOCK ) == PWR_RAM3_MEMORY_BLOCK ) || \
          ( ( BLOCK ) == PWR_RAM2_16_MEMORY_BLOCK ) ||                                            \
          ( ( BLOCK ) == PWR_RAM2_48_MEMORY_BLOCK ) || ( ( BLOCK ) == PWR_RAM1_MEMORY_BLOCK ) )
#elif defined( PWR_PMCR_SRAM2_16LSO )
#define IS_PWR_MEMORY_BLOCK( BLOCK )                        \
...
#define IS_PWR_AVD_MODE( MODE )                                                                \
        ( ( ( MODE ) == PWR_AVD_MODE_IT_RISING ) || ( ( MODE ) == PWR_AVD_MODE_IT_FALLING ) || \
          ...
          ( ( MODE ) == PWR_AVD_MODE_EVENT_FALLING ) || ( ( MODE ) == PWR_AVD_MODE_NORMAL ) || \
