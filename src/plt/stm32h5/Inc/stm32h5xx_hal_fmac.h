...
#endif /* USE_HAL_FMAC_REGISTER_CALLBACKS */
{
        ...
        DMA_HandleTypeDef* hdmaPreload; /*!< FMAC peripheral preloaded data (X1, X2 and Y) DMA
                                           handle parameters */

#if ( USE_HAL_FMAC_REGISTER_CALLBACKS == 1 )
        void ( *ErrorCallback )( struct __FMAC_HandleTypeDef* hfmac ); /*!< FMAC error callback */

        void ( *HalfGetDataCallback )( struct __FMAC_HandleTypeDef* hfmac ); /*!< FMAC get half data
                                                                                callback          */
        ...
        void ( *OutputDataReadyCallback )( struct __FMAC_HandleTypeDef* hfmac ); /*!< FMAC output
                                                                                    data ready
                                                                                    callback      */

        void ( *FilterConfigCallback )( struct __FMAC_HandleTypeDef* hfmac ); /*!< FMAC filter
                                                                                 configuration
                                                                                 callback   */

        void ( *FilterPreloadCallback )( struct __FMAC_HandleTypeDef* hfmac ); /*!< FMAC filter
                                                                                  preload callback
                                                                                */
        ...
        void ( *MspDeInitCallback )( struct __FMAC_HandleTypeDef* hfmac ); /*!< FMAC Msp DeInit
                                                                              callback */
        ...
        __IO HAL_FMAC_StateTypeDef RdState; /*!< FMAC state related to read operations (access to Y
                                               buffer) This parameter can be a value of @ref
                                               HAL_FMAC_StateTypeDef */

        __IO HAL_FMAC_StateTypeDef WrState; /*!< FMAC state related to write operations (access to
                                               X1 buffer) This parameter can be a value of @ref
                                               HAL_FMAC_StateTypeDef */
 ...
 */
typedef void ( *pFMAC_CallbackTypeDef )( FMAC_HandleTypeDef* hfmac ); /*!< pointer to an FMAC
                                                                         callback function */
...
typedef struct
{
        uint8_t InputBaseAddress; /*!< Base address of the input buffer (X1) within the internal
                                     memory (0x00 to 0xFF). Ignored if InputBufferSize is set to 0
        ...
        uint32_t InputThreshold; /*!< Input threshold: the buffer full flag will be set if the
                                    number of free spaces in the buffer is lower than this
                                    threshold. This parameter can be a value of @ref
                                    FMAC_Data_Buffer_Threshold. */

        uint8_t CoeffBaseAddress; /*!< Base address of the coefficient buffer (X2) within the
                                     internal memory (0x00 to 0xFF). Ignored if CoeffBufferSize is
                                     set to 0 (previous configuration kept). Note: the buffers can
        ...
        uint8_t OutputBaseAddress; /*!< Base address of the output buffer (Y) within the internal
                                        memory (0x00 to 0xFF). Ignored if OuputBufferSize is set to
                                      0 (previous configuration kept). Note: the buffers can overlap
                                      or even coincide exactly. */

        uint8_t OutputBufferSize; /*!< Number of 16-bit words allocated to the output buffer
                                      (including the optional "headroom").
                                       0 if a previous configuration should be kept. */

        uint32_t OutputThreshold; /*!< Output threshold: the buffer empty flag will be set if the
                                     number of unread values in the buffer is lower than this
                                     threshold. This parameter can be a value of @ref
                                     FMAC_Data_Buffer_Threshold. */
 ...
 *            @arg @ref FMAC_IT_SATIEN  Saturation error interrupt enable (this helps in debugging a
 * filter)
 * @retval None
 */
...
#define IS_FMAC_FUNCTION( __FUNCTION__ )                                                           \
        ( ( ( __FUNCTION__ ) == FMAC_FUNC_LOAD_X1 ) ||                                             \
          ( ( __FUNCTION__ ) == FMAC_FUNC_LOAD_X2 ) || ( ( __FUNCTION__ ) == FMAC_FUNC_LOAD_Y ) || \
