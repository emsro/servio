...
typedef struct
{
        ...
        uint32_t Tolerance; /*!< Set RXTOL bit, specifies the tolerance accepted on the received
                               waveforms, it can be a value of CEC_Tolerance : it is either
                               CEC_STANDARD_TOLERANCE or CEC_EXTENDED_TOLERANCE */

        uint32_t BRERxStop; /*!< Set BRESTP bit CEC_BRERxStop : specifies whether or not a Bit
                               Rising Error stops the reception. CEC_NO_RX_STOP_ON_BRE: reception is
                               not stopped. CEC_RX_STOP_ON_BRE:    reception is stopped. */

        uint32_t BREErrorBitGen; /*!< Set BREGEN bit CEC_BREErrorBitGen : specifies whether or not
                                    an Error-Bit is generated on the CEC line upon Bit Rising Error
                                    detection. CEC_BRE_ERRORBIT_NO_GENERATION: no error-bit
                                    generation. CEC_BRE_ERRORBIT_GENERATION:    error-bit generation
                                    if BRESTP is set. */

        uint32_t LBPEErrorBitGen; /*!< Set LBPEGEN bit CEC_LBPEErrorBitGen : specifies whether or
                                     not an Error-Bit is generated on the CEC line upon Long Bit
                                     Period Error detection. CEC_LBPE_ERRORBIT_NO_GENERATION:  no
                                     error-bit generation. CEC_LBPE_ERRORBIT_GENERATION: error-bit
                                     generation. */

        uint32_t BroadcastMsgNoErrorBitGen; /*!< Set BRDNOGEN bit CEC_BroadCastMsgErrorBitGen :
                                               ...
                                               values:
                                                 ...
                                                 1) CEC_BROADCASTERROR_ERRORBIT_GENERATION.
                                                    a) BRE detection: error-bit generation on the
                                               CEC line if BRESTP=CEC_RX_STOP_ON_BRE and
                                               ...
                                               LBPGEN=CEC_LBPE_ERRORBIT_NO_GENERATION.
                                                 ...
                                                 2) CEC_BROADCASTERROR_NO_ERRORBIT_GENERATION.
                                                    no error-bit generation in case neither a) nor
                                               b) are satisfied. Additionally, there is no error-bit
                                               generation in case of Short Bit Period Error
                                               detection in a broadcast message while LSTN bit is
                                               set. */

        uint32_t SignalFreeTimeOption; /*!< Set SFTOP bit CEC_SFT_Option : specifies when SFT timer
                                          starts. CEC_SFT_START_ON_TXSOM SFT:    timer starts when
                                          TXSOM is set by software. CEC_SFT_START_ON_TX_RX_END:  SFT
                                          timer starts automatically at the end of message
...
#endif /* USE_HAL_CEC_REGISTER_CALLBACKS */
{
        ...
        uint8_t const* pTxBuffPtr; /*!< Pointer to CEC Tx transfer Buffer */
...
#if ( USE_HAL_CEC_REGISTER_CALLBACKS == 1 )
        void ( *TxCpltCallback )( struct __CEC_HandleTypeDef* hcec ); /*!< CEC Tx Transfer completed
                                                                         callback  */
        void ( *RxCpltCallback )(
        ...
        void ( *MspDeInitCallback )( struct __CEC_HandleTypeDef* hcec ); /*!< CEC Msp DeInit
                                                                            callback             */
 ...
 */
typedef void ( *pCEC_CallbackTypeDef )( CEC_HandleTypeDef* hcec ); /*!< pointer to an CEC callback
                                                                      function */
...
/** @brief  Set OAR register (without resetting previously set address in case of multi-address
 * mode) To reset OAR, __HAL_CEC_CLEAR_OAR() needs to be called beforehand
 * @param  __HANDLE__ specifies the CEC Handle.
...
HAL_StatusTypeDef
HAL_CEC_UnRegisterCallback( CEC_HandleTypeDef* hcec, HAL_CEC_CallbackIDTypeDef CallbackID );

HAL_StatusTypeDef
HAL_CEC_RegisterRxCpltCallback( CEC_HandleTypeDef* hcec, pCEC_RxCallbackTypeDef pCallback );
...
HAL_StatusTypeDef HAL_CEC_Transmit_IT(
    ...
    uint8_t const*     pData,
    uint32_t           Size );
uint32_t HAL_CEC_GetLastReceivedFrameSize( CEC_HandleTypeDef const* hcec );
...
HAL_CEC_StateTypeDef HAL_CEC_GetState( CEC_HandleTypeDef const* hcec );
uint32_t             HAL_CEC_GetError( CEC_HandleTypeDef const* hcec );
/**
 * @}
 */
