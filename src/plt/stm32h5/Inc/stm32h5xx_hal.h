...
#define SBS_BREAK_FLASH_ECC                                                                 \
        SBS_CFGR2_ECCL /*!< Enable and lock the FLASH ECC double error with TIM1/8/15/16/17 \
                            Break inputs.*/
...
#define SBS_BREAK_LOCKUP                                                                   \
        SBS_CFGR2_CLL /*!< Enable and lock the connection of Cortex-M33 LOCKUP (hardfault) \
...
#define IS_SBS_EPOCH_SELECTION( SELECT )                                                         \
        ( ( ( SELECT ) == SBS_EPOCH_SEL_SECURE ) || ( ( SELECT ) == SBS_EPOCH_SEL_NONSECURE ) || \
...
#define SBS_OBKHDPL_INCR_0                                                                         \
        0x00U /*!< Index to add to the current HDPL to point (through OBK-HDPL) to the next secure \
                 storage areas */
...
#define SBS_MPU_SEC                                                                       \
        ( SBS_CSLCKR_LOCKSMPU << 16U ) /*!< Secure MPU lock (privileged secure code only) \
                                        */
#define SBS_VTOR_AIRCR_SEC                                                               \
        ( SBS_CSLCKR_LOCKSVTAIRCR << 16U ) /*!< VTOR_S and AIRCR lock (privileged secure \
                                                code only) */
#define SBS_LOCK_ALL \
        ( SBS_MPU_NSEC | SBS_VTOR_NSEC | SBS_SAU | SBS_MPU_SEC | SBS_VTOR_AIRCR_SEC ) /*!< All */
#else
#define SBS_LOCK_ALL \
        ( SBS_MPU_NSEC | SBS_VTOR_NSEC ) /*!< All (privileged secure or non-secure only) */
 ...
 *         Enable and lock the connection of Cortex-M33 LOCKUP (Hardfault) output to TIM1/8/15/16/17
 * Break input.
 * @note   The selected configuration is locked and can be unlocked only by system reset.
 */
 ...
 *            @arg @ref SBS_MEMORIES_ERASE_FLAG_IPMEE   Status of End Of Erase for ICACHE and PKA
 * RAMs
 *            @arg @ref SBS_MEMORIES_ERASE_FLAG_MCLR    Status of Erase after Power-on Reset
 * ((SRAM2, BKPRAM, ICACHE, DCACHE, PKA RAMs)
...
#define IS_SBS_OBKHDPL_SELECTION( __SELECT__ )                                                    \
        ( ( ( __SELECT__ ) == SBS_OBKHDPL_INCR_0 ) || ( ( __SELECT__ ) == SBS_OBKHDPL_INCR_1 ) || \
          ( ( __SELECT__ ) == SBS_OBKHDPL_INCR_2 ) || ( ( __SELECT__ ) == SBS_OBKHDPL_INCR_3 ) )
