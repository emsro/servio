...
#endif /* USE_HAL_PKA_REGISTER_CALLBACKS */
{
...
#if ( USE_HAL_PKA_REGISTER_CALLBACKS == 1 )
        void ( *OperationCpltCallback )( struct __PKA_HandleTypeDef* hpka ); /*!< PKA End of
                                                                                operation callback
                                                                              */
        ...
        void ( *MspDeInitCallback )( struct __PKA_HandleTypeDef* hpka ); /*!< PKA Msp DeInit
                                                                            callback       */
#endif /* USE_HAL_PKA_REGISTER_CALLBACKS */
} PKA_HandleTypeDef;
/**
 ...
 * @{
 */
typedef void ( *pPKA_CallbackTypeDef )( PKA_HandleTypeDef* hpka ); /*!< Pointer to a PKA callback
                                                                      function */
/**
 * @}
 */
...
typedef struct
{
        ...
        uint8_t const* coefA; /*!< Pointer to curve coefficient |a| (Array of modulusSize elements)
                               */
        uint8_t const* coefB; /*!< pointer to curve coefficient b */
        uint8_t const* modulus; /*!< Pointer to curve modulus value p (Array of modulusSize
                                   elements) */
        ...
        uint8_t const* pointY; /*!< Pointer to point P coordinate yP (Array of modulusSize elements)
                                */
        uint8_t const* scalarMul;  /*!< Pointer to scalar multiplier k   (Array of scalarMulSize
                                      elements) */
        uint8_t const* primeOrder; /*!< pointer to order of the curve */
} PKA_ECCMulInTypeDef;
...
typedef struct
{
        uint32_t modulusSize;    /*!< Number of element in coefA, coefB, modulus, pointX and pointY
                                    arrays */
        uint32_t       coefSign; /*!< Curve coefficient a sign */
        uint8_t const* coefA; /*!< Pointer to curve coefficient |a| (Array of modulusSize elements)
                               */
        ...
        uint8_t const* pointY; /*!< Pointer to point P coordinate yP (Array of modulusSize elements)
                                */
        uint32_t const* pMontgomeryParam; /*!< pointer to montgomery param R2 (modulus N) */
...
typedef struct
{
        ...
        uint32_t       coefSign;       /*!< Curve coefficient a sign */
        uint8_t const* coef;       /*!< Pointer to curve coefficient |a|     (Array of modulusSize
                                      elements) */
        uint8_t const* modulus;    /*!< Pointer to curve modulus value p     (Array of modulusSize
                                      elements) */
        uint8_t const* basePointX; /*!< Pointer to curve base point xG       (Array of modulusSize
                                      elements) */
        uint8_t const* basePointY; /*!< Pointer to curve base point yG       (Array of modulusSize
                                      elements) */
        uint8_t const* pPubKeyCurvePtX; /*!< Pointer to public-key curve point xQ (Array of
                                           modulusSize elements) */
        uint8_t const* pPubKeyCurvePtY; /*!< Pointer to public-key curve point yQ (Array of
                                           modulusSize elements) */
        uint8_t const* RSign; /*!< Pointer to signature part r          (Array of primeOrderSize
                                 elements) */
        uint8_t const* SSign; /*!< Pointer to signature part s          (Array of primeOrderSize
                                 elements) */
        uint8_t const* hash;  /*!< Pointer to hash of the message e     (Array of primeOrderSize
                                 elements) */
        uint8_t const* primeOrder; /*!< Pointer to order of the curve n      (Array of
...
typedef struct
{
        ...
        uint8_t const* coef;    /*!< Pointer to curve coefficient |a|     (Array of modulusSize
                                   elements) */
        uint8_t const* coefB;   /*!< Pointer to B coefficient             (Array of modulusSize
                                   elements) */
        uint8_t const* modulus; /*!< Pointer to curve modulus value p     (Array of modulusSize
                                   elements) */
        uint8_t const* integer; /*!< Pointer to random integer k          (Array of primeOrderSize
                                   elements) */
        uint8_t const* basePointX; /*!< Pointer to curve base point xG       (Array of modulusSize
                                      elements) */
        uint8_t const* basePointY; /*!< Pointer to curve base point yG       (Array of modulusSize
                                      elements) */
        uint8_t const* hash; /*!< Pointer to hash of the message       (Array of primeOrderSize
                                elements) */
        uint8_t const* privateKey; /*!< Pointer to private key d             (Array of
                                      primeOrderSize elements) */
        uint8_t const* primeOrder; /*!< Pointer to order of the curve n      (Array of
                                      primeOrderSize elements) */
...
typedef struct
{
        ...
        uint8_t const* pOp1;    /*!< Pointer to Operand 1 */
        uint8_t const* pExp;    /*!< Pointer to Exponent */
        uint8_t const* pMod;    /*!< Pointer to Operand 1 */
        uint8_t const* pPhi;    /*!< Pointer to Phi value */
...
typedef struct
{
        ...
        uint8_t const*  pOp1;    /*!< Pointer to Operand              (Array of OpSize elements) */
        uint8_t const*  pMod;    /*!< Pointer to modulus              (Array of OpSize elements) */
        uint32_t const* pMontgomeryParam; /*!< Pointer to Montgomery parameter (Array of expSize/4
                                             elements) */
...
typedef struct
{
        ...
        uint32_t       coefSign;       /*!< curve coefficient a sign */
        uint8_t const* coefA;          /*!< pointer to curve coefficient |a| */
        uint8_t const* modulus;        /*!< pointer to curve modulus value p */
        uint8_t const* integerK;       /*!< pointer to cryptographically secure random integer k */
        uint8_t const* integerM;       /*!< pointer to cryptographically secure random integer m */
        uint8_t const* basePointX1;    /*!< pointer to curve base first point coordinate x */
        uint8_t const* basePointY1;    /*!< pointer to curve base first point coordinate y */
        uint8_t const* basePointZ1;    /*!< pointer to curve base first point coordinate z */
        uint8_t const* basePointX2;    /*!< pointer to curve base second point coordinate x */
        uint8_t const* basePointY2;    /*!< pointer to curve base second point coordinate y */
        uint8_t const* basePointZ2;    /*!< pointer to curve base second point coordinate z */
...
typedef struct
{
        uint32_t        modulusSize;      /*!< curve modulus p length */
        uint8_t const*  modulus;          /*!< pointer to curve modulus value p */
        uint8_t const*  basePointX;       /*!< pointer to curve base point coordinate x */
        uint8_t const*  basePointY;       /*!< pointer to curve base point coordinate y */
        uint8_t const*  basePointZ;       /*!< pointer to curve base point coordinate z */
        uint32_t const* pMontgomeryParam; /*!< pointer to montgomery parameter R2 modulus n*/
...
#define __HAL_PKA_GET_IT_SOURCE( __HANDLE__, __INTERRUPT__ )                                    \
        ( ( ( ( __HANDLE__ )->Instance->CR & ( __INTERRUPT__ ) ) == ( __INTERRUPT__ ) ) ? SET : \
                                                                                          RESET )
...
HAL_StatusTypeDef HAL_PKA_ModExpFastMode(
    PKA_HandleTypeDef*           hpka,
