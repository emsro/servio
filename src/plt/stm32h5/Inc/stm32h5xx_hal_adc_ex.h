 ...
 * InjectedSingleDiff, InjectedOffsetNumber, InjectedOffset, InjectedOffsetSign,
 ...
 * 'InjectedOffsetSign', 'InjectedOffsetSaturation', 'AutoInjectedConv': ADC enabled without
...
typedef struct
{
        ...
        uint32_t InjectedSamplingTime; /*!< Sampling time value to be set for the selected channel.
                                          ...
                                          case of usage of internal measurement channels (VrefInt,
        ...
        uint32_t InjectedSingleDiff; /*!< Selection of single-ended or differential input.
                                        ...
                                        used in a regular and/or injected group. It overwrites the
                                        last setting. Note: Refer to Reference Manual to ensure the
                                        selected channel is available in differential mode. Note:
                                        When configuring a channel 'i' in differential mode, the
                                        channel 'i+1' is not usable separately. Note: This parameter
                                        must be modified when ADC is disabled (before ADC start
                                        conversion or after ADC stop conversion). If ADC is enabled,
                                        this parameter setting is bypassed without error reporting
                                        (as it can be the expected behavior in case of another
        ...
        FunctionalState InjectedOffsetSaturation; /*!< Define if the offset should be saturated upon
                                                   under or over flow. This parameter value can be
                                                   ENABLE or DISABLE. Note: This parameter must be
        ...
        FunctionalState
            InjectedDiscontinuousConvMode; /*!< Specifies whether the conversions sequence of ADC
                                         ...
                                         Discontinuous mode can be enabled only if continuous mode
                                         is disabled. This parameter can be set to ENABLE or
                                         DISABLE. Note: This parameter must be modified when ADC is
                                         disabled (before ADC start conversion or after ADC stop
                                         conversion). Note: For injected group, discontinuous mode
                                         converts the sequence channel by channel (discontinuous
                                         length fixed to 1 rank). Caution: this setting impacts the
        ...
        FunctionalState AutoInjectedConv; /*!< Enables or disables the selected ADC group injected
                                             automatic conversion after regular one This parameter
                                             can be set to ENABLE or DISABLE. Note: To use Automatic
                                             injected conversion, discontinuous mode must be
                                             disabled ('DiscontinuousConvMode' and
                                                     'InjectedDiscontinuousConvMode' set to DISABLE)
                                               Note: To use Automatic injected conversion, injected
                                             group external triggers must be disabled
                                             ('ExternalTrigInjecConv' set to
                                                     ADC_INJECTED_SOFTWARE_START)
                                               Note: In case of DMA used with regular group: if DMA
                                             configured in normal mode (single shot) JAUTO will be
                                             stopped upon DMA transfer complete. To maintain JAUTO
                                             always enabled, DMA must be configured in circular
                                             mode. Caution: this setting impacts the entire injected
                                             group. Therefore, call of
                                             HAL_ADCEx_InjectedConfigChannel() to configure a
                                             channel on injected group can impact the configuration
                                             of other channels previously set. */

        FunctionalState QueueInjectedContext; /*!< Specifies whether the context queue feature is
                                                 enabled. This parameter can be set to ENABLE or
                                                 DISABLE. If context queue is enabled, injected
                                                 sequencer&channels configurations are queued on up
                                                 to 2 contexts. If a new injected context is set
                                                 when queue is full, error is triggered by
                                                 ...
                                                 calls to HAL_ADCEx_InjectedConfigChannel() as the
                                                   ...
                                                   Caution: this setting impacts the entire injected
                                                 group. Therefore, call of
                                                 HAL_ADCEx_InjectedConfigChannel() to configure a
        ...
        uint32_t ExternalTrigInjecConv; /*!< Selects the external event used to trigger the
                                           ...
                                           parameter can be a value of
                                             ...
                                             Caution: this setting impacts the entire injected
                                           group. Therefore, call of
                                           HAL_ADCEx_InjectedConfigChannel() to configure a channel
                                                      on injected group can impact the configuration
                                           of other channels previously set. */
        ...
        ADC_InjOversamplingTypeDef InjecOversampling; /*!< Specifies the Oversampling parameters.
                                                         ...
                                                         parameter can be modified only if there is
                                                         no conversion is ongoing (both ADSTART and
                                                         JADSTART cleared).*/
...
/** @defgroup ADC_HAL_EC_MULTI_DMA_TRANSFER_RESOLUTION  Multimode - DMA transfer mode depending on
 * ADC resolution
 * @{
 */
...
#define ADC_FORCE_MODE_INDEPENDENT( __HANDLE__ ) \
        LL_ADC_SetMultimode(                     \
            __LL_ADC_COMMON_INSTANCE( ( __HANDLE__ )->Instance ), LL_ADC_MULTI_INDEPENDENT )
...
#define IS_ADC_CHANNEL( __HANDLE__, __CHANNEL__ )                                           \
        ( ( ( __CHANNEL__ ) == ADC_CHANNEL_0 ) || ( ( __CHANNEL__ ) == ADC_CHANNEL_1 ) ||   \
          ...
          ( ( __CHANNEL__ ) == ADC_CHANNEL_VBAT ) || ( ( __CHANNEL__ ) == ADC_CHANNEL_VDDCORE ) )
...
#define IS_ADC_OFFSET_NUMBER( __OFFSET_NUMBER__ )         \
        ( ( ( __OFFSET_NUMBER__ ) == ADC_OFFSET_NONE ) || \
          ...
          ( ( __OFFSET_NUMBER__ ) == ADC_OFFSET_3 ) || ( ( __OFFSET_NUMBER__ ) == ADC_OFFSET_4 ) )
...
#define IS_ADC_RIGHT_BIT_SHIFT( __SHIFT__ )                                                       \
        ( ( ( __SHIFT__ ) == ADC_RIGHTBITSHIFT_NONE ) ||                                          \
          ( ( __SHIFT__ ) == ADC_RIGHTBITSHIFT_1 ) || ( ( __SHIFT__ ) == ADC_RIGHTBITSHIFT_2 ) || \
          ( ( __SHIFT__ ) == ADC_RIGHTBITSHIFT_3 ) || ( ( __SHIFT__ ) == ADC_RIGHTBITSHIFT_4 ) || \
          ( ( __SHIFT__ ) == ADC_RIGHTBITSHIFT_5 ) || ( ( __SHIFT__ ) == ADC_RIGHTBITSHIFT_6 ) || \
          ( ( __SHIFT__ ) == ADC_RIGHTBITSHIFT_7 ) || ( ( __SHIFT__ ) == ADC_RIGHTBITSHIFT_8 ) )
...
uint32_t HAL_ADCEx_Calibration_GetValue( ADC_HandleTypeDef const* hadc, uint32_t SingleDiff );
...
uint32_t          HAL_ADCEx_MultiModeGetValue( ADC_HandleTypeDef const* hadc );
#endif /* ADC_MULTIMODE_SUPPORT */

/* ADC retrieve conversion value intended to be used with polling or interruption */
uint32_t HAL_ADCEx_InjectedGetValue( ADC_HandleTypeDef const* hadc, uint32_t InjectedRank );
...
HAL_StatusTypeDef HAL_ADCEx_InjectedConfigChannel(
    ADC_HandleTypeDef*              hadc,
    ADC_InjectionConfTypeDef const* pConfigInjected );
#if defined( ADC_MULTIMODE_SUPPORT )
HAL_StatusTypeDef
HAL_ADCEx_MultiModeConfigChannel( ADC_HandleTypeDef* hadc, ADC_MultiModeTypeDef const* pMultimode );
#endif /* ADC_MULTIMODE_SUPPORT */
