...
typedef struct
{
        uint32_t DataType; /*!< no swap (32-bit data), half word swap (16-bit data), byte swap
                              (8-bit data) or bit swap (1-bit data). This parameter can be a value
                              of @ref HASH_Data_Type.       */
...
#endif /* (USE_HAL_HASH_REGISTER_CALLBACKS) */
{
...
#if ( USE_HAL_HASH_REGISTER_CALLBACKS == 1 )
        void ( *InCpltCallback )( struct __HASH_HandleTypeDef* hhash ); /*!< HASH input completion
                                                                           callback            */

        void ( *DgstCpltCallback )( struct __HASH_HandleTypeDef* hhash ); /*!< HASH digest
        ...
        void ( *MspDeInitCallback )( struct __HASH_HandleTypeDef* hhash ); /*!< HASH Msp DeInit
                                                                              callback */
 ...
 */
typedef void ( *pHASH_CallbackTypeDef )( HASH_HandleTypeDef* hhash ); /*!< pointer to a HASH common
                                                                         callback functions */
...
#define HASH_FLAG_DINIS                                                                \
        HASH_SR_DINIS /*!< 16 locations are free in the DIN : new block can be entered \
                           in the Peripheral */
...
#define HASH_FLAG_DINNE \
        HASH_CR_DINNE /*!< DIN not empty : input buffer contains at least one word of data*/
/**
 * @}
 */
 ...
 *            @arg @ref HASH_FLAG_DINNE DIN not empty : the input buffer contains at least one word
 * of data.
                    ...
                    HASH_ALGOSELECTION_SHA224 ) ?                                               \
                      ...
                      ( ( READ_BIT( ( __HANDLE__ )->Instance->CR, HASH_CR_ALGO ) ==             \
                          HASH_ALGOSELECTION_SHA256 ) ?                                         \
                            32U :                                                               \
                            ( ( READ_BIT( ( __HANDLE__ )->Instance->CR, HASH_CR_ALGO ) ==       \
                                HASH_ALGOSELECTION_SHA384 ) ?                                   \
                                  48U :                                                         \
                                  ( ( READ_BIT( ( __HANDLE__ )->Instance->CR, HASH_CR_ALGO ) == \
                                      HASH_ALGOSELECTION_SHA512_224 ) ?                         \
                                        28U :                                                   \
                                        ( ( READ_BIT(                                           \
                                                ( __HANDLE__ )->Instance->CR, HASH_CR_ALGO ) == \
...
#define IS_HASH_DATATYPE( __DATATYPE__ )                                                        \
        ( ( ( __DATATYPE__ ) == HASH_NO_SWAP ) || ( ( __DATATYPE__ ) == HASH_HALFWORD_SWAP ) || \
