...
#define DAC_LFSRUNMASK_BITS7_0                                                                     \
        ( DAC_CR_MAMP1_2 | DAC_CR_MAMP1_1 | DAC_CR_MAMP1_0 ) /*!< Unmask DAC channel LFSR bit[7:0] \
...
#define DAC_TRIANGLEAMPLITUDE_255                                                                  \
        ( DAC_CR_MAMP1_2 | DAC_CR_MAMP1_1 | DAC_CR_MAMP1_0 ) /*!< Select max triangle amplitude of \
                                                                255 */
...
#define DAC_TRIANGLEAMPLITUDE_4095                                                                 \
        ( DAC_CR_MAMP1_3 | DAC_CR_MAMP1_1 | DAC_CR_MAMP1_0 ) /*!< Select max triangle amplitude of \
                                                                4095 */
...
#define IS_DAC_TRIGGER( TRIGGER )                                                                 \
        ( ( ( TRIGGER ) == DAC_TRIGGER_NONE ) || ( ( TRIGGER ) == DAC_TRIGGER_T1_TRGO ) ||        \
          ...
          ( ( TRIGGER ) == DAC_TRIGGER_T15_TRGO ) || ( ( TRIGGER ) == DAC_TRIGGER_LPTIM1_CH1 ) || \
          ( ( TRIGGER ) == DAC_TRIGGER_LPTIM2_CH1 ) || ( ( TRIGGER ) == DAC_TRIGGER_EXT_IT9 ) ||  \
...
#define IS_DAC_CHIP_CONNECTION( CONNECT )                \
        ( ( ( CONNECT ) == DAC_CHIPCONNECT_EXTERNAL ) || \
          ( ( CONNECT ) == DAC_CHIPCONNECT_INTERNAL ) || ( ( CONNECT ) == DAC_CHIPCONNECT_BOTH ) )
...
HAL_StatusTypeDef HAL_DACEx_DualSetValue(
    ...
    uint32_t           Data1,
    uint32_t           Data2 );
uint32_t HAL_DACEx_DualGetValue( DAC_HandleTypeDef const* hdac );
...
uint32_t HAL_DACEx_GetTrimOffset( DAC_HandleTypeDef const* hdac, uint32_t Channel );
