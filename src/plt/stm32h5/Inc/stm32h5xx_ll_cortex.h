...
#define LL_MPU_CTRL_HFNMI_PRIVDEF_NONE                                                       \
        0U /*!< Background region access not allowed, MPU disabled for Hardfaults, NMIs, and \
              exception handlers when FAULTMASK=1 */
#define LL_MPU_CTRL_HARDFAULT_NMI                                                           \
        2U /*!< Background region access not allowed, MPU enabled for Hardfaults, NMIs, and \
              exception handlers when FAULTMASK=1 */
#define LL_MPU_CTRL_PRIVILEGED_DEFAULT                                                         \
        4U /*!< Background region privileged-only access allowed, MPU disabled for Hardfaults, \
              NMIs, and exception handlers when FAULTMASK=1 */
#define LL_MPU_CTRL_HFNMI_PRIVDEF                                                             \
        6U /*!< Background region privileged-only access allowed, MPU enabled for Hardfaults, \
              NMIs, and exception handlers when FAULTMASK=1 */
/**
 * @}
 */
...
#define LL_MPU_DEVICE_nGnRnE \
        0x0U /*!< Device non-Gathering, non-Reordering, no Early write acknowledgement */
...
{
        return (
            ( READ_BIT( SysTick->CTRL, SysTick_CTRL_TICKINT_Msk ) ==
...
{
        return (
            ( READ_BIT( MPU->CTRL, MPU_CTRL_ENABLE_Msk ) == ( MPU_CTRL_ENABLE_Msk ) ) ? 1UL : 0UL );
}
...
{
        return (
            ( READ_BIT( MPU_NS->CTRL, MPU_CTRL_ENABLE_Msk ) == ( MPU_CTRL_ENABLE_Msk ) ) ? 1UL :
                                                                                           0UL );
}
#endif /* __ARM_FEATURE_CMSE */
    ...
    uint32_t LimitAddress )
{
        ...
        WRITE_REG(
            MPU->RLAR, ( ( LimitAddress & MPU_RLAR_LIMIT_Msk ) | AttrIndx | MPU_RLAR_EN_Msk ) );
}
 ...
 * @retval None
 */
__STATIC_INLINE void LL_MPU_ConfigRegion_NS(
    uint32_t Region,
    uint32_t Attributes,
    uint32_t AttrIndx,
    uint32_t BaseAddress,
    uint32_t LimitAddress )
{
        ...
        /* Set region limit address, memory attributes index and enable region */
        WRITE_REG(
            MPU_NS->RLAR, ( ( LimitAddress & MPU_RLAR_LIMIT_Msk ) | AttrIndx | MPU_RLAR_EN_Msk ) );
}
#endif /* __ARM_FEATURE_CMSE */
...
LL_MPU_ConfigRegionAddress( uint32_t Region, uint32_t BaseAddress, uint32_t LimitAddress )
{
        /* Set Region number */
        WRITE_REG( MPU->RNR, Region );
 ...
 */
__STATIC_INLINE void
LL_MPU_ConfigRegionAddress_NS( uint32_t Region, uint32_t BaseAddress, uint32_t LimitAddress )
{
        /* Set Region number */
        WRITE_REG( MPU_NS->RNR, Region );
...
__STATIC_INLINE uint32_t LL_MPU_GetRegionAccess_NS( uint32_t Region )
{
        ...
        return (
            READ_REG( MPU_NS->RBAR & ( MPU_RBAR_XN_Msk | MPU_RBAR_AP_Msk | MPU_RBAR_SH_Msk ) ) );
}
#endif /* __ARM_FEATURE_CMSE */
