...
{
        return (
            ( READ_BIT( ICACHE->IER, ICACHE_IER_BSYENDIE ) == ( ICACHE_IER_BSYENDIE ) ) ? 1UL :
                                                                                          0UL );
}
...
{
        return (
            ( READ_BIT( ICACHE->IER, ICACHE_IER_ERRIE ) == ( ICACHE_IER_ERRIE ) ) ? 1UL : 0UL );
}
...
{
        return (
            ( READ_BIT( ICACHE->SR, ICACHE_SR_BSYENDF ) == ( ICACHE_SR_BSYENDF ) ) ? 1UL : 0UL );
}
...
__STATIC_INLINE uint32_t LL_ICACHE_IsEnabledRegion( uint32_t Region )
{
        return (
            ( READ_BIT(
                  *( (__IO uint32_t*) ( &( ICACHE->CRR0 ) + ( 1U * Region ) ) ),
...
{
        return ( READ_BIT(
            *( (__IO uint32_t*) ( &( ICACHE->CRR0 ) + ( 1U * Region ) ) ), ICACHE_CRRx_HBURST ) );
}
...
{
        return ( READ_BIT(
            *( (__IO uint32_t*) ( &( ICACHE->CRR0 ) + ( 1U * Region ) ) ), ICACHE_CRRx_MSTSEL ) );
}
...
void LL_ICACHE_ConfigRegion(
    uint32_t                             Region,
    LL_ICACHE_RegionTypeDef const* const pICACHE_RegionStruct );
