...
typedef struct
{
        ...
        uint32_t NbSectors; /*!< Number of sectors to be erased.
                                 This parameter can be a value between 1 and (max number of sectors
                               in the bank - value of initial sector)*/
...
typedef struct
{
        ...
        uint32_t USERType; /*!< Select the User Option Byte(s) to be configured (used for
                              OPTIONBYTE_USER). This parameter can be a combination of @ref
                              FLASH_OB_USER_Type */
        ...
        uint32_t BootConfig; /*!< Specifies if the Boot Address to be configured: secure or
                                non-secure. This parameter must be a value of @ref
                                FLASH_OB_BOOT_CONFIG enumeration */
        ...
        uint32_t HDPStartSector; /*!< Start sector of HDP area (used for OPTIONBYTE_HDP).
                                      This parameter must be a value between 0 and (max number of
...
typedef struct
{
        ...
        uint32_t BBAttributesType; /*!< Block-Based Attributes type.
                                        ...
                                        */
        uint32_t BBAttributes_array[FLASH_BLOCKBASED_NB_REG]; /*!< Each bit specifies the
                                                                 block-based attribute configuration
                                                                   of a sector:
                                                                   0 means sector non-protected, 1
                                                                 means sector protected. Protection
                                                                 (secure or privilege) depends on
...
typedef struct
{
        ...
        uint32_t Address;       /*!< Flash operation Address offset.
                                     This parameter is given by bank, and must be a value between 0x0 and
                                   0xFFFF0 */
...
#define FLASH_TYPEERASE_MASSERASE_NS                                                       \
        ( FLASH_CR_BER | FLASH_CR_MER | FLASH_NON_SECURE_MASK ) /*!< Non-secure flash mass \
                                                                     erase activation */
...
#define FLASH_ECC_AREA_EDATA_BANK2 \
        ( FLASH_ECCR_DATA_ECC | FLASH_ECCR_BK_ECC ) /*!< FLASH high-cycle data area       */
...
#define OPTIONBYTE_ALL                                                                     \
        ( OPTIONBYTE_WRP | OPTIONBYTE_PROD_STATE | OPTIONBYTE_USER | OPTIONBYTE_BOOTADDR | \
          OPTIONBYTE_BOOT_LOCK | OPTIONBYTE_OTP_LOCK | OPTIONBYTE_HDP | OPTIONBYTE_EDATA | \
          OPTIONBYTE_WMSEC ) /*!< All option                                               \
byte configuration */
#else
#if defined( FLASH_EDATAR_EDATA_EN )
#define OPTIONBYTE_ALL                                                                     \
        ( OPTIONBYTE_WRP | OPTIONBYTE_PROD_STATE | OPTIONBYTE_USER | OPTIONBYTE_BOOTADDR | \
          OPTIONBYTE_BOOT_LOCK | OPTIONBYTE_OTP_LOCK | OPTIONBYTE_HDP |                    \
          OPTIONBYTE_EDATA ) /*!< All option byte configuration */
#else
#define OPTIONBYTE_ALL                                                                     \
        ( OPTIONBYTE_WRP | OPTIONBYTE_PROD_STATE | OPTIONBYTE_USER | OPTIONBYTE_BOOTADDR | \
          OPTIONBYTE_BOOT_LOCK | OPTIONBYTE_OTP_LOCK |                                     \
...
#define OB_USER_ALL                                                                               \
        ( OB_USER_BOR_LEV | OB_USER_BORH_EN | OB_USER_IWDG_SW | OB_USER_WWDG_SW |                 \
          OB_USER_NRST_STOP | OB_USER_NRST_STDBY | OB_USER_IO_VDD_HSLV | OB_USER_IO_VDDIO2_HSLV | \
          OB_USER_IWDG_STOP | OB_USER_IWDG_STDBY | OB_USER_BOOT_UBE | OB_USER_SWAP_BANK |         \
          OB_USER_SRAM1_3_RST | OB_USER_SRAM2_RST | OB_USER_BKPRAM_ECC | OB_USER_SRAM3_ECC |      \
          OB_USER_SRAM2_ECC | OB_USER_USBPD_DIS | OB_USER_TZEN )
#else
#define OB_USER_ALL                                                                               \
        ( OB_USER_BOR_LEV | OB_USER_BORH_EN | OB_USER_IWDG_SW | OB_USER_WWDG_SW |                 \
          OB_USER_NRST_STOP | OB_USER_NRST_STDBY | OB_USER_IO_VDD_HSLV | OB_USER_IO_VDDIO2_HSLV | \
          OB_USER_IWDG_STOP | OB_USER_IWDG_STDBY | OB_USER_SWAP_BANK | OB_USER_SRAM1_RST |        \
          OB_USER_SRAM2_RST | OB_USER_BKPRAM_ECC | OB_USER_SRAM3_ECC | OB_USER_SRAM2_ECC |        \
          OB_USER_SRAM1_ECC )
...
#define OB_SRAM1_3_RST_NOT_ERASE                                                         \
        FLASH_OPTSR2_SRAM1_3_RST /*!< SRAM1 and SRAM3 are not erased when a system reset \
                                      occurs */
#endif                           /* FLASH_OPTSR2_SRAM1_3_RST */
/**
 * @}
...
#define FLASH_PROGRAMMING_DELAY_1                                                          \
        FLASH_ACR_WRHIGHFREQ_0 /*!< programming delay set for Flash running between 84 MHz \
                                    and 168 MHz */
#define FLASH_PROGRAMMING_DELAY_2                                                           \
        FLASH_ACR_WRHIGHFREQ_1 /*!< programming delay set for Flash running between 168 MHz \
                                    and 250 MHz */
/**
 * @}
 */
...
#define FLASH_NSPRIV_GRANTED                                                           \
        0x00000000U /*!< access to non-secure Flash registers is granted to privileged \
...
#define FLASH_SPRIV_GRANTED                                                           \
        0x00000000U /*!< access to secure Flash registers is granted to privileged or \
                         unprivileged access */
#if defined( FLASH_PRIVCFGR_SPRIV )
#define FLASH_SPRIV_DENIED                                                                    \
        FLASH_PRIVCFGR_SPRIV /*!< access to secure Flash registers is denied to non-privilege \
                                  access */
...
#define FLASH_OPERATION_TYPE_OBKSWAP                                                    \
        ( FLASH_OPSR_CODE_OP_2 | FLASH_OPSR_CODE_OP_1 | FLASH_OPSR_CODE_OP_0 ) /*!< OBK \
                                                                       swap operation */
#endif                                                                         /* FLASH_SR_OBKERR */
/**
 * @}
...
#endif /* __ARM_FEATURE_CMSE */
HAL_StatusTypeDef HAL_FLASHEx_ConfigHDPExtension( FLASH_HDPExtensionTypeDef const* pHDPExtension );
/**
 * @}
 */
...
#define FLASH_TYPEPROGRAM_OB \
        ( 0x00008000U | FLASH_NON_SECURE_MASK )  /*!< Program Option Bytes operation type */
...
#define IS_FLASH_TYPEERASE( VALUE )                   \
        ( ( ( VALUE ) == FLASH_TYPEERASE_SECTORS ) || \
          ( ( VALUE ) == FLASH_TYPEERASE_MASSERASE ) || ( ( VALUE ) == FLASH_TYPEERASE_OBK_ALT ) )
#else
#define IS_FLASH_TYPEERASE( VALUE ) \
...
#define IS_OB_PRODUCT_STATE( STATE )                                                            \
        ( ( ( STATE ) == OB_PROD_STATE_OPEN ) || ( ( STATE ) == OB_PROD_STATE_PROVISIONING ) || \
          ( ( STATE ) == OB_PROD_STATE_IROT_PROVISIONED ) ||                                    \
          ( ( STATE ) == OB_PROD_STATE_TZ_CLOSED ) || ( ( STATE ) == OB_PROD_STATE_CLOSED ) ||  \
          ( ( STATE ) == OB_PROD_STATE_LOCKED ) || ( ( STATE ) == OB_PROD_STATE_REGRESSION ) || \
...
#define IS_OB_USER_IO_VDDIO2_HSLV( VALUE ) \
        ( ( ( VALUE ) == OB_IO_VDDIO2_HSLV_DISABLE ) || ( ( VALUE ) == OB_IO_VDDIO2_HSLV_ENABLE ) )
