...
typedef struct
{
        ...
        uint32_t FunctionalMode; /*!< Set OPAMP functional mode by setting internal connections:
                                    ...
                                    OPAMP_LL_EC_FUNCTIONAL_MODE
                                      @note If OPAMP is configured in mode PGA, the gain can be
                                    configured using function @ref LL_OPAMP_SetPGAGain(). This
                                    feature can be modified afterwards using unitary function @ref
                                    LL_OPAMP_SetFunctionalMode(). */

        uint32_t InputNonInverting; /*!< Set OPAMP input non-inverting connection.
                                         This parameter can be a value of @ref
                                       OPAMP_LL_EC_INPUT_NONINVERTING This feature can be modified
                                       afterwards using unitar function @ref
        ...
        uint32_t InputInverting; /*!< Set OPAMP inverting input connection.
                                    ...
                                    standalone or PGA with external capacitors for filtering
                                    circuit. Otherwise (OPAMP in mode follower), OPAMP inverting
                                    input is not used (not connected to GPIO pin), this parameter is
                                    discarded. This feature can be modified afterwards using unitary
...
#define LL_OPAMP_MODE_FOLLOWER                                                                   \
        ( OPAMP_CSR_VMSEL_1 | OPAMP_CSR_VMSEL_0 ) /*!< OPAMP functional mode, OPAMP operation in \
                                                     follower */
#define LL_OPAMP_MODE_PGA \
        ( OPAMP_CSR_VMSEL_1 ) /*!< OPAMP functional mode, OPAMP operation in PGA      */
#define LL_OPAMP_MODE_PGA_IO0                                                               \
        ( OPAMP_CSR_PGGAIN_2 | OPAMP_CSR_VMSEL_1 ) /*!< In PGA mode, the inverting input is \
                                                      connected to VINM0 for filtering */
#define LL_OPAMP_MODE_PGA_IO0_BIAS                                                              \
        ( OPAMP_CSR_PGGAIN_3 | OPAMP_CSR_VMSEL_1 ) /*!< In PGA mode, the inverting input is     \
                                                        connected to VINM0.                     \
...
/** @defgroup OPAMP_LL_EC_MODE_PGA_GAIN OPAMP PGA gain (relevant when OPAMP is in functional mode
 * PGA)
 * @note Gain sign:
...
#define LL_OPAMP_INPUT_NONINVERT_DAC                                                         \
        OPAMP_CSR_VPSEL_0 /*!< OPAMP non inverting input connected internally to DAC channel \
                               (DAC1_CH1 for OPAMP1)                                         \
                               Note: On this STM32 series, all OPAMPx are not available on   \
                               all devices. Refer to device datasheet for more details   */
 ...
 * @note OPAMP inverting input is used with OPAMP in mode standalone or PGA with negative gain or
 * bias. Otherwise (OPAMP in mode follower), OPAMP inverting input is not used (not connected to
 * GPIO pin).
 * @{
 */
...
/** @defgroup OPAMP_LL_EC_TRIMMING_TRANSISTORS_DIFF_PAIR OPAMP trimming of transistors differential
 * pair NMOS or PMOS
 * @{
 */
...
#define LL_OPAMP_TRIMMING_NMOS_VREF_50PC_VDDA                                                      \
        ( OPAMP_OTR_TRIMOFFSETN | ( OPAMP_CSR_CALSEL_1 << 4 ) ) /*!< OPAMP trimming of transistors \
                                                                   ...
                                                                   (internal reference voltage set \
                                                                   to 0.5*Vdda). */
...
/** @defgroup OPAMP_LL_EF_CONFIGURATION_OPAMP_INSTANCE Configuration of OPAMP hierarchical scope:
 * OPAMP instance
 * @{
 */
 ...
 *         @arg @ref LL_OPAMP_MODE_CALIBRATION
 */
__STATIC_INLINE uint32_t LL_OPAMP_GetMode( OPAMP_TypeDef const* OPAMPx )
{
...
__STATIC_INLINE void LL_OPAMP_SetFunctionalMode( OPAMP_TypeDef* OPAMPx, uint32_t FunctionalMode )
{
        ...
        MODIFY_REG(
            OPAMPx->CSR,
            OPAMP_CSR_PGGAIN_3 | OPAMP_CSR_PGGAIN_2 | OPAMP_CSR_VMSEL | OPAMP_CSR_CALON,
            FunctionalMode );
}
 ...
 *         @arg @ref LL_OPAMP_MODE_PGA_IO0_IO1_BIAS
 */
__STATIC_INLINE uint32_t LL_OPAMP_GetFunctionalMode( OPAMP_TypeDef const* OPAMPx )
{
 ...
 *         @arg @ref LL_OPAMP_PGA_GAIN_16_OR_MINUS_15
 */
__STATIC_INLINE uint32_t LL_OPAMP_GetPGAGain( OPAMP_TypeDef const* OPAMPx )
{
 ...
 *         @arg @ref LL_OPAMP_POWERMODE_HIGHSPEED
 */
__STATIC_INLINE uint32_t LL_OPAMP_GetPowerMode( OPAMP_TypeDef const* OPAMPx )
{
 ...
 *         @arg @ref LL_OPAMP_INPUT_NONINVERT_DAC
 */
__STATIC_INLINE uint32_t LL_OPAMP_GetInputNonInverting( OPAMP_TypeDef const* OPAMPx )
{
 ...
 *         @arg @ref LL_OPAMP_INPUT_INVERT_CONNECT_NO
 */
__STATIC_INLINE uint32_t LL_OPAMP_GetInputInverting( OPAMP_TypeDef const* OPAMPx )
{
 ...
 *         @arg @ref LL_OPAMP_TRIMMING_USER
 */
__STATIC_INLINE uint32_t LL_OPAMP_GetTrimmingMode( OPAMP_TypeDef const* OPAMPx )
{
 ...
 *             pair NMOS and PMOS)
 */
__STATIC_INLINE uint32_t LL_OPAMP_GetCalibrationSelection( OPAMP_TypeDef const* OPAMPx )
{
 ...
 */
__STATIC_INLINE uint32_t LL_OPAMP_IsCalibrationOutputSet( OPAMP_TypeDef const* OPAMPx )
{
 ...
 * @retval 0x0...0x1F
 */
__STATIC_INLINE uint32_t LL_OPAMP_GetTrimmingValue(
    OPAMP_TypeDef const* OPAMPx,
    uint32_t             PowerMode,
    uint32_t             TransistorsDiffPair )
{
        ...
        /* containing other bits reserved for other purpose.                        */
        return (
            uint32_t) ( READ_BIT( *preg, ( TransistorsDiffPair & OPAMP_TRIMMING_VALUE_MASK ) ) >>
                        ( ( TransistorsDiffPair == LL_OPAMP_TRIMMING_NMOS ) ?
                              OPAMP_OTR_TRIMOFFSETN_Pos :
                              OPAMP_OTR_TRIMOFFSETP_Pos ) );
}
 ...
 * @retval State of bit (1 or 0).
 */
__STATIC_INLINE uint32_t LL_OPAMP_IsEnabled( OPAMP_TypeDef const* OPAMPx )
{
        return (
            ( READ_BIT( OPAMPx->CSR, OPAMP_CSR_OPAMPxEN ) == ( OPAMP_CSR_OPAMPxEN ) ) ? 1UL : 0UL );
}
...
ErrorStatus LL_OPAMP_Init( OPAMP_TypeDef* OPAMPx, LL_OPAMP_InitTypeDef const* OPAMP_InitStruct );
