...
#endif /* USE_HAL_HCD_REGISTER_CALLBACKS */
{
...
#if defined( USB_DRD_FS )
        uint32_t ep0_PmaAllocState; /*!< EP0 PMA allocation State (allocated, virtual Ch, EP0
                                       direction) */
        ...
        void ( *ConnectCallback )( struct __HCD_HandleTypeDef* hhcd );      /*!< USB OTG HCD Connect
                                                                               callback            */
        void ( *DisconnectCallback )( struct __HCD_HandleTypeDef* hhcd );   /*!< USB OTG HCD
                                                                               Disconnect callback   */
        void ( *PortEnabledCallback )( struct __HCD_HandleTypeDef* hhcd );  /*!< USB OTG HCD Port
                                                                               Enable callback  */
        void ( *PortDisabledCallback )( struct __HCD_HandleTypeDef* hhcd ); /*!< USB OTG HCD Port
                                                                               Disable callback */
        void ( *HC_NotifyURBChangeCallback )(
            struct __HCD_HandleTypeDef* hhcd,
            uint8_t                     chnum,
            HCD_URBStateTypeDef         urb_state ); /*!< USB OTG HCD Host Channel Notify URB Change
callback  */

        void ( *MspInitCallback )( struct __HCD_HandleTypeDef* hhcd );   /*!< USB OTG HCD Msp Init
                                                                            callback           */
        void ( *MspDeInitCallback )( struct __HCD_HandleTypeDef* hhcd ); /*!< USB OTG HCD Msp DeInit
                                                                            callback         */
#endif /* USE_HAL_HCD_REGISTER_CALLBACKS */
} HCD_HandleTypeDef;
/**
...
#if ( USE_HAL_HCD_REGISTER_CALLBACKS == 1U )
/** @defgroup HAL_HCD_Callback_ID_enumeration_definition HAL USB OTG HCD Callback ID enumeration
 * definition
...
typedef void ( *pHCD_CallbackTypeDef )( HCD_HandleTypeDef* hhcd ); /*!< pointer to a common USB OTG
                                                                      HCD callback function  */
 ...
 * @retval Counter value
 */
__STATIC_INLINE uint16_t HCD_GET_CH_DBUF0_CNT( HCD_TypeDef const* Instance, uint16_t bChNum )
{
        UNUSED( Instance );
        __IO uint32_t count = 10U;
 ...
 * @retval Counter value
 */
__STATIC_INLINE uint16_t HCD_GET_CH_DBUF1_CNT( HCD_TypeDef const* Instance, uint16_t bChNum )
{
        UNUSED( Instance );
        __IO uint32_t count = 10U;
