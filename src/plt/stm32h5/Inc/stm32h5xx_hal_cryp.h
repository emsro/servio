...
typedef struct
{
        ...
        uint32_t  DataWidthUnit;  /*!< Payload Data Width Unit, this parameter can be value of @ref
                                     CRYP_Data_Width_Unit */
        uint32_t HeaderWidthUnit; /*!< Header Width Unit, this parameter can be value of @ref
                                     CRYP_Header_Width_Unit */
        ...
        uint32_t KeyMode; /*!< Key mode selection, this parameter can be value of @ref CRYP_Key_Mode
                           */
        uint32_t KeySelect; /*!< Only for SAES : Key selection, this parameter can be value of @ref
                               CRYP_Key_Select */
        uint32_t KeyProtection; /*!< Only for SAES : Key protection, this parameter can be value of
                                   @ref CRYP_Key_Protection */
...
#endif /* USE_HAL_CRYP_REGISTER_CALLBACKS */
{
        ...
        void ( *InCpltCallback )( struct __CRYP_HandleTypeDef* hcryp );  /*!< CRYP Input FIFO
                                                                            transfer completed
                                                                            callback  */
        void ( *OutCpltCallback )( struct __CRYP_HandleTypeDef* hcryp ); /*!< CRYP Output FIFO
                                                                            transfer completed
                                                                            callback */
        ...
        void ( *MspInitCallback )( struct __CRYP_HandleTypeDef* hcryp ); /*!< CRYP Msp Init callback
                                                                          */
        void ( *MspDeInitCallback )( struct __CRYP_HandleTypeDef* hcryp ); /*!< CRYP Msp DeInit
                                                                              callback  */
        ...
        CRYP_ConfigTypeDef Init_saved; /*!< copy of CRYP required parameters when processing is
                                          suspended */

        uint32_t* pCrypInBuffPtr_saved; /*!< copy of CRYP input pointer when processing is suspended
                                         */

        uint32_t* pCrypOutBuffPtr_saved; /*!< copy of CRYP output pointer when processing is
                                            suspended */

        uint32_t CrypInCount_saved; /*!< copy of CRYP input data counter when processing is
                                       suspended */

        uint32_t CrypOutCount_saved; /*!< copy of CRYP output data counter when processing is
                                        suspended */
        ...
        uint16_t CrypHeaderCount_saved; /*!< copy of CRYP header data counter when processing is
                                           suspended */
 ...
 */
typedef void ( *pCRYP_CallbackTypeDef )( CRYP_HandleTypeDef* hcryp ); /*!< pointer to a common CRYP
                                                                         callback function */
...
#define CRYP_KEYMODE_WRAPPED \
        AES_CR_KMOD_0 /*!< Only for SAES, Wrapped key: to encrypt or decrypt AES keys */
...
#define CRYP_KEYSEL_HW                                                                       \
        AES_CR_KEYSEL_0 /*!< Only for SAES, Hardware key : derived hardware unique key (DHUK \
                           256-bit) */
#define CRYP_KEYSEL_SW \
        AES_CR_KEYSEL_1 /*!< Only for SAES, Software key : boot hardware key BHK (256-bit) */
#define CRYP_KEYSEL_HSW \
        AES_CR_KEYSEL_2 /*!< Only for SAES, DHUK XOR BHK Hardware unique key XOR software key */
...
#define CRYP_KEYPROT_ENABLE                                                                     \
        AES_CR_KEYPROT /*!< Only for SAES, Key protection between 2 applications with different \
                          security contexts */
#define CRYP_KEYPROT_DISABLE                                                                    \
        0x00000000U /*!< Only for SAES, Key not protected between 2 applications with different \
                       security contexts */
/**
 * @}
 */
...
#define __HAL_CRYP_GET_FLAG( __HANDLE__, __FLAG__ )                                                \
        ( ( ( __FLAG__ ) == CRYP_FLAG_KEYVALID ) ?                                                 \
          ...
          ( ( __FLAG__ ) == CRYP_FLAG_BUSY ) ?                                                     \
              ( ( ( ( __HANDLE__ )->Instance->SR & ( CRYP_FLAG_BUSY ) ) == ( CRYP_FLAG_BUSY ) ) ?  \
                    SET :                                                                          \
                    RESET ) :                                                                      \
          ...
          ( ( __FLAG__ ) == CRYP_FLAG_KEIF ) ?                                                     \
              ( ( ( ( __HANDLE__ )->Instance->ISR & ( CRYP_FLAG_KEIF ) ) == ( CRYP_FLAG_KEIF ) ) ? \
                    SET :                                                                          \
                    RESET ) :                                                                      \
 ...
 *            @arg @ref CRYP_CLEAR_RWEIF Read (RDERR), Write (WRERR) or Read/write (RWEIF) Error
 * Flag Clear
...
#define IS_CRYP_BUFFERSIZE( ALGO, DATAWIDTH, SIZE )                                              \
        ( ( ( ( ( ALGO ) == CRYP_AES_CTR ) ) &&                                                  \
            ( ( ( ( DATAWIDTH ) == CRYP_DATAWIDTHUNIT_WORD ) && ( ( ( SIZE ) % 4U ) == 0U ) ) || \
              ( ( ( DATAWIDTH ) == CRYP_DATAWIDTHUNIT_BYTE ) &&                                  \
                ( ( ( SIZE ) % 16U ) == 0U ) ) ) ) ||                                            \
