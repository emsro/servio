...
acquisition::adc_states acquisition::next_side_state( adc_states inpt )
{
    switch ( inpt ) {
        case READ_POSITION:
            return READ_VCC;
        case READ_VCC:
            // this skips the temp reading, turns out that for some reason READ_TEMP is much slower
            //            return READ_TEMP;
...
void acquisition::period_elapsed_irq( int8_t power_direction )
{
    ...
    if ( period_i_ % 2 == 0 ) {
        ...
        // For some reason the Stop is necessary to call even thou the ADC should've finished by
        // now
