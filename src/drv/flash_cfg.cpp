...
namespace
{
        ...
        bool cfg_write( uint32_t addr, std::span< const std::byte > buffer )
        {
                static constexpr std::size_t n = 128 / 8;
                using dword                    = std::array< std::byte, n >;

                while ( !buffer.empty() ) {
                        std::span< const std::byte > tmp =
                        ...
                        const HAL_StatusTypeDef status = HAL_FLASH_Program(
                            ...
                            addr,
                            std::bit_cast< uint32_t >( var.data() ) );
                        if ( status != HAL_OK )
                                fw::stop_exec();
...
em::result flash_storage::store_page( std::span< const std::byte > data )
{
        const cfg::page* page = cfg::find_next_page( _pages );
...
{
        const cfg::page* last_page = cfg::find_latest_page( _pages );
